 ...
 */
    ...
    HEDLEY_INTEL_VERSION_CHECK(13, 0, 0) ||                                    \
    HEDLEY_PGI_VERSION_CHECK(19, 4, 0) || HEDLEY_ARM_VERSION_CHECK(4, 1, 0) || \
    HEDLEY_TINYC_VERSION_CHECK(0, 9, 24) ||                                    \
    HEDLEY_TI_ARMCL_VERSION_CHECK(16, 9, 0) ||                                 \
    HEDLEY_TI_CL2000_VERSION_CHECK(16, 9, 0) ||                                \
    HEDLEY_TI_CL6X_VERSION_CHECK(8, 0, 0) ||                                   \
...
#define SIMDE_ALIGN_ASSUME_TO_UNCHECKED(Pointer, Alignment)                 \
  HEDLEY_REINTERPRET_CAST(                                                  \
      ...
      __builtin_assume_aligned(                                             \
          HEDLEY_CONST_CAST(void*,                                          \
                            HEDLEY_REINTERPRET_CAST(const void*, Pointer)), \
          Alignment))
...
#if defined(__cplusplus)
template <typename T>
HEDLEY_ALWAYS_INLINE static T* simde_align_assume_to_unchecked(
    T* ptr, const size_t alignment)
#else
HEDLEY_ALWAYS_INLINE static void* simde_align_assume_to_unchecked(
    void* ptr, const size_t alignment)
#endif
{
...
#define SIMDE_ALIGN_ASSUME_TO_UNCHECKED(Pointer, Alignment)                    \
  simde_align_assume_to_unchecked(                                             \
      HEDLEY_CONST_CAST(void*, HEDLEY_REINTERPRET_CAST(const void*, Pointer)), \
      (Alignment))
#endif
#endif
...
#if defined(__cplusplus)
template <typename T>
static HEDLEY_ALWAYS_INLINE T* simde_align_assume_to_checked_uncapped(
    T* ptr, const size_t alignment, const char* file, int line,
    const char* ptrname)
#else
static HEDLEY_ALWAYS_INLINE void* simde_align_assume_to_checked_uncapped(
    void* ptr, const size_t alignment, const char* file, int line,
    const char* ptrname)
#endif
{
  if (HEDLEY_UNLIKELY((HEDLEY_REINTERPRET_CAST(SIMDE_ALIGN_INTPTR_T_, (ptr)) %
...
#if defined(__cplusplus)
#define SIMDE_ALIGN_ASSUME_TO(Pointer, Alignment)                          \
  simde_align_assume_to_checked_uncapped((Pointer), (Alignment), __FILE__, \
                                         __LINE__, #Pointer)
#else
#define SIMDE_ALIGN_ASSUME_TO(Pointer, Alignment)                              \
...
#define SIMDE_ALIGN_CAST(Type, Pointer)                                \
  (__extension__({                                                     \
    ...
    _Pragma("GCC diagnostic ignored \"-Wcast-align\"") Type simde_r_ = \
