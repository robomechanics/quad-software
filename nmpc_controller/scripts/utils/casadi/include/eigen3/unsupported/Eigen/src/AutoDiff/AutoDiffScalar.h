 ...
 *
 * \param DerivativeType the vector type used to store/represent the
 * derivatives. The base scalar type as well as the number of derivatives to
 * compute are determined from this type. Typical choices include, e.g., \c
 * Vector4f for 4 derivatives, or \c VectorXf if the number of derivatives is
 * not known at compile time, and/or, the number of derivatives is large. Note
 * that DerivativeType can also be a reference (e.g., \c VectorXf&) to wrap a
 *                 existing vector into an AutoDiffScalar.
 *                 Finally, DerivativeType can also be any Eigen compatible
 * expression.
 *
    ...
    : public internal::auto_diff_special_op<
          ...
          !internal::is_same<
              typename internal::traits<
                  typename internal::remove_all<DerivativeType>::type>::Scalar,
              typename NumTraits<
                  typename internal::traits<typename internal::remove_all<
                      DerivativeType>::type>::Scalar>::Real>::value> {
 public:
...
//                            NumTraits<Scalar>::Real>::value>
{
  ...
  //   typedef auto_diff_scalar_op<DerivativeType, typename
  ...
  AutoDiffScalar<DerivativeType>& derived() {
    return *static_cast<AutoDiffScalar<DerivativeType>*>(this);
  }
  ...
  friend inline const AutoDiffScalar<DerType&> operator+(
      const Real& a, const AutoDiffScalar<DerivativeType>& b) {
    return AutoDiffScalar<DerType&>(a + b.value(), b.derivatives());
  }
...
inline typename CleanedUpDerType<DerType>::type(min)(
    const AutoDiffScalar<DerType>& x, const T& y) {
  typedef typename CleanedUpDerType<DerType>::type ADS;
  return (x <= y ? ADS(x) : ADS(y));
}
template <typename DerType, typename T>
inline typename CleanedUpDerType<DerType>::type(max)(
    const AutoDiffScalar<DerType>& x, const T& y) {
  typedef typename CleanedUpDerType<DerType>::type ADS;
  return (x >= y ? ADS(x) : ADS(y));
}
