...
// used for refering to a Pointer on TensorEvaluator class. While the
// TensorExpression is a device-agnostic type and need MakePointer class for
// type conversion, the TensorEvaluator class can be specialized for a device,
// hence it is possible to construct different types of temproray storage memory
...
struct MakeSYCLPointer {
  typedef Eigen::TensorSycl::internal::RangeAccess<
      cl::sycl::access::mode::read_write, T>
      Type;
};

template <typename T>
EIGEN_STRONG_INLINE const Eigen::TensorSycl::internal::RangeAccess<
    cl::sycl::access::mode::read_write, T>&
constCast(const Eigen::TensorSycl::internal::RangeAccess<
          cl::sycl::access::mode::read_write, T>& data) {
  return data;
}
