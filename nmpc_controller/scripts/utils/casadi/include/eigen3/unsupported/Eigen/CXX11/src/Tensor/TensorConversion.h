...
struct PacketConverter<TensorEvaluator, SrcPacket, TgtPacket, 8, 1> {
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE TgtPacket packet(Index index) const {
    ...
    SrcPacket src1 = m_impl.template packet<LoadMode>(index);
    SrcPacket src2 =
        m_impl.template packet<LoadMode>(index + 1 * SrcPacketSize);
    SrcPacket src3 =
        m_impl.template packet<LoadMode>(index + 2 * SrcPacketSize);
    SrcPacket src4 =
        m_impl.template packet<LoadMode>(index + 3 * SrcPacketSize);
    SrcPacket src5 =
        m_impl.template packet<LoadMode>(index + 4 * SrcPacketSize);
    SrcPacket src6 =
        m_impl.template packet<LoadMode>(index + 5 * SrcPacketSize);
    SrcPacket src7 =
        m_impl.template packet<LoadMode>(index + 6 * SrcPacketSize);
    SrcPacket src8 =
        m_impl.template packet<LoadMode>(index + 7 * SrcPacketSize);
    TgtPacket result = internal::pcast<SrcPacket, TgtPacket>(
...
template <typename TensorEvaluator, typename SrcPacket, typename TgtPacket,
          int TgtCoeffRatio>
struct PacketConverter<TensorEvaluator, SrcPacket, TgtPacket, 1,
                       TgtCoeffRatio> {
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE
  PacketConverter(const TensorEvaluator& impl)
...
struct ConversionSubExprEvalAsync {
  static EIGEN_STRONG_INLINE void run(Eval& impl, EvalPointerType,
...
struct ConversionSubExprEvalAsync<true, Eval, EvalPointerType,
  ...
  static EIGEN_STRONG_INLINE void run(Eval& impl, EvalPointerType data,
                                      EvalSubExprsCallback done) {
...
struct CoeffConv {
  ...
  static EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE TargetType
...
template <typename SrcPacket, typename TargetPacket, int LoadMode,
          bool ActuallyVectorize, bool IsSameT>
struct PacketConv {
  typedef typename internal::unpacket_traits<SrcPacket>::type SrcType;
...
template <typename SrcPacket, typename TargetPacket, int LoadMode>
struct PacketConv<SrcPacket, TargetPacket, LoadMode,
                  /*ActuallyVectorize=*/false, /*IsSameT=*/true> {
  typedef typename internal::unpacket_traits<TargetPacket>::type TargetType;
  static const int PacketSize = internal::unpacket_traits<TargetPacket>::size;
...
struct TensorEvaluator<const TensorConversionOp<TargetType, ArgType>, Device> {
  ...
  EIGEN_STRONG_INLINE bool evalSubExprsIfNeeded(EvaluatorPointerType data) {
    return ConversionSubExprEval<IsSameType, TensorEvaluator<ArgType, Device>,
                                 EvaluatorPointerType>::run(m_impl, data);
  }
  ...
  EIGEN_STRONG_INLINE void evalSubExprsIfNeededAsync(
    ...
    ConversionSubExprEvalAsync<IsSameType, TensorEvaluator<ArgType, Device>,
                               ...
                               EvalSubExprsCallback>::run(m_impl, data,
                                                          std::move(done));
  }
#endif
  ...
  coeff(Index index) const {
    return internal::CoeffConv<SrcType, TargetType, IsSameType>::run(m_impl,
                                                                     index);
  }
  ...
  packet(Index index) const {
    ...
    const bool Vectorizable =
        IsSameType ? TensorEvaluator<ArgType, Device>::PacketAccess
                   : int(TensorEvaluator<ArgType, Device>::PacketAccess) &
                         int(internal::type_casting_traits<
                             SrcType, TargetType>::VectorizedCast);
