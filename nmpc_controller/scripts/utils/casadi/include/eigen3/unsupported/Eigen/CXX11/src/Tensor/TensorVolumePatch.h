      ...
      switch (op.padding_type()) {
        ...
        case PADDING_SAME: {
          ...
          m_outputCols = numext::ceil(m_input_cols_eff /
                                      static_cast<float>(m_col_strides));
          const Index dz = (m_outputPlanes - 1) * m_plane_strides +
                           m_patch_planes_eff - m_input_planes_eff;
          const Index dy = (m_outputRows - 1) * m_row_strides +
                           m_patch_rows_eff - m_input_rows_eff;
          const Index dx = (m_outputCols - 1) * m_col_strides +
