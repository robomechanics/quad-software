...
/**     \internal Low-level Induced Dimension Reduction algoritm
        ...
        \param iter On input the max number of iteration, on output the number
   of performed iterations. \param relres On input the tolerance error, on
   output an estimation of the relative error. \param S On input Number of the
   dimension of the shadow space. \param smoothing switches residual smoothing
   on. \param angle small omega lead to faster convergence at the expense of
   numerical stability \param replacement switches on a residual replacement
   strategy to increase accuracy of residual at the expense of more Mat*vec
   products \return false in the case of numerical issue, for example a break
   down of IDRS.
*/
template <typename Vector, typename RealScalar>
typename Vector::Scalar omega(const Vector& t, const Vector& s,
                              RealScalar angle) {
  using numext::abs;
...
bool idrs(const MatrixType& A, const Rhs& b, Dest& x,
          const Preconditioner& precond, Index& iter,
          typename Dest::RealScalar& relres, Index S, bool smoothing,
          typename Dest::RealScalar angle, bool replacement) {
  ...
  // A peak in the residual is considered dangerously high if‖ri‖/‖b‖>
  // C(tol/epsilon). With epsilon the
  // relative machine precision. The factor tol/epsilon corresponds to the size
  // of a finite precision number that is so large that the absolute round-off
  // error in this number, when propagated through the process, makes it
  // impossible to achieve the required accuracy.The factor C accounts for the
  // accumulation of round-off errors. This parameter has beenset to 10−3. mp is
  // epsilon/C 10^3 * eps is very conservative, so normally no residual
  // replacements will take place. It only happens if things go very wrong. Too
  // many restarts may ruin the convergence.
  ...
  while (normr > tolb && iter < maxit) {
    ...
    for (Index k = 0; k < S; ++k) {
      ...
      // M(k:s,k) = (G(:,k)'*P(:,k:s))';
      M.block(k, k, S - k, 1) =
          (G.col(k).adjoint() * P.rightCols(S - k)).adjoint();
 ...
 * \brief The Induced Dimension Reduction method (IDR(s)) is a short-recurrences
 * Krylov method for sparse square problems.
 *
 ...
 * IDR(), is a robust and efficient short-recurrence Krylov subspace method for
 * solving large nonsymmetric systems of linear equations.
 *
 * For indefinite systems IDR(S) outperforms both BiCGStab and BiCGStab(L).
 * Additionally, IDR(S) can handle matrices with complex eigenvalues more
 * efficiently than BiCGStab.
 *
 * Many problems that do not converge for BiCGSTAB converge for IDR(s) (for
 * larger values of s). And if both methods converge the convergence for IDR(s)
 * is typically much faster for difficult systems (for example indefinite
 * problems).
 *
 * IDR(s) is a limited memory finite termination method. In exact arithmetic it
 * converges in at most N+N/s iterations, with N the system size.  It uses a
 * fixed number of 4+3s vector. In comparison, BiCGSTAB terminates in 2N
 * iterations and uses 7 vectors. GMRES terminates in at most N iterations, and
 * uses I+3 vectors, with I the number of iterations. Restarting GMRES limits
 * the memory consumption, but destroys the finite termination property.
 *
 ...
 *
 * IDR(s) can also be used in a matrix-free context, see the following \link
...
class IDRS : public IterativeSolverBase<IDRS<_MatrixType, _Preconditioner> > {
  ...
  */
  ...
  void _solve_vector_with_guess_impl(const Rhs& b, Dest& x) const {
    ...
    bool ret =
        internal::idrs(matrix(), b, x, Base::m_preconditioner, m_iterations,
                       m_error, m_S, m_smoothing, m_angle, m_residual);
  ...
  /** Sets the parameter S, indicating the dimension of the shadow space.
   * Default is 4*/
  void setS(Index S) {
    if (S < 1) {
      S = 4;
