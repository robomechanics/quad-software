...
struct CoeffLoader {
  ...
  EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE const typename MakePointer_<
      const typename Tensor::Scalar>::Type
  data() const {
    eigen_assert(false && "unsupported");
    return NULL;
  }
  ...
  // The placeholder accessors require to be bound to a command group handler
  // for SYCL
...
class BaseTensorContractionMapper
 ...
 public:
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE typename internal::enable_if<
      internal::unpacket_traits<PacketT>::size == packet_size, PacketT>::type
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE typename internal::enable_if<
      internal::unpacket_traits<PacketT>::size != packet_size, PacketT>::type
  load(Index i, Index j) const {
    const Index requested_packet_size =
        internal::unpacket_traits<PacketT>::size;
    EIGEN_ALIGN_MAX Scalar data[requested_packet_size];

    const IndexPair<Index> indexPair =
        this->computeIndexPair(i, j, requested_packet_size - 1);
    const Index first = indexPair.first;
    const Index lastIdx = indexPair.second;
...
class TensorContractionSubMapper {
 public:
  ...
  EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE PacketT loadPacket(Index i) const {
    ...
    return m_base_mapper.template loadPacket<PacketT, Alignment>(
  ...
  EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE PacketT load(Index i) const {
    EIGEN_STATIC_ASSERT((internal::is_same<PacketT, PacketT>::value),
    ...
    return m_base_mapper.template loadPacket<PacketT, ActualAlignment>(
        i + m_vert_offset, m_horiz_offset);
  }
...
class TensorContractionInputMapper
 ...
 public:
  ...
  EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE const
      CoeffLoader<Tensor, Tensor::RawAccess, MakePointer_>&
      get_tensor() const {
...
template <typename Scalar_, typename Index_, int side_, typename Tensor_,
          typename nocontract_t_, typename contract_t_, int packet_size_,
          bool inner_dim_contiguous_, bool inner_dim_reordered_, int Alignment_,
          template <class> class MakePointer_>
struct TensorContractionInputMapperTrait<TensorContractionInputMapper<
    Scalar_, Index_, side_, Tensor_, nocontract_t_, contract_t_, packet_size_,
