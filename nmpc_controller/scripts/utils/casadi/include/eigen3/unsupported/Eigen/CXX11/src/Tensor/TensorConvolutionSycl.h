...
template <typename Evaluator, typename CoeffReturnType, typename KernelType,
          typename Index, typename InputDims, typename Kernel_accessor,
          typename Buffer_accessor, convolution_type Conv_Dim>
struct EigenConvolutionKernel;
template <typename Evaluator, typename CoeffReturnType, typename KernelType,
          typename Index, typename InputDims, typename Kernel_accessor,
          typename Buffer_accessor>
struct EigenConvolutionKernel<Evaluator, CoeffReturnType, KernelType, Index,
                              InputDims, Kernel_accessor, Buffer_accessor,
  ...
  EigenConvolutionKernel(
      Local_accessor local_acc_, Evaluator device_evaluator_,
      Kernel_accessor kernel_filter_, Buffer_accessor buffer_acc_,
      internal::IndexMapper<Index, InputDims, 1, Evaluator::Layout>
          indexMapper_,
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE bool boundary_check(
  ...
  void operator()(cl::sycl::nd_item<2> itemID) {
    ...
    const size_t plane_tensor_offset =
        indexMapper.mapGpuInputPlaneToTensorInputOffset(
            itemID.get_global_id(1));
    /// fill the shared memory
    for (size_t i = itemID.get_local_id(0); i < num_input;
         i += itemID.get_local_range()[0]) {
      const size_t local_index = i + plane_kernel_offset;
      const size_t tensor_index =
          plane_tensor_offset +
          indexMapper.mapGpuInputKernelToTensorInputOffset(i + input_offset);

      local_acc[local_index] =
          (((i + input_offset) < (input_range[0] + kernelSize - 1)) &&
           itemID.get_global_id(1) < input_range[1])
    ...
    if (boundary_check(itemID.get_global_id() < input_range)) {
      ...
      const size_t tensor_index =
          indexMapper.mapGpuOutputPlaneToTensorOutputOffset(
              itemID.get_global_id(1)) +
          indexMapper.mapGpuOutputKernelToTensorOutputOffset(
              itemID.get_local_id(0) + first_output_start);
...
struct EigenConvolutionKernel<Evaluator, CoeffReturnType, KernelType, Index,
  ...
  EigenConvolutionKernel(
      Local_accessor local_acc_, Evaluator device_evaluator_,
      Kernel_accessor kernel_filter_, Buffer_accessor buffer_acc_,
      internal::IndexMapper<Index, InputDims, 2, Evaluator::Layout>
          indexMapper_,
  ...
  void operator()(cl::sycl::nd_item<3> itemID) {
    ...
    if (boundary_check(itemID.get_global_id() < input_range)) {
      ...
      for (size_t j = 0; j < kernel_size[1]; j++) {
        ...
        const size_t index = (num_input[0] * (plane_kernel_offset + j +
                                              itemID.get_local_id(1))) +
                             itemID.get_local_id(0);
      ...
      const size_t tensor_index =
          indexMapper.mapGpuOutputPlaneToTensorOutputOffset(
              itemID.get_global_id(2)) +
          indexMapper.mapGpuOutputKernelToTensorOutputOffset(
              itemID.get_local_id(0) + output_offset[0],
...
struct EigenConvolutionKernel<Evaluator, CoeffReturnType, KernelType, Index,
  ...
  EigenConvolutionKernel(
      Local_accessor local_acc_, Evaluator device_evaluator_,
      Kernel_accessor kernel_filter_, Buffer_accessor buffer_acc_,
      internal::IndexMapper<Index, InputDims, 3, Evaluator::Layout>
          indexMapper_,
      const cl::sycl::range<3> kernel_size_,
      const cl::sycl::range<3> input_range_, const size_t numP_)
      : local_acc(local_acc_),
        device_evaluator(device_evaluator_),
        kernel_filter(kernel_filter_),
        buffer_acc(buffer_acc_),
  ...
  void operator()(cl::sycl::nd_item<3> itemID) {
    ...
    auto kernel_ptr = kernel_filter.get_pointer();
    const auto num_input =
        cl::sycl::range<3>{itemID.get_local_range() + kernel_size - 1};

    const auto input_offset = cl::sycl::range<3>{itemID.get_group().get_id() *
                                                 itemID.get_local_range()};

    const auto output_offset = cl::sycl::range<3>{itemID.get_group().get_id() *
                                                      itemID.get_local_range() +
                                                  itemID.get_local_id()};
    ...
    for (size_t p = 0; p < numP; p++) {
      ...
      for (size_t k = itemID.get_local_id(2); k < num_input[2];
        ...
        for (size_t j = itemID.get_local_id(1); j < num_input[1];
          ...
          for (size_t i = itemID.get_local_id(0); i < num_input[0];
               i += itemID.get_local_range()[0]) {
            bool conds = cond_j_dim && (i + input_offset[0] <
                                        (input_range[0] + kernel_size[0] - 1));
      ...
      if (boundary_check(itemID.get_global_id() < input_range)) {
          ...
          for (size_t j = 0; j < kernel_size[1]; j++) {
            for (size_t i = 0; i < kernel_size[0]; i++) {
              const size_t kernel_index =
                  i + kernel_size[0] * (j + kernel_size[1] * k);
              const size_t local_index =
                  ((i + itemID.get_local_id(0)) +
                   num_input[0] *
                       ((j + itemID.get_local_id(1)) +
                        num_input[1] * (k + itemID.get_local_id(2))));
        ...
        const size_t tensor_index =
            ...
            indexMapper.mapGpuOutputKernelToTensorOutputOffset(
                output_offset[0], output_offset[1], output_offset[2]);
        buffer_ptr[tensor_index] = result;
      }
...
struct TensorEvaluator<
  ...
  static const int NumDims = internal::array_size<typename TensorEvaluator<
      InputArgType, Eigen::SyclDevice>::Dimensions>::value;
  static const int NumKernelDims = internal::array_size<Indices>::value;
  typedef typename XprType::Index Index;
  typedef DSizes<Index, NumDims> Dimensions;
  typedef typename TensorEvaluator<KernelArgType, Eigen::SyclDevice>::Dimensions
      KernelDimensions;
  typedef const Eigen::SyclDevice Device;
  typedef typename XprType::CoeffReturnType CoeffReturnType;
  typedef typename PacketType<CoeffReturnType, Eigen::SyclDevice>::type
      PacketReturnType;
  ...
  TensorEvaluator(const XprType &op, const Eigen::SyclDevice &device)
    ...
    EIGEN_STATIC_ASSERT(
        (static_cast<int>(
             TensorEvaluator<InputArgType, Eigen::SyclDevice>::Layout) ==
         static_cast<int>(
             TensorEvaluator<KernelArgType, Eigen::SyclDevice>::Layout)),
        YOU_MADE_A_PROGRAMMING_MISTAKE);

    const typename TensorEvaluator<InputArgType, Eigen::SyclDevice>::Dimensions
        &input_dims = m_inputImpl.dimensions();
    const typename TensorEvaluator<KernelArgType, Eigen::SyclDevice>::Dimensions
  ...
  EIGEN_STRONG_INLINE bool evalSubExprsIfNeeded(EvaluatorPointerType data) {
    ...
    if (data) {
      ...
      m_buf =
          (EvaluatorPointerType)m_device.get((Scalar *)m_device.allocate_temp(
              dimensions().TotalSize() * sizeof(Scalar)));
      executeEval(m_buf);
      return true;
    }
  }
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE void preloadKernel() {
    ...
    if (in_place) {
      ...
      EvaluatorPointerType local = (EvaluatorPointerType)m_device.get(
          (Scalar *)m_device.allocate_temp(kernel_sz));
      typedef TensorEvalToOp<const KernelArgType> EvalTo;
      EvalTo evalToTmp(m_device.get(local), m_kernelArg);
      const bool PacketAccess =
          internal::IsVectorizable<Eigen::SyclDevice, KernelArgType>::value;
      internal::TensorExecutor<const EvalTo, Eigen::SyclDevice,
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE void executeEval(
    ...
    switch (NumKernelDims) {
      case 1: {
        ...
        const size_t local_memory_size =
            (local_range[0] + kernel_size - 1) * (local_range[1]);
        gpu_assert(static_cast<unsigned long>(local_memory_size) <=
        ...
        typedef EigenConvolutionKernel<
            InputEvaluator, CoeffReturnType, Scalar, Index, InputDims,
            typename KernelStorage::Type, EvaluatorPointerType,
            convolution_type::CONV1D>
            ConvKernel;

        m_device.template binary_kernel_launcher<CoeffReturnType, ConvKernel>(
            m_inputImpl, m_kernel, data,
            cl::sycl::nd_range<2>(global_range, local_range), local_memory_size,
            indexMapper, kernel_size,
            cl::sycl::range<2>(input_dim[0], input_dim[1]));
        break;
      }

      case 2: {
        auto kernel_index = std::array<size_t, 2>{
            static_cast<int>(Layout) == static_cast<int>(ColMajor) ? 0 : 1,
            static_cast<int>(Layout) == static_cast<int>(ColMajor) ? 1 : 0};
        auto kernel_size = cl::sycl::range<2>{
        ...
        const size_t local_memory_size = (local_range[0] + kernel_size[0] - 1) *
                                         (local_range[1] + kernel_size[1] - 1) *
                                         local_range[2];
        gpu_assert(static_cast<unsigned long>(local_memory_size) <=
                   m_device.sharedMemPerBlock());
        const array<Index, 2> indices{
            {m_indices[kernel_index[0]], m_indices[kernel_index[1]]}};
        const array<Index, 2> kernel_dims{
            {m_kernelImpl.dimensions()[kernel_index[0]],
        ...
        m_device.template binary_kernel_launcher<CoeffReturnType, ConvKernel>(
            m_inputImpl, m_kernel, data,
            cl::sycl::nd_range<3>(global_range, local_range), local_memory_size,
            indexMapper, kernel_size,
            cl::sycl::range<3>{input_dim[0], input_dim[1], input_dim[2]});
        break;
      }

      case 3: {
        auto kernel_index = std::array<size_t, 3>{
        ...
        auto kernel_size = cl::sycl::range<3>{
            (size_t)m_kernelImpl.dimensions()[kernel_index[0]],
            (size_t)m_kernelImpl.dimensions()[kernel_index[1]],
        ...
        const array<Index, 3> indices{{m_indices[kernel_index[0]],
                                       m_indices[kernel_index[1]],
                                       m_indices[kernel_index[2]]}};
        ...
        const size_t local_memory_size = local_memory_range[0] *
                                         local_memory_range[1] *
                                         local_memory_range[2];
        ...
        m_device.template binary_kernel_launcher<CoeffReturnType, ConvKernel>(
            ...
            cl::sycl::nd_range<3>(global_range, local_range), local_memory_size,
            indexMapper, kernel_size,
            cl::sycl::range<3>(input_dim[0], input_dim[1], input_dim[2]), numP);
        break;
      }
