...
class TensorMap
 ...
 public:
  ...
  typedef typename Eigen::internal::remove_reference<
      typename Eigen::internal::nested<Self>::type>::type Nested;
#else
  ...
  typedef typename internal::conditional<
      bool(internal::is_lvalue<PlainObjectType>::value), Scalar&,
  ...
  EIGEN_DEVICE_FUNC
  EIGEN_STRONG_INLINE TensorMap(StoragePointerType dataPtr)
  ...
  EIGEN_STRONG_INLINE TensorMap(StoragePointerType dataPtr,
      ...
      : m_data(dataPtr),
        m_dimensions(dim1, dim2){EIGEN_STATIC_ASSERT(
            ...
            YOU_MADE_A_PROGRAMMING_MISTAKE)} EIGEN_DEVICE_FUNC
        EIGEN_STRONG_INLINE TensorMap(StoragePointerType dataPtr, Index dim1,
      ...
      : m_data(dataPtr),
        ...
        EIGEN_STRONG_INLINE
        TensorMap(StoragePointerType dataPtr, Index dim1, Index dim2,
                  Index dim3, Index dim4, Index dim5)
      : m_data(dataPtr),
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE StorageRefType operator()(
    ...
    EIGEN_STATIC_ASSERT(sizeof...(otherIndices) + 2 == NumIndices,
                        YOU_MADE_A_PROGRAMMING_MISTAKE)
    eigen_assert(
        internal::all((Eigen::NumTraits<Index>::highest() >= otherIndices)...));
    if (PlainObjectType::Options & RowMajor) {
...
#if EIGEN_HAS_VARIADIC_TEMPLATES
  template <typename... IndexTypes>
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE StorageRefType
