...
#define EIGEN_SYCL_TRY_CATCH(X)                                \
  do {                                                         \
    ...
    EIGEN_CATCH(const cl::sycl::exception& e) {                \
      EIGEN_THROW_X(std::runtime_error(                        \
          "SYCL exception at " + std::string(__FILE__) + ":" + \
...
#if EIGEN_COMP_CLANG  // workaround clang bug (see
 ...
 * deleted when a custom operator= is defined. This also inherits
 * template<OtherDerived> operator=(const OtherDerived&) assignments. With C++11
 * or later this also default-implements the copy-constructor
 */
