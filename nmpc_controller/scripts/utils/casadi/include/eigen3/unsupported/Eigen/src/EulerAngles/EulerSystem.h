...
class EulerSystem {
 ...
 private:
  ...
  static void CalcEulerAngles_imp(
    ...
    const Scalar Rsum =
        sqrt((mat(I_, I_) * mat(I_, I_) + mat(I_, J_) * mat(I_, J_) +
              mat(J_, K_) * mat(J_, K_) + mat(K_, K_) * mat(K_, K_)) /
             2);
    ...
    if (Rsum > 4 * NumTraits<Scalar>::epsilon()) {  // cos(beta) != 0
    ...
    } else if (plusMinus * mat(I_, K_) >
               0) {  // cos(beta) == 0 and sin(beta) == 1
      Scalar spos = mat(J_, I_) +
                    plusMinus * mat(K_, J_);  // 2*sin(alpha + plusMinus * gamma
      Scalar cpos =
          mat(J_, J_) +
          minusPlus * mat(K_, I_);  // 2*cos(alpha + plusMinus * gamma)
      ...
      Scalar sneg =
          plusMinus *
          (mat(K_, J_) +
           minusPlus * mat(J_, I_));  // 2*sin(alpha + minusPlus*gamma)
      Scalar cneg = mat(J_, J_) +
                    plusMinus * mat(K_, I_);  // 2*cos(alpha + minusPlus*gamma)
  ...
  static void CalcEulerAngles_imp(
    ...
    const Scalar Rsum =
        sqrt((mat(I_, J_) * mat(I_, J_) + mat(I_, K_) * mat(I_, K_) +
              mat(J_, I_) * mat(J_, I_) + mat(K_, I_) * mat(K_, I_)) /
             2);
    ...
    if (Rsum > 4 * NumTraits<Scalar>::epsilon()) {  // sin(beta) != 0
      ...
      Scalar spos = plusMinus * mat(K_, J_) +
                    minusPlus * mat(J_, K_);    // 2*sin(alpha + gamma)
      Scalar cpos = mat(J_, J_) + mat(K_, K_);  // 2*cos(alpha + gamma)
      res[0] = atan2(spos, cpos);
      res[2] = 0;
    } else {  // sin(beta) == 0 and cos(beta) == -1
      Scalar sneg = plusMinus * mat(K_, J_) +
                    plusMinus * mat(J_, K_);    // 2*sin(alpha - gamma)
