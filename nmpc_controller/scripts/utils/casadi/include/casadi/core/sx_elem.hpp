...
class CASADI_EXPORT SXElem : public GenericExpression<SXElem>,
 ...
 public:
  ...
  /// \cond INTERNAL
  /// Create an expression from a node: extra dummy argument to avoid ambiguity
  /// for 0/NULL
  ...
  SXElem& operator=(
      double scalar);  // needed since otherwise both a = SXElem(double)
  ...
  /** \brief  Get a pointer to the node
      ...
      \identifier{10n} */
  SXNode* get() const;  // note: constant pointer, not pointer to constant
                        // object! (to allow access to the counter)
  ...
  /** \brief Check if a value is always nonnegative (false negatives are
     allowed)
  ...
  /** \brief Assign the node to something, without invoking the deletion of the
   node,
  ...
  /** \brief Ternary if_else: x ? y : z
      ...
      \identifier{113} */
  friend inline SXElem if_else(const SXElem& x, const SXElem& y,
                               const SXElem& z) {
