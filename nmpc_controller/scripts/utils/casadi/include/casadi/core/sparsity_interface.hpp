...
/** \brief Sparsity interface class
   ...
   using the curiously recurring template pattern (CRTP) idiom.\n
...
class SparsityInterface : public SparsityInterfaceCommon {
 protected:
  // Helper functions
  inline const MatType &self() const {
    return static_cast<const MatType &>(*this);
  }
  inline MatType &self() { return static_cast<MatType &>(*this); }

 public:
  /// \cond CLUTTER
  static std::vector<std::vector<MatType> > blocksplit(
      const MatType &x, const std::vector<casadi_int> &vert_offset,
      const std::vector<casadi_int> &horz_offset);
  static std::vector<std::vector<MatType> > blocksplit(const MatType &x,
  ...
  static std::vector<casadi_int> offset(const std::vector<MatType> &v,
                                        bool vert = true);
  static std::vector<MatType> diagsplit(
      const MatType &x, const std::vector<casadi_int> &output_offset);
  static std::vector<MatType> diagsplit(const MatType &x, casadi_int incr);
  static std::vector<MatType> diagsplit(const MatType &x, casadi_int incr1,
                                        casadi_int incr2);
  ...
  static MatType repmat(const MatType &A,
  ...
  */
   ...
   *   horzcat(horzsplit(x, ...)) = x
      ...
      \identifier{3g} */
  inline friend std::vector<MatType> horzsplit(
      const MatType &x, const std::vector<casadi_int> &offset) {
   ...
   * \seealso horzsplit_n
      ...
      \identifier{3h} */
  inline friend std::vector<MatType> horzsplit(const MatType &x,
                                               casadi_int incr = 1) {
   ...
   * \seealso horzsplit
      ...
      \identifier{277} */
  inline friend std::vector<MatType> horzsplit_n(const MatType &x,
                                                 casadi_int n) {
   ...
   *   vertcat(vertsplit(x, ...)) = x
        ...
        \identifier{3i} */
  inline friend std::vector<MatType> vertsplit(
      const MatType &x, const std::vector<casadi_int> &offset) {
  ...
  /** \brief Helper function, get offsets corresponding to a vector of matrices
      ...
      \identifier{3j} */
  inline friend std::vector<casadi_int> offset(const std::vector<MatType> &v,
                                               bool vert = true) {
   ...
   * \seealso vertsplit_n
      ...
      \identifier{3k} */
  inline friend std::vector<MatType> vertsplit(const MatType &x,
                                               casadi_int incr = 1) {
   ...
   * \seealso vertsplit
      ...
      \identifier{278} */
  inline friend std::vector<MatType> vertsplit_n(const MatType &x,
                                                 casadi_int n) {
   ...
   *   blockcat(blocksplit(x,..., ...)) = x
      ...
      \identifier{3n} */
  inline friend std::vector<std::vector<MatType> > blocksplit(
      const MatType &x, const std::vector<casadi_int> &vert_offset,
      const std::vector<casadi_int> &horz_offset) {
   ...
   * \param vert_incr Defines the increment for block boundaries in row
   dimension
   * \param horz_incr Defines the increment for block boundaries in column
   dimension
   *
   *   blockcat(blocksplit(x,..., ...)) = x
   ...
   *   diagcat(diagsplit(x, ...)) = x
      ...
      \identifier{3q} */
  friend std::vector<MatType> diagsplit(
      const MatType &x, const std::vector<casadi_int> &output_offset1,
   ...
   *   diagcat(diagsplit(x, ...)) = x
      ...
      \identifier{3r} */
  inline friend std::vector<MatType> diagsplit(
      const MatType &x, const std::vector<casadi_int> &output_offset) {
  ...
  /** \brief Multiply-accumulate operation
      ...
      \identifier{3x} */
  inline friend MatType mac(const MatType &x, const MatType &y,
                            const MatType &z) {
  ...
  /** \brief Transpose
      ...
      \identifier{3y} */
  inline friend MatType transpose(const MatType &X) { return X.T(); }

  /** \brief  make a vector

      Reshapes/vectorizes the matrix such that the shape becomes (expr.numel(),
     1). Columns are stacked on top of each other. Same as reshape(expr,
     expr.numel(), 1)
      ...
      \identifier{3z} */
  inline friend MatType vec(const MatType &x) { return MatType::vec(x); }

  /** \brief Returns a reshaped version of the matrix

      \identifier{40} */
  inline friend MatType reshape(const MatType &x, casadi_int nrow,
                                casadi_int ncol) {
  ...
  /** \brief Returns a reshaped version of the matrix, dimensions as a vector
      ...
      \identifier{41} */
  inline friend MatType reshape(const MatType &x,
  ...
  /** \brief Repeat matrix A n times vertically and m times horizontally
      ...
      \identifier{48} */
  inline friend MatType repmat(const MatType &A, casadi_int n,
                               casadi_int m = 1) {
  ...
  /** \brief Repeat matrix A n times vertically and m times horizontally
      ...
      \identifier{49} */
  inline friend MatType repmat(const MatType &A,
                               const std::pair<casadi_int, casadi_int> &rc) {
  ...
  /** \brief Concatenate horizontally, four matrices
      ...
      \identifier{4c} */
  inline friend MatType horzcat(const MatType &x, const MatType &y,
                                const MatType &z, const MatType &w) {
  ...
  inline friend MatType horzcat(const MatType &x, const MatType &y,
                                const MatType &z, const MatType &w,
                                const MatType &v, const MatType &u) {
...
template <typename MatType>
MatType SparsityInterface<MatType>::repmat(const MatType &x, casadi_int n,
                                           casadi_int m) {
  if (n == 1 && m == 1) return x;
...
template <typename MatType>
std::vector<std::vector<MatType> > SparsityInterface<MatType>::blocksplit(
    const MatType &x, const std::vector<casadi_int> &vert_offset,
    const std::vector<casadi_int> &horz_offset) {
...
template <typename MatType>
std::vector<casadi_int> SparsityInterface<MatType>::offset(
    const std::vector<MatType> &v, bool vert) {
...
template <typename MatType>
std::vector<MatType> SparsityInterface<MatType>::diagsplit(
    const MatType &x, const std::vector<casadi_int> &output_offset) {
  casadi_assert(
      x.is_square(),
      "diagsplit(x,incr)::input must be square but got " + x.dim() + ".");
  return MatType::diagsplit(x, output_offset, output_offset);
}

template <typename MatType>
std::vector<MatType> SparsityInterface<MatType>::diagsplit(const MatType &x,
                                                           casadi_int incr) {
...
MatType SparsityInterface<MatType>::mtimes(const std::vector<MatType> &args) {
  ...
  MatType ret = args[0];
  for (casadi_int i = 1; i < args.size(); ++i)
    ret = MatType::mtimes(ret, args[i]);
  return ret;
}

template <typename MatType>
std::vector<MatType> SparsityInterface<MatType>::horzsplit(const MatType &x,
                                                           casadi_int incr) {
  casadi_assert_dev(incr >= 1);
  casadi_int sz2 = x.size2();
...
template <typename MatType>
std::vector<MatType> SparsityInterface<MatType>::vertsplit(const MatType &x,
                                                           casadi_int incr) {
  casadi_assert_dev(incr >= 1);
  casadi_int sz1 = x.size1();
...
template <typename MatType>
std::vector<MatType> SparsityInterface<MatType>::horzsplit_n(const MatType &x,
                                                             casadi_int n) {
  casadi_assert(n >= 0,
                "horzsplit_n(x,n): n (" + str(n) + ") must be non-negative");
...
template <typename MatType>
std::vector<MatType> SparsityInterface<MatType>::vertsplit_n(const MatType &x,
                                                             casadi_int n) {
  casadi_assert(n >= 0,
                "vertsplit_n(x,n): n (" + str(n) + ") must be non-negative");
