...
class GenericMatrix
 ...
 public:
  ...
  /** \brief Check if the sparsity is empty, i.e. if one of the dimensions is
   zero
  ...
  static MatType linspace(const MatType& a, const MatType& b,
                          casadi_int nsteps);
  ...
  static void linear_coeff(const MatType& expr, const MatType& var, MatType& A,
                           MatType& b, bool check);
  /** @}  */
  /// \endcond
  ...
  * xq[j] is used as interplating value
  ...
  /** \brief Compute any contraction of two dense tensors, using index/einstein
     notation
      ...
      \identifier{1bk} */
  ///@{
  inline friend MatType einstein(const MatType& A, const MatType& B,
                                 const MatType& C,
  ...
  static MatType rank1(const MatType& A, const MatType& alpha, const MatType& x,
                       const MatType& y);
  ///@}
  ...
  /** \brief Calculate sum of squares: sum_ij X_ij^2
      ...
      \identifier{1bq} */
  inline friend MatType sumsqr(const MatType& x) { return MatType::sumsqr(x); }
  ...
  /** \brief Matlab's \c linspace command
      ...
      \identifier{1bt} */
  inline friend MatType linspace(const MatType& a, const MatType& b,
                                 casadi_int nsteps) {
  ...
  /** \brief Matlab's \c cross command
      ...
      \identifier{1bu} */
  inline friend MatType cross(const MatType& a, const MatType& b,
                              casadi_int dim = -1) {
  ...
  /** \brief Generate a skew symmetric matrix from a 3-vector
      ...
      \identifier{1bv} */
  inline friend MatType skew(const MatType& a) { return MatType::skew(a); }
  ...
  /** \brief Matrix inverse (experimental)
      ...
      \identifier{1by} */
  inline friend MatType inv_minor(const MatType& A) {
  ...
  /** \brief Matrix inverse
      ...
      \identifier{1c0} */
  inline friend MatType inv(const MatType& A, const std::string& lsolver,
  ...
  /** \brief  Frobenius norm
      ...
      \identifier{1c4} */
  inline friend MatType norm_fro(const MatType& x) {
  ...
  /** \brief Returns difference (n-th order) along given axis (MATLAB
     convention)

      \identifier{1c8} */
  inline friend MatType diff(const MatType& x, casadi_int n = 1,
                             casadi_int axis = -1) {
  ...
  /** \brief   Get the diagonal of a matrix or construct a diagonal
      ...
      \identifier{1cd} */
  inline friend MatType diag(const MatType& A) { return MatType::diag(A); }
   ...
   *
   * If the condition \param ind evaluates to the integer k, where
   0<=k<f.size(),
   * then x[k] will be returned, otherwise \param x_default will be returned.

      \identifier{1cj} */
  inline friend MatType conditional(const MatType& ind,
  ...
  /** \brief  Substitute variable var with expression expr in multiple
     expressions

      \identifier{1cm} */
  friend inline std::vector<MatType> substitute(
      const std::vector<MatType>& ex, const std::vector<MatType>& v,
   ...
   * as well as out of a number of other expressions piggyback
      ...
      \identifier{1cn} */
  inline friend void substitute_inplace(const std::vector<MatType>& v,
  ...
  /** \brief  Solve a system of equations: A*x = b
      ...
      The solve routine works similar to Matlab's backslash when A is square and
     nonsingular. The algorithm used is the following:
      1. A simple forward or backward substitution if A is upper or lower
     triangular
      2. If the linear system is at most 3-by-3, form the inverse via minor
     expansion and multiply
      3. Permute the variables and equations as to get a (structurally) nonzero
     ...
     transformation is formed. Only the permutation part of this is however
     used. An improvement would be to solve block-by-block if there are multiple
     BLT blocks.
  ...
  /** \brief Linearize an expression
      ...
      \identifier{1cs} */
  friend inline MatType linearize(const MatType& f, const MatType& x,
                                  const MatType& x0,
   ...
   *
      \identifier{1ct} */
  friend inline MatType pinv(const MatType& A) { return MatType::pinv(A); }
   ...
   *
      ...
      \identifier{23w} */
  friend inline MatType expm(const MatType& A) { return MatType::expm(A); }
  ...
  /** \brief Calculate the Jacobian and multiply by a vector from the right
      ...
      not necessarily) more efficient if the complete Jacobian is not needed and
     v has few rows.
  ...
  /** \brief Forward directional derivative
      ...
      \identifier{1cx} */
  friend inline std::vector<std::vector<MatType> > forward(
      const std::vector<MatType>& ex, const std::vector<MatType>& arg,
      const std::vector<std::vector<MatType> >& v, const Dict& opts = Dict()) {
  ...
  /** \brief Reverse directional derivative
      ...
      \identifier{1cy} */
  friend inline std::vector<std::vector<MatType> > reverse(
      const std::vector<MatType>& ex, const std::vector<MatType>& arg,
      const std::vector<std::vector<MatType> >& v, const Dict& opts = Dict()) {
  ...
  inline friend MatType hessian(const MatType& ex, const MatType& arg,
                                MatType& output_g, const Dict& opts = Dict()) {
  ...
  /** \brief Find out which variables enter with some order
      ...
      \identifier{1cz} */
  inline friend std::vector<bool> which_depends(const MatType& expr,
                                                const MatType& var,
  ...
  *                  Provided to deal with false positive dependency checks.
  ...
  /** \brief Get a string representation for a binary MatType, using custom
     arguments

      \identifier{1d4} */
  inline friend std::string print_operator(
      const MatType& xb, const std::vector<std::string>& args) {
  ...
  /** \brief Introduce intermediate variables for selected nodes in a graph
      ...
      \identifier{1d5} */
  inline friend void extract(std::vector<MatType>& ex, std::vector<MatType>& v,
  ...
  /** \brief Extract shared subexpressions from an set of expressions
      ...
      \identifier{1d6} */
  inline friend void shared(std::vector<MatType>& ex, std::vector<MatType>& v,
                            std::vector<MatType>& vdef,
  ...
  /** \brief Given a repeated matrix, computes the sum of repeated parts
      ...
      \identifier{1d7} */
  inline friend MatType repsum(const MatType& A, casadi_int n,
                               casadi_int m = 1) {
  ...
  /** \brief Smallest element in a matrix
      ...
      \identifier{1d8} */
  friend inline MatType mmin(const MatType& x) { return MatType::mmin(x); }
  ///@}
  ...
  /** \brief Largest element in a matrix
      ...
      \identifier{1d9} */
  friend inline MatType mmax(const MatType& x) { return MatType::mmax(x); }
  ///@}

  ///@{
  /// Functions called by friend functions defined here
  static MatType jtimes(const MatType& ex, const MatType& arg, const MatType& v,
                        bool tr = false, const Dict& opts = Dict());
  static MatType gradient(const MatType& ex, const MatType& arg,
                          const Dict& opts = Dict());
  static MatType tangent(const MatType& ex, const MatType& arg,
                         const Dict& opts = Dict());
  static MatType linearize(const MatType& f, const MatType& x,
                           const MatType& x0, const Dict& opts = Dict());
...
/** @} */
  ...
  */
  ...
  /** \brief Create an nrow-by-ncol symbolic primitive
      ...
      \identifier{1da} */
  static MatType sym(const std::string& name, casadi_int nrow = 1,
                     casadi_int ncol = 1) {
  ...
  /** \brief  Construct a symbolic primitive with given dimensions
      ...
      \identifier{1db} */
  static MatType sym(const std::string& name,
   ...
   * with symbolic primitives of given sparsity
      ...
      \identifier{1dd} */
  static std::vector<MatType> sym(const std::string& name, const Sparsity& sp,
                                  casadi_int p);
   ...
   * symbolic primitives with given sparsity
      ...
      \identifier{1df} */
  static std::vector<std::vector<MatType> > sym(const std::string& name,
                                                const Sparsity& sp,
                                                casadi_int p, casadi_int r);
  ...
  ///@{
  /** \brief Create a dense matrix or a matrix with specified sparsity with all
     entries zero
...
template <typename MatType>
std::vector<std::vector<MatType> > GenericMatrix<MatType>::sym(
    const std::string& name, const Sparsity& sp, casadi_int p, casadi_int r) {
...
template <typename MatType>
MatType GenericMatrix<MatType>::linspace(const MatType& a, const MatType& b,
                                         casadi_int nsteps) {
  std::vector<MatType> ret(nsteps);
  ret[0] = a;
  MatType step = (b - a) / static_cast<MatType>(nsteps - 1);

  for (casadi_int i = 1; i < nsteps - 1; ++i) ret[i] = a + i * step;
...
template <typename MatType>
MatType GenericMatrix<MatType>::cross(const MatType& a, const MatType& b,
                                      casadi_int dim) {
...
template <typename MatType>
MatType GenericMatrix<MatType>::interp1d(const std::vector<double>& x,
                                         const MatType& v,
                                         const std::vector<double>& xq,
                                         const std::string& mode,
                                         bool equidistant) {
  ...
  casadi_assert(x.size() == v.size1(),
                "interp1d(x, v, xq): dimensions mismatch. v expected to have " +
                    str(x.size()) +
                    " rows,"
                    " but got " +
                    str(v.size1()) + " instead.");
...
MatType GenericMatrix<MatType>::skew(const MatType& a) {
  ...
  MatType z = a(2);
  return blockcat(
      std::vector<std::vector<MatType> >({{0, -z, y}, {z, 0, -x}, {-y, x, 0}}));
}
...
MatType GenericMatrix<MatType>::inv_skew(const MatType& a) {
  ...
  return 0.5 * vertcat(std::vector<MatType>(
...
MatType GenericMatrix<MatType>::tril2symm(const MatType& x) {
  ...
  casadi_assert(x.nnz_upper() - x.nnz_diag() == 0,
                "Sparsity error in tril2symm. Found above-diagonal entries in "
                "argument: " +
                    x.dim());
...
template <typename MatType>
MatType GenericMatrix<MatType>::repsum(const MatType& x, casadi_int n,
                                       casadi_int m) {
...
MatType GenericMatrix<MatType>::triu2symm(const MatType& x) {
  ...
  casadi_assert(x.nnz_lower() - x.nnz_diag() == 0,
                "Sparsity error in triu2symm. Found below-diagonal entries in "
                "argument: " +
                    x.dim());
  return x + x.T() - diag(diag(x));
}
  ...
  try {
    ...
    if (tr) {
      casadi_assert(v.size1() == ex.size1() && v.size2() % ex.size2() == 0,
                    ...
                    " v " +
                        v.dim(false) + ", ex " + ex.dim(false) + ".");
    } else {
      casadi_assert(v.size1() == arg.size1() && v.size2() % arg.size2() == 0,
                    "'v' has inconsistent dimensions: "
                    " v " +
                        v.dim(false) + ", arg " + arg.dim(false) + ".");
    }
...
MatType GenericMatrix<MatType>::gradient(const MatType& ex, const MatType& arg,
  ...
  try {
    ...
    return project(jtimes(ex, arg, MatType::ones(ex.sparsity()), true, opts),
                   arg.sparsity());
  } catch (std::exception& e) {
...
MatType GenericMatrix<MatType>::tangent(const MatType& ex, const MatType& arg,
  ...
  try {
    ...
    return project(jtimes(ex, arg, MatType::ones(arg.sparsity()), false, opts),
                   ex.sparsity());
  } catch (std::exception& e) {
...
template <typename MatType>
MatType GenericMatrix<MatType>::linearize(const MatType& f, const MatType& x,
                                          const MatType& x0, const Dict& opts) {
...
template <typename MatType>
MatType GenericMatrix<MatType>::mpower(const MatType& a, const MatType& b) {
  ...
  casadi_assert(bv - static_cast<double>(N) == 0,
...
template <typename MatType>
MatType GenericMatrix<MatType>::soc(const MatType& x, const MatType& y) {
...
template <typename MatType>
bool GenericMatrix<MatType>::is_linear(const MatType& expr,
                                       const MatType& var) {
  return !any(MatType::which_depends(expr, var, 2, true));
}

template <typename MatType>
bool GenericMatrix<MatType>::is_quadratic(const MatType& expr,
                                          const MatType& var) {
  return is_linear(gradient(expr, var), var);
}

template <typename MatType>
void GenericMatrix<MatType>::quadratic_coeff(const MatType& expr,
                                             const MatType& var, MatType& A,
                                             MatType& b, MatType& c,
                                             bool check) {
  casadi_assert(expr.is_scalar(),
                "'quadratic_coeff' only defined for scalar expressions.");
  A = hessian(expr, var);
  b = substitute(jacobian(expr, var), var, 0).T();
  if (check)
    casadi_assert(!depends_on(A, var),
                  "'quadratic_coeff' called on non-quadratic expression.");
  c = substitute(expr, var, 0);
}

template <typename MatType>
void GenericMatrix<MatType>::linear_coeff(const MatType& expr,
                                          const MatType& var, MatType& A,
                                          MatType& b, bool check) {
  casadi_assert(expr.is_vector(),
                "'linear_coeff' only defined for vector expressions.");
  if (check)
    casadi_assert(is_linear(expr, var),
...
template <typename MatType>
MatType GenericMatrix<MatType>::diff(const MatType& x, casadi_int n,
                                     casadi_int axis) {
  ...
  for (casadi_int i = 0; i < n; ++i) {
    ...
    if (local_axis == 0) {
      if (ret.size1() <= 1) {
        ...
        ret = ret(Slice(1, ret.size1()), Slice()) -
              ret(Slice(0, ret.size1() - 1), Slice());
      }
    } else {
      if (ret.size2() <= 1) {
        ret = MatType::zeros(ret.size1(), 0);
      } else {
        ret = ret(Slice(), Slice(1, ret.size2())) -
              ret(Slice(), Slice(0, ret.size2() - 1));
      }
    }
  }
  return ret;
