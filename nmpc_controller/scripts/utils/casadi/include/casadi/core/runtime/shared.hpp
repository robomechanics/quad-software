...
namespace casadi {
template <typename T>
casadi_int einstein_process(
    const T& A, const T& B, const T& C, const std::vector<casadi_int>& dim_a,
    const std::vector<casadi_int>& dim_b, const std::vector<casadi_int>& dim_c,
    const std::vector<casadi_int>& a, const std::vector<casadi_int>& b,
    const std::vector<casadi_int>& c, std::vector<casadi_int>& iter_dims,
  ...
  std::sort(dim_map_pair.begin(), dim_map_pair.end(),
            [](const std::pair<casadi_int, casadi_int>& a,
               const std::pair<casadi_int, casadi_int>& b) {
...
template <>
inline void Contraction(const bvec_t& a, const bvec_t& b, bvec_t& r) {
  r |= a | b;
}

template <typename T>
void einstein_eval(casadi_int n_iter, const std::vector<casadi_int>& iter_dims,
                   const std::vector<casadi_int>& strides_a,
                   const std::vector<casadi_int>& strides_b,
                   const std::vector<casadi_int>& strides_c, const T* a_in,
                   const T* b_in, T* c_in) {
  if (!n_iter) return;
