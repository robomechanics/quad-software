...
interpolant(const std::string& name, const std::string& solver,
            const std::vector<std::vector<double> >& grid,
            const std::vector<double>& values, const Dict& opts = Dict());
 ...
 * Some interpolant plugins may support the `inline=true` which enables correct
 derivatives
 *
 *
    \identifier{1p3} */
CASADI_EXPORT Function
interpolant(const std::string& name, const std::string& solver,
            const std::vector<std::vector<double> >& grid, casadi_int m = 1,
