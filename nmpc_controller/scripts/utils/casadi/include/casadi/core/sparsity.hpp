 ...
 *
 In this format, the structural non-zero elements are stored in column-major
 order, starting from the upper left corner of the matrix and ending in the
 lower right corner.

 In addition to the dimension (size1(), size2()), (i.e. the number of rows and
 the number of columns respectively), there are also two vectors of integers:

 1. "colind" [length size2()+1], which contains the index to the first non-zero
 element on or after the corresponding column. All the non-zero elements of a
 particular column i are thus the elements with index el that fulfills:
 colind[i] <= el < colind[i+1].

 2. "row" [same length as the number of non-zero elements, nnz()] The rows for
 each of the structural non-zeros.

 Note that with this format, it is cheap to loop over all the non-zero elements
 of a particular column, at constant time per element, but expensive to jump to
 access a location (i, j).

 If the matrix is dense, i.e. length(row) == size1()*size2(), the format reduces
 to standard dense column major format, which allows access to an arbitrary
 element in constant time.

 Since the object is reference counted (it inherits from SharedObject), several
 matrices are allowed to share the same sparsity pattern.

 The implementations of methods marked as such in this class has been taken from
 the CSparse package and modified to fit CasADi data structures and separation
 of sparsity pattern calculation and numerical evaluation. These functions are
 Copyright(c) Timothy A. Davis, 2006-2009 and licensed as a derivative work
 under the GNU LGPL
...
class CASADI_EXPORT Sparsity
 ...
 public:
  ...
  /// Construct from sparsity pattern vectors given in compressed column storage
  /// format
  Sparsity(casadi_int nrow, casadi_int ncol, const casadi_int* colind,
           const casadi_int* row, bool order_rows = false);
  ...
  /** \brief Create the sparsity pattern for a unit vector of length n and a
   nonzero on
  ...
  static Sparsity rowcol(const std::vector<casadi_int>& row,
                         const std::vector<casadi_int>& col, casadi_int nrow,
                         casadi_int ncol);
  ...
  static Sparsity triplet(casadi_int nrow, casadi_int ncol,
                          ...
                          std::vector<casadi_int>& SWIG_OUTPUT(mapping),
                          bool invert_mapping);
  static Sparsity triplet(casadi_int nrow, casadi_int ncol,
                          const std::vector<casadi_int>& row,
                          const std::vector<casadi_int>& col);
  ...
  * Inverse of `find()`
      ...
      \identifier{23g} */
  static Sparsity nonzeros(casadi_int nrow, casadi_int ncol,
   ...
   *
   * Right multiplication of P with a vector leads to the same results as
   indexing that vector with p
   *
   * P @ v = v[p]
   *
   * The inverse of a permutation matrix is equal to its transpose (property of
   orthonormality)
   *

      \identifier{244} */
  static Sparsity permutation(const std::vector<casadi_int>& p,
                              bool invert = false);
  ...
  */
...
#ifndef SWIG
  bool is_equal(casadi_int nrow, casadi_int ncol, const casadi_int* colind,
  ...
  /** \brief Implicit or explicit type conversion to C representation
      ...
      In the C runtime, sparsity patterns are represented as a "const
     casadi_int*". This enables using the C runtime functions using a natural
     syntax.
  ...
  /** \brief The total number of elements, including structural zeros, i.e.
    size2()*size1()
  ...
  /** \brief Number of non-zeros on the diagonal, i.e. the number of elements
     (i, j) with j==i
  ...
  static Sparsity from_file(const std::string& filename,
  ...
  /** \brief Get a reference to the colindex of all column element (see class
     description)
  ...
  /** \brief Get the row for each non-zero entry
     ...
     matrix in sparse triplet format, and together with the colind vector, one
     obtains the sparsity in column compressed format.
  ...
  /** \brief  Get a reference to the colindex of column cc (see class
     description)
  ...
  /** \brief Get the column for each non-zero entry
      ...
      Together with the row-vector, this vector gives the sparsity of the matrix
     in sparse triplet format, i.e. the column and row for each non-zero
     elements
  ...
  Sparsity sub(const std::vector<casadi_int>& rr,
               ...
               std::vector<casadi_int>& SWIG_OUTPUT(mapping),
               bool ind1 = false) const;

  /** \brief Get a set of elements
   *
   * Returns the sparsity of the corresponding elements, with a mapping such
   that
   *   submatrix[k] = originalmatrix[mapping[k]]
  ...
  *              for each non-zero of the new matrix
  ...
  /** \brief Combine two sparsity patterns
      ...
      Returns the new sparsity pattern as well as a mapping with the same length
     as the number of non-zero elements The mapping matrix contains the
     arguments for each nonzero, the first bit indicates if the first argument
     is nonzero, the second bit indicates if the second argument is nonzero
     (note that none of, one of or both of the arguments can be nonzero)
...
#endif  // SWIG
  Sparsity combine(const Sparsity& y, bool f0x_is_zero,
  ...
  /** \brief Intersection of two sparsity patterns
      ...
      Returns the new sparsity pattern as well as a mapping with the same length
     as the number of non-zero elements The value is 1 if the non-zero comes
     from the first (i.e. this) object, 2 if it is from the second and 3 (i.e. 1
     | 2) if from both
  ...
  static void mul_sparsityF(const bvec_t* x, const Sparsity& x_sp,
                            const bvec_t* y, const Sparsity& y_sp, bvec_t* z,
                            const Sparsity& z_sp, bvec_t* w);
   ...
   * Reverse mode.
      ...
      \identifier{cp} */
  static void mul_sparsityR(bvec_t* x, const Sparsity& x_sp, bvec_t* y,
                            const Sparsity& y_sp, bvec_t* z,
                            const Sparsity& z_sp, bvec_t* w);
  ...
  static std::vector<Sparsity> horzsplit(const Sparsity& x,
                                         const std::vector<casadi_int>& offset);
  static std::vector<Sparsity> vertsplit(const Sparsity& x,
                                         const std::vector<casadi_int>& offset);
  static std::vector<Sparsity> diagsplit(
      const Sparsity& x, const std::vector<casadi_int>& offset1,
  ...
  /** \brief Enlarge the matrix along the first dimension (i.e. insert rows)
      ...
      \identifier{cs} */
  void enlargeRows(casadi_int nrow, const std::vector<casadi_int>& rr,
                   bool ind1 = false);

  /** \brief Enlarge the matrix along the second dimension (i.e. insert columns)

      \identifier{ct} */
  void enlargeColumns(casadi_int ncol, const std::vector<casadi_int>& cc,
                      bool ind1 = false);
   ...
   *
   * A Matrix P is permutation matrix if right multiplication with a dense
   vector v leads to
   ...
   *
   * A Matrix S is selection matrix if right multiplication with a dense vector
   leads to
  ...
  * which must have the same length as the number of nonzeros
  ...
  /** \brief Calculate the elimination tree
      ...
      If the parameter ata is false, the algorithm is equivalent to MATLAB's
     etree(A), except that the indices are zero-based. If ata is true, the
     algorithm is equivalent to MATLAB's etree(A, 'col').
  ...
  void qr_sparse(Sparsity& SWIG_OUTPUT(V), Sparsity& SWIG_OUTPUT(R),
                 ...
                 std::vector<casadi_int>& SWIG_OUTPUT(pc),
                 bool amd = true) const;
  ...
  /** \brief Depth-first search on the adjacency graph of the sparsity
      ...
      \identifier{d5} */
  casadi_int dfs(casadi_int j, casadi_int top,
                 std::vector<casadi_int>& SWIG_INOUT(xi),
                 std::vector<casadi_int>& SWIG_INOUT(pstack),
                 const std::vector<casadi_int>& pinv,
                 std::vector<bool>& SWIG_INOUT(marked)) const;

  /** \brief Find the strongly connected components of the bigraph defined by
  ...
  /** \brief Calculate the block triangular form (BTF)
      ...
      The function computes the Dulmage-Mendelsohn decomposition, which allows
     you to reorder the rows and columns of a matrix to bring it into block
     triangular form (BTF).

      It will not consider the distance of off-diagonal elements to the
     diagonal: there is no guarantee you will get a block-diagonal matrix if you
     supply a randomly permuted block-diagonal matrix.
  ...
  /** \brief Get the location of all non-zero elements as they would appear in a
     Dense matrix
  ...
  /** \brief Perform a unidirectional coloring: A greedy distance-2 coloring
     algorithm

      (Algorithm 3.1 in A. H. GEBREMEDHIN, F. MANNE, A. POTHEN)

      \identifier{db} */
  Sparsity uni_coloring(
      const Sparsity& AT = Sparsity(),
  ...
  /** \brief Perform a star coloring of a symmetric matrix:
      ...
      \identifier{dc} */
  Sparsity star_coloring(
      casadi_int ordering = 1,
      casadi_int cutoff = std::numeric_limits<casadi_int>::max()) const;
  ...
  /** \brief Perform a star coloring of a symmetric matrix:
      ...
      Algorithm 4.1 in
        NEW ACYCLIC AND STAR COLORING ALGORITHMS WITH APPLICATION TO COMPUTING
     HESSIANS A. H. GEBREMEDHIN, A. TARAFDAR, F. MANNE, A. POTHEN SIAM J. SCI.
     COMPUT. Vol. 29, No. 3, pp. 1042–1072 (2007)

      Ordering options: None (0), largest first (1)

      \identifier{dd} */
  Sparsity star_coloring2(
      casadi_int ordering = 1,
      casadi_int cutoff = std::numeric_limits<casadi_int>::max()) const;
  ...
  /** \brief Permute rows and/or columns
     ...
     an index vector containing the row for each col. As an alternative, P can
     be transposed (inverted).

      \identifier{df} */
  Sparsity pmult(const std::vector<casadi_int>& p, bool permute_rows = true,
  ...
  *               With this option true, a numeric matrix will be constructed
  ...
  /** \brief Add the nonzero entries of one sparsity pattern to the nonzero
   entries
  ...
  /** \brief Bitwise or of the nonzero entries of one sparsity pattern and the
   nonzero
 ...
 private:
  /// Construct a sparsity pattern from vectors, reuse cached pattern if
  /// possible
  ...
  /// Construct a sparsity pattern from vectors, reuse cached pattern if
  /// possible
  void assign_cached(casadi_int nrow, casadi_int ncol, const casadi_int* colind,
                     const casadi_int* row, bool order_rows = false);
...
/** \brief Generate a hash value incrementally, array
    ...
    \identifier{dq} */
inline void hash_combine(std::size_t& seed, const casadi_int* v,
                         std::size_t sz) {
...
void Sparsity::set(DataType* data, const DataType* val_data,
                   const Sparsity& val_sp) const {
  ...
  if (val_sp == *this) {
    ...
    casadi_error("Sparsity::set<DataType>: shape mismatch. lhs is " + dim() +
                 ", while rhs is " + val_sp.dim() + ".");
  }
}

template <typename DataType>
void Sparsity::add(DataType* data, const DataType* val_data,
                   const Sparsity& val_sp) const {
  // Get dimensions of this
  const casadi_int sz = nnz();
  const casadi_int sz1 = size1();
  ...
  if (val_sp == *this) {
    ...
    casadi_assert(sz2 == val_sz2 && sz1 == val_sz1,
                  "Sparsity::add<DataType>: shape mismatch. lhs is " + dim() +
                      ", while rhs is " + val_sp.dim() + ".");
...
template <typename DataType>
void Sparsity::bor(DataType* data, const DataType* val_data,
                   const Sparsity& val_sp) const {
  // Get dimensions of this
  const casadi_int sz = nnz();
  const casadi_int sz1 = size1();
