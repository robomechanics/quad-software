...
/** \brief MX - Matrix expression
   ...
   SXElem functions.
    ...
    Note that an operation is always "lazy", making a matrix multiplication will
   create a matrix multiplication node, not perform the actual multiplication.
...
class CASADI_EXPORT MX
 ...
 public:
  ...
  /** \brief Check if matrix can be used to define function inputs.
      ...
      Valid inputs for MXFunctions are combinations of Reshape, concatenations
     and SymbolicMX
  ...
  /** \brief Detect duplicate symbolic expressions
     ...
     to casadi_warning. Note: Will mark the node using MX::set_temp. Make sure
     to call reset_input() after usage.
  ...
  ///@{
  void get(MX& SWIG_OUTPUT(m), bool ind1, const Slice& rr,
  ...
  void get(MX& SWIG_OUTPUT(m), bool ind1, const Matrix<casadi_int>& rr,
           const Matrix<casadi_int>& cc) const;
  void get(MX& SWIG_OUTPUT(m), bool ind1, casadi_int rr, casadi_int cc) const {
  ...
  void get_nz(MX& SWIG_OUTPUT(m), bool ind1,
              const Matrix<casadi_int>& kk) const;
  ...
  void get_nz(MX& SWIG_OUTPUT(m), bool ind1, const MX& inner,
              const Slice& outer) const;
  void get_nz(MX& SWIG_OUTPUT(m), bool ind1, const Slice& inner,
  ...
  static std::vector<MX> horzsplit(const MX& x,
                                   const std::vector<casadi_int>& offset);
  static std::vector<MX> diagsplit(const MX& x,
                                   const std::vector<casadi_int>& offset1,
                                   const std::vector<casadi_int>& offset2);
  static std::vector<MX> vertsplit(const MX& x,
                                   const std::vector<casadi_int>& offset);
  ...
  static std::vector<std::vector<MX> > forward(
      const std::vector<MX>& ex, const std::vector<MX>& arg,
      const std::vector<std::vector<MX> >& v, const Dict& opts = Dict());
  static std::vector<std::vector<MX> > reverse(
      const std::vector<MX>& ex, const std::vector<MX>& arg,
      const std::vector<std::vector<MX> >& v, const Dict& opts = Dict());
  ...
  static void substitute_inplace(const std::vector<MX>& v,
                                 std::vector<MX>& vdef, std::vector<MX>& ex,
                                 bool reverse);
  ...
  static MX inv(const MX& A, const std::string& lsolver = "qr",
                const Dict& dict = Dict());
  ...
  static std::string print_operator(const MX& x,
  ...
  static MX if_else(const MX& cond, const MX& if_true, const MX& if_false,
                    bool short_circuit = false);
  static MX conditional(const MX& ind, const std::vector<MX>& x,
  ...
  static MX bspline(const MX& x, const DM& coeffs,
   ...
   * Accepts lookup_mode option.
      ...
      \identifier{r6} */
  static MX interpn_linear(const std::vector<MX>& x, const MX& v,
  ...
  */
   ...
   * If failed, returns the number of rows
      ...
      \identifier{r7} */
  inline friend MX find(const MX& x) { return MX::find(x); }
   ...
   * multiple expressions, preserving nodes
      ...
      \identifier{ra} */
  inline friend std::vector<MX> graph_substitute(const std::vector<MX>& ex,
                                                 const std::vector<MX>& v,
                                                 const std::vector<MX>& vdef) {
    return MX::graph_substitute(ex, v, vdef);
  }
   ...
   *
      \identifier{rb} */
  inline friend MX matrix_expand(
      const MX& e, const std::vector<MX>& boundary = std::vector<MX>(),
   ...
   *
      \identifier{rc} */
  inline friend std::vector<MX> matrix_expand(
  ...
  inline friend MX bspline(const MX& x, const DM& coeffs,
                           const std::vector<std::vector<double> >& knots,
                           const std::vector<casadi_int>& degree, casadi_int m,
                           const Dict& opts = Dict()) {
  ...
  inline friend MX convexify(const MX& H, const Dict& opts = Dict()) {
   ...
   *
      \identifier{re} */
  inline friend MX inv_node(const MX& x) { return MX::inv_node(x); }
  ...
  * An error is raised when the expression contains symbols
      ...
      \identifier{rf} */
  inline friend DM evalf(const MX& expr) { return MX::evalf(expr); }
  ...
  /** \brief Stop first derivatives of an expression wrt to all its symbolic
   variables
   *
   * \seealso stop_diff

      \identifier{25m} */
  inline friend MX no_grad(const MX& expr) { return MX::stop_diff(expr, 1); }

  /** \brief Stop second derivatives of an expression wrt to all its symbolic
   variables
   *
   * \seealso stop_diff

      \identifier{25n} */
  inline friend MX no_hess(const MX& expr) { return MX::stop_diff(expr, 2); }

  /** \brief Stop derivatives of an expression wrt to a select set of symbolic
     variables
...
/** @} */
   ...
   */
  friend std::vector<MX> difference(const std::vector<MX>& a,
  ...
  * Returns itself, but with the side effect of printing the nonzeros along with
  a comment
  ...
  /** \brief Evaluate the MX node with new symbolic dependencies
      ...
      \identifier{rn} */
  void eval_mx(const std::vector<MX>& arg,
               std::vector<MX>& SWIG_OUTPUT(res)) const;
 ...
 private:
  /// Create an expression from a node: extra dummy arguments to avoid ambiguity
  /// for 0/NULL
