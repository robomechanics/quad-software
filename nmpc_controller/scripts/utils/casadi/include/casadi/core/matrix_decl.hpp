...
/** \brief Sparse matrix class. SX and DM are specializations.
   ...
   is a matrix", that is, also scalars and vectors.\n This philosophy makes it
   easy to use and to interface in particularly with Python and Matlab/Octave.\n
    ...
    The storage format is Compressed Column Storage (CCS), similar to that used
   for sparse matrices in Matlab, \n but unlike this format, we do allow for
   elements to be structurally non-zero but numerically zero.\n
...
class CASADI_EXPORT Matrix
    : public MatrixCommon,
      public SWIG_IF_ELSE(GenericExpressionCommon,
                          GenericExpression<Matrix<Scalar>>),
 ...
 public:
  ...
  Matrix(const std::vector<A>& x)
      : sparsity_(Sparsity::dense(x.size(), 1)),
  ...
  void get(Matrix<Scalar>& SWIG_OUTPUT(m), bool ind1,
           const Matrix<casadi_int>& rr) const;
  void get(Matrix<Scalar>& SWIG_OUTPUT(m), bool ind1, const Sparsity& sp) const;
  ///@}

  /// Get a submatrix, two arguments
  ///@{
  void get(Matrix<Scalar>& SWIG_OUTPUT(m), bool ind1, const Slice& rr,
  ...
  void get_nz(Matrix<Scalar>& SWIG_OUTPUT(m), bool ind1,
              const Matrix<casadi_int>& k) const;
  ///@}
  ...
  /** \brief  Create nodes by their ID
      ...
      \identifier{18o} */
  static Matrix<Scalar> binary(casadi_int op, const Matrix<Scalar>& x,
                               const Matrix<Scalar>& y);
  static Matrix<Scalar> unary(casadi_int op, const Matrix<Scalar>& x);
  static Matrix<Scalar> scalar_matrix(casadi_int op, const Matrix<Scalar>& x,
                                      const Matrix<Scalar>& y);
  static Matrix<Scalar> matrix_scalar(casadi_int op, const Matrix<Scalar>& x,
                                      const Matrix<Scalar>& y);
  static Matrix<Scalar> matrix_matrix(casadi_int op, const Matrix<Scalar>& x,
                                      const Matrix<Scalar>& y);
  ///@}
  /// \endcond
  ...
  static bool is_equal(const Matrix<Scalar>& x, const Matrix<Scalar>& y,
                       casadi_int depth = 0);
  ...
  static Matrix<Scalar> jacobian(const Matrix<Scalar>& f,
                                 const Matrix<Scalar>& x,
                                 const Dict& opts = Dict());
  static Sparsity jacobian_sparsity(const Matrix<Scalar>& f,
                                    const Matrix<Scalar>& x);
  static Matrix<Scalar> hessian(const Matrix<Scalar>& f,
                                const Matrix<Scalar>& x,
                                const Dict& opts = Dict());
  static Matrix<Scalar> hessian(const Matrix<Scalar>& f,
                                const Matrix<Scalar>& x, Matrix<Scalar>& g,
                                const Dict& opts = Dict());
  static Matrix<Scalar> substitute(const Matrix<Scalar>& ex,
                                   const Matrix<Scalar>& v,
                                   const Matrix<Scalar>& vdef);
  static std::vector<Matrix<Scalar>> substitute(
  ...
  static void substitute_inplace(const std::vector<Matrix<Scalar>>& v,
                                 ...
                                 std::vector<Matrix<Scalar>>& ex, bool revers);
  ...
  static Matrix<Scalar> expm_const(const Matrix<Scalar>& A,
  ...
  static Matrix<Scalar> inv(const Matrix<Scalar>& A, const std::string& lsolver,
                            const Dict& opts);

  static casadi_int n_nodes(const Matrix<Scalar>& x);
  static std::string print_operator(const Matrix<Scalar>& x,
                                    const std::vector<std::string>& args);
  static void extract(std::vector<Matrix<Scalar>>& ex,
  ...
  static Matrix<Scalar> _bilin(const Matrix<Scalar>& A, const Matrix<Scalar>& x,
                               const Matrix<Scalar>& y);
  ...
  static Matrix<Scalar> mrdivide(const Matrix<Scalar>& x,
                                 const Matrix<Scalar>& y);
  static Matrix<Scalar> mldivide(const Matrix<Scalar>& x,
                                 const Matrix<Scalar>& y);
  ...
  static Matrix<Scalar> project(const Matrix<Scalar>& x, const Sparsity& sp,
                                bool intersect = false);
  static Matrix<Scalar> polyval(const Matrix<Scalar>& p,
                                const Matrix<Scalar>& x);
  static Matrix<Scalar> densify(const Matrix<Scalar>& x,
                                const Matrix<Scalar>& val);
  static Matrix<Scalar> densify(const Matrix<Scalar>& x);
  static Matrix<Scalar> einstein(
  ...
  static Matrix<Scalar> einstein(const Matrix<Scalar>& A,
  ...
  static Matrix<Scalar> blockcat(
      const std::vector<std::vector<Matrix<Scalar>>>& v);
  static Matrix<Scalar> horzcat(const std::vector<Matrix<Scalar>>& v);
  static std::vector<Matrix<Scalar>> horzsplit(
      const Matrix<Scalar>& x, const std::vector<casadi_int>& offset);
  static Matrix<Scalar> vertcat(const std::vector<Matrix<Scalar>>& v);
  static std::vector<Matrix<Scalar>> vertsplit(
      const Matrix<Scalar>& x, const std::vector<casadi_int>& offset);
  static std::vector<Matrix<Scalar>> diagsplit(
      const Matrix<Scalar>& x, const std::vector<casadi_int>& offset1,
      const std::vector<casadi_int>& offset2);
  static Matrix<Scalar> reshape(const Matrix<Scalar>& x, casadi_int nrow,
                                casadi_int ncol);
  static Matrix<Scalar> reshape(const Matrix<Scalar>& x, const Sparsity& sp);
  static Matrix<Scalar> sparsity_cast(const Matrix<Scalar>& x,
                                      const Sparsity& sp);
  static Matrix<Scalar> kron(const Matrix<Scalar>& x, const Matrix<Scalar>& y);
  static Matrix<Scalar> mtimes(const Matrix<Scalar>& x,
                               const Matrix<Scalar>& y);
  static Matrix<Scalar> mac(const Matrix<Scalar>& x, const Matrix<Scalar>& y,
  ...
  static void expand(const Matrix<Scalar>& x, Matrix<Scalar>& weights,
                     Matrix<Scalar>& terms);
  ...
  static Matrix<Scalar> gauss_quadrature(const Matrix<Scalar>& f,
                                         ...
                                         const Matrix<Scalar>& b,
                                         casadi_int order = 5);
  static Matrix<Scalar> gauss_quadrature(
      const Matrix<Scalar>& f, const Matrix<Scalar>& x, const Matrix<Scalar>& a,
      const Matrix<Scalar>& b, casadi_int order, const Matrix<Scalar>& w);
  static std::vector<std::vector<Matrix<Scalar>>> forward(
      const std::vector<Matrix<Scalar>>& ex,
      const std::vector<Matrix<Scalar>>& arg,
      const std::vector<std::vector<Matrix<Scalar>>>& v,
      const Dict& opts = Dict());
  static std::vector<std::vector<Matrix<Scalar>>> reverse(
      const std::vector<Matrix<Scalar>>& ex,
      const std::vector<Matrix<Scalar>>& arg,
      const std::vector<std::vector<Matrix<Scalar>>>& v,
      const Dict& opts = Dict());
  static std::vector<bool> which_depends(const Matrix<Scalar>& expr,
                                         const Matrix<Scalar>& var,
                                         casadi_int order = 1, bool tr = false);
  static Matrix<Scalar> taylor(const Matrix<Scalar>& ex,
  ...
  static Matrix<Scalar> mtaylor(
      const Matrix<Scalar>& ex, const Matrix<Scalar>& x,
      const Matrix<Scalar>& a, casadi_int order,
      const std::vector<casadi_int>& order_contributions);
  static Matrix<Scalar> poly_coeff(const Matrix<Scalar>& ex,
                                   const Matrix<Scalar>& x);
  ...
  static void qr_sparse(const Matrix<Scalar>& A, Matrix<Scalar>& V,
                        Matrix<Scalar>& R, Matrix<Scalar>& beta,
                        std::vector<casadi_int>& prinv,
                        std::vector<casadi_int>& pc, bool amd = true);
  static Matrix<Scalar> qr_solve(
      const Matrix<Scalar>& b, const Matrix<Scalar>& v, const Matrix<Scalar>& r,
      const Matrix<Scalar>& beta, const std::vector<casadi_int>& prinv,
      const std::vector<casadi_int>& pc, bool tr = false);
  static void qr(const Matrix<Scalar>& A, Matrix<Scalar>& Q, Matrix<Scalar>& R);
  static void ldl(const Matrix<Scalar>& A, Matrix<Scalar>& D,
                  Matrix<Scalar>& LT, std::vector<casadi_int>& p,
                  bool amd = true);
  static Matrix<Scalar> ldl_solve(const Matrix<Scalar>& b,
  ...
  static Matrix<Scalar> minor(const Matrix<Scalar>& x, casadi_int i,
                              casadi_int j);
  static Matrix<Scalar> cofactor(const Matrix<Scalar>& A, casadi_int i,
                                 casadi_int j);
  static Matrix<Scalar> chol(const Matrix<Scalar>& A);
  static Matrix<Scalar> norm_inf_mul(const Matrix<Scalar>& x,
                                     const Matrix<Scalar>& y);
  ...
  */
  ...
  /** \brief Get the (i,j) minor matrix
      ...
      \identifier{18q} */
  friend inline Matrix<Scalar> minor(const Matrix<Scalar>& x, casadi_int i,
                                     casadi_int j) {
  ...
  /** \brief Get the (i,j) cofactor matrix
      ...
      \identifier{18r} */
  friend inline Matrix<Scalar> cofactor(const Matrix<Scalar>& x, casadi_int i,
                                        casadi_int j) {
   ...
   * More stable than the classical Gram-Schmidt, but may break down if the rows
   of A
   ...
   * Note that in SWIG, Q and R are returned by value.
      ...
      \identifier{18s} */
  friend inline void qr(const Matrix<Scalar>& A, Matrix<Scalar>& Q,
                        Matrix<Scalar>& R) {
   ...
   * See T. Davis: Direct Methods for Sparse Linear Systems
      ...
      \identifier{18t} */
  friend inline void qr_sparse(const Matrix<Scalar>& A, Matrix<Scalar>& V,
                               Matrix<Scalar>& R, Matrix<Scalar>& beta,
                               std::vector<casadi_int>& prinv,
                               std::vector<casadi_int>& pc, bool amd = true) {
  ...
  /** \brief Solve using a sparse QR factorization
      ...
      \identifier{18u} */
  friend inline Matrix<Scalar> qr_solve(
      const Matrix<Scalar>& b, const Matrix<Scalar>& v, const Matrix<Scalar>& r,
      const Matrix<Scalar>& beta, const std::vector<casadi_int>& prinv,
      const std::vector<casadi_int>& pc, bool tr = false) {
   ...
   * Only guarenteed to work for positive definite matrices.
      ...
      \identifier{18w} */
  friend inline void ldl(const Matrix<Scalar>& A, Matrix<Scalar>& D,
                         Matrix<Scalar>& LT, std::vector<casadi_int>& p,
                         bool amd = true) {
  ...
  /** \brief Solve using a sparse LDL^T factorization
      ...
      \identifier{18x} */
  friend inline Matrix<Scalar> ldl_solve(const Matrix<Scalar>& b,
                                         const Matrix<Scalar>& D,
                                         const Matrix<Scalar>& LT,
                                         const std::vector<casadi_int>& p) {
  ...
  /** \brief Inf-norm of a Matrix-Matrix product
      ...
      \identifier{190} */
  friend inline Matrix<Scalar> norm_inf_mul(const Matrix<Scalar>& x,
                                            const Matrix<Scalar>& y) {
  ...
  /** \brief  Make a matrix sparse by removing numerical zeros
      ...
      \identifier{191} */
  friend inline Matrix<Scalar> sparsify(const Matrix<Scalar>& A,
                                        double tol = 0) {
  ...
  /** \brief Create a piecewise constant function
      ...
      \param tval vector with the discrete values of t at the interval
     transitions (length n-1) \param val vector with the value of the function
     for each interval (length n)
  ...
  /** Create a piecewise linear function
      ...
      \brief tval vector with the the discrete values of t (monotonically
     increasing) \brief val vector with the corresponding function values (same
     length as tval)

                                                                                                              \identifier{194} */
  friend inline Matrix<Scalar> pw_lin(const Matrix<Scalar>& t,
                                      const Matrix<Scalar>& tval,
                                      const Matrix<Scalar>& val) {
  ...
  /** \brief  Integrate f from a to b using Gaussian quadrature with n points
      ...
      \identifier{196} */
  friend inline Matrix<Scalar> gauss_quadrature(const Matrix<Scalar>& f,
                                                const Matrix<Scalar>& x,
                                                const Matrix<Scalar>& a,
                                                const Matrix<Scalar>& b,
                                                casadi_int order = 5) {
  ...
  friend inline Matrix<Scalar> gauss_quadrature(
      const Matrix<Scalar>& f, const Matrix<Scalar>& x, const Matrix<Scalar>& a,
      const Matrix<Scalar>& b, casadi_int order, const Matrix<Scalar>& w) {
   ...
   *
   * \f$(x)=f(a)+f'(a)(x-a)+f''(a)\frac
   {(x-a)^2}{2!}+f'''(a)\frac{(x-a)^3}{3!}+\ldots\f$
   *
   * Example usage:
   * \code
   * taylor(sin(x), x)
   ...
   * \verbatim >>   x \endverbatim
      ...
      \identifier{23q} */
  friend inline Matrix<Scalar> taylor(const Matrix<Scalar>& ex,
                                      const Matrix<Scalar>& x,
                                      const Matrix<Scalar>& a,
                                      casadi_int order = 1) {
   ...
   *
      ...
      \identifier{23r} */
  friend inline Matrix<Scalar> mtaylor(const Matrix<Scalar>& ex,
                                       const Matrix<Scalar>& x,
                                       const Matrix<Scalar>& a,
                                       casadi_int order = 1) {
   ...
   * The argument order_contributions can denote how match each variable
   contributes
   * to the aggregated order. If x=[x, y] and order_contributions=[1, 2], then
   the
   ...
   *
      ...
      \identifier{23s} */
  friend inline Matrix<Scalar> mtaylor(
      const Matrix<Scalar>& ex, const Matrix<Scalar>& x,
      const Matrix<Scalar>& a, casadi_int order,
      const std::vector<casadi_int>& order_contributions) {
...
/** @} */
  ...
  /** \brief Set or reset the depth to which equalities are being checked for
     simplifications

      \identifier{19b} */
  static void set_max_depth(casadi_int eq_depth = 1);

  /** \brief Get the depth to which equalities are being checked for
     simplifications
...
#ifndef SWIG
  /// Print default style
  static void print_default(std::ostream& stream, const Sparsity& sp,
                            const Scalar* nonzeros, bool truncate = true);

  /// Print scalar
  static void print_scalar(std::ostream& stream, const Scalar& e);

  /// Print vector-style
  static void print_vector(std::ostream& stream, const Sparsity& sp,
                           const Scalar* nonzeros, bool truncate = true);

  /// Print scalar
  static void print_sparse(std::ostream& stream, const Sparsity& sp,
                           const Scalar* nonzeros, bool truncate = true);

  /// Get strings corresponding to the nonzeros and the interdependencies
  static void print_split(casadi_int nnz, const Scalar* nonzeros,
                          std::vector<std::string>& nz,
                          std::vector<std::string>& inter);

  /// Print dense matrix-stype
  static void print_dense(std::ostream& stream, const Sparsity& sp,
                          const Scalar* nonzeros, bool truncate = true);
#endif
  ...
  /** \brief Erase a submatrix (leaving structural zeros in its place)
      ...
      \identifier{19f} */
  void erase(const std::vector<casadi_int>& rr,
  ...
  /** \brief Remove columns and rows
      ...
      \identifier{19h} */
  void remove(const std::vector<casadi_int>& rr,
  ...
  friend inline const Scalar* get_ptr(const Matrix<Scalar>& v) {
    return v.ptr();
  }
  ///@}
  ...
  static Matrix<Scalar> triplet(const std::vector<casadi_int>& row,
                                const std::vector<casadi_int>& col,
                                const Matrix<Scalar>& d, casadi_int nrow,
                                casadi_int ncol);
  static Matrix<Scalar> triplet(const std::vector<casadi_int>& row,
                                const std::vector<casadi_int>& col,
                                const Matrix<Scalar>& d,
  ...
  /** \brief Detect duplicate symbolic expressions
      ...
      If there are symbolic primitives appearing more than once, the function
     will return true and the names of the duplicate expressions will be passed
     to casadi_warning. Note: Will mark the node using SXElem::set_temp. Make
     sure to call reset_input() after usage.
  ...
  /** \brief Check if the matrix is constant (note that false negative answers
     are possible)
  ...
  /** \brief  check if the matrix is 0 (note that false negative answers are
     possible)

      \identifier{19x} */
  bool is_zero() const;

  /** \brief  check if the matrix is 1 (note that false negative answers are
     possible)
  ...
  /** \brief  check if the matrix is an identity matrix (note that false
   negative answers
  ...
  /** \brief  Check if the matrix has any zero entries which are not structural
     zeros
  ...
  // @{
  /// Set the 'precision, width & scientific' used in printing and serializing
  /// to streams
  ...
  static Matrix<Scalar> rand(  // NOLINT(runtime/threadsafe_fn)
      casadi_int nrow = 1, casadi_int ncol = 1);
  static Matrix<Scalar> rand(
      const Sparsity& sp);     // NOLINT(runtime/threadsafe_fn)
  static Matrix<Scalar> rand(  // NOLINT(runtime/threadsafe_fn)
      const std::pair<casadi_int, casadi_int>& rc);
  ///@}
   ...
   * \verbatim
   * options:
   *   inline: Indicates if you want everything on a single line (default:
   False)
  ...
  static Matrix<double> from_file(const std::string& filename,
