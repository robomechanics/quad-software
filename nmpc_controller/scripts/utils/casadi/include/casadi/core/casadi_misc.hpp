...
class scoped_checkout {
 public:
  scoped_checkout(const T &proto) : proto_(proto) { mem = proto_.checkout(); }
  ...
  operator casadi_int() const { return mem; }
...
*
     \identifier{1l6} */
CASADI_EXPORT std::vector<casadi_int> range(
 ...
 *
    \identifier{1l7} */
CASADI_EXPORT bool is_range(const std::vector<casadi_int> &v, casadi_int start,
                            casadi_int stop, casadi_int step = 1);

CASADI_EXPORT std::string join(const std::vector<std::string> &l,
...
CASADI_EXPORT std::string replace(const std::string &s, const std::string &p,
                                  const std::string &r);
...
CASADI_EXPORT std::vector<casadi_int> boolvec_to_index(
    const std::vector<bool> &v);

CASADI_EXPORT bool is_equally_spaced(const std::vector<double> &v);

/// Computes a mapping for a (dense) tensor permutation
CASADI_EXPORT std::vector<casadi_int> tensor_permute_mapping(
    const std::vector<casadi_int> &dims, const std::vector<casadi_int> &order);
...
*  \param i List of indices
     ...
     \identifier{1l9} */
template <typename T>
std::vector<T> vector_slice(const std::vector<T> &v,
...
/** \brief permute a list
    ...
    \identifier{1ld} */
template <typename T>
std::vector<T> permute(const std::vector<T> &a,
...
#define casadi_assert_in_range(v, lower, upper)                            \
  casadi_assert(in_range(v, lower, upper),                                 \
                "Out of bounds error. Got elements in range [" +           \
                    ...
                    str(*std::max_element(v.begin(), v.end())) +           \
                    "], which is outside the range [" + str(lower) + "," + \
                    str(upper) + ").")
...
#define casadi_assert_bounded(v, upper)                                \
  casadi_assert(in_range(v, upper),                                    \
                "Out of bounds error. Got elements in range [" +       \
                    str(*std::min_element(v.begin(), v.end())) + "," + \
                    str(*std::max_element(v.begin(), v.end())) +       \
                    "], which exceeds the upper bound " + str(upper) + ".")
...
* The result vector is guaranteed to be monotonously increasing
    ...
    \identifier{1lf} */
CASADI_EXPORT std::vector<casadi_int> complement(
...
*  Duplicates are treated by looking up last occurrence
    ...
    \identifier{1lg} */
CASADI_EXPORT std::vector<casadi_int> lookupvector(
    const std::vector<casadi_int> &v, casadi_int size);
CASADI_EXPORT std::vector<casadi_int> lookupvector(
...
void flatten_nested_vector(const std::vector<std::vector<T> > &nested,
                           std::vector<S> &flat, std::vector<I> &indices);
...
* \param[in] invert_indices Output indices such that
...
bool is_regular(const std::vector<T> &v) {
  for (auto &&vk : v) {
    if (vk != vk || vk == std::numeric_limits<T>::infinity() ||
        vk == -std::numeric_limits<T>::infinity())
      return false;
  }
  return true;
}
...
void linspace(std::vector<T> &v, const F &first, const L &last) {
  ...
  v[0] = first;
  for (unsigned i = 1; i < v.size() - 1; ++i) v[i] = v[i - 1] + increment;
  v[v.size() - 1] = last;
}
