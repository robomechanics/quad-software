 ...
 *
    \identifier{1sw} */
CASADI_EXPORT void detect_simple_bounds(
    const SX& xX, const SX& p, const SX& g, const SX& lbg, const SX& ubg,
    std::vector<casadi_int>& SWIG_OUTPUT(gi), SX& SWIG_OUTPUT(lbx),
    SX& SWIG_OUTPUT(ubx), Function& SWIG_OUTPUT(lam_forward),
    Function& SWIG_OUTPUT(lam_backward));
CASADI_EXPORT void detect_simple_bounds(
    const MX& xX, const MX& p, const MX& g, const MX& lbg, const MX& ubg,
    std::vector<casadi_int>& SWIG_OUTPUT(gi), MX& SWIG_OUTPUT(lbx),
    MX& SWIG_OUTPUT(ubx), Function& SWIG_OUTPUT(lam_forward),
    Function& SWIG_OUTPUT(lam_backward));
//@}
...
void check_sos(casadi_int nx, const std::vector<std::vector<T> >& groups,
  ...
  // Set default types
  if (!groups.empty() && types.empty()) types.resize(sos_num, 1);
  ...
  casadi_assert(types.size() == sos_num, "sos_types has incorrect size");
  ...
  for (casadi_int i = 0; i < weights.size(); ++i) {
    casadi_assert(weights[i].size() == groups[i].size(),
                  "Dimension mismatch in weights for group " + str(i) +
                      ": "
                      "Expected " +
                      str(groups[i].size()) + ", got " +
                      str(weights[i].size()));
  }

  // Checks on contents
  for (casadi_int t : types)
    casadi_assert(t == 1 || t == 2, "SOS type must be either 1 or 2.");
  for (const auto& v : groups)
    casadi_assert(in_range(v, 0, nx), "Index out of bound");
}
