...
/** \brief Function object
    ...
    Function is a reference counted and immutable class; copying a class
   instance is very cheap and its behavior (with some exceptions) is not
   affected by calling its member functions.\n
...
class CASADI_EXPORT Function
 ...
 public:
  ...
  /** \brief Construct an SX function
      ...
      \identifier{1v0} */
  Function(const std::string& name, const std::vector<SX>& ex_in,
           const std::vector<SX>& ex_out, const Dict& opts = Dict());
  Function(const std::string& name, const std::vector<SX>& ex_in,
           const std::vector<SX>& ex_out,
           const std::vector<std::string>& name_in,
           const std::vector<std::string>& name_out, const Dict& opts = Dict());
  ...
  /** \brief Construct an MX function
      ...
      \identifier{1v1} */
  Function(const std::string& name, const std::vector<MX>& ex_in,
           const std::vector<MX>& ex_out, const Dict& opts = Dict());
  Function(const std::string& name, const std::vector<MX>& ex_in,
   ...
   * The names and sparsity patterns of all the inputs and outputs must be
   provided.
   * If sparsities are not provided, all inputs and outputs are assumed to be
   scalar.
   ...
   * The data type used is 'casadi_real', which is typically equal to 'double`
   or
   ...
   *
   * casadi_int fname(const casadi_real** arg, casadi_real** res, casadi_int*
   iw, casadi_real* w, void* mem) {
  ...
  casadi_int size1_in(const std::string& iname) const {
    return size1_in(index_in(iname));
  }
  casadi_int size2_in(casadi_int ind) const;
  casadi_int size2_in(const std::string& iname) const {
    return size2_in(index_in(iname));
  }
  ...
  casadi_int size1_out(const std::string& oname) const {
    return size1_out(index_out(oname));
  }
  casadi_int size2_out(casadi_int ind) const;
  casadi_int size2_out(const std::string& oname) const {
    return size2_out(index_out(oname));
  }
  ...
  casadi_int numel_in(const std::string& iname) const {
    return numel_in(index_in(iname));
  }
  ///@}
  ...
  casadi_int numel_out(const std::string& oname) const {
    return numel_out(index_out(oname));
  }
  ///@}
  ...
  /** \brief Find the index for a string describing a particular entry of an
   input scheme
   *
  ...
  * \param[in] tr   Flip the relationship. Return which expressions contain the
  variables
  ...
  /** \brief Print all information there is to know about a certain option
      ...
      \identifier{1w0} */
  void print_option(const std::string& name,
                    std::ostream& stream = casadi::uout()) const;
  ...
  /** \brief Change option after object creation for debugging
    ...
    * This is only possible for a selected number of options that do not change
    the numerical
    * results of the computation, e.g. to enable a more verbose output or saving
    to file.
  ...
  /** \brief Evaluate with temporary memory allocation
      ...
      \identifier{1w8} */
  void operator()(std::vector<const double*> arg,
                  std::vector<double*> res) const;
  void operator()(std::vector<const bvec_t*> arg,
                  std::vector<bvec_t*> res) const;
  void operator()(std::vector<const SXElem*> arg,
                  std::vector<SXElem*> res) const;
  template <typename D>
  ...
  /** \brief Evaluate numerically with checkout/release
      ...
      \identifier{1wc} */
  int operator()(const double** arg, double** res, casadi_int* iw,
                 double* w) const;
  ...
  /** \brief  Propagate sparsity backward
      ...
      \identifier{1wf} */
  int rev(bvec_t** arg, bvec_t** res, casadi_int* iw, bvec_t* w,
          int mem = 0) const;
  ...
  /** \brief  Create a mapaccumulated version of this function
      ...
      The effect is similar to that of a for-loop with a check-pointing
     instruction after each chunk of iterations with size base.
  ...
  /** \brief Map with reduction
    ...
    up.
      ...
      \identifier{1wk} */
  Function map(const std::string& name, const std::string& parallelization,
               casadi_int n, const std::vector<casadi_int>& reduce_in,
               const std::vector<casadi_int>& reduce_out,
               const Dict& opts = Dict()) const;
  Function map(const std::string& name, const std::string& parallelization,
               casadi_int n, const std::vector<std::string>& reduce_in,
               const std::vector<std::string>& reduce_out,
               const Dict& opts = Dict()) const;
  Function map(casadi_int n, const std::vector<bool>& reduce_in,
               const std::vector<bool>& reduce_out = std::vector<bool>(),
               const Dict& opts = Dict()) const;
  ///@}

  /** \brief returns a new function with a selection of inputs/outputs of the
     original

      \identifier{1wl} */
  Function slice(const std::string& name,
  ...
  /** \brief Conditional call to a function
      ...
      \identifier{1wn} */
  static Function conditional(const std::string& name, const Function& f,
                              const Dict& opts = Dict());
   ...
   *         The first <tt>n_in</tt> inputs correspond to nondifferentiated
   inputs.
   *         The next <tt>n_out</tt> inputs correspond to nondifferentiated
   outputs.
   ...
   *
   *        The functions returned are cached, meaning that if called multiple
   timed
   *        with the same value, then multiple references to the same function
   will be returned.
  ...
  /** \brief Get, if necessary generate, the sparsity of a single Jacobian block
      ...
      \identifier{1wt} */
  Sparsity jac_sparsity(casadi_int oind, casadi_int iind,
                        bool compact = false) const;

  /** \brief Export / Generate C code for the function

      \identifier{1wu} */
  std::string generate(const std::string& fname,
                       const Dict& opts = Dict()) const;
  ...
  /** \brief Export / Generate C code for the dependency function
      ...
      \identifier{1ww} */
  std::string generate_dependencies(const std::string& fname,
                                    const Dict& opts = Dict()) const;

  /** \brief Export an input file that can be passed to generate C code with a
   main
   *
   * \see generate_out
  ...
  /** \brief Export an output file that can be checked with generated C code
   output
   *
   * \see generate_in
   ...
   * Only allowed for (a subset of) SX/MX Functions
      ...
      \identifier{1wz} */
  ///@{
  void export_code(const std::string& lang, const std::string& fname,
  ...
  std::string export_code(const std::string& lang,
                          const Dict& options = Dict()) const;
#ifndef SWIG
  ...
  * Does not perform sparsity checking.
  ...
  /** \brief Get the floating point output argument of an instruction
     (SXFunction)
  ...
  /** \brief Get number of temporary variables needed
      ...
      \identifier{1xq} */
  void sz_work(size_t& sz_arg, size_t& sz_res, size_t& sz_iw,
               size_t& sz_w) const;
  ...
  /** \brief Set the (temporary) work vectors
      ...
      \identifier{1xs} */
  void set_temp(const double** arg, double** res, casadi_int* iw, double* w,
                int mem = 0) const;

  /** \brief Set the (persistent and temporary) work vectors

      \identifier{1xt} */
  void setup(const double** arg, double** res, casadi_int* iw, double* w,
             int mem = 0) const;
  ...
  /** \brief Call using a map
      ...
      \identifier{1xu} */
  template <typename M>
  void call_gen(const std::map<std::string, M>& arg,
                std::map<std::string, M>& res, bool always_inline,
   ...
   * - Underscore, but never as first character and never next to another
   underscore
   *
  ...
  void assert_sparsity_out(casadi_int i, const Sparsity& sp, casadi_int n = 1,
                           bool allow_all_zero_sparse = true) const;
  ...
  /** \brief Get all functions embedded in the expression graphs
    ...
    * \param[in] max_depth  Maximum depth - a negative number indicates no
    maximum
 ...
 protected:
  ...
  /** \brief Called by constructors
      ...
      \identifier{1y8} */
  void construct(const std::string& name, const std::vector<SX>& ex_in,
                 const std::vector<SX>& ex_out,
                 const std::vector<std::string>& name_in,
                 const std::vector<std::string>& name_out, const Dict& opts);
  void construct(const std::string& name, const std::vector<MX>& ex_in,
                 const std::vector<MX>& ex_out,
                 const std::vector<std::string>& name_in,
                 const std::vector<std::string>& name_out, const Dict& opts);
  template <typename M>
 ...
 public:
  ...
  // operator()(const double** arg, double** res, casadi_int* iw, double* w, int
  // mem)?
