...
// dll-interface class 'casadi::CasadiException'
...
// warning C4910: __declspec(dllexport) and extern incompatible on an explicit
// instantiation
...
// Type with a size corresponding to that of double (or smaller) that can be
// used to hold a set of booleans
typedef unsigned long long bvec_t;

// Number of directions we can deal with at a time
// the size of bvec_t in bits (CHAR_BIT is the number of bits per byte, usually
// 8)
...
inline std::vector<std::string> strvec() { return {}; }
...
inline std::string fmtstr(const std::string& fmt,
