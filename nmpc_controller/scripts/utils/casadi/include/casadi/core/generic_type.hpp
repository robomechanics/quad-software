...
/** \brief Generic data type, can hold different types such as bool, casadi_int,
   std::string etc.
...
#ifndef SWIG
template <class T>
T get_from_dict(const std::map<std::string, T>& d, const std::string& key,
 ...
 *
    \identifier{17t} */
CASADI_EXPORT Dict combine(const Dict& first, const Dict& second,
                           bool recurse = false);

/** \brief Update the target dictionary in place with source elements

    \identifier{17u} */
CASADI_EXPORT void update_dict(Dict& target, const Dict& source,
                               bool recurse = false);
#endif
