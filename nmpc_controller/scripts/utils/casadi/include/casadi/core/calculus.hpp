...
inline double printme(double x, double y) {
  ...
  uout() << "|> " << y << " : ";
  uout() << std::setprecision(std::numeric_limits<double>::digits10 + 1)
         << std::scientific;
  uout() << x << std::endl;
  uout().flags(f);
  return x;
}
inline bool is_equal(double x, double y, casadi_int depth = 0) {
  return x == y;
}

// Integer maximum and minimum
inline casadi_int casadi_max(casadi_int x, casadi_int y) {
  return std::max(x, y);
}
inline casadi_int casadi_min(casadi_int x, casadi_int y) {
  return std::min(x, y);
}
...
inline double erfinv(double x) throw() {
  ...
  if (x >= 1) {
    ...
    return -(((1.641345311 * z + 3.429567803) * z - 1.624906493) * z -
             1.970840454) /
    ...
    if (x < 0.7) {
      ...
      y = x *
          (((-0.140543331 * z + 0.914624893) * z - 1.645349621) * z +
           0.886226899) /
          ((((-0.329097515 * z + 0.012229801) * z + 1.442710462) * z -
            2.118377725) *
               z +
           1.0);
    } else {
...
struct UnaryOperation {
  /// Function evaluation
  template <typename T>
  static inline void fcn(const T& x, T& f);

  /// Partial derivatives
  template <typename T>
  static inline void der(const T& x, const T& f, T* d);
};
...
struct BinaryOperation {
  /// Function evaluation
  template <typename T>
  static inline void fcn(const T& x, const T& y, T& f) {
    UnaryOperation<I>::fcn(x, f);
  }

  /// Partial derivatives - binary function
  template <typename T>
  static inline void der(const T& x, const T& y, const T& f, T* d) {
    UnaryOperation<I>::der(x, f, d);
    d[1] = 0;
  }
};
...
struct BinaryOperationE {
  /// Function evaluation
  template <typename T>
  static inline T fcn(const T& x, const T& y) {
    T ret;
...
struct DerBinaryOperation {
  ...
  template <typename T>
  static inline void derf(const T& x, const T& y, T& f, T* d) {
...
struct BinaryOperationSS {
  /// Function evaluation
  template <typename T>
  static inline void fcn(const T& x, const T& y, T& f, casadi_int n) {
...
struct casadi_math {
  ...
  /** \brief Evaluate a built in derivative function
      ...
      \identifier{1ga} */
  static inline void der(unsigned char op, const T& x, const T& y, const T& f,
                         T* d);
...
struct casadi_math<casadi_int> {
  ...
  static inline void fun(unsigned char op, const casadi_int& x,
    ...
    double ff(0);
    casadi_math<double>::fun(op, static_cast<double>(x), static_cast<double>(y),
                             ff);
    f = static_cast<casadi_int>(ff);
  }

  static inline void fun(unsigned char op, const casadi_int* x,
                         const casadi_int* y, casadi_int* f, casadi_int n) {
    for (casadi_int i = 0; i < n; ++i) {
      double ff(0);
      casadi_math<double>::fun(op, static_cast<double>(*x++),
  ...
  /** \brief Evaluate a built in derivative function
      ...
      \identifier{1gh} */
  static inline void der(unsigned char op, const casadi_int& x,
                         const casadi_int& y, const casadi_int& f,
                         casadi_int* d) {
  ...
  /** \brief Evaluate the function and the derivative function
      ...
      \identifier{1gi} */
  static inline void derF(unsigned char op, const casadi_int& x,
                          const casadi_int& y, casadi_int& f, casadi_int* d) {
    double d_real[2] = {static_cast<double>(d[0]), static_cast<double>(d[1])};
    double f_real = static_cast<double>(f);
    casadi_math<double>::derF(op, static_cast<double>(x),
...
template <typename T>
inline void casadi_math<T>::fun(unsigned char op, const T& x, const T& y,
                                T& f) {
  // NOTE: We define the implementation in a preprocessor macro to be able to
  // force inlining,
  //  and to allow extensions in the VM
#define CASADI_MATH_FUN_BUILTIN_GEN(CNAME, X, Y, F, N) \
  case OP_ASSIGN:                                      \
    CNAME<OP_ASSIGN>::fcn(X, Y, F, N);                 \
    break;                                             \
  case OP_ADD:                                         \
    CNAME<OP_ADD>::fcn(X, Y, F, N);                    \
    break;                                             \
  case OP_SUB:                                         \
    CNAME<OP_SUB>::fcn(X, Y, F, N);                    \
    break;                                             \
  case OP_MUL:                                         \
    CNAME<OP_MUL>::fcn(X, Y, F, N);                    \
    break;                                             \
  case OP_DIV:                                         \
    CNAME<OP_DIV>::fcn(X, Y, F, N);                    \
    break;                                             \
  case OP_NEG:                                         \
    CNAME<OP_NEG>::fcn(X, Y, F, N);                    \
    break;                                             \
  case OP_EXP:                                         \
    CNAME<OP_EXP>::fcn(X, Y, F, N);                    \
    break;                                             \
  case OP_LOG:                                         \
    CNAME<OP_LOG>::fcn(X, Y, F, N);                    \
    break;                                             \
  case OP_POW:                                         \
    CNAME<OP_POW>::fcn(X, Y, F, N);                    \
    break;                                             \
  case OP_CONSTPOW:                                    \
    CNAME<OP_CONSTPOW>::fcn(X, Y, F, N);               \
    break;                                             \
  case OP_SQRT:                                        \
    CNAME<OP_SQRT>::fcn(X, Y, F, N);                   \
    break;                                             \
  case OP_SQ:                                          \
    CNAME<OP_SQ>::fcn(X, Y, F, N);                     \
    break;                                             \
  case OP_TWICE:                                       \
    CNAME<OP_TWICE>::fcn(X, Y, F, N);                  \
    break;                                             \
  case OP_SIN:                                         \
    CNAME<OP_SIN>::fcn(X, Y, F, N);                    \
    break;                                             \
  case OP_COS:                                         \
    CNAME<OP_COS>::fcn(X, Y, F, N);                    \
    break;                                             \
  case OP_TAN:                                         \
    CNAME<OP_TAN>::fcn(X, Y, F, N);                    \
    break;                                             \
  case OP_ASIN:                                        \
    CNAME<OP_ASIN>::fcn(X, Y, F, N);                   \
    break;                                             \
  case OP_ACOS:                                        \
    CNAME<OP_ACOS>::fcn(X, Y, F, N);                   \
    break;                                             \
  case OP_ATAN:                                        \
    CNAME<OP_ATAN>::fcn(X, Y, F, N);                   \
    break;                                             \
  case OP_LT:                                          \
    CNAME<OP_LT>::fcn(X, Y, F, N);                     \
    break;                                             \
  case OP_LE:                                          \
    CNAME<OP_LE>::fcn(X, Y, F, N);                     \
    break;                                             \
  case OP_EQ:                                          \
    CNAME<OP_EQ>::fcn(X, Y, F, N);                     \
    break;                                             \
  case OP_NE:                                          \
    CNAME<OP_NE>::fcn(X, Y, F, N);                     \
    break;                                             \
  case OP_NOT:                                         \
    CNAME<OP_NOT>::fcn(X, Y, F, N);                    \
    break;                                             \
  case OP_AND:                                         \
    CNAME<OP_AND>::fcn(X, Y, F, N);                    \
    break;                                             \
  case OP_OR:                                          \
    CNAME<OP_OR>::fcn(X, Y, F, N);                     \
    break;                                             \
  case OP_IF_ELSE_ZERO:                                \
    CNAME<OP_IF_ELSE_ZERO>::fcn(X, Y, F, N);           \
    break;                                             \
  case OP_FLOOR:                                       \
    CNAME<OP_FLOOR>::fcn(X, Y, F, N);                  \
    break;                                             \
  case OP_CEIL:                                        \
    CNAME<OP_CEIL>::fcn(X, Y, F, N);                   \
    break;                                             \
  case OP_FMOD:                                        \
    CNAME<OP_FMOD>::fcn(X, Y, F, N);                   \
    break;                                             \
  case OP_REMAINDER:                                   \
    CNAME<OP_REMAINDER>::fcn(X, Y, F, N);              \
    break;                                             \
  case OP_FABS:                                        \
    CNAME<OP_FABS>::fcn(X, Y, F, N);                   \
    break;                                             \
  case OP_SIGN:                                        \
    CNAME<OP_SIGN>::fcn(X, Y, F, N);                   \
    break;                                             \
  case OP_COPYSIGN:                                    \
    CNAME<OP_COPYSIGN>::fcn(X, Y, F, N);               \
    break;                                             \
  case OP_ERF:                                         \
    CNAME<OP_ERF>::fcn(X, Y, F, N);                    \
    break;                                             \
  case OP_FMIN:                                        \
    CNAME<OP_FMIN>::fcn(X, Y, F, N);                   \
    break;                                             \
  case OP_FMAX:                                        \
    CNAME<OP_FMAX>::fcn(X, Y, F, N);                   \
    break;                                             \
  case OP_INV:                                         \
    CNAME<OP_INV>::fcn(X, Y, F, N);                    \
    break;                                             \
  case OP_SINH:                                        \
    CNAME<OP_SINH>::fcn(X, Y, F, N);                   \
    break;                                             \
  case OP_COSH:                                        \
    CNAME<OP_COSH>::fcn(X, Y, F, N);                   \
    break;                                             \
  case OP_TANH:                                        \
    CNAME<OP_TANH>::fcn(X, Y, F, N);                   \
    break;                                             \
  case OP_ASINH:                                       \
    CNAME<OP_ASINH>::fcn(X, Y, F, N);                  \
    break;                                             \
  case OP_ACOSH:                                       \
    CNAME<OP_ACOSH>::fcn(X, Y, F, N);                  \
    break;                                             \
  case OP_ATANH:                                       \
    CNAME<OP_ATANH>::fcn(X, Y, F, N);                  \
    break;                                             \
  case OP_ATAN2:                                       \
    CNAME<OP_ATAN2>::fcn(X, Y, F, N);                  \
    break;                                             \
  case OP_ERFINV:                                      \
    CNAME<OP_ERFINV>::fcn(X, Y, F, N);                 \
    break;                                             \
  case OP_LIFT:                                        \
    CNAME<OP_LIFT>::fcn(X, Y, F, N);                   \
    break;                                             \
  case OP_PRINTME:                                     \
    CNAME<OP_PRINTME>::fcn(X, Y, F, N);                \
    break;                                             \
  case OP_LOG1P:                                       \
    CNAME<OP_LOG1P>::fcn(X, Y, F, N);                  \
    break;                                             \
  case OP_EXPM1:                                       \
    CNAME<OP_EXPM1>::fcn(X, Y, F, N);                  \
    break;                                             \
  case OP_HYPOT:                                       \
    CNAME<OP_HYPOT>::fcn(X, Y, F, N);                  \
    break;

#define CASADI_MATH_FUN_BUILTIN(X, Y, F) \
  CASADI_MATH_FUN_BUILTIN_GEN(BinaryOperationSS, X, Y, F, 1)

  switch (op) { CASADI_MATH_FUN_BUILTIN(x, y, f) }
}

template <typename T>
inline void casadi_math<T>::fun(unsigned char op, const T* x, const T* y, T* f,
                                casadi_int n) {
  switch (op) { CASADI_MATH_FUN_BUILTIN_GEN(BinaryOperationVV, x, y, f, n) }
}

template <typename T>
inline void casadi_math<T>::fun(unsigned char op, const T* x, const T& y, T* f,
                                casadi_int n) {
  switch (op) { CASADI_MATH_FUN_BUILTIN_GEN(BinaryOperationVS, x, y, f, n) }
}

template <typename T>
inline void casadi_math<T>::fun(unsigned char op, const T& x, const T* y, T* f,
                                casadi_int n) {
  switch (op) { CASADI_MATH_FUN_BUILTIN_GEN(BinaryOperationSV, x, y, f, n) }
}

template <typename T>
inline void casadi_math<T>::der(unsigned char op, const T& x, const T& y,
                                const T& f, T* d) {
  // NOTE: We define the implementation in a preprocessor macro to be able to
  // force inlining, and to allow extensions in the VM
#define CASADI_MATH_DER_BUILTIN(X, Y, F, D)            \
  case OP_ASSIGN:                                      \
    BinaryOperation<OP_ASSIGN>::der(X, Y, F, D);       \
    break;                                             \
  case OP_ADD:                                         \
    BinaryOperation<OP_ADD>::der(X, Y, F, D);          \
    break;                                             \
  case OP_SUB:                                         \
    BinaryOperation<OP_SUB>::der(X, Y, F, D);          \
    break;                                             \
  case OP_MUL:                                         \
    BinaryOperation<OP_MUL>::der(X, Y, F, D);          \
    break;                                             \
  case OP_DIV:                                         \
    BinaryOperation<OP_DIV>::der(X, Y, F, D);          \
    break;                                             \
  case OP_NEG:                                         \
    BinaryOperation<OP_NEG>::der(X, Y, F, D);          \
    break;                                             \
  case OP_EXP:                                         \
    BinaryOperation<OP_EXP>::der(X, Y, F, D);          \
    break;                                             \
  case OP_LOG:                                         \
    BinaryOperation<OP_LOG>::der(X, Y, F, D);          \
    break;                                             \
  case OP_POW:                                         \
    BinaryOperation<OP_POW>::der(X, Y, F, D);          \
    break;                                             \
  case OP_CONSTPOW:                                    \
    BinaryOperation<OP_CONSTPOW>::der(X, Y, F, D);     \
    break;                                             \
  case OP_SQRT:                                        \
    BinaryOperation<OP_SQRT>::der(X, Y, F, D);         \
    break;                                             \
  case OP_SQ:                                          \
    BinaryOperation<OP_SQ>::der(X, Y, F, D);           \
    break;                                             \
  case OP_TWICE:                                       \
    BinaryOperation<OP_TWICE>::der(X, Y, F, D);        \
    break;                                             \
  case OP_SIN:                                         \
    BinaryOperation<OP_SIN>::der(X, Y, F, D);          \
    break;                                             \
  case OP_COS:                                         \
    BinaryOperation<OP_COS>::der(X, Y, F, D);          \
    break;                                             \
  case OP_TAN:                                         \
    BinaryOperation<OP_TAN>::der(X, Y, F, D);          \
    break;                                             \
  case OP_ASIN:                                        \
    BinaryOperation<OP_ASIN>::der(X, Y, F, D);         \
    break;                                             \
  case OP_ACOS:                                        \
    BinaryOperation<OP_ACOS>::der(X, Y, F, D);         \
    break;                                             \
  case OP_ATAN:                                        \
    BinaryOperation<OP_ATAN>::der(X, Y, F, D);         \
    break;                                             \
  case OP_LT:                                          \
    BinaryOperation<OP_LT>::der(X, Y, F, D);           \
    break;                                             \
  case OP_LE:                                          \
    BinaryOperation<OP_LE>::der(X, Y, F, D);           \
    break;                                             \
  case OP_EQ:                                          \
    BinaryOperation<OP_EQ>::der(X, Y, F, D);           \
    break;                                             \
  case OP_NE:                                          \
    BinaryOperation<OP_NE>::der(X, Y, F, D);           \
    break;                                             \
  case OP_NOT:                                         \
    BinaryOperation<OP_NOT>::der(X, Y, F, D);          \
    break;                                             \
  case OP_AND:                                         \
    BinaryOperation<OP_AND>::der(X, Y, F, D);          \
    break;                                             \
  case OP_OR:                                          \
    BinaryOperation<OP_OR>::der(X, Y, F, D);           \
    break;                                             \
  case OP_IF_ELSE_ZERO:                                \
    BinaryOperation<OP_IF_ELSE_ZERO>::der(X, Y, F, D); \
    break;                                             \
  case OP_FLOOR:                                       \
    BinaryOperation<OP_FLOOR>::der(X, Y, F, D);        \
    break;                                             \
  case OP_CEIL:                                        \
    BinaryOperation<OP_CEIL>::der(X, Y, F, D);         \
    break;                                             \
  case OP_FMOD:                                        \
    BinaryOperation<OP_FMOD>::der(X, Y, F, D);         \
    break;                                             \
  case OP_REMAINDER:                                   \
    BinaryOperation<OP_REMAINDER>::der(X, Y, F, D);    \
    break;                                             \
  case OP_FABS:                                        \
    BinaryOperation<OP_FABS>::der(X, Y, F, D);         \
    break;                                             \
  case OP_SIGN:                                        \
    BinaryOperation<OP_SIGN>::der(X, Y, F, D);         \
    break;                                             \
  case OP_COPYSIGN:                                    \
    BinaryOperation<OP_COPYSIGN>::der(X, Y, F, D);     \
    break;                                             \
  case OP_ERF:                                         \
    BinaryOperation<OP_ERF>::der(X, Y, F, D);          \
    break;                                             \
  case OP_FMIN:                                        \
    BinaryOperation<OP_FMIN>::der(X, Y, F, D);         \
    break;                                             \
  case OP_FMAX:                                        \
    BinaryOperation<OP_FMAX>::der(X, Y, F, D);         \
    break;                                             \
  case OP_INV:                                         \
    BinaryOperation<OP_INV>::der(X, Y, F, D);          \
    break;                                             \
  case OP_SINH:                                        \
    BinaryOperation<OP_SINH>::der(X, Y, F, D);         \
    break;                                             \
  case OP_COSH:                                        \
    BinaryOperation<OP_COSH>::der(X, Y, F, D);         \
    break;                                             \
  case OP_TANH:                                        \
    BinaryOperation<OP_TANH>::der(X, Y, F, D);         \
    break;                                             \
  case OP_ASINH:                                       \
    BinaryOperation<OP_ASINH>::der(X, Y, F, D);        \
    break;                                             \
  case OP_ACOSH:                                       \
    BinaryOperation<OP_ACOSH>::der(X, Y, F, D);        \
    break;                                             \
  case OP_ATANH:                                       \
    BinaryOperation<OP_ATANH>::der(X, Y, F, D);        \
    break;                                             \
  case OP_ATAN2:                                       \
    BinaryOperation<OP_ATAN2>::der(X, Y, F, D);        \
    break;                                             \
  case OP_ERFINV:                                      \
    BinaryOperation<OP_ERFINV>::der(X, Y, F, D);       \
    break;                                             \
  case OP_LIFT:                                        \
    BinaryOperation<OP_LIFT>::der(X, Y, F, D);         \
    break;                                             \
  case OP_PRINTME:                                     \
    BinaryOperation<OP_PRINTME>::der(X, Y, F, D);      \
    break;                                             \
  case OP_LOG1P:                                       \
    BinaryOperation<OP_LOG1P>::der(X, Y, F, D);        \
    break;                                             \
  case OP_EXPM1:                                       \
    BinaryOperation<OP_EXPM1>::der(X, Y, F, D);        \
    break;                                             \
  case OP_HYPOT:                                       \
    BinaryOperation<OP_HYPOT>::der(X, Y, F, D);        \
    break;
  switch (op) { CASADI_MATH_DER_BUILTIN(x, y, f, d) }
}

template <typename T>
inline void casadi_math<T>::derF(unsigned char op, const T& x, const T& y, T& f,
                                 T* d) {
  // NOTE: We define the implementation in a preprocessor macro to be able to
  // force inlining, and to allow extensions in the VM
#define CASADI_MATH_DERF_BUILTIN(X, Y, F, D)               \
  case OP_ASSIGN:                                          \
    DerBinaryOperation<OP_ASSIGN>::derf(X, Y, F, D);       \
    break;                                                 \
  case OP_ADD:                                             \
    DerBinaryOperation<OP_ADD>::derf(X, Y, F, D);          \
    break;                                                 \
  case OP_SUB:                                             \
    DerBinaryOperation<OP_SUB>::derf(X, Y, F, D);          \
    break;                                                 \
  case OP_MUL:                                             \
    DerBinaryOperation<OP_MUL>::derf(X, Y, F, D);          \
    break;                                                 \
  case OP_DIV:                                             \
    DerBinaryOperation<OP_DIV>::derf(X, Y, F, D);          \
    break;                                                 \
  case OP_NEG:                                             \
    DerBinaryOperation<OP_NEG>::derf(X, Y, F, D);          \
    break;                                                 \
  case OP_EXP:                                             \
    DerBinaryOperation<OP_EXP>::derf(X, Y, F, D);          \
    break;                                                 \
  case OP_LOG:                                             \
    DerBinaryOperation<OP_LOG>::derf(X, Y, F, D);          \
    break;                                                 \
  case OP_POW:                                             \
    DerBinaryOperation<OP_POW>::derf(X, Y, F, D);          \
    break;                                                 \
  case OP_CONSTPOW:                                        \
    DerBinaryOperation<OP_CONSTPOW>::derf(X, Y, F, D);     \
    break;                                                 \
  case OP_SQRT:                                            \
    DerBinaryOperation<OP_SQRT>::derf(X, Y, F, D);         \
    break;                                                 \
  case OP_SQ:                                              \
    DerBinaryOperation<OP_SQ>::derf(X, Y, F, D);           \
    break;                                                 \
  case OP_TWICE:                                           \
    DerBinaryOperation<OP_TWICE>::derf(X, Y, F, D);        \
    break;                                                 \
  case OP_SIN:                                             \
    DerBinaryOperation<OP_SIN>::derf(X, Y, F, D);          \
    break;                                                 \
  case OP_COS:                                             \
    DerBinaryOperation<OP_COS>::derf(X, Y, F, D);          \
    break;                                                 \
  case OP_TAN:                                             \
    DerBinaryOperation<OP_TAN>::derf(X, Y, F, D);          \
    break;                                                 \
  case OP_ASIN:                                            \
    DerBinaryOperation<OP_ASIN>::derf(X, Y, F, D);         \
    break;                                                 \
  case OP_ACOS:                                            \
    DerBinaryOperation<OP_ACOS>::derf(X, Y, F, D);         \
    break;                                                 \
  case OP_ATAN:                                            \
    DerBinaryOperation<OP_ATAN>::derf(X, Y, F, D);         \
    break;                                                 \
  case OP_LT:                                              \
    DerBinaryOperation<OP_LT>::derf(X, Y, F, D);           \
    break;                                                 \
  case OP_LE:                                              \
    DerBinaryOperation<OP_LE>::derf(X, Y, F, D);           \
    break;                                                 \
  case OP_EQ:                                              \
    DerBinaryOperation<OP_EQ>::derf(X, Y, F, D);           \
    break;                                                 \
  case OP_NE:                                              \
    DerBinaryOperation<OP_NE>::derf(X, Y, F, D);           \
    break;                                                 \
  case OP_NOT:                                             \
    DerBinaryOperation<OP_NOT>::derf(X, Y, F, D);          \
    break;                                                 \
  case OP_AND:                                             \
    DerBinaryOperation<OP_AND>::derf(X, Y, F, D);          \
    break;                                                 \
  case OP_OR:                                              \
    DerBinaryOperation<OP_OR>::derf(X, Y, F, D);           \
    break;                                                 \
  case OP_IF_ELSE_ZERO:                                    \
    DerBinaryOperation<OP_IF_ELSE_ZERO>::derf(X, Y, F, D); \
    break;                                                 \
  case OP_FLOOR:                                           \
    DerBinaryOperation<OP_FLOOR>::derf(X, Y, F, D);        \
    break;                                                 \
  case OP_CEIL:                                            \
    DerBinaryOperation<OP_CEIL>::derf(X, Y, F, D);         \
    break;                                                 \
  case OP_FMOD:                                            \
    DerBinaryOperation<OP_FMOD>::derf(X, Y, F, D);         \
    break;                                                 \
  case OP_REMAINDER:                                       \
    DerBinaryOperation<OP_REMAINDER>::derf(X, Y, F, D);    \
    break;                                                 \
  case OP_FABS:                                            \
    DerBinaryOperation<OP_FABS>::derf(X, Y, F, D);         \
    break;                                                 \
  case OP_SIGN:                                            \
    DerBinaryOperation<OP_SIGN>::derf(X, Y, F, D);         \
    break;                                                 \
  case OP_COPYSIGN:                                        \
    DerBinaryOperation<OP_COPYSIGN>::derf(X, Y, F, D);     \
    break;                                                 \
  case OP_ERF:                                             \
    DerBinaryOperation<OP_ERF>::derf(X, Y, F, D);          \
    break;                                                 \
  case OP_FMIN:                                            \
    DerBinaryOperation<OP_FMIN>::derf(X, Y, F, D);         \
    break;                                                 \
  case OP_FMAX:                                            \
    DerBinaryOperation<OP_FMAX>::derf(X, Y, F, D);         \
    break;                                                 \
  case OP_INV:                                             \
    DerBinaryOperation<OP_INV>::derf(X, Y, F, D);          \
    break;                                                 \
  case OP_SINH:                                            \
    DerBinaryOperation<OP_SINH>::derf(X, Y, F, D);         \
    break;                                                 \
  case OP_COSH:                                            \
    DerBinaryOperation<OP_COSH>::derf(X, Y, F, D);         \
    break;                                                 \
  case OP_TANH:                                            \
    DerBinaryOperation<OP_TANH>::derf(X, Y, F, D);         \
    break;                                                 \
  case OP_ASINH:                                           \
    DerBinaryOperation<OP_ASINH>::derf(X, Y, F, D);        \
    break;                                                 \
  case OP_ACOSH:                                           \
    DerBinaryOperation<OP_ACOSH>::derf(X, Y, F, D);        \
    break;                                                 \
  case OP_ATANH:                                           \
    DerBinaryOperation<OP_ATANH>::derf(X, Y, F, D);        \
    break;                                                 \
  case OP_ATAN2:                                           \
    DerBinaryOperation<OP_ATAN2>::derf(X, Y, F, D);        \
    break;                                                 \
  case OP_ERFINV:                                          \
    DerBinaryOperation<OP_ERFINV>::derf(X, Y, F, D);       \
    break;                                                 \
  case OP_LIFT:                                            \
    DerBinaryOperation<OP_LIFT>::derf(X, Y, F, D);         \
    break;                                                 \
  case OP_PRINTME:                                         \
    DerBinaryOperation<OP_PRINTME>::derf(X, Y, F, D);      \
    break;                                                 \
  case OP_LOG1P:                                           \
    DerBinaryOperation<OP_LOG1P>::derf(X, Y, F, D);        \
    break;                                                 \
  case OP_EXPM1:                                           \
    DerBinaryOperation<OP_EXPM1>::derf(X, Y, F, D);        \
    break;                                                 \
  case OP_HYPOT:                                           \
    DerBinaryOperation<OP_HYPOT>::derf(X, Y, F, D);        \
    break;
  switch (op) { CASADI_MATH_DERF_BUILTIN(x, y, f, d) }
}
...
template <typename T>
inline std::string casadi_math<T>::print(unsigned char op, const std::string& x,
                                         const std::string& y) {
...
template <typename T>
inline std::string casadi_math<T>::print(unsigned char op,
                                         const std::string& x) {
