 ...
 * They can just use slicing utility from the host language ( M[0:6]  in Python,
 M(1:7) )
...
class CASADI_EXPORT Slice
 ...
 public:
  ...
  casadi_int start;
  /// stop value: use std::numeric_limits<casadi_int>::max() to indicate
  /// unboundedness
  casadi_int stop;
  casadi_int step;
...
Slice CASADI_EXPORT to_slice(const std::vector<casadi_int>& v,
                             bool ind1 = false);

/// Construct nested slices from an index vector (requires is_slice2(v) to be
/// true)
std::pair<Slice, Slice> CASADI_EXPORT
to_slice2(const std::vector<casadi_int>& v);

/// Check if an index vector can be represented more efficiently as a slice
bool CASADI_EXPORT is_slice(const std::vector<casadi_int>& v,
                            bool ind1 = false);
