...
#ifdef SWIGPYTHON
    %
    {namespace casadi{// Redirect printout
                      static void pythonlogger(const char* s,
                                               std::streamsize num, bool error){
...
// Never for Octave
static bool mexcheckinterrupted() { return false; }
...
void mexclearinterrupted() { utSetInterruptPending(false); }
...
% init % {
  ...
  if (path.rfind(sep) != std::string::npos &&
      path.substr(path.rfind(sep) + 1) == "+casadi")
    path = path.substr(0, path.rfind(sep));
...
// Turn off the warnings that certain methods are effectively ignored, this
// seams to be a false warning, for example vertcat(SXVector), vertcat(DMVector)
// and vertcat(MXVector) appears to work fine
...
import contextlib
      ...
      class _copyableObject(object) :def __copy__(self) : return self.__class__(self)

                                                      def __deepcopy__(self, dummy = None) : return self.__class__(self)

                                                                                 _object = object = _copyableObject

                                                                       _swig_repr_default = _swig_repr def _swig_repr(self) : if hasattr (self, 'repr') : return self.repr() else : return _swig_repr_default(self)

                                                                                                                                                    def DM_from_array(m, check_only = True) :import numpy as np if isinstance (m, np.ndarray) : if len (m.shape)> 2 : return False try :m = m.astype(float, casting = "same_kind", copy = False) except: return False if check_only: return True else :shape = m.shape +(1, 1) nrow, ncol = shape[0], shape[1] return (nrow, ncol, m.flat) return False

                                                                                                                                                                                                                                                                                                                                                                                                                                                                       def IM_from_array(m, check_only = True) :import numpy as np if isinstance (m, np.ndarray) : if len (m.shape)> 2 : return False try :m = m.astype(int, casting = "same_kind", copy = False) except: return False if check_only: return True else :shape = m.shape +(1, 1) nrow, ncol = shape[0], shape[1] return (nrow, ncol, m.flat) return False

                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        def SX_from_array(m, check_only = True) :import numpy as np if isinstance (m, np.ndarray) : if len (m.shape)> 2 : return False if m.dtype != object: return None shape = m.shape +(1, 1) nrow, ncol = shape[0], shape[1] return (nrow, ncol, m.flat) return False

                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         def DM_from_csc(m, check_only = True) : if hasattr (m, "tocsc") :m = m.tocsc() if m.__class__.__name__ == "csc_matrix" : if len (m.shape) != 2 : return False if check_only: return True return m.shape +(m.indptr.flat, m.indices.flat, m.data.flat) return False
...
% feature("customdoc:main", "  $brief\n\n::\n\n$overview\n$main");
  ...
  //%feature("compactdefaultargs","0") casadi::taylor; // taylor function has a
  //default argument for which the namespace is not recognised by SWIG
...
#define SWIG_FILE_WITH_INIT
      ...
      %
      {
#define SWIG_Error_return(code, msg)                                         \
  {                                                                          \
    std::cerr << "Error occured in CasADi SWIG interface code:" << std::endl \
              << "  " << msg << std::endl;                                   \
    SWIG_Error(code, msg);                                                   \
    return 0;                                                                \
  }
      ...
      % fragment("casadi_decl", "header", fragment = "casadi_extra_decl"){
            namespace casadi{/* Check if Null or None */
...
#endif  // SWIGMATLAB
  template <typename M>
  bool to_ptr(GUESTOBJECT * p, std::vector<M> * *m);
  template <typename M>
  bool to_ptr(GUESTOBJECT * p, std::vector<std::vector<M> > * *m);
  template <typename M>
  GUESTOBJECT* from_ptr(const std::vector<M>* a);
  ...
  bool to_ptr(GUESTOBJECT * p, std::pair<M1, M2> * *m);
  template <typename M1, typename M2>
  GUESTOBJECT* from_ptr(const std::pair<M1, M2>* a);

  // std::map
  template <typename M>
  bool to_ptr(GUESTOBJECT * p, std::map<std::string, M> * *m);
  template <typename M>
  GUESTOBJECT* from_ptr(const std::map<std::string, M>* a);
  ...
  GUESTOBJECT* full(const DM& m, bool simplify = false) {
    ...
    PyObject* cr = PyObject_CallMethodObjArgs(
        p, method_name, (simplify ? Py_True : Py_False), 0);
  ...
  PyObject* get_Python_helper(const std::string& name) {
    % #if PY_VERSION_HEX < 0x03070000 PyObject* module =
        PyImport_AddModule("casadi");
    % #else PyObject* c_name = PyString_FromString("casadi");
  ...
  template <class T>
  bool casadi_object_from_fun(GUESTOBJECT * p, T * *m, const std::string& fun,
                              const std::function<bool(PyObject*, T**)>& conv) {
  ...
  bool SX_from_array_conv(GUESTOBJECT * p, casadi::SX * *m) {
    ...
    casadi_int nrow;
    to_val(PyTuple_GetItem(p, 0), &nrow);
    casadi_int ncol;
    to_val(PyTuple_GetItem(p, 1), &ncol);
    if (m) {
  ...
  bool DM_from_csc_conv(GUESTOBJECT * p, casadi::DM * *m) {
    ...
    casadi_int ncol;
    to_val(PyTuple_GetItem(p, 1), &ncol);
    **m = casadi::Matrix<double>(casadi::Sparsity(nrow, ncol, colind, row),
                                 data, false);
    return true;
  }

  bool SX_from_array(GUESTOBJECT * p, casadi::SX * *m) {
    return casadi_object_from_fun<casadi::SX>(p, m, "SX_from_array",
                                              SX_from_array_conv);
  }

  bool IM_from_array(GUESTOBJECT * p, casadi::IM * *m) {
    return casadi_object_from_fun<casadi::IM>(p, m, "IM_from_array",
                                              IM_from_array_conv);
  }

  bool DM_from_array(GUESTOBJECT * p, casadi::DM * *m) {
    return casadi_object_from_fun<casadi::DM>(p, m, "DM_from_array",
                                              DM_from_array_conv);
  }

  bool DM_from_csc(GUESTOBJECT * p, casadi::DM * *m) {
    return casadi_object_from_fun<casadi::DM>(p, m, "DM_from_csc",
                                              DM_from_csc_conv);
  }
...
% fragment("casadi_aux", "header", fragment = "casadi_decl"){
      namespace casadi{template <typename M>
                       bool to_val(GUESTOBJECT * p, M* m){// Copy the pointer
                                                          M* m2 = m;
...
template <typename M>
bool to_generic(GUESTOBJECT* p, GenericType** m) {
  if (m) {
    // Temporary
    M tmp, *tmp_ptr = &tmp;
...
% fragment("casadi_bool", "header", fragment = "casadi_aux",
           fragment = SWIG_AsVal_frag(bool)){namespace casadi{
      bool to_ptr(GUESTOBJECT * p, bool** m){// Treat Null
...
% fragment("casadi_int", "header", fragment = "casadi_aux",
           fragment = SWIG_AsVal_frag(int), fragment = SWIG_AsVal_frag(long),
           fragment = SWIG_AsVal_frag(long long)){namespace casadi{
      bool to_ptr(GUESTOBJECT * p, casadi_int** m){// Treat Null
                                                   if (is_null(p)) return false;
...
GUESTOBJECT* from_ptr(const casadi_int* a) {
  ...
  if (*a > PyInt_GetMax() || *a < -(PyInt_GetMax() - 1))
    return PyLong_FromLongLong(*a);
...
% fragment("casadi_double", "header", fragment = "casadi_aux",
           fragment = SWIG_AsVal_frag(double)){namespace casadi{
      bool to_ptr(GUESTOBJECT * p, double** m){// Treat Null
                                               if (is_null(p)) return false;
...
% fragment("casadi_vector", "header", fragment = "casadi_aux"){namespace casadi{

#ifdef SWIGMATLAB

      // Cell array
      template <typename M>
      bool to_ptr_cell(GUESTOBJECT * p, std::vector<M>** m){
          // Cell arrays (only row vectors)
          if (mxGetClassID(p) == mxCELL_CLASS){casadi_int nrow = mxGetM(p),
                                               ncol = mxGetN(p);
...
bool to_ptr(GUESTOBJECT* p, std::vector<casadi_int>** m) {
  ...
  if (mxIsLogical(p) && !mxIsLogicalScalar(p) &&
      mxGetNumberOfDimensions(p) == 2 && (mxGetM(p) <= 1 || mxGetN(p) <= 1)) {
    casadi_int n = mxGetM(p) * mxGetN(p);
...
bool to_ptr(GUESTOBJECT* p, std::vector<M>** m) {
  ...
  if (PyDict_Check(p) || PyString_Check(p) || PySet_Check(p) ||
      PyUnicode_Check(p))
    return false;
...
% fragment("casadi_vectorvector", "header",
           fragment = "casadi_aux"){namespace casadi{

#ifdef SWIGMATLAB

      // Cell array
      template <typename M>
      bool to_ptr_cell2(GUESTOBJECT * p, std::vector<std::vector<M> >** m){
          // Cell arrays (only row vectors)
          if (mxGetClassID(p) == mxCELL_CLASS){casadi_int nrow = mxGetM(p),
                                               ncol = mxGetN(p);
if (true) {
...
% fragment("casadi_function", "header",
           fragment = "casadi_aux"){namespace casadi{
      bool to_ptr(GUESTOBJECT * p, Function** m){// Treat Null
                                                 if (is_null(p)) return false;
...
GUESTOBJECT* from_ptr(const Function* a) {
  return SWIG_NewPointerObj(new Function(*a), $descriptor(casadi::Function*),
                            SWIG_POINTER_OWN);
}
}  // namespace casadi
}
...
% fragment("casadi_generictype", "header", fragment = "casadi_aux"){
      namespace casadi{bool to_ptr(GUESTOBJECT * p, GenericType** m){
#ifdef SWIGPYTHON
          if (p == Py_None){if (m) ** m = GenericType();
return true;
}
#endif  // SWIGPYTHON
...
if (to_generic<casadi_int>(p, m) || to_generic<double>(p, m) ||
...
// Check if it can be converted to boolean (last as e.g. can be converted to
// boolean)
...
GUESTOBJECT* from_ptr(const GenericType* a) {
  switch (a->getType()) {
    case OT_BOOL:
      return from_tmp(a->as_bool());
    case OT_INT:
      return from_tmp(a->as_int());
    case OT_DOUBLE:
      return from_tmp(a->as_double());
    case OT_STRING:
      return from_tmp(a->as_string());
    case OT_INTVECTOR:
      return from_tmp(a->as_int_vector());
    case OT_INTVECTORVECTOR:
      return from_tmp(a->as_int_vector_vector());
    case OT_BOOLVECTOR:
      return from_tmp(a->as_bool_vector());
    case OT_DOUBLEVECTOR:
      return from_tmp(a->as_double_vector());
    case OT_DOUBLEVECTORVECTOR:
      return from_tmp(a->as_double_vector_vector());
    case OT_STRINGVECTOR:
      return from_tmp(a->as_string_vector());
    case OT_DICT:
      return from_tmp(a->as_dict());
    case OT_FUNCTION:
      return from_tmp(a->as_function());
    case OT_FUNCTIONVECTOR:
      return from_tmp(a->as_function_vector());
#ifdef SWIGPYTHON
    case OT_NULL:
    case OT_VOIDPTR:
      return Py_None;
...
% fragment("casadi_string", "header", fragment = "casadi_aux"){
      namespace casadi{bool to_ptr(GUESTOBJECT * p, std::string** m){
          // Treat Null
          if (is_null(p)) return false;
...
% fragment("casadi_slice", "header", fragment = "casadi_aux"){namespace casadi{
      bool to_ptr(GUESTOBJECT * p, Slice** m){// Treat Null
                                              if (is_null(p)) return false;
...
if (PySlice_Check(p)) {
  ...
  if (m) {
    (**m).start = (r->start == Py_None || PyNumber_AsSsize_t(r->start, NULL) <=
                                              std::numeric_limits<int>::min())
                      ? std::numeric_limits<casadi_int>::min()
                      : PyInt_AsLong(r->start);
    (**m).stop = (r->stop == Py_None || PyNumber_AsSsize_t(r->stop, NULL) >=
...
GUESTOBJECT* from_ptr(const Slice* a) {
  return SWIG_NewPointerObj(new Slice(*a), $descriptor(casadi::Slice*),
                            SWIG_POINTER_OWN);
}
...
% fragment("casadi_map", "header", fragment = "casadi_aux"){namespace casadi{
      template <typename M>
      bool to_ptr(GUESTOBJECT * p, std::map<std::string, M>** m){
#ifdef SWIGPYTHON
          if (PyDict_Check(p)){PyObject * key, *value;
Py_ssize_t pos = 0;
...
#elif defined(SWIGMATLAB)
          if (mxIsStruct(p) && mxGetM(p) == 1 &&
              mxGetN(p) == 1){casadi_int len = mxGetNumberOfFields(p);
...
GUESTOBJECT* from_ptr(const std::map<std::string, M>* a) {
  ...
  for (typename std::map<std::string, M>::const_iterator it = a->begin();
       it != a->end(); ++it) {
  ...
  for (casadi_int k = 0; k < fields.size(); ++k)
    mxSetFieldByNumber(p, 0, k, fields[k]);
  return p;
#else
  return 0;
#endif
...
% fragment("casadi_pair", "header", fragment = "casadi_aux"){namespace casadi{
#ifdef SWIGMATLAB
      bool to_ptr(GUESTOBJECT * p, std::pair<casadi_int, casadi_int>** m){
          // (casadi_int,casadi_int) mapped to 2-by-1 double matrix
          if (mxIsDouble(p) && mxGetNumberOfDimensions(p) == 2 &&
              !mxIsSparse(p) && mxGetM(p) == 1 &&
              mxGetN(p) == 2){double* data = static_cast<double*>(mxGetData(p));
...
% fragment("casadi_sx", "header", fragment = "casadi_aux"){namespace casadi{
      bool to_ptr(GUESTOBJECT * p, SX** m){// Treat Null
                                           if (is_null(p)) return false;

// SX already?
if (SWIG_IsOK(SWIG_ConvertPtr(p, reinterpret_cast<void**>(m),
                              $descriptor(casadi::Matrix<casadi::SXElem>*),
                              0))) {
  return true;
}
...
GUESTOBJECT* from_ptr(const SX* a) {
  return SWIG_NewPointerObj(new SX(*a),
                            $descriptor(casadi::Matrix<casadi::SXElem>*),
                            SWIG_POINTER_OWN);
}
}  // namespace casadi
...
% fragment("casadi_sxelem", "header", fragment = "casadi_aux"){namespace casadi{
      bool to_ptr(GUESTOBJECT * p, SXElem** m){// Treat Null
                                               if (is_null(p)) return false;
...
GUESTOBJECT* from_ptr(const SXElem* a) { return from_ref(SX(*a)); }
}  // namespace casadi
}

% fragment("casadi_mx", "header", fragment = "casadi_decl"){namespace casadi{
      bool to_ptr(GUESTOBJECT * p, MX** m){// Treat Null
                                           if (is_null(p)) return false;
...
GUESTOBJECT* from_ptr(const MX* a) {
  return SWIG_NewPointerObj(new MX(*a), $descriptor(casadi::MX*),
                            SWIG_POINTER_OWN);
}
}  // namespace casadi
}

% fragment("casadi_dmatrix", "header",
           fragment = "casadi_aux"){namespace casadi{
#ifdef SWIGPYTHON
...
GUESTOBJECT* from_ptr(const DM* a) {
  return SWIG_NewPointerObj(new DM(*a), $descriptor(casadi::Matrix<double>*),
                            SWIG_POINTER_OWN);
}
}  // namespace casadi
}

% fragment("casadi_sparsity", "header",
           fragment = "casadi_aux"){namespace casadi{
      bool to_ptr(GUESTOBJECT * p, Sparsity** m){// Treat Null
                                                 if (is_null(p)) return false;
...
GUESTOBJECT* from_ptr(const Sparsity* a) {
  return SWIG_NewPointerObj(new Sparsity(*a), $descriptor(casadi::Sparsity*),
                            SWIG_POINTER_OWN);
}
}  // namespace casadi
}
...
% fragment("casadi_imatrix", "header", fragment = "casadi_aux",
           fragment = SWIG_AsVal_frag(int)){namespace casadi{
      bool to_ptr(GUESTOBJECT * p, IM** m){// Treat Null
                                           if (is_null(p)) return false;
...
%
    ...
    % fragment("casadi_all", "header",
               fragment =
                   "casadi_aux,casadi_extra,casadi_bool,casadi_int,casadi_"
                   "double,casadi_vector,casadi_vectorvector,casadi_function,"
                   "casadi_generictype,casadi_string,casadi_slice,casadi_map,"
                   "casadi_pair,casadi_sx,casadi_sxelem,casadi_mx,casadi_"
                   "dmatrix,casadi_sparsity,casadi_imatrix"){}
#endif  // SWIGXML
    ...
    % typemap(typecheck, noblock = 1, precedence = xPrec,
              fragment = "casadi_all") xType {
...
% typemap(in, doc = xName, noblock = 1, fragment = "casadi_all") xType {
  if (!casadi::to_val($input, &$1))
    SWIG_exception_fail(SWIG_TypeError,
                        "Failed to convert input $argnum to type '" xName "'.");
}
...
%
    ...
    % typemap(typecheck, noblock = 1, precedence = xPrec,
              fragment = "casadi_all") const xType& {
  $1 = casadi::to_ptr($input, static_cast<xType**>(0));
}

// Pass input by reference, convert argument
% typemap(in, doc = xName, noblock = 1,
          fragment = "casadi_all") const xType&(xType m) {
  $1 = &m;
  if (!casadi::to_ptr($input, &$1))
    SWIG_exception_fail(SWIG_TypeError,
                        "Failed to convert input $argnum to type '" xName "'.");
}
...
% typemap(freearg, noblock = 1) const xType& {}
    ...
    // Return-by-value
    % typemap(out, doc = xName, noblock = 1, fragment = "casadi_all") xType,
    const xType {
  if (!($result = casadi::from_ref($1)))
    SWIG_exception_fail(SWIG_TypeError,
                        "Failed to convert output to type '" xName "'.");
}

// Return a const-ref behaves like return-by-value
% typemap(out, doc = xName, noblock = 1, fragment = "casadi_all") const xType& {
  if (!($result = casadi::from_ptr($1)))
    SWIG_exception_fail(SWIG_TypeError,
                        "Failed to convert output to type '" xName "'.");
}
...
% typemap(directorin, noblock = 1, fragment = "casadi_all") xType, const xType {
  if (!($input = casadi::from_ref($1)))
    % dirout_fail(SWIG_TypeError,
                  "For director inputs, failed to convert input to " xName ".");
}

// Directorin typemap; as output
% typemap(directorin, noblock = 1, fragment = "casadi_all") const xType& {
  if (!($input = casadi::from_ptr(&$1)))
    % dirout_fail(SWIG_TypeError,
                  "For director inputs, failed to convert input to " xName ".");
}
...
// ... but kept as inputs
% typemap(in, doc = xName, noblock = 1,
          fragment = "casadi_all") xType& INOUT(xType m) {
  $1 = &m;
  if (!casadi::to_ptr($input, &$1))
    SWIG_exception_fail(SWIG_TypeError,
                        "Failed to convert input to type '" xName "'.");
}
...
%
    ...
    % define % casadi_template(xName, xPrec, xType...) % template() xType;
% casadi_input_typemaps(xName, xPrec, xType) %
    ...
    % define PREC_DICT 21 % enddef % define PREC_SPARSITY 90 % enddef
    % define PREC_IVector 92 % enddef % define PREC_IVectorVector 92 % enddef
    % define PREC_VECTOR 92 % enddef % define PREC_PAIR_SLICE_SLICE 93 % enddef
    % define PREC_SLICE 94 % enddef
    % define PREC_PAIR_IVector_IVector 96 % enddef % define PREC_IM 97 % enddef
    % define PREC_DVector 99 % enddef % define PREC_DM 100 % enddef
    % define PREC_DMVector 101 % enddef
    % define PREC_DMVectorVector 101 % enddef % define PREC_SX 103 % enddef
    % define PREC_SXVector 103 % enddef
    % define PREC_SXVectorVector 103 % enddef % define PREC_MX 104 % enddef
    % define PREC_MXVector 105 % enddef
    % define PREC_MXVectorVector 106 % enddef % define PREC_CREATOR 150 % enddef
    % define PREC_STRING 180 % enddef % define PREC_FUNCTION 200 % enddef
    % define PREC_GENERICTYPE 201 %
    enddef
#ifndef SWIGXML
...
#ifdef SWIGPYTHON
% typemap(in, doc = "memoryview(ro)", noblock = 1,
          fragment = "casadi_all")(const double* a,
                                   casadi_int size)(Py_buffer* buffer) {
  if (!PyMemoryView_Check($input))
    SWIG_exception_fail(SWIG_TypeError, "Must supply a MemoryView.");
  buffer = PyMemoryView_GET_BUFFER($input);
  $1 = static_cast<double*>(buffer->buf);  // const double cast comes later
  $2 = buffer->len;
}

% typemap(in, doc = "memoryview(rw)", noblock = 1,
          fragment = "casadi_all")(double* a,
                                   casadi_int size)(Py_buffer* buffer) {
  if (!PyMemoryView_Check($input))
    SWIG_exception_fail(SWIG_TypeError, "Must supply a writable MemoryView.");
  buffer = PyMemoryView_GET_BUFFER($input);
  if (buffer->readonly)
    SWIG_exception_fail(SWIG_TypeError, "Must supply a writable MemoryView.");
...
// Directorin typemap; as output
% typemap(directorin, noblock = 1, fragment = "casadi_all")(
      const double** arg,
      const std::vector<casadi_int>& sizes_arg)(PyObject* my_tuple) {
  PyObject* arg_tuple = PyTuple_New($2.size());
  for (casadi_int i = 0; i < $2.size(); ++i) {
#ifdef WITH_PYTHON3
    PyObject* buf =
        $1[i] ? PyMemoryView_FromMemory(
                    reinterpret_cast<char*>(const_cast<double*>($1[i])),
                    $2[i] * sizeof(double), PyBUF_READ)
              : SWIG_Py_Void();
#else
    PyObject* buf = $1[i] ? PyBuffer_FromMemory(const_cast<double*>($1[i]),
                                                $2[i] * sizeof(double))
                          : SWIG_Py_Void();
#endif
...
% typemap(directorin, noblock = 1,
          fragment = "casadi_all")(double** res,
  ...
  for (casadi_int i = 0; i < $2.size(); ++i) {
    ...
    PyObject* buf =
        $1[i] ? PyMemoryView_FromMemory(
                    reinterpret_cast<char*>(const_cast<double*>($1[i])),
                    $2[i] * sizeof(double), PyBUF_WRITE)
              : SWIG_Py_Void();
#else
    PyObject* buf =
        $1[i] ? PyBuffer_FromReadWriteMemory($1[i], $2[i] * sizeof(double))
              : SWIG_Py_Void();
#endif
...
% casadi_typemaps(L_STR, PREC_STRING, std::string) %
    ...
    casadi_template(LL LL L_STR LR LR, PREC_VECTOR,
                    std::vector<std::vector<std::string> >) %
    casadi_typemaps("Sparsity", PREC_SPARSITY, casadi::Sparsity) %
    casadi_template(LL "Sparsity" LR, PREC_SPARSITY,
                    std::vector<casadi::Sparsity>) %
    casadi_template(LL LL "Sparsity" LR LR, PREC_SPARSITY,
                    std::vector<std::vector<casadi::Sparsity> >) %
    casadi_template(LDICT("Sparsity"), PREC_SPARSITY,
                    std::map<std::string, casadi::Sparsity>) %
    casadi_template(LDICT(LL "Sparsity" LR), PREC_SPARSITY,
                    std::map<std::string, std::vector<casadi::Sparsity> >) %
    casadi_template(LPAIR(LDICT("Sparsity"), "[" L_STR "]"), PREC_SPARSITY,
                    std::pair<std::map<std::string, casadi::Sparsity>,
                              std::vector<std::string> >) %
    casadi_typemaps(L_BOOL, SWIG_TYPECHECK_BOOL, bool) %
    casadi_template("[" L_BOOL "]", SWIG_TYPECHECK_BOOL, std::vector<bool>) %
    casadi_template("[[" L_BOOL "]]", SWIG_TYPECHECK_BOOL,
...
#endif
    % casadi_template(LABEL, SWIG_TYPECHECK_INTEGER,
                      std::pair<casadi_int, casadi_int>)
#undef LABEL
    % casadi_template("[" L_INT "]", PREC_IVector, std::vector<casadi_int>) %
    casadi_template(LL "[" L_INT "]" LR, PREC_IVectorVector,
                    std::vector<std::vector<casadi_int> >) %
    casadi_typemaps(L_DOUBLE, SWIG_TYPECHECK_DOUBLE, double) %
    casadi_template("[" L_DOUBLE "]", SWIG_TYPECHECK_DOUBLE,
                    std::vector<double>) %
    casadi_template(LL "[" L_DOUBLE "]" LR, SWIG_TYPECHECK_DOUBLE,
                    std::vector<std::vector<double> >) %
    casadi_typemaps("SXElem", PREC_SX, casadi::SXElem) %
    casadi_template(LL "SXElem" LR, PREC_SXVector,
                    std::vector<casadi::SXElem>) %
    casadi_typemaps("SX", PREC_SX, casadi::Matrix<casadi::SXElem>) %
    casadi_template(LL "SX" LR, PREC_SXVector,
                    std::vector<casadi::Matrix<casadi::SXElem> >) %
    casadi_template(
        LL LL "SX" LR LR, PREC_SXVectorVector,
        std::vector<std::vector<casadi::Matrix<casadi::SXElem> > >) %
    casadi_template(LDICT("SX"), PREC_SX,
                    std::map<std::string, casadi::Matrix<casadi::SXElem> >) %
    casadi_typemaps("MX", PREC_MX, casadi::MX) %
    casadi_template(LL "MX" LR, PREC_MXVector, std::vector<casadi::MX>) %
    casadi_template(LL LL "MX" LR LR, PREC_MXVectorVector,
                    std::vector<std::vector<casadi::MX> >) %
    casadi_template(LDICT("MX"), PREC_MX, std::map<std::string, casadi::MX>) %
    casadi_template(LPAIR("MX", "MX"), PREC_MXVector,
                    std::pair<casadi::MX, casadi::MX>) %
    casadi_typemaps("DM", PREC_DM, casadi::Matrix<double>) %
    casadi_template(LL "DM" LR, PREC_DMVector,
                    std::vector<casadi::Matrix<double> >) %
    casadi_template(LL LL "DM" LR LR, PREC_DMVectorVector,
                    std::vector<std::vector<casadi::Matrix<double> > >) %
    casadi_template(LDICT("DM"), PREC_DM,
                    std::map<std::string, casadi::Matrix<double> >) %
    ...
    casadi_template(LL "GenericType" LR, PREC_GENERICTYPE,
    ...
    casadi_template(LL "Function" LR, PREC_FUNCTION,
                    std::vector<casadi::Function>) %
    casadi_template(LPAIR("Function", "Function"), PREC_FUNCTION,
                    std::pair<casadi::Function, casadi::Function>) %
    casadi_template(L_DICT, PREC_DICT,
                    std::map<std::string, casadi::GenericType>) %
    casadi_template(LDICT(LL L_STR LR), PREC_DICT,
...
// Matlab is index-1 based
  ...
  if (!casadi::to_val($input, &$1))
    SWIG_exception_fail(SWIG_TypeError,
                        "Failed to convert input $argnum to type ' index '.");
  if ($1 == 0)
    SWIG_exception_fail(SWIG_TypeError, "Index starts at 1, got index '0'.");
  if ($1 >= 1) $1--;
}
#endif
...
% {
#include <pythonrun.h>
      void SigIntHandler(casadi_int){std::cerr << "Keyboard Interrupt"
                                               << std::endl;
...
% pythoncode % {
  try : from numpy import pi, inf except:
  pass
...
% pythoncode % {
  def veccat(*args) : try:
    if len (args)
      == 0 : return DM(0, 1) except : pass return _veccat(args)
                                          def vertcat(*args)
          : try:
        if len (args)
          == 0 : return DM(0, 1) except : pass return _vertcat(args)
                                              def horzcat(*args)
              : try:
            if len (args)
              == 0 : return DM(1, 0) except : pass return _horzcat(args)
                                                  def diagcat(*args)
                  : try:
                if len (args)
                  == 0 : return DM(0, 0) except : pass return _diagcat(args) def
                                                  vvcat(args)
                      : try:
                    if len (args)
                      == 0 : return DM(0, 1) except : pass return _veccat(args)
                          ...
                          : try:
                        if len (args)
                          == 0 : return DM(0, 1) except
                              : pass return _vertcat(args) def
                                hcat(args)
                              : try:
                            if len (args)
                              == 0 : return DM(1, 0) except
                                  : pass return _horzcat(args) def
                                    dcat(args)
                                  : try:
                                if len (args)
                                  == 0 : return DM(0, 0) except
                                      : pass return _diagcat(args) %
}
...
% define % matrix_helpers(Type) % pythoncode % {
    @property
    def shape(self):
...
% enddef
    ...
    % define % python_array_wrappers(arraypriority) % pythoncode % {
  __array_priority__ = arraypriority

      def
      __array_wrap__(self, out_arr, context = None)
      : if context is None : return out_arr name = context[0].__name__ args =
                                 list(context[1])

                                     if len (context[1]) == 3
      : raise Exception("Error with %s. Looks like you are using an assignment "
                        "operator, such as 'a+=b' where 'a' is a numpy type. "
                        "This is not supported, and cannot be supported "
                        "without changing numpy." %
                        name)

            if "vectorized" in name : name = name[:-len(" (vectorized)")]
      ...
      : name = conversion[name] if len (context[1]) == 2 and
               context[1][1] is self and
               not(context[1][0] is self)
      : name = 'r' + name args.reverse() if not(hasattr(self, name)) or
               ('mul' in name)
      : name = '__' + name +
               '__' fun = getattr(self, name) return fun(*args [1:])
      ...
      : conversion = {
          ...
          "equal" : "eq",
          "not_equal" : "ne"
        } name = ufunc.__name__ inputs = list(inputs) if len (inputs) == 3
      : import warnings warnings.warn(
            "Error with %s. Looks like you are using an assignment operator, "
            "such as 'a+=b' where 'a' is a numpy type. This is not supported, "
            "and cannot be supported without changing numpy." %
                name,
            RuntimeWarning) return NotImplemented if "vectorized" in name
      : name = name
  [:-len(" (vectorized)")] if name in conversion
      : name = conversion[name] if len (inputs) == 2 and inputs[1] is self and
               not(inputs[0] is self)
      : name = 'r' + name inputs.reverse() if not(hasattr(self, name)) or
               ('mul' in name) : name = '__' + name + '__' try : assert method
    == "__call__" fun = getattr(self, name) return fun(*inputs [1:])
                            except Exception as e : if "Dimension mismatch" in
                                                    str(e)
        : import sys if sys.version_info[0] < 3
        : raise RuntimeError(str(e)) else : raise e
#Fall back to numpy conversion
                                                new_inputs =
                            list(inputs) try : new_inputs
      [0] =
          new_inputs[0]
                  .full() except : import warnings warnings
                  .warn(
                      "Implicit conversion of symbolic CasADi type to numeric "
                      "matrix not supported.\n" +
                          "This may occur when you pass a CasADi object to a "
                          "numpy function.\n" +
                          "Use an equivalent CasADi function instead of that "
                          "numpy function.",
                      RuntimeWarning) return NotImplemented return new_inputs[0]
                  .__array_ufunc__(ufunc, method, *new_inputs, **kwargs)

                                       def
                                       __array__(self, *args, **kwargs)
          : import numpy as n if len (args) > 1 and
          isinstance(args[1], tuple) and isinstance(args[1][0], n.ufunc) and
          isinstance(args[1][0], n.ufunc) and len(args[1]) > 1 and
          args[1][0].nin ==
              len(args[1][1])
          : if len (args[1][1]) ==
            3 : raise Exception(
                    "Error with %s. Looks like you are using an assignment "
                    "operator, such as 'a+=b'. This is not supported when 'a' "
                    "is a numpy type, and cannot be supported without changing "
                    "numpy itself. Either upgrade a to a CasADi type first, or "
                    "use 'a = a + b'. " %
                    args[1][0].__name__) return n
                    .array([n.nan]) else : if hasattr (self, '__array_custom__')
          : return self.__array_custom__(*args, **kwargs) else : try:
        return self.full() except : if self.is_scalar(True)
            :
#Needed for #2743
              E = n.empty((), dtype = object) E[()] = self return E else
            : raise Exception("Implicit conversion of symbolic CasADi type to "
                              "numeric matrix not supported.\n" +
                              "This may occur when you pass a CasADi object to "
                              "a numpy function.\n" +
                              "Use an equivalent CasADi function instead of "
                              "that numpy function.")
...
#ifdef SWIGXML
    % define % matrix_helpers(Type) % enddef
#endif

#ifdef SWIGMATLAB
    % {namespace casadi{
          /// Helper function: Convert ':' to Slice
          inline Slice char2Slice(char ch){casadi_assert_dev(ch == ':');
return Slice();
}
}  // namespace casadi
%
...
const Type paren(const Matrix<casadi_int>& rr,
...
void paren_asgn(const Type& m, const Matrix<casadi_int>& rr) {
  $self->set(m, true, rr);
}
void paren_asgn(const Type& m, const Sparsity& sp) { $self->set(m, true, sp); }
void paren_asgn(const Type& m, char rr, char cc) {
  $self->set(m, true, casadi::char2Slice(rr), casadi::char2Slice(cc));
}
void paren_asgn(const Type& m, char rr, const Matrix<casadi_int>& cc) {
  $self->set(m, true, casadi::char2Slice(rr), cc);
}
void paren_asgn(const Type& m, const Matrix<casadi_int>& rr, char cc) {
...
const Type brace(char rr) const {
  Type m;
  $self->get_nz(m, true, casadi::char2Slice(rr));
  return m;
}
const Type brace(const Matrix<casadi_int>& rr) const {
  Type m;
  $self->get_nz(m, true, rr);
  return m;
}

// Set nonzeros (index-1)
void setbrace(const Type& m, char rr) {
  $self->set_nz(m, true, casadi::char2Slice(rr));
}
void setbrace(const Type& m, const Matrix<casadi_int>& rr) {
  $self->set_nz(m, true, rr);
}
...
casadi_int numel(casadi_int k) const { return 1; }
...
// Needed for brace syntax to access nonzeros
casadi_int numel(const std::vector<casadi_int>& k) const { return 1; }

// Needed because original numel call gets hidden by the above extend overloads
casadi_int numel() const { return $self->numel(); }
...
% enddef
    ...
    namespace casadi{
        ...
        extend PrintableCommon{
            ...
            % pythoncode % {
                def __str__(self) :
                    return self.str() def repr(self) : return self.type_name() +
                '(' + self.str() + ')' %
              }
#endif  // SWIGPYTHON
#ifdef SWIGMATLAB
            % matlabcode %
            {function s = repr(self) s = [self.type_name() '(' self.str() ')'];
end %
}
#endif  // SWIGMATLAB
}
...
// Logic for pickling
    ...
    /* There is no reason to expose the Slice class to e.g. Python or MATLAB.
       Only if an interfaced language lacks a slice type, the type should be
       exposed here */
...
DECL M casadi_horzcat(const std::vector<M>& v) { return horzcat(v); }
DECL M casadi_vertcat(const std::vector<M>& v) { return vertcat(v); }
DECL std::vector<M> casadi_horzsplit(const M& v,
...
DECL std::vector<M> casadi_vertsplit(const M& v,
                                     const std::vector<casadi_int>& offset) {
  return vertsplit(v, offset);
}
DECL std::vector<casadi_int> casadi_offset(const std::vector<M>& v,
                                           bool vert = true) {
  return offset(v, vert);
}
DECL std::vector<M> casadi_vertsplit(const M& v, casadi_int incr = 1) {
  return vertsplit(v, incr);
}
DECL std::vector<M> casadi_vertsplit_n(const M& v, casadi_int n) {
  return vertsplit_n(v, n);
}
DECL M casadi_blockcat(const M& A, const M& B, const M& C, const M& D) {
  return vertcat(horzcat(A, B), horzcat(C, D));
}
DECL std::vector<std::vector<M> > casadi_blocksplit(
    const M& x, const std::vector<casadi_int>& vert_offset,
    const std::vector<casadi_int>& horz_offset) {
  return blocksplit(x, vert_offset, horz_offset);
}
DECL std::vector<std::vector<M> > casadi_blocksplit(const M& x,
                                                    casadi_int vert_incr = 1,
                                                    casadi_int horz_incr = 1) {
  return blocksplit(x, vert_incr, horz_incr);
}
DECL M casadi_diagcat(const std::vector<M>& A) { return diagcat(A); }
DECL std::vector<M> casadi_diagsplit(
    const M& x, const std::vector<casadi_int>& output_offset1,
    const std::vector<casadi_int>& output_offset2) {
  return diagsplit(x, output_offset1, output_offset2);
}
DECL std::vector<M> casadi_diagsplit(
    const M& x, const std::vector<casadi_int>& output_offset) {
  return diagsplit(x, output_offset);
}
DECL std::vector<M> casadi_diagsplit(const M& x, casadi_int incr = 1) {
  return diagsplit(x, incr);
}
DECL std::vector<M> casadi_diagsplit(const M& x, casadi_int incr1,
                                     casadi_int incr2) {
  return diagsplit(x, incr1, incr2);
}
DECL M casadi_veccat(const std::vector<M>& x) { return veccat(x); }
DECL M casadi_mtimes(const M& x, const M& y) { return mtimes(x, y); }
DECL M casadi_mtimes(const std::vector<M>& args) { return mtimes(args); }
DECL M casadi_mac(const M& X, const M& Y, const M& Z) { return mac(X, Y, Z); }
DECL M casadi_transpose(const M& X) { return X.T(); }
DECL M casadi_vec(const M& a) { return vec(a); }
...
DECL M casadi_reshape(const M& a, const Sparsity& sp) { return reshape(a, sp); }
DECL M casadi_sparsity_cast(const M& a, const Sparsity& sp) {
  return sparsity_cast(a, sp);
}
DECL casadi_int casadi_sprank(const M& A) { return sprank(A); }
...
DECL M casadi_kron(const M& a, const M& b) { return kron(a, b); }
...
DECL M casadi_sum2(const M& x) { return sum2(x); }
DECL M casadi_sum1(const M& x) { return sum1(x); }
#endif
% enddef
    ...
    % define SPARSITY_INTERFACE_ALL(DECL, FLAG)
          SPARSITY_INTERFACE_FUN(DECL, (FLAG | IS_SPARSITY), Sparsity)
              SPARSITY_INTERFACE_FUN(DECL, (FLAG | IS_MX), MX)
                  SPARSITY_INTERFACE_FUN(DECL, (FLAG | IS_DMATRIX),
                                         Matrix<double>)
                      SPARSITY_INTERFACE_FUN(DECL, (FLAG | IS_SX),
                                             Matrix<SXElem>) %
    enddef
#ifdef SWIGMATLAB
...
#if FLAG & IS_MEMBER
DECL M casadi_mpower(const M& x, const M& n) { return mpower(x, n); }

DECL M casadi_mrdivide(const M& x, const M& y) { return mrdivide(x, y); }

DECL M casadi_mldivide(const M& x, const M& y) { return mldivide(x, y); }

DECL std::vector<M> casadi_symvar(const M& x) { return symvar(x); }

DECL M casadi_bilin(const M& A, const M& x, const M& y) {
  return bilin(A, x, y);
}

DECL M casadi_bilin(const M& A, const M& x) { return bilin(A, x); }

DECL M casadi_rank1(const M& A, const M& alpha, const M& x, const M& y) {
  return rank1(A, alpha, x, y);
}

DECL M casadi_sumsqr(const M& X) { return sumsqr(X); }

DECL M casadi_linspace(const M& a, const M& b, casadi_int nsteps) {
  return linspace(a, b, nsteps);
}

DECL M casadi_logsumexp(const M& a) { return logsumexp(a); }
...
DECL M casadi_soc(const M& x, const M& y) { return soc(x, y); }

DECL M casadi_cross(const M& a, const M& b, casadi_int dim = -1) {
  return cross(a, b, dim);
}

DECL M casadi_skew(const M& a) { return skew(a); }

DECL M casadi_inv_skew(const M& a) { return inv_skew(a); }

DECL M casadi_det(const M& A) { return det(A); }

DECL M casadi_inv_minor(const M& A) { return inv_minor(A); }

DECL M casadi_inv(const M& A) { return inv(A); }
...
DECL M casadi_trace(const M& a) { return trace(a); }

DECL M casadi_tril2symm(const M& a) { return tril2symm(a); }

DECL M casadi_triu2symm(const M& a) { return triu2symm(a); }

DECL M casadi_norm_fro(const M& x) { return norm_fro(x); }

DECL M casadi_norm_2(const M& x) { return norm_2(x); }

DECL M casadi_norm_1(const M& x) { return norm_1(x); }

DECL M casadi_norm_inf(const M& x) { return norm_inf(x); }

DECL M casadi_dot(const M& x, const M& y) { return dot(x, y); }

DECL M casadi_nullspace(const M& A) { return nullspace(A); }

DECL M casadi_polyval(const M& p, const M& x) { return polyval(p, x); }

DECL M casadi_diag(const M& A) { return diag(A); }

DECL M casadi_unite(const M& A, const M& B) { return unite(A, B); }

DECL M casadi_densify(const M& x) { return densify(x); }

DECL M casadi_project(const M& A, const Sparsity& sp, bool intersect = false) {
  return project(A, sp, intersect);
}

DECL M casadi_if_else(const M& cond, const M& if_true, const M& if_false,
...
DECL M casadi_solve(const M& A, const M& b) { return solve(A, b); }

DECL M casadi_solve(const M& A, const M& b, const std::string& lsolver,
                    const casadi::Dict& opts = casadi::Dict()) {
  return solve(A, b, lsolver, opts);
}

DECL M casadi_pinv(const M& A) { return pinv(A); }
...
DECL M casadi_expm_const(const M& A, const M& t) { return expm_const(A, t); }

DECL M casadi_expm(const M& A) { return expm(A); }
...
DECL M casadi_gradient(const M& ex, const M& arg) { return gradient(ex, arg); }

DECL M casadi_tangent(const M& ex, const M& arg) { return tangent(ex, arg); }
...
DECL void casadi_quadratic_coeff(const M& ex, const M& arg, M& OUTPUT1,
                                 M& OUTPUT2, M& OUTPUT3, bool check = true) {
...
DECL casadi_int casadi_n_nodes(const M& A) { return n_nodes(A); }
...
DECL casadi::DM casadi_evalf(const M& x) { return evalf(x); }
DECL std::vector<M> casadi_cse(const std::vector<M>& e) { return cse(e); }
DECL M casadi_cse(const M& e) { return cse(e); }

#endif  // FLAG & IS_MEMBER

#if FLAG & IS_GLOBAL
DECL std::vector<std::vector<M> > casadi_forward(
    const std::vector<M>& ex, const std::vector<M>& arg,
    const std::vector<std::vector<M> >& v, const Dict& opts = Dict()) {
  return forward(ex, arg, v, opts);
}

DECL std::vector<std::vector<M> > casadi_reverse(
    const std::vector<M>& ex, const std::vector<M>& arg,
    const std::vector<std::vector<M> >& v, const Dict& opts = Dict()) {
...
DECL void casadi_extract(const std::vector<M>& ex, std::vector<M>& OUTPUT1,
                         std::vector<M>& OUTPUT2, std::vector<M>& OUTPUT3,
                         const Dict& opts = Dict()) {
  OUTPUT1 = ex;
  extract(OUTPUT1, OUTPUT2, OUTPUT3, opts);
}

DECL void casadi_shared(const std::vector<M>& ex, std::vector<M>& OUTPUT1,
                        std::vector<M>& OUTPUT2, std::vector<M>& OUTPUT3,
...
DECL M casadi_erfinv(const M& x) {
  using casadi::erfinv;
  return erfinv(x);
}
DECL M casadi_sign(const M& x) {
  using casadi::sign;
  return sign(x);
}
...
DECL M casadi_simplify(const M& x) {
  using casadi::simplify;
  return simplify(x);
}
DECL bool casadi_is_equal(const M& x, const M& y, casadi_int depth = 0) {
  using casadi::is_equal;
  return is_equal(x, y, depth);
}
DECL M casadi_copysign(const M& x, const M& y) { return copysign(x, y); }
DECL M casadi_constpow(const M& x, const M& y) {
  using casadi::constpow;
  return constpow(x, y);
}
#endif  // FLAG & IS_MEMBER
% enddef
...
#if FLAG & IS_MEMBER
DECL M casadi_all(const M& x) { return all(x); }

DECL M casadi_any(const M& x) { return any(x); }

DECL M casadi_adj(const M& A) { return adj(A); }
...
DECL void casadi_qr_sparse(const M& A, M& OUTPUT1, M& OUTPUT2, M& OUTPUT3,
                           std::vector<casadi_int>& OUTPUT4,
                           std::vector<casadi_int>& OUTPUT5, bool amd = true) {
...
DECL void casadi_ldl(const M& A, M& OUTPUT1, M& OUTPUT2,
                     std::vector<casadi_int>& OUTPUT3, bool amd = true) {
...
DECL M casadi_chol(const M& A) { return chol(A); }

DECL M casadi_norm_inf_mul(const M& x, const M& y) {
  return norm_inf_mul(x, y);
}

DECL M casadi_sparsify(const M& A, double tol = 0) { return sparsify(A, tol); }
...
DECL M casadi_heaviside(const M& x) { return heaviside(x); }

DECL M casadi_rectangle(const M& x) { return rectangle(x); }

DECL M casadi_triangle(const M& x) { return triangle(x); }

DECL M casadi_ramp(const M& x) { return ramp(x); }

DECL M casadi_gauss_quadrature(const M& f, const M& x, const M& a, const M& b,
                               casadi_int order = 5) {
...
DECL M casadi_taylor(const M& ex, const M& x, const M& a = 0,
                     casadi_int order = 1) {
  return taylor(ex, x, a, order);
}

DECL M casadi_mtaylor(const M& ex, const M& x, const M& a,
                      casadi_int order = 1) {
...
DECL M casadi_poly_coeff(const M& ex, const M& x) { return poly_coeff(ex, x); }

DECL M casadi_poly_roots(const M& p) { return poly_roots(p); }

DECL M casadi_eig_symbolic(const M& m) { return eig_symbolic(m); }
...
#if FLAG & IS_MEMBER
DECL M casadi_find(const M& x) { return find(x); }
DECL M casadi_low(const M& v, const M& p, const Dict& options = Dict()) {
  return low(v, p, options);
}
DECL M casadi_inv_node(const M& x) { return inv_node(x); }
#endif  // FLAG & IS_MEMBER

#if FLAG & IS_GLOBAL
DECL std::vector<M> casadi_matrix_expand(
    const std::vector<M>& e, const std::vector<M>& boundary = std::vector<M>(),
...
DECL std::vector<M> casadi_graph_substitute(const std::vector<M>& ex,
                                            const std::vector<M>& v,
                                            const std::vector<M>& vdef) {
  return graph_substitute(ex, v, vdef);
}
DECL M casadi_bspline(const M& x, const DM& coeffs,
                      const std::vector<std::vector<double> >& knots,
                      const std::vector<casadi_int>& degree, casadi_int m,
                      const Dict& opts = Dict()) {
  return bspline(x, coeffs, knots, degree, m, opts);
}
DECL M casadi_bspline(const M& x, const M& coeffs,
                      const std::vector<std::vector<double> >& knots,
                      const std::vector<casadi_int>& degree, casadi_int m,
                      const Dict& opts = Dict()) {
  return bspline(x, coeffs, knots, degree, m, opts);
}
DECL M casadi_convexify(const M& H, const Dict& opts = Dict()) {
...
DECL M casadi_no_hess(const M& expr) { return no_hess(expr); }
DECL M casadi_no_grad(const M& expr) { return no_grad(expr); }

#endif
% enddef

    % define MX_ALL(DECL, FLAG) MX_FUN(DECL, (FLAG | IS_MX), MX) % enddef
...
% extend casadi::Matrix<double> { % template(DM) Matrix<SXElem>; };
...
% extend Matrix<double> {
  ...
  GUESTOBJECT* full() const { return full(*$self); }

  // Convert to a sparse matrix
  GUESTOBJECT* sparse() const { return sparse(*$self); }
}
...
% extend Matrix<double>{
      ...
      // Normally, because of the presence of __array__, an ufunctor like
      // nump.sqrt will unleash its activity on the output of __array__ However,
      // we wish DM to remain a DM So when we receive a call from a functor, we
      // return a dummy empty array and return the real result during the
      // postprocessing (__array_wrap__) of the functor.
      % pythoncode % {
          def __array_custom__(self, *args, **kwargs) : if "dtype" in kwargs and
          not(isinstance(kwargs["dtype"], n.double)) : return n
                  .array(self.full(), dtype = kwargs["dtype"]) else :
                      return self.full() %
        }

      % pythoncode %
      {
        def tocsc(self) : import numpy as np import warnings with warnings
            .catch_warnings() : warnings.simplefilter("ignore") from scipy
            .sparse import csc_matrix return csc_matrix(
                (self.nonzeros(), self.row(), self.colind()),
                shape = self.shape, dtype = np.double)
                def toarray(self, simplify = False) : import numpy as np if
        simplify : if self.is_scalar() : return float(self)
            elif self.is_vector() : return np.array(self.T.elements()) return np
            .array(self.T.elements())
            .reshape(self.shape) %
      }

#ifdef WITH_PYTHON3
      % pythoncode % {
          def __bool__(self) : if self.numel() !=
          1 : raise Exception(
              "Only a scalar can be cast to a float") if self.nnz() ==
          0 : return False return float(self) != 0 %
        }
#else
      % pythoncode % {
          def __nonzero__(self) : if self.numel() !=
          1 : raise Exception(
              "Only a scalar can be cast to a float") if self.nnz() ==
          0 : return False return float(self) != 0 %
        }
      ...
      % pythoncode % {def __abs__(self) : return abs(float(self)) % }
...
% extend Matrix<double>{
      ...
      % matlabcode %
      {function s = saveobj(obj) try s.serialization = obj.serialize();
catch exception warning(
    ['Serializing of CasADi DM failed:' getReport(exception)]);
s = struct;
end end %
}  // namespace casadi
% matlabcode_static % {
  function obj = loadobj(s) try if isstruct (s) obj =
      casadi.DM.deserialize(s.serialization);
  else obj = s;
  end catch exception warning(
      ['Serializing of CasADi DM failed:' getReport(exception)]);
  s = struct;
  end end %
}
...
% extend Sparsity{
    ...
    ['Serializing of CasADi Sparsity failed:' getReport(exception)]);
s = struct;
end end %
}
% matlabcode_static % {
  function obj = loadobj(s) try if isstruct (s) obj =
      casadi.Sparsity.deserialize(s.serialization);
  else obj = s;
  end catch exception warning(
      ['Serializing of CasADi Sparsity failed:' getReport(exception)]);
  s = struct;
...
% extend Function {
  % matlabcode % {
    function s = saveobj(obj) try s.serialization = obj.serialize();
        ...
        ['Serializing of CasADi Function failed:' getReport(exception)]);
    s = struct;
    end end %
  }
  % matlabcode_static % {
    function obj = loadobj(s) try if isstruct (s) obj =
        casadi.Function.deserialize(s.serialization);
    else obj = s;
    end catch exception warning(
        ['Serializing of CasADi Function failed:' getReport(exception)]);
    s = struct;
...
#ifdef SWIGPYTHON
    % extend casadi::Sparsity{ % pythoncode % {
        @property def shape(self) : return (self.size1(), self.size2())

            @property def T(self) : return _casadi
                .transpose(self)

                    def __array__(self, *args, **kwargs) : return DM.ones(self)
                .full() %
      }};
...
namespace casadi {
% extend Matrix<SXElem>{ % matrix_helpers(casadi::Matrix<casadi::SXElem>)
...
% extend casadi::Matrix<casadi::SXElem> { % template(SX) Matrix<double>; };

% include<casadi / core / mx.hpp>

    % extend casadi::MX{ % matrix_helpers(casadi::MX)
#ifdef SWIGPYTHON
...
% extend GenericMatrixCommon{
      % matlabcode %
      {function varargout = spy(self, varargin)[varargout{1 : nargout}] =
           spy(sparse(casadi.DM(self.sparsity(), 1)), varargin{ : });
end function varargout =
    subsref(self, s) if numel (s) == 1 && strcmp(s.type, '()')[varargout{1}] =
        paren(self, s.subs{ : });
elseif numel(s) == 1 &&
    strcmp(s.type, '{}')[varargout{1}] = brace(self, s.subs{ : });
else[varargout{1 : nargout}] = builtin('subsref', self, s);
end end function self = subsasgn(self, s, v) if numel (s) == 1 &&
                        strcmp(s.type, '()') paren_asgn(self, v, s.subs{ : });
elseif numel(s) == 1 && strcmp(s.type, '{}') brace_asgn(self, v, s.subs{ : });
else self = builtin('subsasgn', self, s, v);
end end function out = sum(self, varargin) narginchk(1, 3);
if nargin
  == 1 if is_vector (self) if is_column (self) out = sum1(self);
else
  out = sum2(self);
end else out = sum1(self);
end else i = varargin{1};
if i
  == 1 out = sum1(self);
elseif i == 2 out = sum2(self);
else error('sum argument (if present) must be 1 or 2');
end end end function out = norm(self, varargin) narginchk(1, 2);
% 2 - norm by default if nargin == 1 ind = 2;
else ind = varargin{1};
      end
      % Typecheck
...
end end function varargout = paren(self, varargin) if nargin == 1 ||
                             (nargin >= 2 && ischar(varargin{1})) % Named inputs
    : return struct assert(nargout < 2, 'Syntax error');
assert(mod(nargin, 2) == 1, 'Syntax error');
arg = struct;
        for
          i = 1 : 2 : nargin - 1 assert(ischar(varargin{i}), 'Syntax error');
        arg.(varargin{i}) = varargin{i + 1};
        end res = self.call(arg);
        varargout{1} = res;
        else % Ordered inputs : return variable number of outputs res =
            self.call(varargin);
        assert(nargout <= numel(res), 'Too many outputs');
        for
          i = 1 : max(min(1, numel(res)), nargout) varargout{i} = res{i};
        end end end %
        }
        }
        }
        ...
        %
            ...
            extend MatrixCommon{MATRIX_ALL(static inline, IS_MEMBER)} %
            extend MX {
          MX_ALL(static inline, IS_MEMBER)
          const MX brace(const casadi::MX& rr) const {
            casadi::MX m;
            $self->get_nz(m, true, rr);
            return m;
          }
...
#define FLAG (IS_GLOBAL | IS_MEMBER)
        ...
        % extend GenericExpressionCommon {
          % pythoncode % {
            def __hash__(self) : try:
              return self.element_hash() except : return SharedObject
                                                      .__hash__(self) def
                                                      __matmul__(x, y)
                  : return _casadi.mtimes(x, y) def __rmatmul__(x, y)
...
#endif
        namespace casadi {
        % extend GenericExpressionCommon{ % pythoncode % {
          def __add__(x, y) : return _casadi
              .plus(x, y) def __radd__(x, y) :
                  return _casadi.plus(y, x) def __sub__(x, y) : return _casadi
              .minus(x, y) def __rsub__(x, y) :
                  return _casadi.minus(y, x) def __mul__(x, y) : return _casadi
              .times(x, y) def __rmul__(x, y) :
                  return _casadi.times(y, x) def __div__(x, y) : return _casadi
              .rdivide(x, y) def __rdiv__(x, y) : return _casadi
              .rdivide(y, x) def __truediv__(x, y) : return _casadi
              .rdivide(x, y) def __rtruediv__(x, y) :
                  return _casadi.rdivide(y, x) def __lt__(x, y) :
                      return _casadi.lt(x, y) def __rlt__(x, y) : return _casadi
              .lt(y, x) def __le__(x, y) :
                  return _casadi.le(x, y) def __rle__(x, y) :
                      return _casadi.le(y, x) def __gt__(x, y) : return _casadi
              .lt(y, x) def __rgt__(
                  x,
                  y) : return _casadi.lt(x, y) def __ge__(x, y) :
                  return _casadi.le(y, x) def __rge__(x, y) :
                      return _casadi.le(x, y) def __eq__(x, y) : return _casadi
              .eq(x, y) def __req__(x, y) :
                  return _casadi.eq(y, x) def __ne__(x, y) :
                      return _casadi.ne(x, y) def __rne__(x, y) : return _casadi
              .ne(y, x) def __pow__(x, n) :
                  return _casadi.power(x, n) def __rpow__(n, x) : return _casadi
              .power(x, n) def __arctan2__(x, y) : return _casadi
              .atan2(x, y) def __rarctan2__(y, x) :
                  return _casadi.atan2(x, y) def fmin(x, y) :
                      return _casadi.fmin(x, y) def fmax(x, y) : return _casadi
              .fmax(x, y) def __fmin__(x, y) :
                  return _casadi.fmin(x, y) def __rfmin__(y, x) : return _casadi
              .fmin(x, y) def __fmax__(x, y) :
                  return _casadi.fmax(x, y) def __rfmax__(y, x) : return _casadi
              .fmax(x, y) def logic_and(x, y) : return _casadi
              .logic_and(x, y) def logic_or(x, y) :
                  return _casadi.logic_or(x, y) def fabs(x) :
                      return _casadi.fabs(x) def sqrt(x) :
                          return _casadi.sqrt(x) def sin(x) :
                              return _casadi.sin(x) def cos(x) : return _casadi
              .cos(x) def tan(x) :
                  return _casadi.tan(x) def arcsin(x) :
                      return _casadi.asin(x) def arccos(x) :
                          return _casadi.acos(x) def arctan(x) : return _casadi
              .atan(x) def sinh(x) :
                  return _casadi.sinh(x) def cosh(x) :
                      return _casadi.cosh(x) def tanh(x) :
                          return _casadi.tanh(x) def arcsinh(x) : return _casadi
              .asinh(x) def arccosh(
                  x) : return _casadi.acosh(x) def arctanh(x) :
                  return _casadi.atanh(x) def exp(x) :
                      return _casadi.exp(x) def log(x) :
                          return _casadi.log(x) def log10(x) : return _casadi
              .log10(x) def log1p(x) :
                  return _casadi.log1p(x) def expm1(x) :
                      return _casadi.expm1(x) def floor(x) :
                          return _casadi.floor(x) def ceil(x) :
                              return _casadi.ceil(x) def erf(x) : return _casadi
              .erf(x) def sign(x) :
                  return _casadi.sign(x) def fmod(x, y) :
                      return _casadi.mod(x, y) def hypot(x, y) : return _casadi
              .hypot(x, y) def remainder(x, y) : return _casadi
              .remainder(x, y) def __copysign__(x, y) : return _casadi
              .copysign(x, y) def __rcopysign__(y, x) : return _casadi
              .copysign(x, y) def copysign(x, y) : return _casadi
              .copysign(x, y) def rcopysign(y, x) : return _casadi
              .copysign(x, y) def __constpow__(x, y) : return _casadi
              .constpow(x, y) def __rconstpow__(y, x) : return _casadi
              .constpow(x, y) def constpow(x, y) : return _casadi
              .constpow(x, y) def rconstpow(y, x) :
          ...
          % pythoncode % {
            def __mldivide__(x, y)
                : return _casadi
                      .mldivide(x, y) def __rmldivide__(y, x)
                : return _casadi
                      .mldivide(x, y) def __mrdivide__(x, y)
                : return _casadi
                      .mrdivide(x, y) def __rmrdivide__(y, x)
                : return _casadi
                      .mrdivide(x, y) def __mpower__(x, y)
                : return _casadi
                      .mpower(x, y) def __rmpower__(y, x)
        ...
        % rename("%(regex:/(SERIALIZED_\w+)/_\\1/)s", regextarget = 1,
                 fullname = 1) "casadi::SerializerBase::SERIALIZED_\w+";
#endif  // SWIG_PYTHON

#ifdef SWIGMATLAB
        % rename("internal_pop_type") casadi::DeserializerBase::pop_type;
        % rename("%(regex:/(SERIALIZED_\w+)/internal_\\1/)s", regextarget = 1,
                 fullname = 1) "casadi::SerializerBase::SERIALIZED_\w+";
#endif  // SWIG_PYTHON
...
#ifdef SWIGPYTHON
            % extend casadi::DeserializerBase{ % pythoncode % {
                def unpack(self) : type =
                    SerializerBase.type_to_string(self._pop_type()) f =
                        getattr(self, "blind_unpack_" + type) return f() %
              }}
#endif  // SWIGPYTHON
#ifdef SWIGMATLAB
            % extend casadi::DeserializerBase{
                  % matlabcode %
                  {function out = unpack(self) type =
                       casadi.SerializerBase.type_to_string(
                           self.internal_pop_type);
        ...
        // Return-by-value
        % typemap(out, doc = "double", noblock = 1,
                  fragment = "casadi_all") casadi::native_DM {
          if (!($result = full_or_sparse($1, true)))
            SWIG_exception_fail(SWIG_TypeError,
                                "Failed to convert output to type 'double'.");
        }

        % apply casadi_int& OUTPUT{Opti::ConstraintType & OUTPUT};

        % typemap(argout, noblock = 1, fragment = "casadi_all")
                casadi::Opti::ConstraintType& OUTPUT {
          % append_output(casadi::from_ptr((casadi_int*)$1));
        }

        % typemap(in, doc = "Opti.ConstraintType", noblock = 1, numinputs = 0)
                casadi::Opti::ConstraintType& OUTPUT(
                    casadi::Opti::ConstraintType m) {
          $1 = &m;
        }

#ifdef SWIGPYTHON

        % define make_property(class, name) % rename(_##name) class## ::##name;
        % extend class {
          % pythoncode % { @property def name(self) : return self._##name() % }
        } % enddef
        ...
        % define make_property_opti(name) make_property(casadi::Opti, name);
        % enddef

                make_property(casadi::OptiSol, debug);
        make_property_opti(f) make_property_opti(g) make_property_opti(x)
            make_property_opti(p) make_property_opti(lam_g)
                make_property_opti(lbg) make_property_opti(ubg)
                    make_property_opti(nx) make_property_opti(np)
        ...
        % extend class {
          % pythoncode % {
            def parameter(self, *args) : import sys import os try : frame
              = sys._getframe(1) except
                  : frame = {} meta = {} if frame is None else {
                "stacktrace" : {
                  "file" : os.path.abspath(frame.f_code.co_filename),
                      "line" : frame.f_lineno, "name" : frame.f_code.co_name
                }
              }
            ret = self._parameter(*args) if len (meta) > 0
                : self.update_user_dict(ret, meta) return ret

                      def
                      variable(self, *args)
                : import sys import os try : frame
              = sys._getframe(1) except
                  : frame = {} meta = {} if frame is None else {
                "stacktrace" : {
                  "file" : os.path.abspath(frame.f_code.co_filename),
            ...
            ret = self._variable(*args) if len (meta) > 0
                : self.update_user_dict(ret, meta) return ret

                      def
                      subject_to(self, *args)
                : if len (args) ==
                  0 : return self._subject_to() import sys import os try : frame
              = sys._getframe(1) except
                  : frame = {} meta = {} if frame is None else {
                "stacktrace" : {
                  "file" : os.path.abspath(frame.f_code.co_filename),
            ...
            ret = self._subject_to(*args) if len (meta) > 0
                : self.update_user_dict(args[0], meta) return ret %
          }
        } % enddef
        ...
        % extend class {
          % matlabcode % {
            function out = variable(self, varargin) st =
                dbstack('-completenames', 1);
            if length (st)
              > 0 meta = struct('stacktrace', st(1));
            else
              meta = struct;
            end out = self.internal_variable(varargin{ : });
            self.update_user_dict(out, meta);
            end function out = parameter(self, varargin) st =
                dbstack('-completenames', 1);
            if length (st)
              > 0 meta = struct('stacktrace', st(1));
            else
              meta = struct;
            end out = self.internal_parameter(varargin{ : });
            self.update_user_dict(out, meta);
            end function[] = subject_to(self, varargin) if length (varargin) ==
                             0 self.internal_subject_to();
            return;
            end st = dbstack('-completenames', 1);
            if length (st)
              > 0 meta = struct('stacktrace', st(1));
            else
              meta = struct;
            end self.internal_subject_to(varargin{ : });
...
#ifdef SWIGPYTHON
            % extend casadi::Opti{ % pythoncode % {

                @staticmethod def _callback(self, fh = None) : if fh is
                None : self.callback_class();
        return class OptiCallbackHelper(OptiCallback)
            : def __init__(self, callback)
            : OptiCallback.__init__(self) self.callback = callback

                                                              def call(self, i)
            : self.callback(i)

                  self._fh = fh self._cb = OptiCallbackHelper(fh);
        ...
        %
            extend casadi::Opti{
                ...
                {function[] = callback(self, varargin)
                                  casadi.OptiCallbackHelper.callback_setup(
                                      self, varargin{ : }) end % }}
#endif
