...
VEG_INLINE constexpr auto vector_grow_choose(usize current_cap,
                                             usize new_cap) noexcept -> usize {
...
struct relocate_pointer<T, false> {
  static constexpr void* (*value)(void*, void const*,
...
struct CloneImpl<true> {
  ...
  static VEG_CPP14(constexpr) void fn(  //
      RefMut<A> alloc, RefMut<C> cloner, T* out, T* out_end,
...
struct CloneImpl<false> {
  ...
  static void fn(  //
    ...
    for (; _.fn.ptr < _.fn.ptr_end; ++_.fn.ptr, ++in) {
      mem::construct_with(_.fn.ptr, CloneFn<false, T, A, C>{
                                        _.fn.alloc,
                                        _.fn.cloner,
                                        in,
                                    });
...
template <typename T, typename A, typename C>
VEG_CPP14(constexpr)
void slice_clone(RefMut<A> alloc, RefMut<C> cloner, T* out, T* out_end,
                 T const* in)
    VEG_NOEXCEPT_IF(VEG_CONCEPT(alloc::nothrow_clone<C, T, A>)) {
  CloneImpl<VEG_CONCEPT(alloc::nothrow_clone<C, T, A>)>::fn(alloc, cloner, out,
                                                            out_end, in);
}

template <typename T, typename A, typename C>
VEG_CPP14(constexpr)
void slice_clone_from(RefMut<A> alloc, RefMut<C> cloner, T* out, T* out_end,
  ...
  while (true) {
    ...
    mem::Cloner<C>::clone_from(  //
        RefMut<C>(cloner), mut(*out), ref(*in), RefMut<A>(alloc));
    ++out;
    ++in;
  }
}
...
template <typename T>
auto relocate_backward(void* out, void const* in,
                       usize nbytes) noexcept -> void* {
  T* out_T = static_cast<T*>(out);
  T* in_T = const_cast<T*>(static_cast<T const*>(in));
  usize n = nbytes / sizeof(T);
...
struct AllocCleanup {
  ...
  VEG_INLINE VEG_CPP14(constexpr) void operator()()
    ...
    if (data != nullptr) {
      mem::Alloc<A>::dealloc(RefMut<A>(alloc), static_cast<void*>(data),
                             mem::Layout(layout));
    }
  }
};
...
auto alloc_and_copy(RefMut<A> alloc, RefMut<C> cloner, T const* data, usize len)
  ...
  _collections::slice_clone(_.fn.alloc, cloner, static_cast<T*>(block.data),
                            static_cast<T*>(block.data) + len, data);
...
auto realloc_and_append(  //
    RefMut<A> alloc, RefMut<C> cloner, mem::AllocBlock out, usize out_len,
    T const* in, usize in_len)
  ...
  if (out.byte_cap >= (in_len + out_len) * sizeof(T)) {
    mem::AllocBlock block = mem::Alloc<A>::grow(
        RefMut<A>(alloc), static_cast<void*>(out.data),
        mem::Layout{out.byte_cap, alignof(T)}, out_len * sizeof(T),
    ...
    _collections::slice_clone(  //
        destroy.fn.alloc, destroy.fn.cloner,
        static_cast<T*>(block.data) + out_len,
        static_cast<T*>(block.data) + in_len, in);
    // disable destruction
    ...
    // copy construct elements
    _collections::slice_clone(  //
        alloc, cloner, static_cast<T*>(out.data) + out_len,
        static_cast<T*>(out.data) + in_len, in);
  }
  return out;
}
...
struct CloneFromImpl<false> {
  template <typename T, typename A, typename C>
  static void fn(RefMut<A> lhs_alloc, RefMut<C> cloner,
                 vector::RawVector<T>& lhs_raw, Ref<A> rhs_alloc,
    ...
    if (!(lhs_alloc == rhs_alloc)) {
      ...
      mem::Alloc<A>::dealloc(  //
          RefMut<A>(lhs_alloc), static_cast<void*>(data),
    ...
    _collections::slice_clone_from(  //
        lhs_alloc, cloner, lhs_copy.data, lhs_copy.data + assign_len,
        rhs_raw.data);

    // destroy from the shared len until end of lhs
    lhs_raw.end = lhs_raw.data + assign_len;
    _collections::backward_destroy(  //
        lhs_alloc, cloner, lhs_copy.data + assign_len, lhs_copy.end);
  ...
  static void fn(RefMut<A> lhs_alloc, RefMut<C> cloner,
    ...
    if (need_to_realloc) {
      mem::AllocBlock block = _collections::alloc_and_copy(  //
          lhs_alloc, cloner, rhs_raw.data, rhs_raw.len());
      lhs_raw.data = static_cast<T*>(block.data);
      lhs_raw.end_alloc = lhs_raw.data + block.byte_cap / sizeof(T);
    } else {
      _collections::slice_clone(  //
          lhs_alloc, cloner, lhs_copy.data, lhs_copy.data + rhs_raw.len(),
          rhs_raw.data);
    }
...
template <typename T, typename A, typename C>
VEG_INLINE void clone_from(RefMut<A> lhs_alloc, RefMut<C> cloner,
                           vector::RawVector<T>& lhs_raw, Ref<A> rhs_alloc,
                           vector::RawVector<T> const rhs_raw)
    VEG_NOEXCEPT_IF(VEG_CONCEPT(nothrow_copy_assignable<A>) &&     //
                    VEG_CONCEPT(alloc::nothrow_alloc<A>) &&        //
  ...
  _collections::CloneFromImpl<
      mem::Cloner<C>::template trivial_clone<T>::value>::fn(lhs_alloc, cloner,
                                                            lhs_raw, rhs_alloc,
                                                            rhs_raw);
}
}  // namespace _collections
  ...
  VEG_INLINE ~VecAlloc()
    ...
    if ((raw.data != nullptr) && (raw.end_alloc != 0)) {
      ...
      _detail::__sanitizer_annotate_contiguous_container(  //
          raw.data, raw.end_alloc, raw.data, raw.end_alloc);
#endif

      mem::Alloc<A>::dealloc(
          mut((*this)[0_c]), static_cast<void*>(raw.data),
...
struct VecImpl {
 ...
 private:
  VEG_INLINE void _reserve_grow_exact_impl(Unsafe /*tag*/, usize new_cap)
    ...
    mem::AllocBlock new_block = mem::Alloc<A>::grow(
        this->alloc_mut(unsafe), static_cast<void*>(raw.data),
        mem::Layout{
            usize(byte_capacity()),
            alignof(T),
        },
  ...
  VEG_INLINE void _reserve_one_more(Unsafe /*tag*/) {
    this->_reserve_grow_exact(
        unsafe, 1 + isize(_detail::_collections::vector_grow_compute(
                        usize(capacity()))));
  }
 ...
 public:
  ...
  VEG_INLINE
  VecImpl(Unsafe /*unsafe*/, FromRawParts /*tag*/, vector::RawVector<T> rawvec,
  ...
  explicit VecImpl(VecImpl const& rhs)
    ...
    mem::AllocBlock blk = _detail::_collections::alloc_and_copy(
        this->alloc_mut(unsafe), mut(mem::DefaultCloner{}), rhs_raw.data,
        usize(rhs.len()));
  ...
  auto operator=(VecImpl const& rhs)
    ...
    if (this != mem::addressof(rhs)) {
      ...
      _detail::_collections::clone_from(
          this->alloc_mut(unsafe), mut(mem::DefaultCloner{}),
          this->raw_mut(unsafe).get(), rhs.alloc_ref(), rhs.raw_ref().get());
    }
    return *this;
  }
  ...
  VEG_INLINE auto pop_mid_unchecked(Unsafe /*unsafe*/, isize i)
    ...
    // this block does not throw
    {
      ...
      _detail::_collections::relocate<T>(  //
          elem, elem + 1, sizeof(T) * usize(len() - i - 1));
    }
    --raw_mut(unsafe).get().end;
  ...
  VEG_TEMPLATE(typename U = T, requires(VEG_CONCEPT(constructible<U>)),
               void resize, (n, isize))
  VEG_NOEXCEPT_IF(VEG_CONCEPT(alloc::nothrow_grow<A>) &&
  ...
  VEG_TEMPLATE(typename U = T, requires(VEG_CONCEPT(constructible<U>)),
               void resize_for_overwrite, (n, isize))
  VEG_NOEXCEPT_IF(VEG_CONCEPT(alloc::nothrow_grow<A>) &&
                  VEG_CONCEPT(nothrow_constructible<T>)) {
    vector::RawVector<T>& raw = raw_mut(unsafe).get();
  ...
  VEG_TEMPLATE(typename Fn, requires(VEG_CONCEPT(fn_once<Fn, T>)),
               VEG_INLINE void push_mid_with, (fn, Fn), (i, isize))
  VEG_NOEXCEPT_IF(VEG_CONCEPT(nothrow_fn_once<Fn, T>) &&
    ...
    reserve(len() + 1);
    {
      ...
      _detail::_collections::relocate_backward<T>(  //
          elem + 1, elem, sizeof(T) * usize(raw.end - elem));
  ...
  VEG_TEMPLATE(typename Fn, requires(VEG_CONCEPT(fn_once<Fn, T>)),
               VEG_INLINE void push_with_unchecked, (/*tag*/, Unsafe), (fn, Fn))
  VEG_NOEXCEPT_IF(VEG_CONCEPT(nothrow_fn_once<Fn, T>) &&
                  VEG_CONCEPT(alloc::nothrow_alloc<A>)) {
    __VEG_ASAN_ANNOTATE();
  ...
  VEG_TEMPLATE(typename Fn, requires(VEG_CONCEPT(fn_once<Fn, T>)),
               VEG_INLINE void push_with, (fn, Fn))
  VEG_NOEXCEPT_IF(VEG_CONCEPT(nothrow_fn_once<Fn, T>) &&
                  VEG_CONCEPT(alloc::nothrow_alloc<A>)) {
    vector::RawVector<T> raw = this->raw_ref().get();
  ...
  VEG_NODISCARD VEG_INLINE auto operator[](isize i) const
      VEG_NOEXCEPT->T const& {
...
template <typename T, typename A = mem::SystemAlloc,
          ...
          mem::CopyAvailable Copy = mem::CopyAvailableFor<T>::value>
struct Vec
    : private _detail::_vector::adl::AdlBase,
      private meta::if_t<  //
          Copy == mem::CopyAvailable::no, _detail::NoCopy, _detail::Empty>,
