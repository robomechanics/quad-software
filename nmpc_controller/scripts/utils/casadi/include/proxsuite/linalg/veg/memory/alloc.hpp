...
namespace alignment {
    ...
    (defined(_LIBCPP_HAS_ALIGNED_ALLOC) ||     \
     defined(_LIBCPP_HAS_C11_FEATURES)) &&     \
    defined(PROXSUITE_WITH_CPP_17)
VEG_INLINE void* aligned_alloc(std::size_t alignment, std::size_t size) {
...
constexpr inline bool is_alignment(std::size_t value) {
...
inline void* aligned_alloc(std::size_t alignment, std::size_t size) {
  assert(is_alignment(alignment));
  enum { N = alignment_of<void*>::value };
  if (alignment < N) {
    alignment = N;
  }
...
struct DtorAvailableFor
    : meta::constant<mem::DtorAvailable, VEG_CONCEPT(nothrow_destructible<T>)
...
struct Alloc<SystemAlloc> {
  ...
  VEG_INLINE static void dealloc(RefMut<SystemAlloc> /*alloc*/, void* ptr,
  ...
  VEG_NODISCARD VEG_NO_INLINE static auto realloc(
      RefMut<SystemAlloc> /*alloc*/, void* ptr, Layout layout, usize new_size,
      usize copy_size, RelocFn reloc) noexcept -> mem::AllocBlock {
    void* new_ptr;  // NOLINT
  ...
  VEG_NODISCARD VEG_INLINE static auto grow(
    ...
    return realloc(VEG_FWD(alloc), ptr, layout, new_size, layout.byte_size,
                   reloc);
  }
...
struct Cloner<DefaultCloner> {
  ...
  VEG_INLINE static void destroy(RefMut<DefaultCloner> /*cloner*/, T* ptr,
                                 RefMut<Alloc> /*alloc*/)
      VEG_NOEXCEPT_IF(VEG_CONCEPT(nothrow_destructible<T>)) {
    mem::destroy_at(ptr);
  }
  VEG_TEMPLATE((typename T, typename Alloc), requires(VEG_CONCEPT(copyable<T>)),
               VEG_NODISCARD VEG_INLINE static auto clone,
               (/*cloner*/, RefMut<DefaultCloner>), (rhs, Ref<T>),
               (/*alloc*/, RefMut<Alloc>))
  VEG_NOEXCEPT_IF(VEG_CONCEPT(nothrow_copyable<T>))->T { return T(rhs.get()); }
  VEG_TEMPLATE((typename T, typename Alloc), requires(VEG_CONCEPT(copyable<T>)),
               VEG_INLINE static void clone_from,
               (/*cloner*/, RefMut<DefaultCloner>), (lhs, RefMut<T>),
               (rhs, Ref<T>), (/*alloc*/, RefMut<Alloc>))
