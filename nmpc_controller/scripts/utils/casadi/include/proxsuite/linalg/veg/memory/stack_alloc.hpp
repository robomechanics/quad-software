...
struct BumpAllocLayout {
  ...
  auto _alloc(mem::Layout layout) noexcept -> mem::AllocBlock {
    ...
    VEG_ASSERT_ALL_OF(  //
        (layout.align <= MaxAlign), (given_bytes < rem_bytes));
  ...
  auto _grow_last(void* ptr, mem::Layout old_layout, usize new_byte_size,
  ...
  auto _grow_any(void* ptr, mem::Layout old_layout, usize new_byte_size,
                 mem::RelocFn reloc) noexcept -> mem::AllocBlock {
...
struct Alloc<BumpAlloc<MaxAlign>> {
  ...
  VEG_INLINE static void dealloc(RefMut ref, void* ptr,
                                 mem::Layout layout) noexcept {
    ImplMut(ref.get())._dealloc_any(ptr, layout);
  }
  VEG_INLINE static auto grow(RefMut ref, void* ptr, mem::Layout old_layout,
                              usize new_byte_size,
