...
extern "C" void __sanitizer_annotate_contiguous_container /* NOLINT */ (
    const void* begin, const void* end, const void* old_mid,
    const void* new_mid);
#endif
...
namespace mem {
inline auto memmove(void* dest, void const* src,
...
namespace concepts {
namespace alloc {
VEG_CONCEPT_EXPR((typename A), (A), dealloc,
                 mem::Alloc<A>::dealloc(  //
                     VEG_DECLVAL(RefMut<A>), VEG_DECLVAL(void*),
                     VEG_DECLVAL(mem::Layout)),
                 true);

VEG_CONCEPT_EXPR((typename A), (A), alloc,
                 mem::Alloc<A>::alloc(VEG_DECLVAL(RefMut<A>),
                                      VEG_DECLVAL(mem::Layout)),
                 VEG_CONCEPT(same<ExprType, mem::AllocBlock>));

VEG_CONCEPT_EXPR((typename A), (A), owns_alloc,
                 mem::Alloc<A>::owns(VEG_DECLVAL(Ref<A>),
                                     VEG_DECLVAL(mem::Layout)),
                 VEG_CONCEPT(same<ExprType, bool>));

VEG_CONCEPT_EXPR((typename A), (A), grow,
                 mem::Alloc<A>::grow(VEG_DECLVAL(RefMut<A>), VEG_DECLVAL(void*),
                                     VEG_DECLVAL(mem::Layout),
                                     VEG_DECLVAL(usize),
                                     VEG_DECLVAL(mem::RelocFn)),
                 VEG_CONCEPT(same<ExprType, mem::AllocBlock>));

VEG_CONCEPT_EXPR((typename A), (A), shrink,
...
VEG_CONCEPT_EXPR((typename C, typename T, typename A), (C, T, A), destroy,
                 mem::Cloner<C>::destroy(  //
                     VEG_DECLVAL(RefMut<C>), VEG_DECLVAL(T*),
                     VEG_DECLVAL(RefMut<A>)),
                 true);

VEG_CONCEPT_EXPR((typename C, typename T, typename A), (C, T, A), clone,
                 mem::Cloner<C>::clone(  //
                     VEG_DECLVAL(RefMut<C>), VEG_DECLVAL(Ref<T>),
                     VEG_DECLVAL(RefMut<A>)),
                 VEG_CONCEPT(same<ExprType, T>));

VEG_CONCEPT_EXPR((typename C, typename T, typename A), (C, T, A), clone_from,
