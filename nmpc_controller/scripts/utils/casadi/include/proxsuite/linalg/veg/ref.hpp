...
struct mut {
  VEG_TEMPLATE(typename T, requires(!VEG_CONCEPT(const_type<meta::unref_t<T>>)),
               constexpr auto operator(), (r, T&&))
  const noexcept -> RefMut<meta::uncvref_t<T>> { return mem::addressof(r); }
};
struct clone {
  VEG_TEMPLATE(typename T, requires(VEG_CONCEPT(copyable<T>)),
               VEG_INLINE constexpr auto operator(), (arg, Ref<T>))
