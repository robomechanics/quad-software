...
struct proxsuite::linalg::veg::mem::Alloc<
  ...
  VEG_INLINE static void dealloc(RefMut /*alloc*/, void* ptr,
                                 Layout l) noexcept {
    return Alloc<SystemAlloc>::dealloc(mut(SystemAlloc{}), ptr,
                                       adjusted_layout(l));
  }

  VEG_NODISCARD VEG_INLINE static auto alloc(
      RefMut /*alloc*/, Layout l) noexcept -> mem::AllocBlock {
    return Alloc<SystemAlloc>::alloc(mut(SystemAlloc{}), adjusted_layout(l));
  }

  VEG_NODISCARD VEG_INLINE static auto grow(
      RefMut /*alloc*/, void* ptr, Layout l, usize new_size,
      RelocFn reloc) noexcept -> mem::AllocBlock {
    return Alloc<SystemAlloc>::grow(mut(SystemAlloc{}), ptr, adjusted_layout(l),
                                    new_size, reloc);
  }
  VEG_NODISCARD VEG_INLINE static auto shrink(
      RefMut /*alloc*/, void* ptr, Layout l, usize new_size,
      RelocFn reloc) noexcept -> mem::AllocBlock {
    return Alloc<SystemAlloc>::shrink(mut(SystemAlloc{}), ptr,
                                      adjusted_layout(l), new_size, reloc);
  }
};
...
struct Ldlt {
 private:
  ...
  using LView = Eigen::TriangularView<
      Eigen::Map<  //
          ColMat const, Eigen::Unaligned, Eigen::OuterStride<DYN>>,
      Eigen::UnitLower>;
  using LViewMut = Eigen::TriangularView<
      Eigen::Map<  //
          ColMat, Eigen::Unaligned, Eigen::OuterStride<DYN>>,
      Eigen::UnitLower>;

  using LTView = Eigen::TriangularView<
      Eigen::Map<  //
          RowMat const, Eigen::Unaligned, Eigen::OuterStride<DYN>>,
      Eigen::UnitUpper>;
  using LTViewMut = Eigen::TriangularView<
      Eigen::Map<  //
          RowMat, Eigen::Unaligned, Eigen::OuterStride<DYN>>,
      Eigen::UnitUpper>;
  ...
  using StorageSimdVec = proxsuite::linalg::veg::Vec<
      ...
      proxsuite::linalg::veg::meta::if_t<
          _detail::should_vectorize<T>::value, _detail::SimdAlignedSystemAlloc,
 ...
 public:
  ...
  void reserve(isize cap) noexcept {
    ...
    for (isize i = 0; i < n; ++i) {
      ...
      std::move_backward(  //
          ptr + col * stride, ptr + col * stride + n,
   ...
   */
  void delete_at(isize const* indices, isize r,
    ...
    proxsuite::linalg::dense::ldlt_delete_rows_and_cols_sort_indices(  //
        ld_col_mut(), indices_actual, r, stack);
   ...
   * @param stack workspace memory stack
   */
  void insert_block_at(isize i, Eigen::Ref<ColMat const> a,
                       proxsuite::linalg::veg::dynstack::DynStackMut stack) {
    isize n = dim();
    isize r = a.cols();
    ...
    proxsuite::linalg::dense::ldlt_insert_rows_and_cols(ld_col_mut(), i_actual,
                                                        permuted_a, stack);
  }
  ...
  void diagonal_update_clobber_indices(  //
      isize* indices, isize r, Eigen::Ref<Vec const> alpha,
      proxsuite::linalg::veg::dynstack::DynStackMut stack) {
    if (r == 0) {
      return;
    }
    ...
    std::sort(positions, positions + r,
              [indices](isize i, isize j) noexcept -> bool {
    ...
    proxsuite::linalg::dense::_detail::rank_r_update_clobber_w_impl(
        util::submatrix(ld_col_mut(), first, first, n, n), _w.data(),
        _w.outerStride(), _alpha.data(),
        _detail::IndicesR{
            first,
            0,
            r,
   ...
   */
  void rank_r_update(  //
      Eigen::Ref<ColMat const> w, Eigen::Ref<Vec const> alpha,
      proxsuite::linalg::veg::dynstack::DynStackMut stack) {
    auto n = dim();
    auto r = w.cols();
    if (r == 0) {
    ...
    proxsuite::linalg::dense::rank_r_update_clobber_inputs(ld_col_mut(), _w,
                                                           _alpha);
  }
  ...
  auto ld_col() const noexcept
      -> Eigen::Map<  //
          ColMat const, Eigen::Unaligned, Eigen::OuterStride<DYN>> {
    return {ld_storage.ptr(), dim(), dim(), stride};
  }
  auto ld_col_mut() noexcept
      -> Eigen::Map<  //
          ColMat, Eigen::Unaligned, Eigen::OuterStride<DYN>> {
    return {ld_storage.ptr_mut(), dim(), dim(), stride};
  }
  auto ld_row() const noexcept
      -> Eigen::Map<  //
          RowMat const, Eigen::Unaligned, Eigen::OuterStride<DYN>> {
    return {
        ld_storage.ptr(),
  ...
  void factorize(Eigen::Ref<ColMat const> mat /* NOLINT */,
    ...
    proxsuite::linalg::dense::_detail::compute_permutation(  //
        perm.ptr_mut(), perm_inv.ptr_mut(), util::diagonal(mat));
