...
void update_proximal_parameters(Settings<T>& settings, Results<T>& results,
                                Workspace<T, I>& work, optional<T> rho_new,
...
void warm_start(optional<VecRef<T>> x_wm, optional<VecRef<T>> y_wm,
                ...
                Settings<T>& settings, Model<T, I>& model) {
...
void qp_setup(QpView<T, I> qp, Results<T>& results, Model<T, I>& data,
              Workspace<T, I>& work, Settings<T>& settings, P& precond,
  ...
  work.setup_impl(qp, data, settings, execute_preconditioner_or_not, precond,
                  P::scale_qp_in_place_req(proxsuite::linalg::veg::Tag<T>{}, n,
                                           n_eq, n_in));
 ...
 */
template <typename T, typename I>
auto have_same_structure(proxsuite::linalg::sparse::MatRef<T, I> a,
  ...
  for (usize j = 0; j < static_cast<usize>(a.ncols()); ++j) {
    ...
    if (n_elems != n_elems_to_compare) return false;
 ...
 */
template <typename T, typename I>
void copy(proxsuite::linalg::sparse::MatMut<T, I> a,
