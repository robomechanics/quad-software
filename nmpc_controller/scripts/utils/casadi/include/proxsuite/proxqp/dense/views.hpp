...
#define LDLT_IMPL_GET_PARAMS_0(NParams, ...)                  \
  __VEG_PP_TUPLE_FOR_EACH(LDLT_IMPL_GET_PARAM, (__VA_ARGS__), \
...
struct max2 {
  template <typename T>
  VEG_INLINE constexpr auto operator()(T const& a,
                                       T const& b) const -> T const& {
    return a > b ? a : b;
  }
};
struct min2 {
...
struct ElementAccess<Layout::colmajor> {
  template <typename T>
  VEG_INLINE static constexpr auto offset(T* ptr, isize row, isize col,
  ...
  template <typename T>
  VEG_INLINE static void transpose_if_rowmajor(T* ptr, isize dim,
                                               isize outer_stride) {
...
auto is_eigen_matrix_base_impl(void const volatile*)
...
template <typename Dummy, typename Fallback, template <typename...> class F,
          typename... Ts>
...
template <typename T>
using LayoutImpl = proxsuite::linalg::veg::meta::constant<
    Layout, (bool(T::IsRowMajor) ? rowmajor : colmajor)>;
...
using CompTimeCols =
    detail::Detected<proxsuite::linalg::veg::meta::constant<isize, 0>,
                     detail::CompTimeColsImpl, T>;
template <typename T>
using CompTimeRows =
    detail::Detected<proxsuite::linalg::veg::meta::constant<isize, 0>,
                     detail::CompTimeRowsImpl, T>;
template <typename T>
using CompTimeInnerStride =
    detail::Detected<proxsuite::linalg::veg::meta::constant<isize, 0>,
                     detail::CompTimeInnerStrideImpl, T>;
template <typename T>
using GetLayout =
    detail::Detected<proxsuite::linalg::veg::meta::constant<
                         Layout, Layout(static_cast<unsigned char>(-1))>,
                     detail::LayoutImpl, T>;
}  // namespace eigen
...
struct VectorView {
  ...
  VEG_TEMPLATE(typename Vec, requires(LDLT_CONCEPT(eigen_vector_view<Vec, T>)),
               VEG_INLINE VectorView, (/*tag*/, FromEigen),
...
struct VectorViewMut {
  ...
  VEG_TEMPLATE(typename Vec,
               ...
               VEG_INLINE VectorViewMut, (/*tag*/, FromEigen),
               (vec, Vec&&)) noexcept
      : data(vec.data()), dim(vec.rows()) {}
...
struct StridedVectorView {
  ...
  VEG_INLINE auto to_eigen() const
    ...
    return detail::EigenVecMap<T, Eigen::InnerStride<Eigen::Dynamic>>(
        data, Eigen::Index(dim), Eigen::Index(1),
...
struct StridedVectorViewMut {
  ...
  VEG_INLINE
  StridedVectorViewMut(FromPtrSizeStride /*tag*/, T* _data, isize _dim,
                       isize _stride) noexcept
      : data(_data), dim(_dim), stride(_stride) {}

  VEG_TEMPLATE(typename Vec,
               requires(LDLT_CONCEPT(eigen_strided_vector_view_mut<Vec, T>)),
               VEG_INLINE StridedVectorViewMut, (/*tag*/, FromEigen),
               (vec, Vec&&)) noexcept
      : data(vec.data()), dim(vec.rows()), stride(vec.innerStride()) {}
...
struct MatrixView {
  ...
  VEG_INLINE MatrixView(FromPtrRowsColsStride /*tag*/, T const* _data,
                        isize _rows, isize _cols, isize _outer_stride) noexcept
  ...
  VEG_TEMPLATE(typename Mat,
               ...
               VEG_INLINE MatrixView, (/*tag*/, FromEigen),
  ...
  VEG_INLINE auto block(isize row, isize col, isize nrows,
 ...
 public:
  VEG_INLINE auto col(isize c) const noexcept
      -> proxsuite::linalg::veg::meta::if_t<(L == colmajor), VectorView<T>,
                                            StridedVectorView<T>> {
    return col_impl(proxsuite::linalg::veg::meta::constant<Layout, L>{}, c);
  }
  VEG_INLINE auto row(isize r) const noexcept
      -> proxsuite::linalg::veg::meta::if_t<(L == rowmajor), VectorView<T>,
                                            StridedVectorView<T>> {
    return trans().col(r);
  }
  ...
  VEG_INLINE auto to_eigen() const noexcept -> detail::EigenMatMap<T, L> {
    return detail::EigenMatMap<T, L>(
        data, Eigen::Index(rows), Eigen::Index(cols),
...
struct MatrixViewMut {
  ...
  VEG_INLINE MatrixViewMut(FromPtrRowsColsStride /*tag*/, T* _data, isize _rows,
                           isize _cols, isize _outer_stride) noexcept
      : data(_data), rows(_rows), cols(_cols), outer_stride(_outer_stride) {}

  VEG_TEMPLATE(typename Mat,
               requires(LDLT_CONCEPT(eigen_view<Mat, T>) &&
                        eigen::GetLayout<unref<Mat>>::value == L),
               VEG_INLINE MatrixViewMut, (/*tag*/, FromEigen),
               (mat, Mat&&)) noexcept
      : data(mat.data()),
        rows(mat.rows()),
        cols(mat.cols()),
 ...
 public:
  VEG_INLINE auto col(isize c) const noexcept
      -> proxsuite::linalg::veg::meta::if_t<(L == colmajor), VectorViewMut<T>,
                                            StridedVectorViewMut<T>> {
    return col_impl(proxsuite::linalg::veg::meta::constant<Layout, L>{}, c);
  }
  VEG_INLINE auto row(isize r) const noexcept
      -> proxsuite::linalg::veg::meta::if_t<(L == rowmajor), VectorViewMut<T>,
                                            StridedVectorViewMut<T>> {
    return trans().col(r);
  }
  VEG_INLINE auto trans() const noexcept
...
namespace detail {
template <typename T>
void noalias_mul_add(MatrixViewMut<T, colmajor> dst,
                     MatrixView<T, colmajor> lhs, MatrixView<T, colmajor> rhs,
                     T factor) {
...
#define LAZY_PRODUCT(a, b) a.operator*(b)
  ...
  if (dst.cols == 1 && dst.rows == 1) {
    ...
    MapMut(dst.data, dst.rows, dst.cols, Stride(dst.outer_stride))
        .noalias()
        .operator+=(factor * LAZY_PRODUCT(Map(lhs.data, lhs.rows, lhs.cols,
                                              Stride(lhs.outer_stride)),
                                          Map(rhs.data, rhs.rows, rhs.cols,
                                              Stride(rhs.outer_stride))));
  }
#endif
...
template <typename T>
void noalias_mul_add_vec(VectorViewMut<T> dst, MatrixView<T, colmajor> lhs,
                         VectorView<T> rhs, T factor) {
...
template <typename T>
void assign_cwise_prod(VectorViewMut<T> out, StridedVectorView<T> lhs,
...
template <typename T>
void trans_tr_unit_up_solve_in_place_on_right(MatrixView<T, colmajor> tr,
...
template <typename T>
void apply_diag_inv_on_right(MatrixViewMut<T, colmajor> out,
...
template <typename T>
void apply_diag_on_right(MatrixViewMut<T, colmajor> out, StridedVectorView<T> d,
                         MatrixView<T, colmajor> in) {
  if (out.cols == 1) {
    out.col(0).to_eigen() =
...
template <typename T>
void noalias_mul_sub_tr_lo(MatrixViewMut<T, colmajor> out,
  ...
  if (lhs.cols == 1) {
    out.to_eigen().template triangularView<Eigen::Lower>().operator-=(
        lhs.col(0).to_eigen().operator*(
            Eigen::Map<Eigen::Matrix<T, 1, Eigen::Dynamic> const>(rhs.data, 1,
                                                                  rhs.cols)));
  } else {
    out.to_eigen().template triangularView<Eigen::Lower>().operator-=(
