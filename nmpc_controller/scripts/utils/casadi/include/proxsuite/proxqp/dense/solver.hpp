 ...
 */
template <typename T>
void refactorize(const Model<T>& qpmodel, Results<T>& qpresults,
                 Workspace<T>& qpwork, T rho_new) {
 ...
 */
template <typename T>
void mu_update(const Model<T>& qpmodel, Results<T>& qpresults,
               Workspace<T>& qpwork, T mu_eq_new, T mu_in_new) {
  proxsuite::linalg::veg::dynstack::DynStackMut stack{
      proxsuite::linalg::veg::from_slice_mut, qpwork.ldl_stack.as_mut()};
...

  {
    ...
    qpwork.ldl.diagonal_update_clobber_indices(indices, n_eq + n_c,
                                               rank_update_alpha, stack);
  }
 ...
 */
template <typename T>
void iterative_residual(const Model<T>& qpmodel, Results<T>& qpresults,
                        Workspace<T>& qpwork, isize inner_pb_dim) {
...
void iterative_solve_with_permut_fact(  //
    const Settings<T>& qpsettings, const Model<T>& qpmodel,
    Results<T>& qpresults, Workspace<T>& qpwork, T eps, isize inner_pb_dim) {
  qpwork.err.setZero();
  i32 it = 0;
  i32 it_stability = 0;
 ...
 */
template <typename T>
void bcl_update(const Settings<T>& qpsettings, Results<T>& qpresults,
                Workspace<T>& qpwork, T& primal_feasibility_lhs_new,
                T& bcl_eta_ext, T& bcl_eta_in,

                T bcl_eta_ext_init, T eps_in_min,

                T& new_bcl_mu_in, T& new_bcl_mu_eq, T& new_bcl_mu_in_inv,
                T& new_bcl_mu_eq_inv
 ...
 */
template <typename T>
void Martinez_update(const Settings<T>& qpsettings, Results<T>& qpresults,
                     T& primal_feasibility_lhs_new,
                     T& primal_feasibility_lhs_old, T& bcl_eta_in, T eps_in_min,
 ...
 */
template <typename T>
auto compute_inner_loop_saddle_point(const Model<T>& qpmodel,
                                     Results<T>& qpresults,
 ...
 */
template <typename T>
void primal_dual_semi_smooth_newton_step(const Settings<T>& qpsettings,
                                         const Model<T>& qpmodel,
                                         Results<T>& qpresults,
                                         Workspace<T>& qpwork, T eps) {
  /* MUST BE
  ...
  iterative_solve_with_permut_fact(  //
      qpsettings, qpmodel, qpresults, qpwork, eps, inner_pb_dim);
 ...
 */
template <typename T>
void primal_dual_newton_semi_smooth(const Settings<T>& qpsettings,
                                    const Model<T>& qpmodel,
  ...
  */
  ...
  for (i64 iter = 0; iter <= qpsettings.max_iter_in; ++iter) {
    ...
    primal_dual_semi_smooth_newton_step<T>(qpsettings, qpmodel, qpresults,
                                           qpwork, eps_int);
    ...
    */
    ...
    bool is_primal_infeasible = global_primal_residual_infeasibility(
        VectorViewMut<T>{from_eigen, ATdy}, VectorViewMut<T>{from_eigen, CTdz},
        VectorViewMut<T>{from_eigen, dy}, VectorViewMut<T>{from_eigen, dz},
        qpwork, qpsettings, ruiz);

    bool is_dual_infeasible = global_dual_residual_infeasibility(
        VectorViewMut<T>{from_eigen, Adx}, VectorViewMut<T>{from_eigen, Cdx},
        VectorViewMut<T>{from_eigen, Hdx}, VectorViewMut<T>{from_eigen, dx},
        qpwork, qpsettings, qpmodel, ruiz);
...
void qp_solve(  //
  ...
  */
  ...
  if (qpwork.dirty) {  // the following is used when a solve has already been
    ...
    switch (qpsettings.initial_guess) {
      case InitialGuessStatus::EQUALITY_CONSTRAINED_INITIAL_GUESS: {
        proxsuite::proxqp::dense::setup_factorization(qpwork, qpmodel,
                                                      qpresults);
        compute_equality_constrained_initial_guess(qpwork, qpsettings, qpmodel,
                                                   qpresults);
        break;
      ...
      case InitialGuessStatus::COLD_START_WITH_PREVIOUS_RESULT: {
        ...
        ruiz.scale_primal_in_place(
            {proxsuite::proxqp::from_eigen,
             qpresults
                 .x});  // meaningful for when there is an upate of the model
  ...
  for (i64 iter = 0; iter < qpsettings.max_iter; ++iter) {
    ...
    global_primal_residual(
        qpmodel, qpresults, qpwork, ruiz, primal_feasibility_lhs,
        primal_feasibility_eq_rhs_0, primal_feasibility_in_rhs_0,
        primal_feasibility_eq_lhs, primal_feasibility_in_lhs);

    global_dual_residual(qpresults, qpwork, qpmodel, ruiz, dual_feasibility_lhs,
                         dual_feasibility_rhs_0, dual_feasibility_rhs_1,
                         dual_feasibility_rhs_3, rhs_duality_gap, duality_gap);
    ...
    primal_dual_newton_semi_smooth(qpsettings, qpmodel, qpresults, qpwork, ruiz,
                                   bcl_eta_in);
    ...
    global_primal_residual(
        qpmodel, qpresults, qpwork, ruiz, primal_feasibility_lhs_new,
        primal_feasibility_eq_rhs_0, primal_feasibility_in_rhs_0,
        primal_feasibility_eq_lhs, primal_feasibility_in_lhs);
      ...
      is_dual_feasible =
          ...
          (qpsettings.eps_abs +
           qpsettings.eps_rel *
               std::max(
                   std::max(dual_feasibility_rhs_3, dual_feasibility_rhs_0),
                   std::max(dual_feasibility_rhs_1,
                            qpwork.dual_feasibility_rhs_2)));
    ...
    if (qpsettings.bcl_update) {
      bcl_update(qpsettings, qpresults, qpwork, primal_feasibility_lhs_new,
                 bcl_eta_ext, bcl_eta_in, bcl_eta_ext_init, eps_in_min,

                 new_bcl_mu_in, new_bcl_mu_eq, new_bcl_mu_in_inv,
                 new_bcl_mu_eq_inv);
    } else {
      Martinez_update(qpsettings, qpresults, primal_feasibility_lhs_new,
                      primal_feasibility_lhs, bcl_eta_in, eps_in_min,
                      new_bcl_mu_in, new_bcl_mu_eq, new_bcl_mu_in_inv,
                      new_bcl_mu_eq_inv);
    }
    // COLD RESTART
