...
#define simde_assert_type_full(prefix, suffix, T, fmt, a, op, b)        \
  do {                                                                  \
    ...
    if (!(simde_tmp_a_ op simde_tmp_b_)) {                              \
      simde_errorf("assertion failed: %s %s %s (" prefix "%" fmt suffix \
                   " %s " prefix "%" fmt suffix ")\n",                  \
...
#define simde_assert_double_equal(a, b, precision)                      \
  do {                                                                  \
    ...
    if (HEDLEY_UNLIKELY(simde_tmp_diff_ > 1e-##precision)) {            \
      simde_errorf("assertion failed: %s == %s (%0." #precision         \
                   "g == %0." #precision "g)\n",                        \
...
#define simde_assert_memory_equal(size, a, b)                               \
  do {                                                                      \
    ...
    if (HEDLEY_UNLIKELY(                                                    \
            memcmp(simde_tmp_a_, simde_tmp_b_, simde_tmp_size_)) != 0) {    \
      size_t simde_tmp_pos_;                                                \
      for (simde_tmp_pos_ = 0; simde_tmp_pos_ < simde_tmp_size_;            \
           simde_tmp_pos_++) {                                              \
        if (simde_tmp_a_[simde_tmp_pos_] != simde_tmp_b_[simde_tmp_pos_]) { \
          simde_errorf(                                                     \
              "assertion failed: memory %s == %s, at offset "               \
              "%" SIMDE_SIZE_MODIFIER "u\n",                                \
              #a, #b, simde_tmp_pos_);                                      \
          break;                                                            \
        }                                                                   \
      }                                                                     \
...
#define simde_assert_memory_not_equal(size, a, b)                             \
  do {                                                                        \
    ...
    if (HEDLEY_UNLIKELY(                                                      \
            memcmp(simde_tmp_a_, simde_tmp_b_, simde_tmp_size_)) == 0) {      \
      simde_errorf("assertion failed: memory %s != %s (%" SIMDE_SIZE_MODIFIER \
                   "u bytes)\n",                                              \
                   #a, #b, simde_tmp_size_);                                  \
    }                                                                         \
    SIMDE_PUSH_DISABLE_MSVC_C4127_                                            \
...
#define simde_assert_char(a, op, b)                                          \
  simde_assert_type_full("'\\x", "'", char, "02" SIMDE_CHAR_MODIFIER "x", a, \
                         op, b)
