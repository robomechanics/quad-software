...
auto at_most(T const& expr, const Scalar value) PROXSUITE_DEDUCE_RET(
...
template <typename T, typename Scalar>
auto at_least(T const& expr, const Scalar value) PROXSUITE_DEDUCE_RET(
    (expr.array() > value).select(expr, T::Constant(expr.rows(), value)));
...
auto select(Condition const& condition, T const& expr, const Scalar value)
