...
VEG_INLINE void unroll_impl(
    proxsuite::linalg::veg::meta::index_sequence<Is...> /*unused*/, Fn fn) {
  VEG_EVAL_ALL(fn(Is));
}
...
VEG_INLINE void rank_r_update_inner_loop_iter(  //
    _simd::Pack<T, N> const* p_p, _simd::Pack<T, N> const* p_mu, T* inout_l,
    T* pw, isize w_stride) {
...
struct RankRUpdateLoopImpl<false> {
  ...
  VEG_INLINE static void fn(isize n, T* inout_l, T* pw, isize w_stride,
                            T const* p, T const* mu) noexcept {
    ...
    while (inout_l < inout_l_finish) {
      _detail::rank_r_update_inner_loop_iter<R>(p_p, p_mu, inout_l, pw,
                                                w_stride);
      ++inout_l;
      ++pw;
    }
...
template <usize R, typename T>
VEG_INLINE void rank_r_update_inner_loop(isize n, T* inout_l, T* pw,
                                         isize w_stride, T const* p,
                                         T const* mu) {
...
void rank_r_update_clobber_w_impl(  //
    LD ld, T* pw, isize w_stride, T* palpha, Fn r_fn) {
  ...
  for (isize j = 0; j < n; ++j) {
    ...
    while (true) {
      ...
      (*fn_table[r_chunk - 1])(  //
          rem, util::matrix_elem_addr(ld, j + 1, j), pw + 1 + r_done * w_stride,
          w_stride, p_array, mu_array);
...
template <typename LD, typename W,
          typename T = typename proxsuite::linalg::veg::uncvref_t<LD>::Scalar>
void rank_1_update_clobber_w(LD&& ld, W&& w,
                             proxsuite::linalg::veg::DoNotDeduce<T> alpha) {
  _detail::rank_r_update_clobber_w_impl(  //
      util::to_view_dyn(ld), w.data(), 0,
      proxsuite::linalg::veg::mem::addressof(alpha), _detail::ConstantR{1});
}

template <typename LD, typename W, typename A,
          typename T = typename proxsuite::linalg::veg::uncvref_t<LD>::Scalar>
void rank_r_update_clobber_inputs(LD&& ld, W&& w, A&& alpha) {
  isize r = w.cols();
  _detail::rank_r_update_clobber_w_impl(  //
      util::to_view_dyn(ld), w.data(), w.outerStride(), alpha.data(),
      _detail::ConstantR{r});
}
}  // namespace dense
}  // namespace linalg
