...
namespace concepts {
VEG_DEF_CONCEPT(typename T, index,
...
struct Boolean {
  ...
  VEG_NODISCARD VEG_INLINE explicit constexpr operator bool() const
      VEG_NOEXCEPT {
    return T == yes;
  }
};
...
struct Fix : _detail::idx::adl::IdxBase<Fix<N>> {
  ...
  VEG_TEMPLATE((isize M), requires((M != N)), constexpr Fix,
               (/*arg*/, Fix<M>)) = delete;
...
#define VEG_OP(Op, Name, TypeName)                                          \
  VEG_TEMPLATE((typename R), requires(VEG_CONCEPT(index<R>)),               \
               VEG_NODISCARD VEG_INLINE constexpr auto operator Op, (b, R)) \
...
#undef VEG_OP
  ...
  VEG_TEMPLATE(
      ...
      VEG_NODISCARD VEG_INLINE constexpr auto operator/, (b, R))
...
#define VEG_CMP(Name, TypeName, Op)                                       \
  VEG_TEMPLATE(                                                           \
      (typename R), requires(VEG_CONCEPT(index<R>)),                      \
      VEG_NODISCARD VEG_INLINE constexpr auto operator Op, /* NOLINT */   \
      (b, R))                                                             \
  const VEG_NOEXCEPT->typename _detail::binary_traits<Fix, R>::TypeName { \
...
constexpr auto parse_num(char const* str, u64 len, u64 radix,
                         Error e) VEG_NOEXCEPT -> u64 {
  return (len == 0) ? 0
                    : radix * parse_num(str, len - 1, radix, e) +
                          (parse_digit(radix)(str[len - 1], e));
}

constexpr auto parse_int(char const* str, u64 len,
                         Error e) VEG_NOEXCEPT -> u64 {
  return (len == 0)  //
             ? e()
             : ((str[0] == '0')    //
...
#define VEG_CMP(Name, TypeName, Op)                                        \
  ...
  VEG_NODISCARD VEG_INLINE static constexpr auto div_fn(
      Fix<N> /*a*/, Fix<M> /*b*/) VEG_NOEXCEPT -> Div {
    return Div();
  }
  VEG_NODISCARD VEG_INLINE static constexpr auto mod_fn(
      Fix<N> /*a*/, Fix<M> /*b*/) VEG_NOEXCEPT -> Mod {
    return Mod();
  }
...
VEG_INLINE constexpr auto operator"" _c() VEG_NOEXCEPT {
  return Fix<_detail::parse_int(_detail::char_seq<Chars...>::value,
                                sizeof...(Chars), _detail::Error{})>{};
}
}  // namespace literals
}  // namespace veg
}  // namespace linalg
