...
inline auto align_next(isize alignment, isize size, void*& ptr,
                       isize& space) VEG_ALWAYS_NOEXCEPT -> void* {
...
struct DynStackArrayDtor<T, false> {
  ...
  VEG_INLINE ~DynStackArrayDtor()
    ...
    proxsuite::linalg::veg::_detail::_collections::backward_destroy(
        mut(mem::SystemAlloc{}), mut(mem::DefaultCloner{}), self.ptr_mut(),
...
struct DynStackMut {
 ...
 public:
  VEG_TEMPLATE((typename T), requires VEG_CONCEPT(constructible<T>),
               VEG_NODISCARD auto make_new, (/*unused*/, Tag<T>), (len, isize),
               (align = alignof(T), isize))
  VEG_NOEXCEPT_IF(VEG_CONCEPT(nothrow_constructible<T>))->DynStackArray<T> {
    assert_valid_len(len);
    DynStackArray<T> get{*this, isize(len), align,
                         _detail::_dynstack::zero_init_fn{}};
    VEG_ASSERT(get.ptr() != nullptr);
    return VEG_FWD(get);
  }

  VEG_TEMPLATE((typename T), requires VEG_CONCEPT(constructible<T>),
               VEG_NODISCARD auto make_new_for_overwrite, (/*unused*/, Tag<T>),
               (len, isize), (align = alignof(T), isize))

  VEG_NOEXCEPT_IF(VEG_CONCEPT(nothrow_constructible<T>))->DynStackArray<T> {
    assert_valid_len(len);
    DynStackArray<T> get{*this, isize(len), align,
                         _detail::_dynstack::default_init_fn{}};
    VEG_ASSERT(get.ptr() != nullptr);
    return VEG_FWD(get);
  }

  template <typename T>
  VEG_NODISCARD auto make_alloc(Tag<T> /*unused*/, isize len,
                                isize align = alignof(T))
      VEG_NOEXCEPT -> DynStackAlloc<T> {
    assert_valid_len(len);
    DynStackAlloc<T> get{*this, isize(len), align,
                         _detail::_dynstack::no_init_fn{}};
    VEG_ASSERT(get.ptr() != nullptr);
    return VEG_FWD(get);
  }
  ...
  void destroy(void const volatile* void_data_end) VEG_NOEXCEPT {
    if (data != nullptr) {
      ...
      VEG_INTERNAL_ASSERT_PRECONDITIONS(  //
          parent_stack_data == data_end, parent_stack_data >= old_position);
...
struct DynStackAlloc : _detail::_dynstack::DynAllocBase {
 ...
 private:
  ...
  DynStackAlloc(DynStackMut& parent_ref, isize alloc_size, isize align, Fn fn)
    ...
    void* const ptr =
        _detail::align_next(align, alloc_size * isize(sizeof(T)),
                            parent_ref.stack_data, parent_ref.stack_bytes);
