  ...
  VEG_INLINE static VEG_CPP14(constexpr) void fn(RefMut<A> alloc,
    ...
    while (true) {
      ...
      mem::Cloner<C>::destroy(  //
          RefMut<C>(cloner), static_cast<T*>(ptr_end), RefMut<A>(alloc));
    }
  }
};
...
struct DestroyImpl<false> {
  ...
  VEG_INLINE static VEG_CPP20(constexpr) void fn(RefMut<A> alloc,
    ...
    while (true) {
      ...
      --_.fn.ptr_end;
      mem::Cloner<C>::destroy(  //
          RefMut<C>(_.fn.cloner), static_cast<T*>(_.fn.ptr_end),
          RefMut<A>(_.fn.alloc));
    }
  }
};
...
void backward_destroy(RefMut<A> alloc, RefMut<C> cloner, T* ptr, T* ptr_end)
  ...
  DestroyImpl<VEG_CONCEPT(alloc::nothrow_destroy<C, T, A>)>::fn(alloc, cloner,
                                                                ptr, ptr_end);
}
}  // namespace _collections
}  // namespace _detail
