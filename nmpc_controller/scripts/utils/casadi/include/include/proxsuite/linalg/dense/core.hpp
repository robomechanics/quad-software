...
#define __LDLT_TEMP_VEC_IMPL(Type, Name, Rows, Stack, Make)                 \
  auto LDLT_ID(vec_storage) = (Stack).Make(                                 \
      ::proxsuite::linalg::veg::Tag<__VEG_PP_REMOVE_PAREN(Type)>{}, (Rows), \
...
#define __LDLT_TEMP_MAT_IMPL(Type, Name, Rows, Cols, Stack, Make)       \
  ...
  auto LDLT_ID(vec_storage) = (Stack).Make(                             \
      ...
      ::proxsuite::linalg::dense::_detail::align<__VEG_PP_REMOVE_PAREN( \
          Type)>());                                                    \
  auto Name /* NOLINT */ = ::Eigen::Map<                                \
      ::Eigen::Matrix<__VEG_PP_REMOVE_PAREN(Type), ::Eigen::Dynamic,    \
...
template <typename T>
using OwnedRows =
    Eigen::Matrix<typename T::Scalar, Eigen::Dynamic, T::ColsAtCompileTime,
                  bool(T::IsRowMajor) ? Eigen::RowMajor : Eigen::ColMajor>;

template <typename T>
using OwnedCols =
    Eigen::Matrix<typename T::Scalar, T::RowsAtCompileTime, Eigen::Dynamic,
                  bool(T::IsRowMajor) ? Eigen::RowMajor : Eigen::ColMajor>;

template <typename T>
using OwnedColVector = Eigen::Matrix<  //
    typename T::Scalar, Eigen::Dynamic, 1, Eigen::ColMajor>;
template <typename T>
using OwnedRowVector = Eigen::Matrix<  //
    typename T::Scalar, 1, Eigen::Dynamic, Eigen::RowMajor>;
...
struct ElemAddrImpl<false> {
  template <typename T>
  static auto fn(T* ptr, isize row, isize col, isize outer_stride,
...
struct RowColAccessImpl<true> {
  template <typename T>
  using Col = Eigen::Map<
      const_if<ptr_is_const<decltype(VEG_DECLVAL(T&&).data())>::value,
...
using StrideOf = Eigen::Stride<  //
    T::OuterStrideAtCompileTime, T::InnerStrideAtCompileTime>;
}  // namespace _detail

namespace util {
template <bool COLMAJOR, typename T>
auto elem_addr(T* ptr, isize row, isize col, isize outer_stride,
               isize inner_stride) noexcept -> T* {
  return _detail::ElemAddrImpl<!COLMAJOR>::fn(ptr, row, col, outer_stride,
                                              inner_stride);
}

template <typename Mat>
auto matrix_elem_addr(Mat&& mat, isize row,
                      isize col) noexcept -> decltype(mat.data()) {
  return util::elem_addr<!bool(
      proxsuite::linalg::veg::uncvref_t<Mat>::IsRowMajor)>(  //
      mat.data(), row, col, mat.outerStride(), mat.innerStride());
}

template <typename T>
auto col(T&& mat, isize col_idx) noexcept ->
    typename _detail::RowColAccessImpl<!bool(
        proxsuite::linalg::veg::uncvref_t<T>::IsRowMajor)>::template Col<T> {
  return _detail::RowColAccessImpl<!bool(
      proxsuite::linalg::veg::uncvref_t<T>::IsRowMajor)>::col(mat, col_idx);
}
template <typename T>
auto row(T&& mat, isize row_idx) noexcept ->
    typename _detail::RowColAccessImpl<!bool(
        proxsuite::linalg::veg::uncvref_t<T>::IsRowMajor)>::template Row<T> {
  return _detail::RowColAccessImpl<!bool(
      proxsuite::linalg::veg::uncvref_t<T>::IsRowMajor)>::row(mat, row_idx);
}

template <typename Mat>
auto trans(Mat&& mat) noexcept
    -> Eigen::Map<  //
        _detail::const_if<
...
template <typename Mat>
auto diagonal(Mat&& mat) noexcept
    -> Eigen::Map<  //
        _detail::const_if<
            _detail::ptr_is_const<decltype(mat.data())>::value,
            Eigen::Matrix<  //
                typename proxsuite::linalg::veg::uncvref_t<Mat>::Scalar,
                Eigen::Dynamic, 1, Eigen::ColMajor>>,
...
template <typename Mat>
auto submatrix(Mat&& mat, isize row_start, isize col_start, isize nrows,
  ...
  return {
      util::elem_addr<!bool(
          proxsuite::linalg::veg::uncvref_t<Mat>::IsRowMajor)>(
          mat.data(), row_start, col_start, mat.outerStride(),
          mat.innerStride()),
      nrows,
      ncols,
      _detail::StrideOf<proxsuite::linalg::veg::uncvref_t<Mat>>{
...
template <typename Mat>
auto to_view_dyn_rows(Mat&& mat) noexcept
    -> Eigen::Map<
        _detail::const_if<
            _detail::ptr_is_const<decltype(mat.data())>::value,
...
template <typename Mat>
auto to_view_dyn_cols(Mat&& mat) noexcept
    -> Eigen::Map<
        _detail::const_if<
            _detail::ptr_is_const<decltype(mat.data())>::value,
...
template <typename Mat>
auto to_view_dyn(Mat&& mat) noexcept
    -> Eigen::Map<
        _detail::const_if<
            _detail::ptr_is_const<decltype(mat.data())>::value,
            _detail::OwnedMatrix<proxsuite::linalg::veg::uncvref_t<Mat>>>,
...
template <typename Mat>
auto subrows(Mat&& mat, isize row_start, isize nrows) noexcept
    -> Eigen::Map<
        _detail::const_if<
            _detail::ptr_is_const<decltype(mat.data())>::value,
            _detail::OwnedRows<proxsuite::linalg::veg::uncvref_t<Mat>>>,
...
template <typename Mat>
auto subcols(Mat&& mat, isize col_start, isize ncols) noexcept
    -> Eigen::Map<
        _detail::const_if<
            _detail::ptr_is_const<decltype(mat.data())>::value,
            _detail::OwnedCols<proxsuite::linalg::veg::uncvref_t<Mat>>>,
...
void noalias_mul_add_impl(Dst dst, Lhs lhs, Rhs rhs, T factor) {
  VEG_ASSERT_ALL_OF(dst.rows() == lhs.rows(), dst.cols() == rhs.cols(),
                    lhs.cols() == rhs.rows());
...
#define LAZY_PRODUCT(a, b) a.operator*(b)
  ...
  if ((dst.rows() < 20) && (dst.cols() < 20) && (rhs.rows() < 20)) {
    ...
    auto dst_ = MapMut(dst.data(), dst.rows(), dst.cols(), {dst.outerStride()});
...
void noalias_mul_add(Dst&& dst, Lhs const& lhs, Rhs const& rhs, T factor) {
  _detail::noalias_mul_add_impl(
      ...
      util::submatrix(rhs, 0, 0, rhs.rows(), rhs.cols()), factor);
}
}  // namespace util
template <typename T>
auto temp_mat_req(proxsuite::linalg::veg::Tag<T> /*tag*/, isize rows,
                  isize cols) noexcept
