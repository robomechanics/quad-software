...
#define __VEG_IMPL_BIND_ID_SEQ(/* NOLINT */                                 \
                               CV_Auto, Identifiers, Tuple, Tuple_Size,     \
                               TupleId)                                     \
...
#define VEG_BIND(CV_Auto, Identifiers, Tuple)              \
  __VEG_IMPL_BIND_ID_SEQ(CV_Auto, Identifiers, Tuple,      \
...
#define __VEG_IMPL_LEAF_ONCE(Tuple, I, ...) /* NOLINT */                 \
  (static_cast<__VA_ARGS__&&>(                                           \
      static_cast<                                                       \
          ::proxsuite::linalg::veg::tuple::TupleLeaf<I, __VA_ARGS__>&&>( \
          (Tuple).inner)                                                 \
          .leaf))
...
struct unpack {
  VEG_TEMPLATE(
      ...
      (args, IndexedTuple<proxsuite::linalg::veg::meta::index_sequence<Is...>,
                          Ts...>&&),
      (fn, Fn))
  const VEG_NOEXCEPT_IF(
      VEG_CONCEPT(
          nothrow_fn_once<
              Fn, proxsuite::linalg::veg::meta::invoke_result_t<Fn, Ts&&...>,
              Ts&&...>))
      ->proxsuite::linalg::veg::meta::invoke_result_t<Fn, Ts&&...> {
...
struct map_i {
  VEG_TEMPLATE(
      ...
      requires(VEG_ALL_OF(VEG_CONCEPT(
          fn_once<  //
              inner_ith<Fn&, Is>,
              proxsuite::linalg::veg::meta::invoke_result_t<inner_ith<Fn&, Is>,
                                                            Ts>,
              Ts>))),
      VEG_NODISCARD VEG_INLINE VEG_CPP14(constexpr) auto operator(),
      (args, IndexedTuple<proxsuite::linalg::veg::meta::index_sequence<Is...>,
                          Ts...>&&),
      (fn, Fn))
  const VEG_NOEXCEPT_IF(
      VEG_ALL_OF(VEG_CONCEPT(nothrow_fn_once<  //
                             ...
                             proxsuite::linalg::veg::meta::invoke_result_t<
                                 inner_ith<Fn&, Is>, Ts>,
                             Ts>)))
      ->Tuple<proxsuite::linalg::veg::meta::invoke_result_t<inner_ith<Fn&, Is>,
                                                            Ts>...> {
...
struct map {
  VEG_TEMPLATE(
      (typename Fn, typename... Ts, usize... Is),
      requires(VEG_ALL_OF(VEG_CONCEPT(
          fn_mut<Fn, proxsuite::linalg::veg::meta::invoke_result_t<Fn&, Ts&&>,
                 Ts&&>))),
      VEG_NODISCARD VEG_INLINE VEG_CPP14(constexpr) auto operator(),
      (args, IndexedTuple<proxsuite::linalg::veg::meta::index_sequence<Is...>,
                          Ts...>&&),
...
struct IndexedTuple<meta::index_sequence<Is...>, Ts...> {
  ...
  VEG_TEMPLATE((typename _, typename... Fns),
               ...
               VEG_INLINE constexpr IndexedTuple, (/*tag*/, InPlace<_>),
               (... fns, Fns))
  ...
  VEG_TEMPLATE((isize I), requires(static_cast<usize>(I) < sizeof...(Ts)),
               VEG_NODISCARD VEG_INLINE VEG_CPP14(constexpr) auto operator[],
               (/*arg*/, Fix<I>)) &&
      VEG_NOEXCEPT_IF(
          VEG_CONCEPT(nothrow_movable<ith<static_cast<usize>(I), Ts...>>))
      -> ith<static_cast<usize>(I), Ts...> {
    return __VEG_IMPL_LEAF_ONCE(*this, static_cast<usize>(I),
                                ith<static_cast<usize>(I), Ts...>);
  }
  ...
  VEG_TEMPLATE((isize I), requires(static_cast<usize>(I) < sizeof...(Ts)),
    ...
    return __VEG_IMPL_LEAF_MUT(*this, static_cast<usize>(I),
                               ith<static_cast<usize>(I), Ts...>);
  }
  ...
  const & VEG_NOEXCEPT->ith<static_cast<usize>(I), Ts...> const& {
    return __VEG_IMPL_LEAF(*this, static_cast<usize>(I),
                           ith<static_cast<usize>(I), Ts...>);
  }
};
}  // namespace tuple
...
struct TupleBaseInfoImpl {
  ...
  using IndexedTuple = proxsuite::linalg::veg::tuple::IndexedTuple<
      meta::make_index_sequence<sizeof...(Ts)>, Ts...>;
};
...
namespace concepts {
namespace tuple {
VEG_DEF_CONCEPT(typename T, tuple,
...
VEG_NODISCARD VEG_INLINE constexpr auto get(
    tuple::IndexedTuple<proxsuite::linalg::veg::meta::index_sequence<Is...>,
                        Ts...> const& tup)
...
VEG_INLINE static constexpr auto tuple_fwd(
    tuple::IndexedTuple<meta::index_sequence<Is...>, Ts...>&& tup)
...
struct with {
  VEG_TEMPLATE(
      ...
      VEG_NODISCARD VEG_INLINE constexpr auto operator(), (... args, Fns))
  const VEG_NOEXCEPT_IF(
      VEG_ALL_OF(VEG_CONCEPT(
          nothrow_fn_once<Fns,
...
struct zip {
  ...
  template <typename... Tuples>
  using Zip = proxsuite::linalg::veg::meta::detected_t<
      PreZip, typename meta::TupleBaseInfo<Tuples>::Tuple...>;

  VEG_TEMPLATE(
      (typename... Tuples),
      requires(VEG_ALL_OF(VEG_CONCEPT(tuple::tuple<Tuples>)) &&
               VEG_CONCEPT(all_same<tuple::meta::tuple_size<Tuples>...>)),
      VEG_NODISCARD VEG_INLINE constexpr auto operator(), (... tups, Tuples))
 ...
 private:
  ...
  VEG_INLINE static constexpr auto pre_apply(
    ...
    return zip::from_ref_to_result(
        Tag<proxsuite::linalg::veg::meta::type_sequence_zip<
            Tuple, typename _detail::meta_::IndexedToTuple<Tuples>::Type...>>{},
  ...
  struct Converter<Tuple<InnerTargets...>> {
    using Type =
        ConverterImpl<proxsuite::linalg::veg::meta::make_index_sequence<
                          sizeof...(InnerTargets)>,
                      InnerTargets...>;
  };
...
struct cat {
  ...
  template <typename... Tuples>
  using Concat = proxsuite::linalg::veg::meta::detected_t<
      PreConcat, typename meta::TupleBaseInfo<Tuples>::Tuple...>;
 ...
 private:
  ...
  VEG_INLINE static constexpr auto pre_apply(
    ...
    return cat::template from_ref_to_result(
        Tag<proxsuite::linalg::veg::meta::type_sequence_cat<Tuple,
                                                            Tuples...>>{},
...
struct deref_assign {
  VEG_TEMPLATE(
      ...
      VEG_INLINE VEG_CPP14(constexpr) void operator(),
      (ts, IndexedTuple<proxsuite::linalg::veg::meta::index_sequence<Is...>,
                        RefMut<Ts>...>),
      (us, IndexedTuple<proxsuite::linalg::veg::meta::index_sequence<Is...>,
                        Ref<Us>...>))
  const VEG_NOEXCEPT_IF(
