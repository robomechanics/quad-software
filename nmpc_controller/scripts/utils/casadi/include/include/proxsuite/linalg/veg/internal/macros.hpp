...
#define VEG_CONCEPT_EXPR(Tpl, TplNames, Name, Expr, ...)                      \
  ...
  VEG_DEF_CONCEPT(                                                            \
      Tpl, Name,                                                              \
      _veg_detail::test_sfinae_##Name<void, __VEG_PP_REMOVE_PAREN1(           \
                                                TplNames)>::TestExpr::value); \
  VEG_DEF_CONCEPT(Tpl, nothrow_##Name, noexcept(Expr));                       \
  VEG_DEF_CONCEPT(                                                            \
      Tpl, xnothrow_##Name,                                                   \
      _veg_detail::test_sfinae_##Name<                                        \
          void, __VEG_PP_REMOVE_PAREN1(TplNames)>::NothrowTestExpr::value);   \
  VEG_NOM_SEMICOLON
...
#define VEG_DEF_CONCEPT(Tpl, Name, ...)                            \
  __VEG_IMPL_DEF_CONCEPT(                                          \
      Tpl, Name, (__VA_ARGS__),                                    \
      ::proxsuite::linalg::veg::meta::bool_constant<__VA_ARGS__>); \
  VEG_TEMPLATE(                                                    \
      Tpl, requires(__VA_ARGS__), constexpr auto check_##Name,     \
...
#define __VEG_IMPL_SFINAE(_, Param)              \
  , ::proxsuite::linalg::veg::meta::enable_if_t< \
        __VEG_PP_ID(__VEG_PP_UNWRAP Param), int> = 0

#define __VEG_IMPL_OVERLOAD(Name_Tpl, Param)                              \
  template <__VEG_PP_REMOVE_PAREN(__VEG_PP_TAIL Name_Tpl),                \
            typename ::proxsuite::linalg::veg::_detail::_meta::enable_if< \
                __VEG_PP_ID(__VEG_PP_UNWRAP Param), int>::type = 0>       \
...
#define VEG_DEF_CONCEPT_BOOL_CONJUNCTION_IMPL(Tpl, Name, Base, Seq)        \
  __VEG_IMPL_DEF_CONCEPT(Tpl, Name, (__VEG_PP_REMOVE_PAREN1(Base)::value), \
...
#define VEG_TEMPLATE(TParams, Constraint, Attr_Name, ...)            \
  __VEG_IMPL_TEMPLATE(Attr_Name, TParams,                            \
...
#define VEG_CONSTRAINED_MEMBER_FN(Constraint, Attr_Name, Params, ...)      \
  VEG_TEMPLATE(                                                            \
      ...
      Attr_Name, __VEG_PP_REMOVE_PAREN(Params))                            \
  __VA_ARGS__
...
#define VEG_TEMPLATE_EXPLICIT(TParams, Constraint, Explicit_Cond, Attr_Name, \
                              Params, ...)                                   \
  VEG_TEMPLATE(TParams, Constraint, Explicit_Cond Attr_Name,                 \
               __VEG_PP_REMOVE_PAREN(Params))                                \
  __VA_ARGS__

#define VEG_TEMPLATE_CVT_EXPLICIT(TParams, Constraint, Explicit_Cond, Attr, \
                                  Type, ...)                                \
  template <__VEG_PP_REMOVE_PAREN(TParams)>                                 \
...
#define VEG_TEMPLATE_EXPLICIT(TParams, Constraint, Explicit_Cond, Attr_Name, \
  ...
  VEG_TEMPLATE(                                                              \
      (__VEG_PP_REMOVE_PAREN TParams,                                        \
       ::proxsuite::linalg::veg::meta::enable_if_t<                          \
           (__VEG_PP_CAT2(__VEG_IMPL_PREFIX_, Explicit_Cond)), int> = 0),    \
      Constraint, explicit Attr_Name, __VEG_PP_REMOVE_PAREN(Params))         \
  __VA_ARGS__                                                                \
  VEG_TEMPLATE(                                                              \
      (__VEG_PP_REMOVE_PAREN TParams,                                        \
       ::proxsuite::linalg::veg::meta::enable_if_t<                          \
           !(__VEG_PP_CAT2(__VEG_IMPL_PREFIX_, Explicit_Cond)), unsigned> =  \
           0),                                                               \
      Constraint, Attr_Name, __VEG_PP_REMOVE_PAREN(Params))                  \
  __VA_ARGS__
...
#define VEG_TEMPLATE_CVT_EXPLICIT(TParams, Constraint, Explicit_Cond, Attr, \
  ...
  VEG_TEMPLATE_CVT(                                                         \
      (__VEG_PP_REMOVE_PAREN TParams,                                       \
       ::proxsuite::linalg::veg::meta::enable_if_t<                         \
           (__VEG_PP_CAT2(__VEG_IMPL_PREFIX_, Explicit_Cond)), int> = 0),   \
      Constraint, explicit Attr, __VEG_PP_REMOVE_PAREN(Type))               \
  __VA_ARGS__                                                               \
                                                                            \
  VEG_TEMPLATE_CVT(                                                         \
      (__VEG_PP_REMOVE_PAREN TParams,                                       \
       ::proxsuite::linalg::veg::meta::enable_if_t<                         \
           !(__VEG_PP_CAT2(__VEG_IMPL_PREFIX_, Explicit_Cond)), unsigned> = \
           0),                                                              \
      Constraint, Attr, __VEG_PP_REMOVE_PAREN(Type))                        \
  __VA_ARGS__
#endif
...
#define __VEG_IMPL_TEMPLATE(Attr_Name, TParams, Constraint, ...)   \
  template <__VEG_PP_REMOVE_PAREN(TParams)>                        \
    requires Constraint                                            \
  Attr_Name __VEG_PP_TUPLE_TRANSFORM_I(__VEG_IMPL_PARAM_EXPAND, _, \
                                       (__VA_ARGS__))
#else
...
#define __VEG_IMPL_TEMPLATE(Attr_Name, TParams, Constraint, ...)    \
  template <__VEG_PP_REMOVE_PAREN(TParams)>                         \
  Attr_Name __VEG_PP_TUPLE_TRANSFORM_I(__VEG_IMPL_TEMPLATE2_HELPER, \
...
#define VEG_DEF_CONCEPT_BUILTIN_OR_INTERNAL(Tpl, Name, ...)     \
  VEG_DEF_CONCEPT(                                              \
      Tpl, Name,                                                \
      VEG_HAS_BUILTIN_OR(__is_##Name, __is_##Name(__VA_ARGS__), \
...
template <typename... Ts>
constexpr auto make_simple_tuple(Empty /*dummy*/,
                                 Ts... args) noexcept -> SimpleTuple<Ts...> {
#if !defined(VEG_WITH_CXX17_SUPPORT)
...
namespace meta {
template <bool B, typename T = void>
using enable_if_t = _detail::_meta::discard_1st<
    typename _detail::_meta::enable_if<B, void>::type, T>;
...
struct Str {
 ...
 public:
  VEG_INLINE constexpr Str(Unsafe /*unsafe*/, FromRawParts /*from_raw_parts*/,
                           CharUnit const* ptr, isize len) noexcept
      : _{ptr, len} {};
...
template <typename LiteralType, usize... Is>
struct ExtractCharsImplExpr<LiteralType,
                            _meta::integer_sequence<usize, Is...>> {
  using Type = StrLiteralConstant<CharUnit(LiteralType::value()[Is])...>;
};

template <typename LiteralType>
auto extract_chars(LiteralType /*unused*/) ->
  ...
  struct _veglib_impl_member_extract {                                         \
    ...
    using MemberTuple =                                                        \
        decltype(::proxsuite::linalg::veg::_detail::make_simple_tuple(         \
            ::proxsuite::linalg::veg::_detail::Empty {                         \
            } __VEG_PP_TUPLE_FOR_EACH(__VEG_IMPL_GET_MEMBER_PTR, _,            \
                                      (__VA_ARGS__))));                        \
    ...
    static constexpr auto member_name_ptrs() noexcept                          \
      ...
      return {{__VEG_PP_TUPLE_FOR_EACH(__VEG_IMPL_GET_MEMBER_NAME_PTR, _,      \
                                       (__VA_ARGS__))}};                       \
    }                                                                          \
    static constexpr auto member_name_lens() noexcept                          \
        -> ::proxsuite::linalg::veg::_detail::Array_<                          \
            ::proxsuite::linalg::veg::usize,                                   \
            __VEG_PP_TUPLE_SIZE((__VA_ARGS__))> {                              \
      return {{__VEG_PP_TUPLE_FOR_EACH(__VEG_IMPL_GET_MEMBER_NAME_LEN, _,      \
                                       (__VA_ARGS__))}};                       \
    }                                                                          \
  };                                                                           \
