...
struct QP {
  ...
  void init(optional<MatRef<T>> H, optional<VecRef<T>> g, optional<MatRef<T>> A,
            ...
            optional<VecRef<T>> u, bool compute_preconditioner = true,
    ...
    if (settings.initial_guess ==
      ...
      work.refactorize =
          true;  // necessary for the first solve (then refactorize only if
                 // there is an update of the matrices)
    } else {
    ...
    proxsuite::proxqp::dense::setup(H, g, A, b, C, l, u, settings, model, work,
                                    results, ruiz, preconditioner_status);
   ...
   */
  void update(optional<MatRef<T>> H, optional<VecRef<T>> g,
              optional<MatRef<T>> A, optional<VecRef<T>> b,
              optional<MatRef<T>> C, optional<VecRef<T>> l,
    ...
    proxsuite::proxqp::dense::setup(/* avoid double assignation */
                                    ...
                                    optional_MatRef(nullopt),
                                    optional_VecRef(nullopt),
                                    optional_VecRef(nullopt), settings, model,
                                    work, results, ruiz, preconditioner_status);
  ...
  void solve() {
    qp_solve(  //
        settings, model, results, work, ruiz);
  };
  /*!
...
proxqp::Results<T> solve(
    ...
    optional<VecRef<T>> b, optional<MatRef<T>> C, optional<VecRef<T>> l,
    optional<VecRef<T>> u, optional<VecRef<T>> x = nullopt,
    optional<VecRef<T>> y = nullopt, optional<VecRef<T>> z = nullopt,
    optional<T> eps_abs = nullopt, optional<T> eps_rel = nullopt,
    optional<T> rho = nullopt, optional<T> mu_eq = nullopt,
    optional<T> mu_in = nullopt, optional<bool> verbose = nullopt,
    bool compute_preconditioner = true, bool compute_timings = false,
