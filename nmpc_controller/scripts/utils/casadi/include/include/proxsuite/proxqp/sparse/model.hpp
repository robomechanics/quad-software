...
struct Model {
   ...
   */
  Model(isize dim, isize n_eq, isize n_in) : dim(dim), n_eq(n_eq), n_in(n_in) {
    ...
    b.setZero();
    u.setZero();
    u.fill(+infinite_bound_value);  // in case it appears u is nullopt (i.e.,
                                    // the problem is only lower bounded)
    l.fill(-infinite_bound_value);  // in case it appears l is nullopt (i.e.,
  ...
  auto kkt() const -> proxsuite::linalg::sparse::MatRef<T, I> {
    ...
    auto nnz = isize(
        proxsuite::linalg::sparse::util::zero_extend(kkt_col_ptrs[n_tot]));
    return {
...
struct SparseModel {
  ...
  template <typename Vector_g, typename Vector_b, typename Vector_u,
            typename Vector_l>
  ...
  auto as_view() -> proxqp::dense::QpView<Scalar> {
    return {
        ...
        {proxqp::from_eigen, A},
        {proxqp::from_eigen, b},
        {proxqp::from_ptr_rows_cols_stride, nullptr, 0, proxqp::isize(H.rows()),
         0},
