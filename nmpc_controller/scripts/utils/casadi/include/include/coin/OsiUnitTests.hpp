...
/** A function that tests that a lot of problems given in MPS files (mostly the
 NETLIB problems) solve properly with all the specified solvers.
 *
 ...
 * the compilation took 10-15 minutes and the machine pages (has 256M core
 * memory!)...
 */
...
/*! \brief Test outcomes
  ...
  A global TestOutcomes object to store test outcomes during the run of the unit
  test for an OSI.
 */
...
/*! \brief Print an error message
  ...
  Formatted as "XxxSolverInterface testing issue: message" where Xxx is the
  string provided as \p solverName.
...
/*! \brief Print an error message, specifying the test name and condition
  ...
  Formatted as "XxxSolverInterface testing issue: testname failed: testcond"
  where Xxx is the OsiStrParam::OsiSolverName parameter of the \p si. Flushes
  std::cout before printing to std::cerr.
*/
void failureMessage(const std::string &solverName, const std::string &testname,
                    const std::string &testcond);

/// \overload
void failureMessage(const OsiSolverInterface &si, const std::string &testname,
                    const std::string &testcond);
...
bool equivalentVectors(const OsiSolverInterface *si1,
                       const OsiSolverInterface *si2, double tol,
                       const double *v1, const double *v2, int size);

/*! \brief Compare two problems for equality

  Compares the problems held in the two solvers: constraint matrix, row and
  column bounds, column type, and objective. Rows are checked using upper and
  lower bounds and using sense, bound, and range.
*/
...
/*! \brief Compare a packed vector with an expanded vector
  ...
  Checks that all values present in the packed vector are present in the full
  vector and checks that there are no extra entries in the full vector. Uses
  CoinRelFltEq with the default tolerance.
*/
bool isEquivalent(const CoinPackedVectorBase &pv, int n, const double *fv);

/*! \brief Process command line parameters.

 An unrecognised keyword which is not in the \p ignorekeywords map will trigger
 the help message and a return value of false. For each keyword in \p
 ignorekeywords, you can specify the number of following parameters that should
 be ignored.
...
bool processParameters(int argc, const char **argv,
                       ...
                       const std::map<std::string, int> &ignorekeywords =
...
class TestOutcome {
 public:
  ...
  TestOutcome(const std::string &comp, const std::string &tst, const char *cond,
              SeverityLevel sev, const char *file, int line, bool exp = false)
      : component(comp),
        testname(tst),
        testcond(cond),
        severity(sev),
...
class TestOutcomes : public std::list<TestOutcome> {
 public:
  ...
  /*! \brief Count total and expected outcomes at given severity level
      ...
      Given a severity level, walk the list of outcomes and count the total
     number of outcomes at this severity level and the number expected.
    */
  void getCountBySeverity(TestOutcome::SeverityLevel sev, int &total,
                          int &expected) const;
};
...
bool OsiUnitTestAssertSeverityExpected(
    bool condition, const char *condition_str, const char *filename, int line,
    const Component &component, const std::string &testname,
    TestOutcome::SeverityLevel severity, bool expected) {
  if (condition) {
    OsiUnitTest::outcomes.add(component, testname, condition_str,
                              OsiUnitTest::TestOutcome::PASSED, filename, line,
                              false);
  ...
  OsiUnitTest::outcomes.add(component, testname, condition_str, severity,
                            filename, line, expected);
  OsiUnitTest::failureMessage(component, testname, condition_str);
  switch (OsiUnitTest::haltonerror) {
    case 2: {
      if (severity >= OsiUnitTest::TestOutcome::ERROR) std::abort();
      break;
    }
    case 1: {
...
#define OSIUNITTEST_ADD_OUTCOME(component, testname, testcondition, severity, \
                                expected)                                     \
...
/*! \brief Test for a condition and record the result
  ...
  If it succeeds, record the result as OsiUnitTest::TestOutcome::PASSED and
  print a message for OsiUnitTest::verbosity >= 2. If it fails, record the test
  as failed with \p severity and \p expected and react as specified by
  OsiUnitTest::haltonerror.

  \p failurecode is executed when failure is not fatal.
*/
#define OSIUNITTEST_ASSERT_SEVERITY_EXPECTED(                               \
    condition, failurecode, component, testname, severity, expected)        \
  {                                                                         \
    if (!OsiUnitTestAssertSeverityExpected(condition, #condition, __FILE__, \
                                           __LINE__, component, testname,   \
                                           severity, expected)) {           \
      failurecode;                                                          \
    }                                                                       \
  }

/*! \brief Perform a test with severity OsiUnitTest::TestOutcome::ERROR, failure
   not expected.
*/
...
/*! \brief Perform a test with severity OsiUnitTest::TestOutcome::WARNING,
   failure not expected.
*/
#define OSIUNITTEST_ASSERT_WARNING(condition, failurecode, component, \
                                   testname)                          \
  OSIUNITTEST_ASSERT_SEVERITY_EXPECTED(                               \
      condition, failurecode, component, testname,                    \
      OsiUnitTest::TestOutcome::WARNING, false)

/*! \brief Perform a test surrounded by a try/catch block

  \p trycode is executed in a try/catch block; if there's no throw the test is
  deemed to have succeeded and is recorded in OsiUnitTest::outcomes with status
  OsiUnitTest::TestOutcome::PASSED. If the \p trycode throws a CoinError, the
  failure is recorded with status \p severity and \p expected and the value of
  OsiUnitTest::haltonerror is consulted. If the failure is not fatal, \p
  catchcode is executed. If any other error is thrown, the failure is recorded
  as for a CoinError and \p catchcode is executed (haltonerror is not
  consulted).
*/
#define OSIUNITTEST_CATCH_SEVERITY_EXPECTED(trycode, catchcode, component,    \
                                            testname, severity, expected)     \
  {                                                                           \
    try {                                                                     \
      trycode;                                                                \
      OSIUNITTEST_ADD_OUTCOME(component, testname,                            \
                              #trycode " did not throw exception",            \
                              OsiUnitTest::TestOutcome::PASSED, false);       \
      if (OsiUnitTest::verbosity >= 2) {                                      \
        std::string successmsg(__FILE__                                       \
                               ":" OSIUNITTEST_QUOTEME(__LINE__) ": ");       \
        successmsg = successmsg + testname;                                   \
        successmsg =                                                          \
            successmsg + " (code \'" #trycode "\') did not throw exception";  \
      ...
      if (e.className().length() > 0) errmsg << " in " << e.className();      \
      if (e.methodName().length() > 0) errmsg << " in " << e.methodName();    \
      if (e.lineNumber() >= 0)                                                \
      ...
      switch (OsiUnitTest::haltonerror) {                                     \
        case 2: {                                                             \
          if (severity >= OsiUnitTest::TestOutcome::ERROR) abort();           \
          break;                                                              \
        }                                                                     \
        case 1: {                                                             \
          std::cout << std::endl                                              \
      ...
      OSIUNITTEST_ADD_OUTCOME(component, testname, errmsg.c_str(), severity,  \
                              false);                                         \
...
/*! \brief Perform a try/catch test with severity
   OsiUnitTest::TestOutcome::ERROR, failure not expected.
*/
#define OSIUNITTEST_CATCH_ERROR(trycode, catchcode, component, testname)       \
  OSIUNITTEST_CATCH_SEVERITY_EXPECTED(trycode, catchcode, component, testname, \
                                      OsiUnitTest::TestOutcome::ERROR, false)

/*! \brief Perform a try/catch test with severity
   OsiUnitTest::TestOutcome::WARNING, failure not expected.
*/
