...
class BabSetupBase {
 public:
  ...
  struct CuttingMethod {
    ...
    bool normal;
    bool always;
    CuttingMethod() : atSolution(false), normal(true), always(false) {}
  ...
  struct HeuristicMethod {
    ...
    HeuristicMethod(const HeuristicMethod &other)
        : id(other.id), heuristic(other.heuristic) {}
  };
  ...
  enum NodeComparison {
    ...
    dynamic /** Dynamic strategy, see <a
               href="http://www.coin-or.org/Doxygen/Cbc/class_cbc_branch_dynamic_decision.html">
                CbcBranchActual.hpp </a> for explanations.*/
    ,
    bestGuess /** Best guessed integer solution is subtree below, based on
                 pseudo costs */
  };
  ...
  enum TreeTraversal {
    ...
    DiveFromBest /** dive from top node of the heap untill it gets to a leaf of
                    the tree. Uses Bonmin::CbcDiver.*/
    ,
    ProbedDive /** Eplore two kids before following on dive.*/,
    DfsDiveFromBest /** dive from top node of the heap with more elaborate
                       strategy (see options doc). Uses Bonmin::CbcDfsDiver.*/
    ,
    DfsDiveDynamic /** Same as DfsDiveFromBest, but after a prescribed number of
                      integer solution are found switch to best-bound and if too
                      many node switches to depth-first. Uses
                      Bonmin::CbcDfsDiver.*/
  };
  ...
  void setNonlinearSolver(OsiTMINLPInterface *s) { nonlinearSolver_ = s; }
  /** @} */
  ...
  virtual void setBabDefaultOptions(
      Ipopt::SmartPtr<Bonmin::RegisteredOptions> roptions) {}
  /** Register all the options for this algorithm instance.*/
  static void registerAllOptions(
      Ipopt::SmartPtr<Bonmin::RegisteredOptions> roptions);

  /** Get the options from default text file (bonmin.opt) if don't already have
   * them.*/
  ...
  /** May print documentation of options if options print_options_documentation
   * is set to yes.*/
  void mayPrintDoc();

  /** Get prefix to use for options.*/
  const char *prefix() const { return prefix_.c_str(); }

  /** Set the value for options, output...*/
  void setOptionsAndJournalist(
      Ipopt::SmartPtr<Bonmin::RegisteredOptions> roptions,
  ...
  OsiTMINLPInterface *nonlinearSolver() { return nonlinearSolver_; }
  /** Pointer to the continuous solver to use for relaxations. */
  OsiSolverInterface *continuousSolver() { return continuousSolver_; }
  /** list of cutting planes methods to apply with their frequencies. */
  CuttingMethods &cutGenerators() { return cutGenerators_; }
  /** list of Heuristic methods to use. */
  HeuristicMethods &heuristics() { return heuristics_; }
  /** branching method to use. */
  OsiChooseVariable *branchingMethod() { return branchingMethod_; }
  /** Method used to compare nodes. */
  NodeComparison &nodeComparisonMethod() { return nodeComparisonMethod_; }
  /** Method used to traverse tree.*/
  TreeTraversal treeTraversalMethod() { return treeTraversalMethod_; }
  /** Return value of integer parameter. */
  int getIntParameter(const IntParameter &p) const { return intParam_[p]; }
  /** Return value of double parameter.*/
  double getDoubleParameter(const DoubleParameter &p) const {
    return doubleParam_[p];
  }
  /** Return value of integer parameter. */
  void setIntParameter(const IntParameter &p, const int v) { intParam_[p] = v; }
  /** Return value of double parameter.*/
  ...
  void gatherParametersValues() { gatherParametersValues(options_); }
  ...
  const vector<OsiObject *> &objects() const { return objects_; }

  /** Access to extra objects.*/
  vector<OsiObject *> &objects() { return objects_; }

  void addCutGenerator(CuttingMethod &cg) { cutGenerators_.push_back(cg); }

  void set_linearizer(TMINLP2OsiLP *linearizer) { linearizer_ = linearizer; }
