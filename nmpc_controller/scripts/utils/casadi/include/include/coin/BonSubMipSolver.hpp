...
class BabSetupBase;
/** A very simple class to provide a common interface for solving MIPs with
 * Cplex and Cbc.*/
class SubMipSolver {
 public:
   ...
   */
  const double *getLastSolution() { return integerSolution_; }

  double getLowerBound() { return lowBound_; }
  ...
  /** update cutoff, put OA constraints in cs as lazy constraints and optimize
   * MIP. */
  void optimize_with_lazy_constraints(double cutoff, int loglevel,
                                      double maxTime, const OsiCuts &cs);

  /** Returns lower bound. */
  inline double lowBound() { return lowBound_; }

  /** returns optimality status. */
  inline bool optimal() { return optimal_; }

  /** Returns number of nodes in last solve.*/
  inline int nodeCount() { return nodeCount_; }

  /** Returns number of simplex iterations in last solve.*/
  inline int iterationCount() { return iterationCount_; }
