...
class ClpGubMatrix : public ClpPackedMatrix {
 public:
  ...
  /** This is local to Gub to allow synchronization:
         ...
         mode=6  - return 1 if there may be changing bounds on variable (column
     generation) mode=7  - do extra restores for column generation mode=8  -
     make sure set is clean mode=9  - adjust lower, upper on set by incoming
     */
  ...
  ClpGubMatrix(const ClpGubMatrix &wholeModel, int numberRows,
               const int *whichRows, int numberColumns,
               const int *whichColumns);
  ClpGubMatrix(const CoinPackedMatrix &wholeModel, int numberRows,
               const int *whichRows, int numberColumns,
               const int *whichColumns);
  ...
  /** This takes over ownership (for space reasons) and is the
         real constructor*/
  ClpGubMatrix(ClpPackedMatrix *matrix, int numberSets, const int *start,
               const int *end, const double *lower, const double *upper,
  ...
  inline int *backward() const { return backward_; }
