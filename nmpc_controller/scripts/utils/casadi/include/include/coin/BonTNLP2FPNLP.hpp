 ...
 * The extra function is set_dist_to_point_obj(size_t n, const double *, const
 * int *)
 */
class TNLP2FPNLP : public Ipopt::TNLP {
 public:
  ...
  TNLP2FPNLP(const Ipopt::SmartPtr<Ipopt::TNLP> tnlp,
             double objectiveScalingFactor = 100);

  /** Build using tnlp as source problem and using other for all other
   * parameters..*/
  TNLP2FPNLP(const Ipopt::SmartPtr<TNLP> tnlp,
             const Ipopt::SmartPtr<TNLP2FPNLP> other);

  /** Default destructor */
  virtual ~TNLP2FPNLP();
  //@}
  void use(Ipopt::SmartPtr<TNLP> tnlp) { tnlp_ = GetRawPtr(tnlp); }
  ...
  void set_rhs_local_branching_constraint(
   ...
   */
  void set_dist_to_point_obj(size_t n, const Ipopt::Number* vals,
  ...
  virtual bool get_starting_point(Ipopt::Index n, bool init_x, Ipopt::Number* x,
    ...
    int ret_code = tnlp_->get_starting_point(n, init_x, x, init_z, z_L, z_U, m2,
                                             init_lambda, lambda);
    return ret_code;
  }
  ...
  //@{
  void setObjectiveScaling(double value) { objectiveScalingFactor_ = value; }
  double getObjectiveScaling() const { return objectiveScalingFactor_; }

 private:
  /** @name Internal methods to help compute the distance, its gradient and
   * hessian */
  //@{
  /** Compute the norm-2 distance to the current point to which distance is
   * minimized. */
  ...
  //@{
  /// Indices of the variables for which distance is minimized (i.e. indices of
  /// integer variables in a feasibility pump setting)
  vector<Ipopt::Index> inds_;
  /// Values of the point to which we separate (if x is the point vals_[i]
  /// should be x[inds_[i]] )
  vector<Ipopt::Number> vals_;
  /** value for the convex combination to take between original objective and
   * distance function. ( take lambda_ * distance + (1-lambda) sigma f(x).*/
  double lambda_;
