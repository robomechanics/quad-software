...
class CglTreeInfo {
  ...
  */
  ...
  int *originalColumns;
  /** Replacement array.  Before Branch and Cut it may be beneficial to
     strengthen rows rather than adding cuts.  If this array is not NULL then
     the cut generator can place a pointer to the stronger cut in this array
     which is number of rows in size.

      A null (i.e. zero elements and free rhs) cut indicates that the row is
  ...
  */
  ...
  /// Destructor
  virtual ~CglTreeInfo();
  /// Take action if cut generator can fix a variable (toValue -1 for down, +1
  /// for up)
...
typedef struct {
  ...
  // unsigned int sequence:31; //  variable (in matrix) (but also see
  // cliqueRow_)
  unsigned int fixes;
} CliqueEntry;
...
class CglTreeProbingInfo : public CglTreeInfo {
 public:
  ...
  OsiSolverInterface *analyze(const OsiSolverInterface &si,
                              int createSolver = 0, int numberExtraCliques = 0,
  ...
  virtual bool fixes(int variable, int toValue, int fixedVariable,
                     bool fixedToLower);
  /** Initalizes fixing arrays etc - returns >0 if we want to save info
      0 if we don't and -1 if is to be used */
  ...
  inline int *integerVariable() const { return integerVariable_; }
  /// Backward look up
  inline int *backward() const { return backward_; }
  /// Number of variables
  inline int numberVariables() const { return numberVariables_; }
