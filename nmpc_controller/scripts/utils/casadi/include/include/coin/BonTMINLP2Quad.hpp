...
class TMINLP2TNLPQuadCuts : public Bonmin::TMINLP2TNLP {
 public:
  ...
  /** Returns the vector of constraint values in x (appends constraint values
   * for quadratics).*/
  ...
  /** Add some linear or quadratic cuts to the problem formulation
      if some of the OsiRowCuts are quadratic they will be well understood as
     long as safe is true.*/
  ...
  void reset_objective() { obj_.clear(); }
