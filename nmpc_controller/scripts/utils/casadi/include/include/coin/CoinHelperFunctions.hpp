...
template <class T>
inline void CoinCopyN(const T *from, const CoinBigIndex size, T *to) {
  if (size == 0 || from == to) return;

#ifndef NDEBUG
  if (size < 0)
    throw CoinError("trying to copy negative number of entries", "CoinCopyN",
                    "");
#endif
...
*/
template <class T>
inline void CoinCopy(const T *first, const T *last, T *to) {
...
template <class T>
inline void CoinDisjointCopyN(const T *from, const CoinBigIndex size, T *to) {
#ifndef _MSC_VER
  if (size == 0 || from == to) return;
...
template <class T>
inline void CoinDisjointCopy(const T *first, const T *last, T *to) {
...
template <class T>
inline T *CoinCopyOfArray(const T *array, const CoinBigIndex size) {
  if (array) {
...
/*! \brief Return an array of length \p size filled with first copySize from \p
  array, or null if \p array is null.
*/

template <class T>
inline T *CoinCopyOfArrayPartial(const T *array, const CoinBigIndex size,
...
template <class T>
inline T *CoinCopyOfArray(const T *array, const CoinBigIndex size, T value) {
  T *arrayNew = new T[size];
  if (array) {
    std::memcpy(arrayNew, array, size * sizeof(T));
  } else {
    CoinBigIndex i;
    for (i = 0; i < size; i++) arrayNew[i] = value;
  }
  return arrayNew;
}
...
template <class T>
inline T *CoinCopyOfArrayOrZero(const T *array, const CoinBigIndex size) {
  T *arrayNew = new T[size];
  if (array) {
    std::memcpy(arrayNew, array, size * sizeof(T));
  } else {
...
template <class T>
inline void CoinMemcpyN(const T *from, const CoinBigIndex size, T *to) {
#ifndef _MSC_VER
#ifdef USE_MEMCPY
  ...
  // Some debug so check
  if (size < 0)
    throw CoinError("trying to copy negative number of entries", "CoinMemcpyN",
                    "");
  ...
  for (CoinBigIndex n = static_cast<CoinBigIndex>(size >> 3); n > 0;
       --n, from += 8, to += 8) {
    to[0] = from[0];
    to[1] = from[1];
    to[2] = from[2];
...
#else
template <class T>
inline void CoinMemcpyN(const T *COIN_RESTRICT from, CoinBigIndex size,
                        T *COIN_RESTRICT to) {
#ifdef USE_MEMCPY
  std::memcpy(to, from, size * sizeof(T));
#else
...
/** This helper function copies an array to another location. The two arrays
    ...
    last" entry; the target array is given by its first entry. */
template <class T>
inline void CoinMemcpy(const T *first, const T *last, T *to) {
...
template <class T>
inline void CoinFillN(T *to, const CoinBigIndex size, const T value) {
  if (size == 0) return;

#ifndef NDEBUG
  if (size < 0)
    throw CoinError("trying to fill negative number of entries", "CoinFillN",
                    "");
#endif
#if 1
...
/** This helper function fills an array with a given value. For speed 8
    ...
    last" entry. */
template <class T>
inline void CoinFill(T *first, T *last, const T value) {
...
template <class T>
inline void CoinZeroN(T *to, const CoinBigIndex size) {
#ifdef USE_MEMCPY
  // Use memset - seems faster on Intel with gcc
#ifndef NDEBUG
  // Some debug so check
  if (size < 0)
    throw CoinError("trying to fill negative number of entries", "CoinZeroN",
                    "");
#endif
...
inline void CoinCheckDoubleZero(double *to, const CoinBigIndex size) {
  CoinBigIndex n = 0;
...
/// This Debug helper function checks an array is all zero
inline void CoinCheckIntZero(int *to, const CoinBigIndex size) {
  CoinBigIndex n = 0;
  for (CoinBigIndex j = 0; j < size; j++) {
    if (to[j]) n++;
  }
...
/** This helper function fills an array with a given value. For speed 8
    ...
    last" entry. */
template <class T>
inline void CoinZero(T *first, T *last) {
...
template <class T>
inline T CoinMax(const T x1, const T x2) {
  return (x1 > x2) ? x1 : x2;
}
...
template <class T>
inline T CoinMin(const T x1, const T x2) {
  return (x1 < x2) ? x1 : x2;
}
...
/** Return the absolute value of the argument. This function was introduced
    ...
    differently. */
template <class T>
inline T CoinAbs(const T value) {
...
/** This helper function tests whether the entries of an array are sorted
    ...
    and by its size. */
template <class T>
inline bool CoinIsSorted(const T *first, const CoinBigIndex size) {
...
/** This helper function tests whether the entries of an array are sorted
    ...
    last" entry. */
template <class T>
inline bool CoinIsSorted(const T *first, const T *last) {
...
template <class T>
inline void CoinIotaN(T *first, const CoinBigIndex size, T init) {
  if (size == 0) return;

#ifndef NDEBUG
  if (size < 0) throw CoinError("negative number of entries", "CoinIotaN", "");
#endif
#if 1
...
template <class T>
inline void CoinIota(T *first, const T *last, T init) {
...
inline T *CoinDeleteEntriesFromArray(T *arrayFirst, T *arrayLast,
  ...
  if (!(CoinIsSorted(firstDelPos, lastDelPos) &&
    ...
    delNum = static_cast<CoinBigIndex>(
        std::unique(delSortedPos, delSortedPos + delNum) - delSortedPos);
  }
  ...
  for (CoinBigIndex i = 0; i < last; ++i) {
    ...
    CoinCopy(arrayFirst + copyFirst, arrayFirst + copyLast, arrayFirst + size);
  ...
  if (delSortedPos) delete[] delSortedPos;
...
inline void CoinSeedRandom(int iseed) { CoinDrand48(true, iseed); }
...
inline int CoinStrNCaseCmp(const char *s0, const char *s1, const size_t len) {
...
template <class T>
inline int CoinFromFile(T *&array, CoinBigIndex size, FILE *fp,
  ...
  int returnCode = 0;
  if (size != newSize && (newSize || array)) returnCode = 2;
  if (newSize) {
    array = new T[newSize];
    numberRead =
        static_cast<CoinBigIndex>(fread(array, sizeof(T), newSize, fp));
    if (numberRead != newSize) returnCode = 1;
  } else {
    array = NULL;
  }
  return returnCode;
...
inline int CoinStrlenAsInt(const char *string) {
...
class CoinThreadRandom {
 public:
  ...
  //@{
  /** Default constructor. */
  CoinThreadRandom() { seed_ = 12345678; }
  /** Constructor wih seed. */
  CoinThreadRandom(int seed) { seed_ = seed; }
  /** Destructor */
  ~CoinThreadRandom() {}
  // Copy
  CoinThreadRandom(const CoinThreadRandom &rhs) { seed_ = rhs.seed_; }
  // Assignment
  ...
  inline void randomize(int n = 0) {
    if (!n) n = seed_ & 255;
    for (int i = 0; i < n; i++) randomDouble();
  }
  //@}
