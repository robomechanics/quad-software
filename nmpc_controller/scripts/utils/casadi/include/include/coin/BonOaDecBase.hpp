...
class OaDecompositionBase : public CglCutGenerator {
 public:
  /** Small class to manipulatee various things in an OsiSolverInterface and
     restore them. The OsiSolverInterface manipulated may already exist or may
     be cloned from another one.*/
  class solverManip {
   public:
    /** Constructor. */
    ...
    OsiSolverInterface *si() { return si_; }
  ...
  void assignNlpInterface(OsiTMINLPInterface *nlp) { nlp_ = nlp; }

  /// Assign an OsiTMINLPInterface
  void assignLpInterface(OsiSolverInterface *si) { lp_ = si; }

  bool reassignLpsolver() { return reassignLpsolver_; }
  /** Set objects.*/
  void setObjects(OsiObject **objects, int nObjects) {
    objects_ = objects;
    nObjects_ = nObjects;
  ...
  inline void setLeaveSiUnchanged(bool yesno) { leaveSiUnchanged_ = yesno; }
  ...
  struct Parameters {
    ...
    const CbcStrategy *strategy() const { return strategy_; }
  ...
  Parameters &parameter() { return parameters_; }

  const Parameters &parameter() const { return parameters_; }

  void setLogLevel(int level) { handler_->setLogLevel(level); }

  void setReassignLpSolver(bool v) { reassignLpsolver_ = v; }
 ...
 protected:
  ...
  virtual double performOa(OsiCuts &cs, solverManip &lpManip, BabInfo *babInfo,
                           double &, const CglTreeInfo &info) const = 0;
  ...
  /** Saved cuts: in some cases when using OA to check feasible solution
     algorithm may loop because Cbc removes inactive cuts. To overcome this we
     can impose that no OA cut can be discarded by Cbc but this consumes too
     much memory in some cases. Here we do it another way: cuts generated at
     current node are saved if algorithm seems to enter a loop we impose the
     needed cuts to be kept.*/
  ...
  class OaDebug {
   public:
    ...
    void printEndOfProcedureDebugMessage(const OsiCuts &cs, bool foundSolution,
                                         double solValue, double milpBound,
                                         bool isInteger, bool feasible,
