...
class CglLandP : public CglCutGenerator {
  friend void CglLandPUnitTest(OsiSolverInterface *si,
 ...
 public:
  enum SelectionRules {
    ...
    initialReducedCosts /** Select only those rows which had initialy a 0
                           reduced cost.*/
  };
  ...
  enum ExtraCutsMode {
    ...
    WhenEnteringBasis /** Generate cuts as soon as a structural enters the
                         basis.*/
    ,
    AllViolatedMigs /** Generate all violated Mixed integer Gomory cuts in the
  ...
  class Parameters : public CglParam {
   public:
    ...
    /** Max number of pivots before we generate the cut
      \default 20 */
    int pivotLimit;
    /** Max number of pivots at regular nodes. Put a value if you want it lower
     than the global pivot limit. \default 100.*/
    ...
    /// @name Flags
    ///@{
    /** Do we use tableau row or the disjunction (I don't really get that there
     * should be a way to always use the tableau)*/
    bool useTableauRow;
    /** Do we apply Egon Balas's Heuristic for modularized cuts */
    bool modularize;
    /** Do we strengthen the final cut (always do if modularize is 1) */
    bool strengthen;
    /** Wether to limit or not the number of mistaken RC (when perturbation is
     * applied).*/
  ...
  LAP::Validator &validator() { return validator_; }
  ...
  class SimplexInterfaceError : public CoinError {
   public:
    SimplexInterfaceError()
        : CoinError("Invalid conversion to simplex interface", "CglLandP",
                    "CglLandP") {}
  };
 ...
 private:
  ...
  /** Some informations that will be changed by the pivots and that we want to
   * keep*/
  struct CachedData {
    ...
    ~CachedData();
    /** Indices of basic variables in starting basis (ordered if variable
     * basics_[i] s basic in row i)*/
    int *basics_;
  ...
  int getSortedFractionals(CoinPackedVector &xFrac, const CachedData &data,
  ...
  bool canLift_;
  /** Store some extra cut which could be cheaply generated but do not cut
   * current incumbent.*/
  OsiCuts extraCuts_;
};
