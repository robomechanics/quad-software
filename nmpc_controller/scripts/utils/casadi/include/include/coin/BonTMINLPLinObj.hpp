...
namespace Bonmin {
/** From a TMINLP, this class adapts to another TMINLP where the original
 objective is transformed into a constraint by adding an extra variable which is
 minimized.
    ...
    \end{array}
    \f]
    The objective is put as first constraint of the problem and the extra
 variable is the last one.
 .*/
class TMINLPLinObj : public Bonmin::TMINLP {
 public:
   ...
   *  the hessian. Call tminlp_ one  but number of constraints and non-zeroes in
   * the jacobian is stored internally.*/
   ...
   * In first call nothing to change. In later just fix the values for the
   * simple concaves and remove entries corresponding to nonConvex constraints.
   */
  ...
  virtual void finalize_solution(TMINLP::SolverReturn status, Ipopt::Index n,
                                 ...
                                 Ipopt::Number obj_value) {
    return tminlp_->finalize_solution(status, n - 1, x, obj_value);
  }
  //@}
