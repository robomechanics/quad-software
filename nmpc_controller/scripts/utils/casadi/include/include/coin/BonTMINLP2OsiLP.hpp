...
/** A transformer class to build outer approximations i.e. transfomrs nonlinear
 * programs into linear programs.*/
class TMINLP2OsiLP : public Ipopt::ReferencedObject {
 public:
  /** Default constructor.*/
  TMINLP2OsiLP() : tiny_(-0.), very_tiny_(-0.) {}
  ...
  virtual void extract(OsiSolverInterface *si, const double *x,
                       bool getObj) = 0;
  ...
  /** Get OAs of nonlinear constraints in x.*/
  virtual void get_oas(OsiCuts &cs, const double *x, bool getObj,
                       bool global) const = 0;
 ...
 protected:
  ...
  inline bool cleanNnz(double &value, double colLower, double colUpper,
                       ...
                       double &lb, double &ub, double tiny,
...
// A procedure to try to remove small coefficients in OA cuts (or make it non
// small
inline bool TMINLP2OsiLP::cleanNnz(double &value, double colLower,
                                   double colUpper, double rowLower,
                                   double rowUpper, double colsol, double &lb,
                                   double &ub, double tiny,
                                   double veryTiny) const {
  ...
  if (colLoBounded && !pos && rowNotUpBounded) {
    ...
    return 0;
  } else if (colLoBounded && pos && rowNotLoBounded) {
    ub += value * (colsol - colLower);
    return 0;
  } else if (colUpBounded && pos && rowNotUpBounded) {
    lb += value * (colsol - colUpper);
    return 0;
  } else if (colUpBounded && !pos && rowNotLoBounded) {
