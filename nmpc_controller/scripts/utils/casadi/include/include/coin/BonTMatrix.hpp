...
struct TMat {
  ...
  /** Default constructor.*/
  TMat() : iRow_(NULL), jCol_(NULL), value_(NULL), nnz_(0), capacity_(0) {}
  ...
  const RowS &nonEmptyRows() const { return nonEmptyRows_; }

  /** Get number of non empty cols.*/
  int numNonEmptyCols();

  /** Get the list of non empty row.*/
  const RowS &nonEmptyCols() const { return nonEmptyCols_; }

 private:
  /** Structure for ordering matrix.*/
  struct TMatOrdering {
    TMat *M_;
    TMatOrdering(TMat *M) : M_(M) {}
  };

  /** Structure for ordering matrix by columns.*/
  struct ColumnOrder : public TMatOrdering {
    ColumnOrder(TMat *M) : TMatOrdering(M) {}

    bool operator()(const int &i, const int &j) {
      if (M_->jCol_[i] < M_->jCol_[j]) return true;
  ...
  struct RowOrder : public TMatOrdering {
    RowOrder(TMat *M) : TMatOrdering(M) {}
    bool operator()(const int &i, const int &j) {
      if (M_->iRow_[i] < M_->iRow_[j]) return true;
 ...
 public:
  ...
  const vector<int> &orderByColumns() {
    ...
    std::sort(columnOrdering_.begin(), columnOrdering_.end(),
              ColumnOrder(this));
