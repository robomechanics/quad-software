...
/* Forward declaration, the function will be defined in BonAmplTMINLP.cpp, if
 * ASL is available */
namespace Ipopt {
...
class RegisteredOptions : public Ipopt::RegisteredOptions {
 public:
  ...
  RegisteredOptions() : Ipopt::RegisteredOptions() {}
  ...
  inline bool isValidForBOA(const std::string &option) {
    optionExists(option);
    std::map<std::string, int>::iterator i = bonOptInfos_.find(option);
    if (i != bonOptInfos_.end()) return (i->second) & (1 << validInOA);
    return true;
  }
  ...
  inline bool isValidForBBB(const std::string &option) {
    optionExists(option);
    std::map<std::string, int>::iterator i = bonOptInfos_.find(option);
    if (i != bonOptInfos_.end()) return (i->second) & (1 << validInBBB);
    return true;
  }
  ...
  inline bool isValidForBEcp(const std::string &option) {
    optionExists(option);
    std::map<std::string, int>::iterator i = bonOptInfos_.find(option);
    if (i != bonOptInfos_.end()) return (i->second) & (1 << validInEcp);
    return true;
  }
  ...
  inline bool isValidForBiFP(const std::string &option) {
    optionExists(option);
    std::map<std::string, int>::iterator i = bonOptInfos_.find(option);
    if (i != bonOptInfos_.end()) return (i->second) & (1 << validIniFP);
    return true;
  }
  ...
  inline bool isValidForCbc(const std::string &option) {
    optionExists(option);
    std::map<std::string, int>::iterator i = bonOptInfos_.find(option);
    if (i != bonOptInfos_.end()) return (i->second) & (1 << validInCbc);
    return true;
  }
  ...
  /** Ouptut a bonmin.opt file with options default values and short
   * descriptions.*/
  void writeBonminOpt(std::ostream &os, ExtraCategoriesInfo which);

  /** Get info about what a category is taking care of (e.g., Ipopt, Bonmin,
   * FilterSQP,...) .*/
  ExtraCategoriesInfo categoriesInfo(const std::string &s) {
    std::map<std::string, ExtraCategoriesInfo>::iterator i =
        categoriesInfos_.find(s);
    if (i == categoriesInfos_.end()) return IpoptCategory;
    return i->second;
  }

  /* Forward declaration, the function will be defined in BonAmplTMINLP.cpp*/
  void fillAmplOptionList(ExtraCategoriesInfo which,
                          Ipopt::AmplOptionsList *amplOptList);

 private:
  /** Output Latex table of options.*/
  void chooseOptions(ExtraCategoriesInfo which,
                     std::list<Ipopt::RegisteredOption *> &options);
  /** Output html table of options.*/
  void writeHtmlOptionsTable(std::ostream &os,
                             std::list<Ipopt::RegisteredOption *> &options);
