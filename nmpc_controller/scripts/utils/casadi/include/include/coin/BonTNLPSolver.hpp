...
/** This is a generic class for calling an NLP solver to solve a TNLP.
    A TNLPSolver is able to solve and resolve a problem, it has some options
   (stored with Ipopt OptionList structure and registeredOptions) it produces
   some statistics (in SolveStatisctics and sometimes some errorCodes.
*/
class TNLPSolver : public Ipopt::ReferencedObject {
 public:
  enum ReturnStatus /** Standard return statuses for a solver*/ {
    ...
    illDefinedProblem =
        -4 /** The solver finds that the problem is not well defined. */,
    illegalOption = -5 /** An option is not valid. */,
    externalException = -6 /** Some unrecovered exception occurred in an
                              external tool used by the solver. */
    ,
    exception = -7 /** Some unrocevered exception */,
    solvedOptimal = 1 /** Problem solved to an optimal solution.*/,
    solvedOptimalTol =
        2 /** Problem solved to "acceptable level of tolerance. */,
  ...
  class UnsolvedError {
   public:
    ...
    UnsolvedError(int errorNum = -10000,
                  ...
                  std::string name = "")
        : errorNum_(errorNum), model_(model), name_(name) {
      if (name_ == "") {
#ifndef NDEBUG
    ...
    /** Return error number. */
    int errorNum() const { return errorNum_; }
    /** destructor. */
  ...
  virtual ReturnStatus OptimizeTNLP(
      const Ipopt::SmartPtr<Ipopt::TNLP> &tnlp) = 0;

  /// Resolves a problem expresses as a TNLP
  virtual ReturnStatus ReOptimizeTNLP(
      const Ipopt::SmartPtr<Ipopt::TNLP> &tnlp) = 0;
  ...
  virtual CoinWarmStart *getUsedWarmStart(
      Ipopt::SmartPtr<TMINLP2TNLP> tnlp) const = 0;
  ...
  /// Get the prefix
  const char *prefix() { return prefix_.c_str(); }
  ...
  bool isError(ReturnStatus &r) { return r < 0; }
