...
class HotInfo : public OsiHotInfo {
 public:
  ...
  HotInfo(OsiSolverInterface* solver, const OsiBranchingInformation* info,
  ...
  /// up infeasibility
  double upInfeasibility() const { return infeasibilities_[1]; }

  /// down infeasibility
  double downInfeasibility() const { return infeasibilities_[0]; }
...
/** This class chooses a variable to branch on
    ...
    from OsiChooseVariable). This class implements a simple strong branching
   algorithm where the changes in the objective value induced by branching on a
   specific object are estimated with the pure virtual function fill_changes.
*/
...
class BonChooseVariable : public OsiChooseVariable {
  ...
  */
  ...
  enum CandidateSortCriterion { DecrPs = 0, IncrPs, DecrInfeas, IncrInfeas };
#endif
 ...
 public:
  ...
  enum chooseVariableReturnStatuses {
    ...
    canFixAndStrongBranch /** Found variable to fix and also has remaining
                             candidate for strong branching.*/
    ,
    canFixAndBranch /** Found variable to fix and also has a (non-strong)
                       branching candidate.*/
    ,
    canFixNoCandidate /** Can fix variables but does not have strong branching
                         candidates.*/
  };
  ...
  double computeUsefulness(const double MAXMIN_CRITERION, const double upMult,
                           const double dowMult, const double value,
  ...
  */
  ...
  void setCbcModel(CbcModel* cbc_model) { cbc_model_ = cbc_model; }
...
#endif
  /** Always strong branch that many first candidate in the list regardless of
   * numberTrusted.*/
  int minNumberStrongBranch_;
  /** Stores the pseudo costs. */
  OsiPseudoCosts pseudoCosts_;
  /** Wether or not to trust strong branching results for updating pseudo
   * costs.*/
