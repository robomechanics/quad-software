...
class ClpModel {
 public:
  ...
  /** Copy constructor. May scale depending on mode
         -1 leave mode as is
         0 -off, 1 equilibrium, 2 geometric, 3, auto, 4
     auto-but-as-initialSolve-in-bab
     */
  ...
  /** Subproblem constructor.  A subset of whole model is created from the
     ...
     */
  ClpModel(const ClpModel *wholeModel, int numberRows, const int *whichRows,
 ...
 public:
  ...
  void copyRowNames(const std::vector<std::string> &rowNames, int first,
                    int last);
  /// Copies in Column names - modifies names first .. last-1
  void copyColumnNames(const std::vector<std::string> &columnNames, int first,
                       int last);
  /// Copies in Row names - modifies names first .. last-1
  ...
  /** Write the problem in MPS format to the specified file.
     ...
     */
  int writeMps(const char *filename, int formatType = 0, int numberAcross = 2,
  ...
  inline int getNumRows() const { return numberRows_; }
  /// Number of columns
  inline int getNumCols() const { return numberColumns_; }
  ...
  inline int solveType() const { return solveType_; }
  inline void setSolveType(int type) { solveType_ = type; }
  ...
  /** Secondary status of problem - may get extended
     ...
     and dual infeasibilities 5 - giving up in primal with flagged variables 6 -
     failed due to empty problem check 7 - postSolve says not optimal 8 - failed
     due to bad element check 9 - status was 3 and stopped on time 10 - status
     was 3 but stopped as primal feasible 11 - status was 1/2 from presolve
     found infeasible or unbounded 100 up - translation of enum from
     ClpEventHandler
     */
  ...
  /** Set a single column lower and upper bound */
  inline void setColBounds(int elementIndex, double lower, double upper) {
  ...
  inline double objectiveScale() const { return objectiveScale_; }
  inline void setObjectiveScale(double value) { objectiveScale_ = value; }
  /// Scaling of rhs and bounds
  inline double rhsScale() const { return rhsScale_; }
  inline void setRhsScale(double value) { rhsScale_ = value; }
  /// Sets or unsets scaling, 0 -off, 1 equilibrium, 2 geometric, 3 auto, 4
  /// auto-but-as-initialSolve-in-bab
  void scaling(int mode = 1);
  /** If we constructed a "really" scaled model then this reverses the
     operation. Quantities may not be exactly as they were before due to
     rounding errors */
  void unscale();
  /// Gets scalingFlag
  ...
  inline double *objective(const double *solution, double &offset,
  ...
  inline const double *getRowObjCoefficients() const { return rowObjective_; }
  /// Column Lower
  ...
  /** Small element value - elements less than this set to zero,
        default is 1.0e-20 */
  inline double getSmallElementValue() const { return smallElement_; }
  inline void setSmallElementValue(double value) { smallElement_ = value; }
  /// Row Matrix
  ...
  inline void setObjectiveValue(double value) {
    objectiveValue_ =
        (value + dblParam_[ClpObjOffset]) / optimizationDirection_;
  }
  ...
  inline double *ray() const { return ray_; }
  /// just test if infeasibility or unbounded Ray exists
  inline bool rayExists() const { return (ray_ != NULL); }
  /// just delete ray if exists
  inline void deleteRay() {
    delete[] ray_;
    ray_ = NULL;
  }
  /// Access internal ray storage. Users should call infeasibilityRay() or
  /// unboundedRay() instead.
  inline const double *internalRay() const { return ray_; }
  /// See if status (i.e. basis) array exists (partly for OsiClp)
  inline bool statusExists() const { return (status_ != NULL); }
  /// Return address of status (i.e. basis) array
  /// (char[numberRows+numberColumns])
  inline unsigned char *statusArray() const { return status_; }
  ...
  inline void setUserPointer(void *pointer) { userPointer_ = pointer; }
  inline void *getUserPointer() const { return userPointer_; }
  ...
  inline int whatsChanged() const { return whatsChanged_; }
  inline void setWhatsChanged(int value) { whatsChanged_ = value; }
  /// Number of threads (not really being used)
  inline int numberThreads() const { return numberThreads_; }
  ...
  /// Event handler
  inline ClpEventHandler *eventHandler() const { return eventHandler_; }
  ...
  /// Row names
  inline const std::vector<std::string> *rowNames() const { return &rowNames_; }
  inline const std::string &rowName(int iRow) const { return rowNames_[iRow]; }
...
#endif
  /// Objective methods
  inline ClpObjective *objectiveAsObject() const { return objective_; }
  ...
  int emptyProblem(int *infeasNumber = NULL, double *infeasSum = NULL,
  ...
  /**@name Parameter set/get methods
     ...
     implemented for the solver or simply the value of the parameter is out of
     the range the solver accepts. If a parameter setting call returns false
     check the details of your solver.
  ...
  /** For advanced options
         ...
         64 -Treat problem as feasible until last minute (i.e. minimize
     infeasibilities) 128 - Switch off all matrix sanity checks 256 - No row
     copy 512 - If not in values pass, solution guaranteed, skip as much as
     possible 1024 - In branch and bound 2048 - Don't bother to re-factorize if
     < 20 iterations 4096 - Skip some optimality checks 8192 - Do Primal when
     cleaning up primal 16384 - In fast dual (so we can switch off things) 32768
     - called from Osi 65536 - keep arrays around as much as possible (also use
     maximumR/C) 131072 - transposeTimes is -1.0 and can skip basic and fixed
         ...
         NOTE - many applications can call Clp but there may be some short cuts
                which are taken which are not guaranteed safe from all
     applications. Vetted applications will have a bit set and the code may test
     this At present I expect a few such applications - if too many I will have
     to re-think.  It is up to application owner to change the code if she/he
     needs these short cuts.  I will not debug unless in Coin repository.  See
     COIN_CLP_VETTED comments. 0x01000000 is Cbc (and in branch and bound)
         0x02000000 is in a different branch and bound
     */
  inline unsigned int specialOptions() const { return specialOptions_; }
...
#define COIN_CBC_USING_CLP 0x01000000
 ...
 protected:
  ...
  void gutsOfLoadModel(int numberRows, int numberColumns, const double *collb,
  ...
  inline bool permanentArrays() const { return (specialOptions_ & 65536) != 0; }
