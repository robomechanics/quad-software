...
/** ReferencedObject class.
     ...
     *
     * class MyClass : public Coin::ReferencedObject // must derive from
   ReferencedObject
     *    {
     *      ...
     *    }
...
#define dbg_smartptr_verbosity 0
  ...
  SmartPtr(T *ptr) : ptr_(NULL) { (void)SetFromRawPtr_(ptr); }

  /** Destructor, automatically decrements the reference count, deletes
   * the object if necessary.*/
  ~SmartPtr() { ReleasePointer_(); }
  //@}
  ...
  SmartPtr<T> &operator=(T *rhs) { return SetFromRawPtr_(rhs); }
