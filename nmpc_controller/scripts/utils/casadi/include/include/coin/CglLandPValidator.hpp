...
//[5] = {"Accepted", "violation too small", "small coefficient too small", "big
//dynamic","too dense"}
...
class Validator {
 public:
  ...
  Validator(double maxFillIn = 1., double maxRatio = 1e8,
            double minViolation = 0, bool scale = false, double rhsScale = 1);

  /** Clean an OsiCut */
  int cleanCut(OsiRowCut& aCut, const double* solCut,
               const OsiSolverInterface& si, const CglParam& par,
               const double* colLower, const double* colUpper);
  /** Clean an OsiCut by another method */
  int cleanCut2(OsiRowCut& aCut, const double* solCut,
                const OsiSolverInterface& si, const CglParam& par,
                const double* colLower, const double* colUpper);
  /** Call the cut cleaner */
  int operator()(OsiRowCut& aCut, const double* solCut,
                 const OsiSolverInterface& si, const CglParam& par,
                 const double* colLower, const double* colUpper) {
  ...
  void setRhsScale(double v) { rhsScale_ = v; }
  /** @} */
  /** @name get functions */
  /** @{ */
  ...
  const char* failureString(int code) const { return rejections_[code]; }
 ...
 private:
  /** max percentage of given formulation fillIn should be accepted for cut
   * fillin.*/
  double maxFillIn_;
