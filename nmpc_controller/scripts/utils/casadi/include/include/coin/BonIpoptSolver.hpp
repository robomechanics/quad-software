...
class IpoptSolver : public TNLPSolver {
 public:
  class UnsolvedIpoptError : public TNLPSolver::UnsolvedError {
   public:
    UnsolvedIpoptError(int errorNum, Ipopt::SmartPtr<TMINLP2TNLP> problem,
                       std::string name)
  ...
  virtual TNLPSolver::ReturnStatus OptimizeTNLP(
      const Ipopt::SmartPtr<Ipopt::TNLP>& tnlp);

  /// Resolves a problem expresses as a TNLP
  virtual TNLPSolver::ReturnStatus ReOptimizeTNLP(
      const Ipopt::SmartPtr<Ipopt::TNLP>& tnlp);
  ...
  virtual CoinWarmStart* getUsedWarmStart(
      Ipopt::SmartPtr<TMINLP2TNLP> tnlp) const;

  /// Get the warm start form the solver
  virtual CoinWarmStart* getWarmStart(
      Ipopt::SmartPtr<Bonmin::TMINLP2TNLP> tnlp) const;
  ...
  /// Get the solver name
  virtual std::string& solverName() { return solverName_; }
  ...
  Ipopt::IpoptApplication& getIpoptApp() { return *app_; }

  virtual int errorCode() const { return (int)optimizationStatus_; }
 ...
 private:
  ...
  TNLPSolver::ReturnStatus solverReturnStatus(
      Ipopt::ApplicationReturnStatus optimization_status) const;
  ...
  /** Flag to indicate if last problem solved had 0 dimension. (in this case
   * Ipopt was not called).*/
