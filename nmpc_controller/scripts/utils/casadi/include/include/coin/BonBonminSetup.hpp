...
enum Algorithm {
  ...
  B_Ecp = 4 /** Bonmin's implemantation of ecp cuts based branch-and-cut a la
               FilMINT.*/
  ,
  B_IFP =
      5 /** Bonmin's implemantation of iterated feasibility pump for MINLP.*/
};
...
class BonminSetup : public BabSetupBase {
 public:
  ...
  virtual BabSetupBase *clone() const { return new BonminSetup(*this); }
  ...
  virtual ~BonminSetup() {}
  /** @name Methods to instantiate: Registering and retrieving options and
  ...
  /** Initialize, read options and create appropriate bonmin setup.*/
  void initialize(const OsiTMINLPInterface &nlpSi,
                  bool createContinuousSolver = true);
  /** Get the algorithm used.*/
  Bonmin::Algorithm getAlgorithm();

  void addCutGenerator(CuttingMethod &cg) { BabSetupBase::addCutGenerator(cg); }

 protected:
  /** Register standard MILP cut generators. */
  static void registerMilpCutGenerators(
      Ipopt::SmartPtr<Bonmin::RegisteredOptions> roptions);
