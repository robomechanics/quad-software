...
class CoinError {
 ...
 private:
  CoinError() : message_(), method_(), class_(), file_(), lineNumber_() {
    // nothing to do here
  }
 ...
 public:
  ...
  inline const std::string &methodName() const { return method_; }
  /// get name of class instantiating error (or hint for assert)
  inline const std::string &className() const { return class_; }
  /// get name of file for assert
  inline const std::string &fileName() const { return file_; }
  ...
  inline void print(bool doPrint = true) const {
    ...
    if (lineNumber_ < 0) {
      ...
      if (class_ != "") std::cout << "Possible reason: " << class_ << std::endl;
    }
  }
  //@}
  ...
  CoinError(std::string message__, std::string methodName__,
            std::string className__, std::string fileName_ = std::string(),
            int line = -1)
      : message_(message__),
...
#define CoinAssertDebug(expression)                                            \
  {                                                                            \
    if (!(expression)) {                                                       \
      throw CoinError(__STRING(expression), __PRETTY_FUNCTION__, "", __FILE__, \
                      __LINE__);                                               \
    }                                                                          \
  }
#define CoinAssertDebugHint(expression, hint)                          \
