...
namespace Bonmin {
/** Generate cuts for the nlp corresponding to continuous relaxation at a
 * node.*/
class OaNlpOptim : public CglCutGenerator {
 public:
  /// Default constructor
  OaNlpOptim(OsiTMINLPInterface *si = NULL, int maxDepth = 10,
             bool addOnlyViolated = false, bool globalCuts = true);
  ...
  virtual CglCutGenerator *clone() const { return new OaNlpOptim(*this); }
  ...
  inline void setMaxDepth(int value) { maxDepth_ = value; }
  inline void setAddOnlyViolated(bool yesno) { addOnlyViolated_ = yesno; }
  inline void setGlobalCuts(bool yesno) { global_ = yesno; }
  inline int getNSolve() { return nSolve_; }
  /**set log level */
