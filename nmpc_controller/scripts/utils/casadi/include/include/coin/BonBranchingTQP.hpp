...
class BranchingTQP : public Ipopt::TNLP {
 public:
  ...
  //@{
  const Ipopt::Number ObjVal() { return obj_val_; }
  const Ipopt::Number* ObjGrad() { return obj_grad_; }
  const Ipopt::Number* ObjHessVals() { return obj_hess_; }
  const Ipopt::Index* ObjHessIRow() { return obj_hess_irow_; }
  const Ipopt::Index* ObjHessJCol() { return obj_hess_jcol_; }
  const Ipopt::Number* ConstrRhs() { return g_vals_; }
  const Ipopt::Number* ConstrJacVals() { return g_jac_; }
  const Ipopt::Index* ConstrJacIRow() { return g_jac_irow_; }
  const Ipopt::Index* ConstrJacJCol() { return g_jac_jcol_; }
  //@}
