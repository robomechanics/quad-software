...
class AuxInfo : public OsiBabSolver {
 public:
  ...
  void setFeasibleNode() { infeasibleNode_ = false; }

  /** Declare the node to be infeasible.*/
  void setInfeasibleNode() { infeasibleNode_ = true; }

  /** Say if current node is found feasible by cut generators.*/
  bool infeasibleNode() { return infeasibleNode_; }
  ...
  void setHasNlpSolution(bool b) { hasNlpSolution_ = b; }
  ...
  /** return objective value of the best solution computed with alternative
      objective function.*/
  double bestObj2() const { return (*bestObj2_)(); }
  /** Set an alternate objective value.*/
  void setBestObj2(double o) { (*bestObj2_)() = o; }
