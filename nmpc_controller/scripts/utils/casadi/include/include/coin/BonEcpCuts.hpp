...
class EcpCuts : public OaDecompositionBase {
 public:
  ...
  /// clone
  CglCutGenerator *clone() const { return new EcpCuts(*this); }
  ...
  double doEcpRounds(OsiSolverInterface &si, bool leaveSiUnchanged,
                     double *violation = NULL);

  void setNumRounds(int value) { numRounds_ = value; }

  void setPropabilityFactor(double value) { beta_ = value; }

  void setAbsViolationTolerance(double value) { abs_violation_tol_ = value; }
  void setRelViolationTolerance(double value) { rel_violation_tol_ = value; }
 ...
 protected:
  ...
  virtual bool doLocalSearch(BabInfo *babInfo) const { return 0; }
