...
class CglDuplicateRow : public CglCutGenerator {
 public:
  ...
  /** Fix variables and find duplicate/dominated rows for the model of the
      ...
      This is a very simple minded idea but I (JJF) am using it in a project so
     thought I might as well add it.  It should really be called before first
     solve and I may modify CBC to allow for that.
     ...
     to their lower bounds (effective rhs just means original with variables
     with nonzero lower bounds subtracted out).
  ...
  */
  ...
  //@{
  /// Get duplicate row list, -1 means still in, -2 means out (all fixed), k>=
  /// means same as row k
  ...
  /**@name We only check for dominated amongst groups of columns whose size <=
   * this */
  //@{
  /// Get
