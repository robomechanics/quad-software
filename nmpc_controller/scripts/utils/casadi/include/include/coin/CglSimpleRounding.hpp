...
/** Simple Rounding Cut Generator Class
 ...
 (Reference: Nemhauser and Wolsey, Integer and Combinatorial Optimization, 1988,
 pg 211.)
*/
...
class CglSimpleRounding : public CglCutGenerator {
 ...
 public:
  /**@name Generate Cuts */
  //@{
  /** Generate simple rounding cuts for the model accessed through the solver
  interface. Insert generated cuts into the cut set cs.
  */
 ...
 private:
  ...
  /// Derive a <= inequality in integer variables from the rowIndex-th
  /// constraint
  bool deriveAnIntegerRow(const OsiSolverInterface& si, int rowIndex,
                          const CoinShallowPackedVector& matrixRow,
                          CoinPackedVector& irow, double& b,
                          bool* negative) const;
