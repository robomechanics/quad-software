...
class CutStrengthener : public Ipopt::ReferencedObject {
  ...
  class StrengtheningTNLP : public Ipopt::TNLP {
   public:
    /** Contructor */
    StrengtheningTNLP(
        Ipopt::SmartPtr<TMINLP> tminlp, const CoinPackedVector& cut,
        bool lower_bound, Ipopt::Index n, const Ipopt::Number* starting_point,
        const double* x_l_orig, const double* x_u_orig,
 ...
 public:
  ...
  bool ComputeCuts(OsiCuts& cs, TMINLP* tminlp, TMINLP2TNLP* problem,
                   const int gindex, CoinPackedVector& cut, double& cut_lb,
                   double& cut_ub, const double g_val, const double g_lb,
                   const double g_ub, int n, const double* x, double infty);
 ...
 private:
  ...
  bool StrengthenCut(
      Ipopt::SmartPtr<TMINLP> tminlp /** current TMINLP */,
      int constr_index /** Ipopt::Index number of the constraint to be
                          strengthened, -1 means objective function */
      ,
  ...
  bool HandleOneCut(bool is_tight, TMINLP* tminlp, TMINLP2TNLP* problem,
                    const double* minlp_lb, const double* minlp_ub,
                    const int gindex, CoinPackedVector& cut, double& cut_lb,
                    double& cut_ub, int n, const double* x, double infty);
