...
class Bab {
 public:
  ...
  enum MipStatuses {
    FeasibleOptimal /** Optimum solution has been found and its optimality
                       proved.*/
    ,
  ...
  /**operator() performs the branchAndBound*/
  virtual void operator()(BabSetupBase* s) { operator()(*s); }

  /** get the best solution known to the problem (is optimal if MipStatus is
    FeasibleOptimal). if no solution is known returns NULL.*/
  const double* bestSolution() const { return bestSolution_; }
  /** return objective value of the bestSolution */
  double bestObj() const { return bestObj_; }

  /** return Mip Status */
  MipStatuses mipStatus() const { return mipStatus_; }
  ...
  int iterationCount() { return mipIterationCount_; }
  /** returns the value of the continuous relaxation. */
  double continuousRelaxation() { return continuousRelaxation_; }

  /** virtual callback function to eventually modify objects for integer
    variable (replace with user set). This is called after
    CbcModel::findIntegers */
  virtual void replaceIntegers(OsiObject** objects, int numberObjects) {}
  /** Get cbc model used to solve. */
  const CbcModel& model() const { return model_; }

  /** Get cbc model used to solve as non-const, in case we want to
      change options before things happen */
  CbcModel& model() { return model_; }
 ...
 protected:
   ...
   * this is not null if and only if there are some non-simple-integer branching
   * objects such as SOS constraints. It is up to Bab to pass them over to
   * appropriate components of the algorithm. */
  OsiObject** objects_;
  /** number of objects.*/
  int nObjects_;
};
