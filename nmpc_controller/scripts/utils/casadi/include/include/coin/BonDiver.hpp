...
namespace Bonmin {
class BabSetupBase;
/** Class to do diving in the tree. Principle is that branch-and-bound follows
   current branch of the tree untill it hits the bottom at which point it goes
   to the best candidate (according to CbcCompare) on the heap.*/
class CbcDiver : public CbcTree {
  ...
  */
  ...
  virtual void endSearch() { nextOnBranch_ = NULL; }
 ...
 * Principle is that branch-and-bound follows current branch of the tree by
 * exploring the two children at each level and continuing the dive on the best
 * one of the two. Untill it hits the bottom at which point it goes to the best
 * candidate (according to CbcCompare) on the heap.*/
...
/** A more elaborate diving class. First there are several modes which can be
   commanded by the Comparison class below. In particular can command to dive to
   find solutions, to try to close the bound as possible or to limit the size of
   the tree.

   The diving goes into the tree doing depth-first search until one of the
   following happens: \li A prescibed \c maxDiveBacktrack_ number of
   backtracking are performed. \li The guessed objective value of the current
   node is worst than the best incumbent. \li The depth of the dive is bigger
   than \c maxDiveDepth_

   In the first case all the nodes are put on the tree and the next node on top
   will be the top of the heap, in the two latter case we just put the node on
   the tree and backtrack in the list of depth-first search nodes.

   \bug This won't work in a non-convex problem where objective does not
   decrease down branches.
 */
class CbcDfsDiver : public CbcTree {
 public:
  enum ComparisonModes {
    Enlarge /** At the very beginning we might want to enlarge the tree just a
               bit*/
    ,
    FindSolutions,
    CloseBound,
  ...
  virtual bool empty();
  /** Give size of the tree.*/
  virtual int size() { return static_cast<int>(nodes_.size()) + diveListSize_; }
  /** @} */
  ...
  */
  ...
  /** Changes the mode of comparison of the tree for "safety reasons" if the
     mode really changes we always finish the current dive and put all the node
     back onto the heap.*/
  void setComparisonMode(ComparisonModes newMode);
  /** get the mode of comparison of the tree.*/
  ComparisonModes getComparisonMode() { return mode_; }
 ...
 protected:
  ...
  int diveListSize_;
  /** Depth of the node from which diving was started (we call this node the
   * diving board).*/
...
class DiverCompare : public CbcCompareBase {
 public:
  ...
  /// Clone
  virtual CbcCompareBase *clone() const { return new DiverCompare(*this); }
  ...
  void setDiver(CbcDfsDiver *diver) { diver_ = diver; }

  /** Set numberSolToStopDive_ */
  void setNumberSolToStopDive(int val) { numberSolToStopDive_ = val; }
