// (C) Copyright International Business Machines Corporation, Carnegie Mellon
// University 2004, 2007 All Rights Reserved. This code is published under the
...
enum Solver {
  EIpopt = 0 /** <a href="http://projects.coin-or.org/Ipopt"> Ipopt </a>
                interior point algorithm.*/
  ,
  EFilterSQP /** <a href="http://www-unix.mcs.anl.gov/~leyffer/solvers.html">
                filterSQP </a> Sequential Quadratic Programming algorithm.*/
  ,
...
class OsiTMINLPInterface : public OsiSolverInterface {
 ...
 public:
   ...
   * Inherited from CoinError, we just want to have a different class to be able
   * to catch errors thrown by OsiTMINLPInterface.
   */
  class SimpleError : public CoinError {
   ...
   public:
    ...
    SimpleError(std::string message, std::string methodName,
                std::string f = std::string(), int l = -1)
        : CoinError(message, methodName, std::string("OsiTMINLPInterface"), f,
                    l) {}
  };
...
#define SimpleError(x, y) SimpleError((x), (y), __FILE__, __LINE__)
  ...
  TNLPSolver::UnsolvedError* newUnsolvedError(
      int num, Ipopt::SmartPtr<TMINLP2TNLP> problem, std::string name) {
  ...
  enum MessagesTypes {
    ...
    WARN_SUCCESS_RANDOM /** Subproblem not solve with warm start but solved with
                           random point*/
    ,
    ...
    ALTERNATE_OBJECTIVE /** Recomputed integer feasible with alternate objective
                           function*/
    ,
    WARN_RESOLVE_BEFORE_INITIAL_SOLVE /** resolve() has been called but there
                                              was no previous call to
                                         initialSolve().
                                         */
    ,
    ERROR_NO_TNLPSOLVER /** Trying to access non-existent TNLPSolver*/,
    WARNING_NON_CONVEX_OA /** Warn that there are equality or ranged constraints
                             and OA may works bad.*/
    ,
    SOLVER_DISAGREE_STATUS /** Different solver gives different status for
                              problem.*/
    ,
    SOLVER_DISAGREE_VALUE /** Different solver gives different optimal value for
                             problem.*/
    ,
  ...
  void initialize(Ipopt::SmartPtr<Bonmin::RegisteredOptions> roptions,
                  ...
                  const std::string& prefix, Ipopt::SmartPtr<TMINLP> tminlp);
  ...
  const char* prefix() const {
    if (!IsValid(app_)) {
      messageHandler()->message(ERROR_NO_TNLPSOLVER, messages_)
          << CoinMessageEol;
      return NULL;
    } else
      return app_->prefix();
  ...
  */
  ...
  virtual void branchAndBound() {
    throw SimpleError("Function not implemented for OsiTMINLPInterface",
                      "branchAndBound()");
  }
  //@}
  ...
  void continuingOnAFailure() { hasContinuedAfterNlpFailure_ = true; }

  // Added by Claudia

  double getNewCutoffDecr() { return newCutoffDecr; }

  void setNewCutoffDecr(double d) { newCutoffDecr = d; }

  /// Did we continue on a failure
  bool hasContinuedOnAFailure() { return hasContinuedAfterNlpFailure_; }
  /// tell to ignore the failures (don't throw, don't fathom, don't report)
  void ignoreFailures() { pretendFailIsInfeasible_ = 2; }
  /// Force current solution to be infeasible
  void forceInfeasible() { problem_->set_obj_value(1e200); }
  /// Force current solution to be branched on (make it fractionnal with small
  /// objective)
  ...
  */
  ...
  inline double getPushFact() const { return pushValue_; }
  ...
  */
  ...
  int nCallOptimizeTNLP() { return nCallOptimizeTNLP_; }
  /** get total time taken to solve NLP's. */
  double totalNlpSolveTime() { return totalNlpSolveTime_; }
  /** get total number of iterations */
  int totalIterations() { return totalIterations_; }
  ...
  */
  ...
  /** Set the primal solution variable values
      ...
      \warning getColSolution will never return this vector (unless it is
     optimal).
  */
  ...
  */
  ...
  //---------------------------------------------------------------------------
  /**@name WarmStart related methods (those should really do nothing for the
   * moment)*/
  //@{
  ...
  */
  ...
  void setWarmStartMode(int mode) { warmStartMode_ = (WarmStartModes)mode; }
  WarmStartModes getWarmStartMode() { return warmStartMode_; }
  ...
  void setNumIterationSuspect(int value) { numIterationSuspect_ = value; }

  /**@name Dummy functions
   * Functions which have to be implemented in an OsiSolverInterface,
   * but which do not do anything (but throwing exceptions) here in the case of
   * a minlp solved using an nlp solver for continuous relaxations */
  //@{
  ...
  virtual void addRow(const CoinPackedVectorBase& vec, const char rowsen,
    ...
    throw SimpleError(
        "OsiTMINLPInterface model does not implement this function.", "addRow");
  }
  /** We have to keep this but it will throw an error.
   */
  virtual void deleteRows(const int num, const int* rowIndices) {
    if (num) freeCachedRowRim();
   ...
   */
  //@{
  void setSolverOutputToDefault() { app_->setOutputToDefault(); }
  void forceSolverOutput(int log_level) { app_->forceSolverOutput(log_level); }
  //@}
  ...
  const TMINLP2TNLP* problem() const { return GetRawPtr(problem_); }

  TMINLP2TNLP* problem() { return GetRawPtr(problem_); }

  const TMINLP* model() const { return GetRawPtr(tminlp_); }

  Bonmin::TMINLP* model() { return GetRawPtr(tminlp_); }

  const Bonmin::TNLPSolver* solver() const { return GetRawPtr(app_); }

  const std::list<Ipopt::SmartPtr<TNLPSolver> >& debug_apps() const {
    return debug_apps_;
  }

  TNLPSolver* solver() { return GetRawPtr(app_); }
  /** \name Methods to build outer approximations */
  //@{
  /** \name Methods to build outer approximations */
  //@{
  /** \brief Extract a linear relaxation of the MINLP.
   * Use user-provided point to build first-order outer-approximation
   * constraints at the optimum. And put it in an OsiSolverInterface.
   */
...
#if 1
  /** \brief Extract a linear relaxation of the MINLP.
   * Solve the continuous relaxation and takes first-order outer-approximation
   * constraints at the optimum. The put everything in an OsiSolverInterface.
   */
  virtual void extractLinearRelaxation(OsiSolverInterface& si, bool getObj = 1,
                                       bool solveNlp = 1) {
    if (solveNlp) initialSolve("build initial OA");
  ...
  void getOuterApproximation(OsiCuts& cs, int getObj, const double* x2,
                             bool global) {
  ...
  /** Get the outer approximation constraints at provided point.
      If x2 is different from NULL only add cuts violated by x2.
   (Only get outer-approximations of nonlinear constraints of the problem.)*/
  void getOuterApproximation(OsiCuts& cs, const double* x, int getObj,
                             const double* x2, bool global) {
    getOuterApproximation(cs, x, getObj, x2, 0., global);
  }

  /** Get the outer approximation constraints at provided point.
      If x2 is different from NULL only add cuts violated by x2 by more than
   delta. (Only get outer-approximations of nonlinear constraints of the
   problem.)*/
  virtual void getOuterApproximation(OsiCuts& cs, const double* x, int getObj,
                                     const double* x2, double theta,
                                     bool global);

  /** Get the outer approximation at provided point for given constraint. */
  virtual void getConstraintOuterApproximation(OsiCuts& cs,
  ...
  void getConstraintOuterApproximation(OsiCuts& cs, int constraintNumber,
                                       const double* x2, bool global) {
    getConstraintOuterApproximation(cs, constraintNumber, getColSolution(), x2,
                                    global);
  }
  ...
  /** Given a point x_bar this solves the problem of finding the point which
   *minimize a convex combination between the distance to  x_bar and the
   *original objective function f(x): \f$ min a * (\sum\limits_{i=1}^n
   *||x_{ind[i]} -\overline{x}_i)||_L) + (1 - a)* s *f(x) \f$ \return Distance
   *between feasibility set a x_bar on components in ind \param n number of
   *elements in array x_bar and ind \param s scaling of the original objective.
   * \param a Combination to take between feasibility and original objective
   *(must be between 0 and 1). \param L L-norm to use (can be either 1 or 2).
   */
  double solveFeasibilityProblem(size_t n, const double* x_bar, const int* ind,
                                 double a, double s, int L);

  /** Given a point x_bar this solves the problem of finding the point which
   * minimize the distance to x_bar while satisfying the additional cutoff
   * constraint: \f$ min \sum\limits_{i=1}^n  ||x_{ind[i]} -\overline{x}_i)||_L$
  ...
  /** Given a point x_bar setup feasibility problem and switch so that every
     call to initialSolve or resolve will solve it.*/
  void switchToFeasibilityProblem(size_t n, const double* x_bar, const int* ind,
                                  double a, double s, int L);

  /** Given a point x_bar setup feasibility problem and switch so that every
     call to initialSolve or resolve will solve it. This is to be used in the
  ...
  void round_and_check(double tolerance, OsiObject** objects = 0,
                       int nObjects = -1) {
  ...
  enum OaMessagesTypes {
    CUT_NOT_VIOLATED_ENOUGH = 0 /** Says that one cut has been generarted, where
                                   from, which is the violation.*/
    ,
  ...
  virtual void applyRowCuts(int numberCuts, const OsiRowCut** cuts) {
    if (numberCuts) freeCachedRowRim();
  ...
  virtual void setAppDefaultOptions(
      Ipopt::SmartPtr<Ipopt::OptionsList> Options);
  ...
  void SetStrongBrachingSolver(
      Ipopt::SmartPtr<StrongBranchingSolver> strong_branching_solver);
  ...
  void get_tolerances(double& tiny, double& very_tiny, double& rhsRelax,
                      double& infty) {
    tiny = tiny_;
    very_tiny = veryTiny_;
    rhsRelax = rhsRelax_;
 ...
 protected:
  ...
  //@{
  virtual std::string appName() { return "bonmin"; }
  //@}
  ...
  void freeCachedData();
  /** Extract rowsense_ vector rhs_ vector and rowrange_ vector from the lower
   * and upper bounds on the constraints */
  ...
  /// If infeasibility for a problem is less than this, let's be carrefull. It
  /// might be feasible
  ...
  /** If not 0 when a problem is not solved (failed to be solved)
      ...
      (i.e. record the fact issue messages to user), if ==2 don't care (somebody
     else will) */
  ...
  /** number iterations above which a problem is considered suspect (-1 is
     considered \f$+ \infty \f$). If in a call to solve a problem takes more
     than that number of iterations it will be output to files.*/
   ...
   */
  int nNonLinear_;
  /** Value for small non-zero element which we will try to remove cleanly in OA
   * cuts.*/
  double tiny_;
  /** Value for small non-zero element which we will take the risk to ignore in
   * OA cuts.*/
  double veryTiny_;
 ...
 protected:
  ...
  /** Procedure that builds a fake basis. Only tries to make basis consistent
   * with constraints activity.*/
 ...
 private:
  ...
  const char* statusAsString() { return statusAsString(optimizationStatus_); }
};
}  // namespace Bonmin
#endif
