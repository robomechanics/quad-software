 ...
 protected:
  CoinTreeNode()
      : depth_(-1),
        ...
        preferred_() {}
  CoinTreeNode(int d, int f = -1, double q = -COIN_DBL_MAX,
               double tlb = -COIN_DBL_MAX, BitVector128 p = BitVector128())
      : depth_(d),
        fractionality_(f),
        quality_(q),
...
#else
    if (x->currentNode()->getDepth() > y->currentNode()->getDepth()) return 1;
...
class CoinSearchTreeBase {
 ...
 public:
  ...
  inline CoinTreeNode *top() const {
    if (size_ == 0 || candidateList_.size() == 0) return NULL;
#ifdef DEBUG_PRINT
    char output[44];
    output[43] = 0;
...
class CoinSearchTree : public CoinSearchTreeBase {
 ...
 protected:
  virtual void realpop() { candidateList_.pop_back(); }
  virtual void fixTop() {
 ...
 protected:
  ...
  virtual void fixTop() {
    ...
    if (size > 1) {
      ...
      for (ch = 2; ch < size; pos = ch, ch *= 2) {
        if (comp_(candidates[ch + 1], candidates[ch])) ++ch;
        if (comp_(s, candidates[ch])) break;
  ...
  virtual void realpush(CoinTreeSiblings *s) {
    ...
    for (ch = pos / 2; ch != 0; pos = ch, ch /= 2) {
      if (comp_(candidates[ch], s)) break;
      candidates[pos] = candidates[ch];
    }
    candidates[pos] = s;
  }
...
class CoinSearchTreeManager {
 ...
 public:
  ...
  virtual ~CoinSearchTreeManager() { delete candidates_; }

  inline void setTree(CoinSearchTreeBase *t) {
    delete candidates_;
    candidates_ = t;
  }
  inline CoinSearchTreeBase *getTree() const { return candidates_; }
  ...
  inline double bestQuality() const { return candidates_->top()->getQuality(); }
