...
class TMINLP : public Ipopt::ReferencedObject {
 public:
  ...
  struct SosInfo {
    ...
    int numNz;
    /** For 0 <= i < nums, start[i] gives the indice of indices and weights
     * arrays at which the description of constraints i begins..*/
    int* starts;
  ...
  struct BranchingInfo {
    ...
    ~BranchingInfo() { gutsOfDestructor(); }
  };
  ...
  class PerturbInfo {
   public:
    /** default constructor. */
    PerturbInfo() : perturb_radius_(NULL) {}

    /** destructor*/
    ~PerturbInfo() { delete[] perturb_radius_; }

    /** Method for setting the perturbation radii. */
    void SetPerturbationArray(Ipopt::Index numvars,
                              const double* perturb_radius);

    /** Method for getting the array for the perturbation radii in
     *  order to use the values. */
    const double* GetPerturbationArray() const { return perturb_radius_; }
  ...
  virtual bool get_nlp_info(Ipopt::Index& n, Ipopt::Index& m,
                            ...
                            Ipopt::TNLP::IndexStyleEnum& index_style) = 0;
  ...
  virtual bool get_variables_linearity(
      Ipopt::Index n, Ipopt::TNLP::LinearityType* var_types) = 0;
  ...
  virtual bool get_constraints_linearity(
      Ipopt::Index m, Ipopt::TNLP::LinearityType* const_types) = 0;
   ...
   *  1e19.
   *  An exception will be thrown if x_l and x_u are not 0,1 for binary
   * variables
   */
  virtual bool get_bounds_info(Ipopt::Index n, Ipopt::Number* x_l,
                               Ipopt::Number* x_u, Ipopt::Index m,
                               Ipopt::Number* g_l, Ipopt::Number* g_u) = 0;
  ...
  virtual bool get_starting_point(Ipopt::Index n, bool init_x, Ipopt::Number* x,
                                  ...
                                  bool init_lambda, Ipopt::Number* lambda) = 0;
  ...
  virtual void finalize_solution(TMINLP::SolverReturn status, Ipopt::Index n,
  ...
  virtual const PerturbInfo* perturbInfo() const { return NULL; }

  /** Say if has a specific function to compute upper bounds*/
  virtual bool hasUpperBoundingObjective() { return false; }

  /** overload this method to return the value of an alternative objective
    function for upper bounding (to use it hasUpperBoundingObjective should
    return true).*/
  ...
  struct MarkedNonConvex {
    ...
    MarkedNonConvex() : cIdx(-1), cRelaxIdx(-1) {}
  ...
  struct SimpleConcaveConstraint {
    /** Default constructor gives "safe" values.*/
    SimpleConcaveConstraint() : xIdx(-1), yIdx(-1), cIdx(-1) {}
  ...
  virtual bool get_constraint_convexities(
      int m, TMINLP::Convexity* constraints_convexities) const {
    CoinFillN(constraints_convexities, m, TMINLP::Convex);
    return true;
  }
  /** Get dimension information on nonconvex constraints.*/
  virtual bool get_number_nonconvex(int& number_non_conv,
  ...
  virtual bool get_constraint_convexities(int number_non_conv,
                                          MarkedNonConvex* non_convs) const {
    assert(number_non_conv == 0);
    return true;
  }
  /** Fill array containing indices of simple concave constraints.*/
  virtual bool get_simple_concave_constraints(
      int number_concave, SimpleConcaveConstraint* simple_concave) const {
   ...
   * applied.*/
  virtual const int* get_const_xtra_id() const { return NULL; }
