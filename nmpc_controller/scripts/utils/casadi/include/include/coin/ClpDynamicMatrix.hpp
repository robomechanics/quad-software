...
/** This implements  a dynamic matrix when we have a limit on the number of
    "interesting rows". This version inherits from ClpPackedMatrix and knows
   that the real matrix is gub.  A later version could use shortest path to
   generate columns.
...
class ClpDynamicMatrix : public ClpPackedMatrix {
 public:
  ...
  /** Returns effective RHS offset if it is being used.  This is used for long
     problems or big dynamic or anywhere where going through full columns is
  ...
  /** Creates a variable.  This is called after partial pricing and will modify
     matrix. Will update bestSequence.
     */
  ...
  void initialProblem();
  /** Adds in a column to gub structure (called from descendant) and returns
   * sequence */
  int addColumn(CoinBigIndex numberEntries, const int *row,
                const double *element, double cost, double lower, double upper,
                int iSet, DynamicStatus status);
  /** If addColumn forces compression then this allows descendant to know what
     to do. If >=0 then entry stayed in, if -1 then entry went out to lower
     bound.of zero. Entries at upper bound (really nonzero) never go out (at
     present).
     */
  ...
  /** This is the real constructor.
         ...
         This resizes model !!!!
         The contents of original matrix in model will be taken over and
     original matrix will be sanitized so can be deleted (to avoid a very small
     memory leak)
      */
  ClpDynamicMatrix(ClpSimplex *model, int numberSets, int numberColumns,
                   const int *starts, const double *lower, const double *upper,
  ...
  inline bool flaggedSlack(int i) const { return (status_[i] & 8) != 0; }
  ...
  inline int numberSets() const { return numberSets_; }
  /// Number of possible gub variables
  inline int numberGubEntries() const { return startSet_[numberSets_]; }
  /// Sets
  inline int *startSets() const { return startSet_; }
  /// Whether flagged
  ...
  inline int numberStaticRows() const { return numberStaticRows_; }
  /// size of working matrix (max)
  inline CoinBigIndex numberElements() const { return numberElements_; }
  inline int *keyVariable() const { return keyVariable_; }
