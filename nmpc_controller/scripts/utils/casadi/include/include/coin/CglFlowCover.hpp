...
class CglFlowCover : public CglCutGenerator {
  ...
  */
  bool generateOneFlowCut(const OsiSolverInterface& si, const int rowLen,
                          int* ind, double* coef, char sense, double rhs,
                          OsiRowCut& flowCut, double& violation);
  ...
  CglFlowRowType determineOneRowType(const OsiSolverInterface& si, int rowLen,
                                     int* ind, double* coef, char sen,
                                     double rhs) const;
  /** Lift functions */
  void liftMinus(double& movement, /* Output */
                 int t, int r, double z, double dPrimePrime, double lambda,
                 double ml, double* M, double* rho) const;

  bool liftPlus(double& alpha, double& beta, int r, double m_j, double lambda,
                double y_j, double x_j, double dPrimePrime, double* M) const;

  //-------------------------------------------------------------------------
  //**@name Query and set the row type of a givne row. */
  //@{
  inline const CglFlowRowType* getRowTypes() const { return rowTypes_; }
  inline CglFlowRowType getRowType(const int i) const { return rowTypes_[i]; }
  ...
  inline void setRowTypes(const CglFlowRowType rt, const int i) {
    if (rowTypes_ != 0)
      ...
      std::cout << "ERROR: Should allocate memory for rowType_ before "
                << "using it " << std::endl;
      throw CoinError("Forgot to allocate memory for rowType_", "setRowType",
                      "CglFlowCover");
    }
  }
  //@}
  ...
  inline void setVubs(CglFlowVUB* vubs) {
    vubs_ = vubs;
    vubs = 0;
  }
  ...
  inline void setVlbs(CglFlowVLB* vlbs) {
    vlbs_ = vlbs;
    vlbs = 0;
  }
