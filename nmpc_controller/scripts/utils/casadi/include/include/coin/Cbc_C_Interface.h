 ...
 * @param nz number of rows (constraints) where this column appears, can be 0 if
 *constraints will be added later
 * @param rows index of rows where this column appears, NULL if rows will be
 *added later
 * @param coefs coefficients that this column appears in its rows, NULL if rows
 *will be added later
 ***/
COINLIBAPI void COINLINKAGE Cbc_addCol(Cbc_Model *model, const char *name,
                                       double lb, double ub, double obj,
                                       char isInteger, int nz, int *rows,
                                       double *coefs);
 ...
 *  @param sense constraint sense: L if <=, G if >=, E if =, R if ranged and N
 * if free
 *  @param rhs right hand size
 * */
COINLIBAPI void COINLINKAGE Cbc_addRow(Cbc_Model *model, const char *name,
                                       int nz, const int *cols,
                                       const double *coefs, char sense,
                                       double rhs);

/** @brief Add SOS constraints to the model using row-order matrix */
COINLIBAPI void COINLINKAGE Cbc_addSOS(Cbc_Model *model, int numRows,
                                       const int *rowStarts,
...
/** Loads a problem (the constraints on the
     ...
     <li> <code>value[k]</code> stores the coefficient of the kth nonzero
   element
     </ul>
    */
 ...
 * @param column name
 **/
COINLIBAPI void COINLINKAGE Cbc_setColName(Cbc_Model *model, int iColumn,
                                           const char *name);
 ...
 * @param name row name
 **/
COINLIBAPI void COINLINKAGE Cbc_setRowName(Cbc_Model *model, int iRow,
                                           const char *name);

/** @brief Sets optimization direction
 *
 * @param model problem object
 * @param sense: direction of optimization (1 - minimize, -1 - maximize, 0 -
 *ignore)
 **/
 ...
 **/
COINLIBAPI void COINLINKAGE Cbc_setRowLower(Cbc_Model *model, int index,
                                            double value);
 ...
 **/
COINLIBAPI void COINLINKAGE Cbc_setRowUpper(Cbc_Model *model, int index,
                                            double value);
 ...
 **/
COINLIBAPI void COINLINKAGE Cbc_setObjCoeff(Cbc_Model *model, int index,
                                            double value);
 ...
 **/
COINLIBAPI void COINLINKAGE Cbc_setColLower(Cbc_Model *model, int index,
                                            double value);
 ...
 **/
COINLIBAPI void COINLINKAGE Cbc_setColUpper(Cbc_Model *model, int index,
                                            double value);
 ...
 *
 **/
COINLIBAPI void COINLINKAGE Cbc_setMIPStart(Cbc_Model *model, int count,
                                            const char **colNames,
 ...
 *
 **/
COINLIBAPI void COINLINKAGE Cbc_setMIPStartI(Cbc_Model *model, int count,
                                             const int colIdxs[],
                                             const double colValues[]);
 ...
 **/
COINLIBAPI void COINLINKAGE Cbc_getRowName(Cbc_Model *model, int iRow,
                                           char *name, size_t maxLength);
 ...
 **/
COINLIBAPI void COINLINKAGE Cbc_getColName(Cbc_Model *model, int iColumn,
                                           char *name, size_t maxLength);
...
COINLIBAPI void COINLINKAGE Cbc_setParameter(Cbc_Model *model, const char *name,
                                             const char *value);
 ...
 *
 * @param model problem object
 * @return vector with the best solution found or NULL if no feasible solution
 *was found
 **/
 ...
 * 0 finished - check isProvenOptimal or isProvenInfeasible to see if solution
 * found (or check value of best solution) 1 stopped - on maxnodes, maxsols,
 * maxtime 2 execution abandoned due to numerical dificulties 5 user programmed
 * interruption
 *
 * @param model problem object
...
COINLIBAPI void COINLINKAGE Osi_getColName(void *osi, int i, char *name,
                                           int maxLen);
...
/** @brief Returns integrality information for columns in OsiSolverInterface
 * object */
...
COINLIBAPI void COINLINKAGE OsiCuts_addRowCut(void *osiCuts, int nz,
                                              const int *idx,
                                              const double *coef, char sense,
                                              double rhs);
