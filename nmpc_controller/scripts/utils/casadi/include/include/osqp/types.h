...
typedef struct {
  ...
  c_int *p;     ///< column pointers (size n+1); col indices (size nzmax) start
             ///< from 0 when using triplet format (direct KKT matrix formation)
...
#ifndef EMBEDDED
  c_int status_polish;  ///< polish status: successful (1), unperformed (0),
                        ///< (-1) unsuccessful
...
typedef struct {
  ...
  c_float adaptive_rho_tolerance;  ///< tolerance X for adapting rho. The new
                                   ///< rho has to be X times larger or 1/X
                                   ///< times smaller than the current one to
  ...
  c_int polish_refine_iter;  ///< number of iterative refinement steps in
                             ///< polishing
