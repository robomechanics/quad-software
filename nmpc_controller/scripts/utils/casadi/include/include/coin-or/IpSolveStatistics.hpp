...
class IPOPTLIB_EXPORT SolveStatistics : public ReferencedObject {
 public:
  ...
  /** @name Accessor methods for retrieving different kind of solver statistics
   * information */
  ///@{
  /** Iteration counts. */
   ...
   */
  IPOPT_DEPRECATED
  Number TotalCPUTime() const { return TotalCpuTime(); }
   ...
   */
  IPOPT_DEPRECATED
  virtual void Infeasibilities(Number& dual_inf, Number& constr_viol,
  ...
  virtual void ScaledInfeasibilities(
      Number& scaled_dual_inf,     ///< scaled dual infeasibility (Gradient of
                                   ///< Lagrangian not zero)
      Number& scaled_constr_viol,  ///< violation of scaled constraints
      Number& scaled_varbounds_viol,   ///< violation of scaled variable bounds
                                       ///< @since 3.14.0
