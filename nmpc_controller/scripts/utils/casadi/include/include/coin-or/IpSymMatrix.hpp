...
class IPOPTLIB_EXPORT SymMatrix : public Matrix {
 public:
   ...
   */
  inline SymMatrix(const SymMatrixSpace* owner_space);
...
class IPOPTLIB_EXPORT SymMatrixSpace : public MatrixSpace {
 public:
   ...
   *  rows and columns).
   */
  SymMatrixSpace(Index dim) : MatrixSpace(dim, dim) {}
  ...
  virtual Matrix* MakeNew() const { return MakeNewSymMatrix(); }
...
/* inline methods */
inline SymMatrix::SymMatrix(const SymMatrixSpace* owner_space)
    : Matrix(owner_space), owner_space_(owner_space) {}

inline Index SymMatrix::Dim() const { return owner_space_->Dim(); }
