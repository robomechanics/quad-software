 ...
 *  \f}
 *  and (invertible diagonal) scaling matrices \f$s_o\f$, \f$s_c\f$, \f$s_d\f$,
 * \f$s_x\f$, this class represents the %NLP \f{eqnarray*}
 *     \mathrm{min}  && s_o f(s_x^{-1} \tilde x), \\
 *     \mathrm{s.t.} && s_c c(s_x^{-1} \tilde x) = 0, &\qquad \tilde y_c \\
 *                   && s_d d_L \leq s_d d(s_x^{-1} \tilde x) \leq s_d d_U,
 * &\qquad \tilde y_d \\
 *                   && s_x x_L \leq \tilde x \leq s_x x_U, &\qquad \tilde z_L,
 * z_U \f} where \f$\tilde x\f$, \f$\tilde y_c\f$, \f$\tilde y_d\f$, \f$\tilde
 * z_L\f$, \f$\tilde z_U\f$, are the primal and dual variables of the scaled
 * problem (though, %Ipopt adds slack variables additionally).
 *
 *  The correspondence between a scaled and its corresponding unscaled solution
 * is \f{eqnarray*}
 *    x   && = s_x^{-1} \tilde x \\
...
class IPOPTLIB_EXPORT OrigIpoptNLP : public IpoptNLP {
 public:
  ...
  virtual bool InitializeStructures(SmartPtr<Vector>& x, bool init_x,
                                    SmartPtr<Vector>& y_c, bool init_y_c,
                                    SmartPtr<Vector>& y_d, bool init_y_d,
                                    SmartPtr<Vector>& z_L, bool init_z_L,
                                    SmartPtr<Vector>& z_U, bool init_z_U,
                                    SmartPtr<Vector>& v_L,
                                    SmartPtr<Vector>& v_U);
  ...
  virtual SmartPtr<const SymMatrix> h(const Vector& x, Number obj_factor,
  ...
  virtual SmartPtr<const Vector> x_L() const { return x_L_; }

  /** Permutation matrix (x_L_ -> x) */
  virtual SmartPtr<const Matrix> Px_L() const { return Px_L_; }
   ...
   * @since 3.14.0
   */
  virtual SmartPtr<const Vector> orig_x_L() const { return orig_x_L_; }

  /** Scaled upper bounds on x */
  virtual SmartPtr<const Vector> x_U() const { return x_U_; }

  /** Permutation matrix (x_U_ -> x) */
  virtual SmartPtr<const Matrix> Px_U() const { return Px_U_; }
   ...
   * @since 3.14.0
   */
  virtual SmartPtr<const Vector> orig_x_U() const { return orig_x_U_; }

  /** Scaled lower bounds on d */
  virtual SmartPtr<const Vector> d_L() const { return d_L_; }

  /** Permutation matrix (d_L_ -> d) */
  virtual SmartPtr<const Matrix> Pd_L() const { return Pd_L_; }
   ...
   * @since 3.14.10
   */
  virtual SmartPtr<const Vector> orig_d_L() const { return orig_d_L_; }

  /** Scaled upper bounds on d */
  virtual SmartPtr<const Vector> d_U() const { return d_U_; }

  /** Permutation matrix (d_U_ -> d) */
  virtual SmartPtr<const Matrix> Pd_U() const { return Pd_U_; }
   ...
   * @since 3.14.10
   */
  virtual SmartPtr<const Vector> orig_d_U() const { return orig_d_U_; }

  virtual SmartPtr<const SymMatrixSpace> HessianMatrixSpace() const {
    return h_space_;
  }

  virtual SmartPtr<const VectorSpace> x_space() const { return x_space_; }
  ///@}
  ...
  ///@{
  virtual Index f_evals() const { return f_evals_; }
  virtual Index grad_f_evals() const { return grad_f_evals_; }
  virtual Index c_evals() const { return c_evals_; }
  virtual Index jac_c_evals() const { return jac_c_evals_; }
  virtual Index d_evals() const { return d_evals_; }
  virtual Index jac_d_evals() const { return jac_d_evals_; }
  virtual Index h_evals() const { return h_evals_; }
  ///@}

  /** Solution Routines - overloaded from IpoptNLP */
  ///@{
  void FinalizeSolution(SolverReturn status, const Vector& x, const Vector& z_L,
                        const Vector& z_U, const Vector& c, const Vector& d,
                        const Vector& y_c, const Vector& y_d, Number obj_value,
                        const IpoptData* ip_data,
                        IpoptCalculatedQuantities* ip_cq);

  bool IntermediateCallBack(AlgorithmMode mode, Index iter, Number obj_value,
