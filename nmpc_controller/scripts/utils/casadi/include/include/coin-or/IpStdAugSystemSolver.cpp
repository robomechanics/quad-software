...
bool StdAugSystemSolver::InitializeImpl(const OptionsList& options,
  ...
  options.GetBoolValue("warm_start_same_structure", warm_start_same_structure_,
                       prefix);
  ...
  if (!warm_start_same_structure_) {
    ...
    ASSERT_EXCEPTION(
        IsValid(augmented_system_), INVALID_WARMSTART,
        "StdAugSystemSolver called with warm_start_same_structure, but "
        "augmented system is not initialized.");
  }

  return linsolver_->Initialize(Jnlst(), IpNLP(), IpData(), IpCq(), options,
                                prefix);
}

ESymSolverStatus StdAugSystemSolver::MultiSolve(
    const SymMatrix* W, Number W_factor, const Vector* D_x, Number delta_x,
    const Vector* D_s, Number delta_s, const Matrix* J_c, const Vector* D_c,
    Number delta_c, const Matrix* J_d, const Vector* D_d, Number delta_d,
    ...
    std::vector<SmartPtr<Vector> >& sol_dV, bool check_NegEVals,
    Index numberOfNegEVals) {
  DBG_START_METH("StdAugSystemSolver::MultiSolve", dbg_verbosity);
  DBG_ASSERT(J_c && J_d &&
             "Currently, you MUST specify J_c and J_d in the augmented system");
  ...
  if (!IsValid(augmented_system_)) {
    ...
    DBG_ASSERT(
        W && J_c &&
        J_d);  // W must exist during the first call to setup the structure!
    CreateAugmentedSpace(*W, *J_c, *J_d, *rhs_xV[0], *rhs_sV[0], *rhs_cV[0],
                         *rhs_dV[0]);
    CreateAugmentedSystem(W, W_factor, D_x, delta_x, D_s, delta_s, *J_c, D_c,
                          delta_c, *J_d, D_d, delta_d, *rhs_xV[0], *rhs_sV[0],
                          *rhs_cV[0], *rhs_dV[0]);
  ...
  retval =
      linsolver_->MultiSolve(*augmented_system_, augmented_rhsV, augmented_solV,
...
void StdAugSystemSolver::CreateAugmentedSpace(
    const SymMatrix& W, const Matrix& J_c, const Matrix& J_d,
    const Vector& proto_x, const Vector& proto_s, const Vector& proto_c,
  ...
  // (1,1) block
  // create the spaces and sum matrix for the upper left corner (W + D_x
  // delta_x*I) of the hessian part for the 1,1 block
  ...
  // (2,2) block
  // create the spaces and diag matrix for the lower right corner (D_s +
  // delta_s*I) of the hessian part, the 2,2 block
...
bool StdAugSystemSolver::AugmentedSystemRequiresChange(
    ...
    const Vector* D_s, Number delta_s, const Matrix& J_c, const Vector* D_c,
    Number delta_c, const Matrix& J_d, const Vector* D_d, Number delta_d) {
  DBG_START_METH("StdAugSystemSolver::AugmentedSystemRequiresChange",
                 dbg_verbosity);
  DBG_ASSERT(augsys_tag_ == augmented_system_->GetTag() &&
             "Someone has changed the augmented system outside of the "
             "AugSystemSolver. This should NOT happen.");
  ...
  if ((W && W->GetTag() != w_tag_) || (!W && w_tag_ != 0) ||
      (W_factor != w_factor_) || (D_x && D_x->GetTag() != d_x_tag_) ||
      (!D_x && d_x_tag_ != 0) || (delta_x != delta_x_) ||
      (D_s && D_s->GetTag() != d_s_tag_) || (!D_s && d_s_tag_ != 0) ||
      (delta_s != delta_s_) || (J_c.GetTag() != j_c_tag_) ||
      (D_c && D_c->GetTag() != d_c_tag_) || (!D_c && d_c_tag_ != 0) ||
      (delta_c != delta_c_) || (J_d.GetTag() != j_d_tag_) ||
      (D_d && D_d->GetTag() != d_d_tag_) || (!D_d && d_d_tag_ != 0) ||
      (delta_d != delta_d_)) {
    return true;
  }
