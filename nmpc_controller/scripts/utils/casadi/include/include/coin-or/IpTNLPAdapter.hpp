 ...
 * g_{U,i}\}\f$ be the indices of equality and inequality constraints,
 * respectively. The dual variables for the constraints are \f$\lambda\f$. The
 * dual variables for the variable bounds are \f$z_L\f$ and \f$z_U\f$.
 *
 ...
 *                   && x_L \leq  x \leq x_U,   &\qquad z_L, z_U
 *  \f}
 *  where \f$c(x) = g_E(x) - g_{L,E}\f$, i.e., corresponding to equality
 * constraints of TNLP, and \f$d(x) = g_I(x)\f$, \f$d_L = g_{L,I}\f$, \f$d_U =
 * g_{U,I}\f$, i.e., corresponding to inequality constraints of TNLP.
 *
 *  The dual variables for the constraints are \f$y_c\f$ and \f$y_d\f$.
 *  The dual variables for the bounds on slack and original variables are
 * \f$s_L\f$, \f$s_U\f$, \f$z_L\f$, \f$z_U\f$.
 *
 ...
 *  fixed_variable_treatment is set to make_parameter, then these variables are
 * not made visible to Ipopt internally. If fixed_variable_treatment is set to
 * make_constraint, then their bounds relaxed and equality constraints \f$x_i -
 * x_{L,i} = 0\f$ are added to the end of \f$c(x) = 0\f$.
 */
class IPOPTLIB_EXPORT TNLPAdapter : public NLP {
 public:
  ...
  virtual bool GetBoundsInformation(const Matrix& Px_L, Vector& x_L,
                                    const Matrix& Px_U, Vector& x_U,
                                    const Matrix& Pd_L, Vector& d_L,
                                    const Matrix& Pd_U, Vector& d_U);

  /** Method for obtaining the starting point for all the iterates. */
  virtual bool GetStartingPoint(SmartPtr<Vector> x, bool need_x,
                                SmartPtr<Vector> y_c, bool need_y_c,
                                SmartPtr<Vector> y_d, bool need_y_d,
                                SmartPtr<Vector> z_L, bool need_z_L,
  ...
  virtual bool Eval_h(const Vector& x, Number obj_factor, const Vector& yc,
                      const Vector& yd, SymMatrix& h);
  ...
  ///@{
  virtual void FinalizeSolution(SolverReturn status, const Vector& x,
  ...
  virtual bool IntermediateCallBack(AlgorithmMode mode, Index iter,
  ...
  virtual void GetQuasiNewtonApproximationSpaces(
      SmartPtr<VectorSpace>& approx_space, SmartPtr<Matrix>& P_approx);
  ...
  SmartPtr<TNLP> tnlp() const { return tnlp_; }
  ...
  void ResortX(const Vector& x, /**< internal values for primal variables x */
               Number* x_orig,  /**< vector to fill with values from x */
               bool usefixedvals =
                   true /**< whether to use stored variable fixings for fixed
                           variables (true), or zero (false) @since 3.14.0 */
  );
  ...
  void ResortG(
      ...
      bool correctrhs =
          false /**< whether to add unscaled rhs-values for constraints that
                   internally correspond to c(x)=0 @since 3.14.0 */
  );

  /** Provides values for lower and upper bounds on variables for given
   * Ipopt-internal vectors.
   *
   * Similar to ResortX, but does so for two arrays and does not set any values
   * for fixed variables.
   * @since 3.14.0
   */
  void ResortBounds(
  ...
  void ResortBnds(
      ...
      Number* x_U_orig,  /**< vector to fill with values from x_U */
      bool clearorig =
          true /**< whether to initialize complete x_L_orig and x_U_orig to 0.0
                  before setting values for non-fixed variables - ignored */
  ) {
    ResortBounds(x_L, x_L_orig, x_U, x_U_orig);
    (void)clearorig;
  }

  /** Provides values for dual multipliers on lower and upper bounds on
   * variables for given Ipopt-internal vectors.
   *
   * Similar to ResortBounds, but also provides dual values for fixed variables
   * if fixed_variable_treatment is set to make_constraint or make_parameter.
   *
   * @attention If there are fixed variables and fixed_variable_treatment is
   * make_parameter (the default), then the Gradient of f(x) and the Jacobian of
   * g(x) may be reevaluated here (that's why the function needs x). Further, in
   * this setting, this function only provides correct bound multipliers for
   * fixed variables if x, y_c, and y_d correspond to the unscaled problem.
   *
   * @return True, if bound multipliers could be assigned. False if there was an
   * evaluation error when calculating bound multipliers for fixed variables.
   * @since 3.14.0
   */
  ...
  void GetFixedVariables(
      ...
      Index*& x_fixed_map, /**< storage for pointer to array that holds indices
                              of fixed variables (has length n_fixed_x, can be
                              NULL if n_fixed_x=0) */
      FixedVariableTreatmentEnum&
          fixed_variable_treatment /**< treatment for fixed variables as used by
                                      TNLP */
  ) const {
  ...
  /** Get mappings between TNLP indices and Ipopt internal indices for variables
   * and constraints.
   *
   * See the various ResortXyz functions on usage.
   *
   * \attention P_x_full_x is set to NULL if there are no fixed variables or
   * fixed_variable_treatment is not make_parameter
   * @since 3.14.0
   */
  void GetPermutationMatrices(
      SmartPtr<const ExpansionMatrix>&
          P_x_full_x, /**< map from TNLP variable indices to Ipopt internal
                         indices, filtered variables get mapped to -1 */
      SmartPtr<const ExpansionMatrix>&
          P_x_x_L, /**< map from indices on lower bounds on x to Ipopt internal
                      indices for x */
      SmartPtr<const ExpansionMatrix>&
          P_x_x_U, /**< map from indices on upper bounds on x to Ipopt internal
                      indices for x */
      SmartPtr<const ExpansionMatrix>&
          P_c_g, /**< map from indices on equality constraints (c(x)=0) into
                    TNLP constraint indices (g_l <= g(x) <= g_u) */
      SmartPtr<const ExpansionMatrix>&
          P_d_g /**< map from indices on inequality constraints (d(x)-s=0) into
                   TNLP constraint indices (g_l <= g(x) <= g_u) */
  ) const {
   ...
   * @since 3.14.0
   */
  const Number* GetC_Rhs() const { return c_rhs_; }
 ...
 private:
  ...
  /** @name Method implementing the detection of linearly dependent equality
   * constraints */
  ...
  /** Pointer to the TNLP class (class specific to Number* vectors and triplet
   * matrices) */
  SmartPtr<TNLP> tnlp_;
  ...
  /** Object that can be used to detect linearly dependent rows in the equality
   * constraint Jacobian */
  ...
  /** Flag indicating if all test values should be printed, or only those
  ...
  /** Flag indicating whether the TNLP with identical structure has already been
   * solved before. */
  ...
  Index nz_jac_c_;
  /** non-zeros of the jacobian of c without added constraints for fixed
   * variables. */
