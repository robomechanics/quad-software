...
class IPOPTLIB_EXPORT ExpansionMatrix : public Matrix {
 ...
 protected:
  ...
  void PrintImplOffset(const Journalist& jnlst, EJournalLevel level,
                       ...
                       Index row_offset, Index col_offset) const;
...
class IPOPTLIB_EXPORT ExpansionMatrixSpace : public MatrixSpace {
 public:
   ...
   */
  ExpansionMatrixSpace(Index NLargeVec, Index NSmallVec, const Index* ExpPos,
                       const int offset = 0);
  ...
  virtual Matrix* MakeNew() const { return MakeNewExpansionMatrix(); }
   ...
   */
  const Index* ExpandedPosIndices() const { return expanded_pos_; }
   ...
   */
  const Index* CompressedPosIndices() const { return compressed_pos_; }
