...
#endif
IPOPTLIB_EXPORT int
Snprintf(char* str, long size, const char* format, ...);

/** Method to calculate new length for a memory increase based on a
 * recommendation and limits in integer type.
 *
 * Checks whether recommended can be represented by T.
 * If so, sets len to min of recommendation and min.
 * If not, sets len to maximal value available for T, if this is larger than
 * current value for len. If not, throws a std::overflow_error exception.
 *
 * @since 3.14.0
 */
template <typename T>
inline void ComputeMemIncrease(
    ...
    const char* context  ///< context from where this function is called - used
                         ///< to setup message for exception
) {
  if (recommended >= std::numeric_limits<T>::max()) {
    ...
    if (len < std::numeric_limits<T>::max()) {
      ...
      what << "Cannot allocate more than "
           << std::numeric_limits<T>::max() * sizeof(T) << " bytes for "
           << context << " due to limitation on integer type";
