...
class IPOPTLIB_EXPORT IpoptAdditionalData : public ReferencedObject {
 public:
   ...
   */
  virtual bool Initialize(const Journalist& jnlst, const OptionsList& options,
...
class IPOPTLIB_EXPORT IpoptData : public ReferencedObject {
 public:
  ...
  bool InitializeDataStructures(IpoptNLP& ip_nlp, bool want_x, bool want_y_c,
                                bool want_y_d, bool want_z_L, bool want_z_U);
   ...
   *  The parameters are taken from the OptionsList object.
   */
  bool Initialize(const Journalist& jnlst, const OptionsList& options,
   ...
   */
  inline void set_trial(SmartPtr<IteratesVector>& trial);
   ...
   */
  void SetTrialPrimalVariablesFromStep(Number alpha, const Vector& delta_x,
   ...
   *  alpha.
   */
  void SetTrialEqMultipliersFromStep(Number alpha, const Vector& delta_y_c,
   ...
   */
  inline void set_delta(SmartPtr<IteratesVector>& delta);
   ...
   */
  inline void set_delta(SmartPtr<const IteratesVector>& delta);
   ...
   * changed) */
  SmartPtr<const SymMatrix> W() { return W_; }

  /** Set Hessian approximation */
  void Set_W(SmartPtr<const SymMatrix> W) { W_ = W; }
   ...
   */
  bool HaveDeltas() const { return have_deltas_; }
   ...
   */
  void SetHaveDeltas(bool have_deltas) { have_deltas_ = have_deltas; }
  ///@}
   ...
   */
  bool HaveAffineDeltas() const { return have_affine_deltas_; }
  ...
  ///@{
  Index iter_count() const { return iter_count_; }
  void Set_iter_count(Index iter_count) { iter_count_ = iter_count; }
  ...
  bool MuInitialized() const { return mu_initialized_; }
  ...
  bool TauInitialized() const { return tau_initialized_; }

  void SetFreeMuMode(bool free_mu_mode) { free_mu_mode_ = free_mu_mode; }
  bool FreeMuMode() const { return free_mu_mode_; }

  /** Setting the flag that indicates if a tiny step (below machine
   *  precision) has been detected
   */
  void Set_tiny_step_flag(bool flag) { tiny_step_flag_ = flag; }
  bool tiny_step_flag() { return tiny_step_flag_; }
  ///@}
   ...
   */
  void Set_tol(Number tol) { tol_ = tol; }
  ///@}
  ...
  ///@{
  Number info_regu_x() const { return info_regu_x_; }
  void Set_info_regu_x(Number regu_x) { info_regu_x_ = regu_x; }
  Number info_alpha_primal() const { return info_alpha_primal_; }
  void Set_info_alpha_primal(Number alpha_primal) {
    info_alpha_primal_ = alpha_primal;
  }
  char info_alpha_primal_char() const { return info_alpha_primal_char_; }
  void Set_info_alpha_primal_char(char info_alpha_primal_char) {
    info_alpha_primal_char_ = info_alpha_primal_char;
  }
  Number info_alpha_dual() const { return info_alpha_dual_; }
  void Set_info_alpha_dual(Number alpha_dual) { info_alpha_dual_ = alpha_dual; }
  Index info_ls_count() const { return info_ls_count_; }
  void Set_info_ls_count(Index ls_count) { info_ls_count_ = ls_count; }
  bool info_skip_output() const { return info_skip_output_; }
  void Append_info_string(const std::string& add_str) {
    info_string_ += add_str;
  }
  const std::string& info_string() const { return info_string_; }
  ...
  Number info_last_output() { return info_last_output_; }
   ...
   */
  int info_iters_since_header() { return info_iters_since_header_; }
  /** increases number of iteration summaries actually printed
   * since last summary header was printed
   */
  void Inc_info_iters_since_header() { info_iters_since_header_++; }
  ...
  TimingStatistics& TimingStats() { return timing_statistics_; }

  /** Return Timing Statistics Object
   *
   * @since 3.14.11
   */
  const TimingStatistics& TimingStats() const { return timing_statistics_; }

  /** Check if additional data has been set */
  bool HaveAddData() { return IsValid(add_data_); }

  /** Get access to additional data object */
  IpoptAdditionalData& AdditionalData() { return *add_data_; }
  ...
  void setPDPert(Number pd_pert_x, Number pd_pert_s, Number pd_pert_c,
                 Number pd_pert_d) {
  ...
  void getPDPert(Number& pd_pert_x, Number& pd_pert_s, Number& pd_pert_c,
                 Number& pd_pert_d) {
...
inline SmartPtr<const IteratesVector> IpoptData::curr() const {
  DBG_ASSERT(IsNull(curr_) || (curr_->GetTag() == debug_curr_tag_ &&
                               curr_->GetTagSum() == debug_curr_tag_sum_));
...
inline SmartPtr<const IteratesVector> IpoptData::trial() const {
  DBG_ASSERT(IsNull(trial_) || (trial_->GetTag() == debug_trial_tag_ &&
                                trial_->GetTagSum() == debug_trial_tag_sum_));

  return trial_;
}

inline SmartPtr<const IteratesVector> IpoptData::delta() const {
  DBG_ASSERT(IsNull(delta_) || (delta_->GetTag() == debug_delta_tag_ &&
                                delta_->GetTagSum() == debug_delta_tag_sum_));
...
inline void IpoptData::CopyTrialToCurrent() {
  curr_ = trial_;
#if IPOPT_CHECKLEVEL > 0
...
inline void IpoptData::set_trial(SmartPtr<IteratesVector>& trial) {
  trial_ = ConstPtr(trial);

#if IPOPT_CHECKLEVEL > 0
  // verify the correct space
  DBG_ASSERT(trial_->OwnerSpace() ==
             static_cast<VectorSpace*>(GetRawPtr(iterates_space_)));
  if (IsValid(trial)) {
...
inline void IpoptData::set_delta(SmartPtr<IteratesVector>& delta) {
...
inline void IpoptData::set_delta(SmartPtr<const IteratesVector>& delta) {
  delta_ = delta;
#if IPOPT_CHECKLEVEL > 0
...
inline void IpoptData::set_delta_aff(SmartPtr<IteratesVector>& delta_aff) {
