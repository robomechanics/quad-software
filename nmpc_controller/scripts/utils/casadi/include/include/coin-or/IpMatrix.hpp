...
class IPOPTLIB_EXPORT Matrix : public TaggedObject {
 public:
  ...
  Matrix(const MatrixSpace* owner_space)
      : TaggedObject(), owner_space_(owner_space), valid_cache_tag_(0) {}
   ...
   * Should be implemented efficiently for the ExpansionMatrix.
   */
  void SinvBlrmZMTdBr(Number alpha, const Vector& S, const Vector& R,
                      const Vector& Z, const Vector& D, Vector& X) const;
  ///@}
   ...
   */
  ///@{
  virtual void Print(SmartPtr<const Journalist> jnlst, EJournalLevel level,
                     EJournalCategory category, const std::string& name,
                     Index indent = 0, const std::string& prefix = "") const;

  virtual void Print(const Journalist& jnlst, EJournalLevel level,
                     EJournalCategory category, const std::string& name,
                     Index indent = 0, const std::string& prefix = "") const;
  ///@}
 ...
 protected:
   ...
   */
  virtual bool HasValidNumbersImpl() const { return true; }
...
class IPOPTLIB_EXPORT MatrixSpace : public ReferencedObject {
 public:
   ...
   */
  MatrixSpace(Index nRows, Index nCols) : nRows_(nRows), nCols_(nCols) {}
...
/* Inline Methods */
inline Index Matrix::NRows() const { return owner_space_->NRows(); }
...
#define DBG_PRINT_MATRIX(__verbose_level, __mat_name, __mat)      \
  if (dbg_jrnl.Verbosity() >= (__verbose_level)) {                \
    if (dbg_jrnl.Jnlst() != NULL) {                               \
      (__mat).Print(dbg_jrnl.Jnlst(), J_ERROR, J_DBG, __mat_name, \
