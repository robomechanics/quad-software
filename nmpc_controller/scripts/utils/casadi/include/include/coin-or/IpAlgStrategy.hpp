...
class IPOPTLIB_EXPORT AlgorithmStrategyObject : public ReferencedObject {
 public:
  ...
  ///@{
  /** Default Constructor */
  AlgorithmStrategyObject() : initialize_called_(false) {}
   ...
   */
  bool Initialize(const Journalist& jnlst, IpoptNLP& ip_nlp, IpoptData& ip_data,
                  IpoptCalculatedQuantities& ip_cq, const OptionsList& options,
   ...
   */
  bool ReducedInitialize(const Journalist& jnlst, const OptionsList& options,
                         const std::string& prefix) {
    initialize_called_ = true;
    // Copy the pointers for the problem defining objects
    jnlst_ = &jnlst;
 ...
 protected:
  ...
  bool HaveIpData() const { return IsValid(ip_data_); }
  ///@}
