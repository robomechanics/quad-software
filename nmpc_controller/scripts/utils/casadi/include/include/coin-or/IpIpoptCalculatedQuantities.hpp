...
class IPOPTLIB_EXPORT IpoptCalculatedQuantities : public ReferencedObject {
 public:
   ...
   */
  bool HaveAddCq() { return IsValid(add_cq_); }
   ...
   *
   *  This considers the inequality constraints with slacks, i.e., the violation
   * of c(x) = 0, d(x) - s = 0,
   *
   *  This value should be used in the line search instead of
   * curr_primal_infeasibility(). What type of norm is used depends on
   ...
   *
   *  This considers the inequality constraints without slacks, i.e., the
   * violation of c(x) = 0, d_L <= d(x) <= d_U, with scaling applied.
   */
   ...
   *
   *  This considers the inequality constraints without slacks, i.e., the
   * violation of c(x) = 0, d_L <= d(x) <= d_U, without scaling applied.
   */
   ...
   *
   * Computes violation of given unscaled solution w.r.t.
   * OrigIpoptNLP::orig_x_L() if current NLP is a OrigIpoptNLP. If current NLP
   * is not an original OrigIpoptNLP, returns zero vector.
   * @since 3.14.0
   */
   ...
   *
   * Computes violation of given unscaled solution w.r.t.
   * OrigIpoptNLP::orig_x_U() if current NLP is a OrigIpoptNLP. If current NLP
   * is not an original OrigIpoptNLP, returns zero vector.
   * @since 3.14.0
   */
  ...
  /** Violation of original variable lower bounds x_L.
   *
   * Computes violation of current (unscaled) solution w.r.t.
   * OrigIpoptNLP::orig_x_L() if current NLP is a OrigIpoptNLP. If current NLP
   * is not an original OrigIpoptNLP, returns zero vector.
   * @since 3.14.0
   */
   ...
   *
   * Computes violation of given scaled solution w.r.t. OrigIpoptNLP::orig_x_L()
   * if current NLP is a OrigIpoptNLP. If current NLP is not an original
   * OrigIpoptNLP, returns zero vector.
   * @since 3.14.0
   */
  SmartPtr<Vector> orig_x_L_violation(const Vector& x);
  /** Violation of scaled original variable upper bounds x_U.
   *
   * Computes violation of given scaled solution w.r.t. OrigIpoptNLP::orig_x_U()
   * if current NLP is a OrigIpoptNLP. If current NLP is not an original
   * OrigIpoptNLP, returns zero vector.
   * @since 3.14.0
   */
  ...
  /** Violation of scaled original variable lower bounds x_L.
   *
   * Computes violation of current (scaled) solution w.r.t.
   * OrigIpoptNLP::orig_x_L() if current NLP is a OrigIpoptNLP. If current NLP
   * is not an original OrigIpoptNLP, returns zero vector.
   * @since 3.14.0
   */
  ...
  Number CalcCentralityMeasure(const Vector& compl_x_L, const Vector& compl_x_U,
   ...
   *  for a given step
   */
  Number primal_frac_to_the_bound(Number tau, const Vector& delta_x,
   ...
   *  for a given step
   */
  Number dual_frac_to_the_bound(Number tau, const Vector& delta_z_L,
   ...
   */
  Number uncached_dual_frac_to_the_bound(Number tau, const Vector& delta_z_L,
                                         const Vector& delta_z_U,
                                         const Vector& delta_v_L,
                                         const Vector& delta_v_U);
  /** Fraction to the boundary from (current) dual variables z and v
   ...
   */
  Number uncached_slack_frac_to_the_bound(Number tau, const Vector& delta_x_L,
  ...
  Number CalcNormOfType(ENormType NormType, const Vector& vec1,
                        const Vector& vec2);

  /** Norm type used for calculating constraint violation */
  ENormType constr_viol_normtype() const { return constr_viol_normtype_; }
   ...
   */
  SmartPtr<IpoptNLP>& GetIpoptNLP() { return ip_nlp_; }
 ...
 private:
  ...
  ///@{
  CachedResults<std::pair<SmartPtr<Vector>, SmartPtr<Vector> > >
      unscaled_curr_orig_x_LU_viol_cache_;
   ...
   *  (uncached)
   */
  SmartPtr<Vector> CalcSlack_L(const Matrix& P, const Vector& x,
                               const Vector& x_bound);
  /** Compute new vector containing the slack to a upper bound
   *  (uncached)
   */
  SmartPtr<Vector> CalcSlack_U(const Matrix& P, const Vector& x,
                               const Vector& x_bound);
  /** Compute barrier term at given point
   *  (uncached)
   */
  Number CalcBarrierTerm(Number mu, const Vector& slack_x_L,
                         const Vector& slack_x_U, const Vector& slack_s_L,
  ...
  Number CalcFracToBound(const Vector& slack_L, Vector& tmp_L,
                         const Matrix& P_L, const Vector& slack_U,
                         Vector& tmp_U, const Matrix& P_U, const Vector& delta,
                         Number tau);
  ...
  void ComputeOptimalityErrorScaling(const Vector& y_c, const Vector& y_d,
                                     ...
                                     Number s_max, Number& s_d, Number& s_c);
