...
class IPOPTLIB_EXPORT DiagMatrix : public SymMatrix {
 public:
  ...
  void SetDiag(const Vector& diag) { diag_ = &diag; }

  /** Get the diagonal elements. */
  SmartPtr<const Vector> GetDiag() const { return diag_; }
...
class IPOPTLIB_EXPORT DiagMatrixSpace : public SymMatrixSpace {
 public:
  ...
  DiagMatrixSpace(Index dim) : SymMatrixSpace(dim) {}

  /** Destructor */
  virtual ~DiagMatrixSpace() {}
  ///@}

  virtual SymMatrix* MakeNewSymMatrix() const { return MakeNewDiagMatrix(); }

  /** Method for creating a new matrix of this specific type. */
  DiagMatrix* MakeNewDiagMatrix() const { return new DiagMatrix(this); }
