...
class IPOPTLIB_EXPORT CompoundVector : public Vector {
 public:
  ...
  SmartPtr<const Vector> GetComp(Index i) const { return ConstComp(i); }
...
class IPOPTLIB_EXPORT CompoundVectorSpace : public VectorSpace {
 public:
  ...
  Index NCompSpaces() const { return ncomp_spaces_; }
  ...
  virtual Vector* MakeNew() const { return MakeNewCompoundVector(); }
...
inline const Vector* CompoundVector::ConstComp(Index i) const {
  DBG_ASSERT(i < NComps());
  DBG_ASSERT(IsValid(comps_[i]) || IsValid(const_comps_[i]));
  if (IsValid(comps_[i])) {
...
inline Vector* CompoundVector::Comp(Index i) {
  DBG_ASSERT(i < NComps());
