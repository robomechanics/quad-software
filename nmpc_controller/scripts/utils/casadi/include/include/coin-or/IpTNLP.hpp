...
/** Base class for all NLP's that use standard triplet matrix form and dense
 * vectors.
 *
 *  This is the standard base class for all NLP's that use the standard triplet
 * matrix form and dense vectors. The class TNLPAdapter then converts this
 * interface to an interface that can be used directly by %Ipopt.
 *
 ...
 *  (i.e. the variable or constraint has no lower bound (-infinity) or upper
 * bound (+infinity)) is set through the option \ref OPT_nlp_lower_bound_inf
 * "nlp_lower_bound_inf" and \ref OPT_nlp_upper_bound_inf "nlp_upper_bound_inf",
 * respectively. To indicate that a variable has no upper or lower bound, set
 * the bound to -ipopt_inf or +ipopt_inf, respectively.
 */
class IPOPTLIB_EXPORT TNLP : public ReferencedObject {
 public:
   ...
   *  @param nnz_jac_g   (out) Storage for the number of nonzero entries in the
   * Jacobian
   *  @param nnz_h_lag   (out) Storage for the number of nonzero entries in the
   * Hessian
   *  @param index_style (out) Storage for the index style,
   *                     the numbering style used for row/col entries in the
   * sparse matrix format (TNLP::C_STYLE: 0-based, TNLP::FORTRAN_STYLE: 1-based;
   * see also \ref TRIPLET)
   */
  // [TNLP_get_nlp_info]
   ...
   *
   * The values of `n` and `m` that were specified in TNLP::get_nlp_info are
   * passed here for debug checking. Setting a lower bound to a value less than
   * or equal to the value of the option \ref OPT_nlp_lower_bound_inf
   * "nlp_lower_bound_inf" will cause %Ipopt to assume no lower bound. Likewise,
   * specifying the upper bound above or equal to the value of the option \ref
   * OPT_nlp_upper_bound_inf "nlp_upper_bound_inf" will cause %Ipopt to assume
   * no upper bound. These options are set to -10<sup>19</sup> and
  ...
  virtual bool get_scaling_parameters(Number& obj_scaling, bool& use_x_scaling,
   ...
   *
   *  @param n      (in) the number of variables \f$x\f$ in the problem; it will
   * have the same value that was specified in TNLP::get_nlp_info
   *  @param init_x (in) if true, this method must provide an initial value for
   * \f$x\f$
   *  @param x      (out) the initial values for the primal variables \f$x\f$
   *  @param init_z (in) if true, this method must provide an initial value for
   * the bound multipliers \f$z^L\f$ and \f$z^U\f$
   ...
   *  @param m      (in) the number of constraints \f$g(x)\f$ in the problem; it
   * will have the same value that was specified in TNLP::get_nlp_info
   *  @param init_lambda (in) if true, this method must provide an initial value
  ...
  virtual bool get_warm_start_iterate(
      IteratesVector&
          warm_start_iterate /**< storage for warm start iterate in the form
                                %Ipopt requires it internally */
  ) {
   ...
   *  @param n     (in) the number of variables \f$x\f$ in the problem; it will
   * have the same value that was specified in TNLP::get_nlp_info
   *  @param x     (in) the values for the primal variables \f$x\f$ at which the
   * objective function \f$f(x)\f$ is to be evaluated
   *  @param new_x (in) false if any evaluation method (`eval_*`) was previously
   * called with the same values in x, true otherwise. This can be helpful when
   * users have efficient implementations that calculate multiple outputs at
   * once. %Ipopt internally caches results from the TNLP and generally, this
   * flag can be ignored.
   ...
   *  @param new_x (in) false if any evaluation method (`eval_*`) was previously
   * called with the same values in x, true otherwise; see also TNLP::eval_f
   *  @param grad_f (out) array to store values of the gradient of the objective
   ...
   *  @param m     (in) the number of constraints \f$g(x)\f$ in the problem; it
   * will have the same value that was specified in TNLP::get_nlp_info
   *  @param g     (out) array to store constraint function values \f$g(x)\f$,
   * do not add or subtract the bound values \f$g^L\f$ or \f$g^U\f$.
   *
   *  @return true if success, false otherwise.
   */
  // [TNLP_eval_g]
  ...
  /** Method to request either the sparsity structure or the values of the
   ...
   *  @param n     (in) the number of variables \f$x\f$ in the problem; it will
   * have the same value that was specified in TNLP::get_nlp_info
   *  @param x     (in) first call: NULL; later calls: the values for the primal
   * variables \f$x\f$ at which the constraint Jacobian \f$\nabla g(x)^T\f$ is
   * to be evaluated
   *  @param new_x (in) false if any evaluation method (`eval_*`) was previously
   * called with the same values in x, true otherwise; see also TNLP::eval_f
   *  @param m     (in) the number of constraints \f$g(x)\f$ in the problem; it
   * will have the same value that was specified in TNLP::get_nlp_info
   *  @param nele_jac (in) the number of nonzero elements in the Jacobian; it
   * will have the same value that was specified in TNLP::get_nlp_info
   *  @param iRow  (out) first call: array of length nele_jac to store the row
   * indices of entries in the Jacobian of the constraints; later calls: NULL
   *  @param jCol  (out) first call: array of length nele_jac to store the
   * column indices of entries in the Jacobian of the constraints; later calls:
   * NULL
   *  @param values (out) first call: NULL; later calls: array of length
   ...
   * have the same value that was specified in TNLP::get_nlp_info
   *  @param x     (in) first call: NULL; later calls: the values for the primal
   * variables \f$x\f$ at which the Hessian is to be evaluated
   *  @param new_x (in) false if any evaluation method (`eval_*`) was previously
   * called with the same values in x, true otherwise; see also TNLP::eval_f
   *  @param obj_factor (in) factor \f$\sigma_f\f$ in front of the objective
   * term in the Hessian
   *  @param m     (in) the number of constraints \f$g(x)\f$ in the problem; it
   * will have the same value that was specified in TNLP::get_nlp_info
   *  @param lambda (in) the values for the constraint multipliers \f$\lambda\f$
   * at which the Hessian is to be evaluated
   *  @param new_lambda (in) false if any evaluation method was previously
   * called with the same values in lambda, true otherwise
   *  @param nele_hess (in) the number of nonzero elements in the Hessian; it
   * will have the same value that was specified in TNLP::get_nlp_info
   *  @param iRow  (out) first call: array of length nele_hess to store the row
   * indices of entries in the Hessian; later calls: NULL
   *  @param jCol  (out) first call: array of length nele_hess to store the
   ...
   *  methods are call by %Ipopt if the \ref QUASI_NEWTON "quasi-Newton
   * approximation" is selected.
   *
   * @{
   */

  /** Return the number of variables that appear nonlinearly in the objective
   * function or in at least one constraint function
   *
  ...
  ///@{
  /** This method is called when the algorithm has finished (successfully or
   * not) so the TNLP can digest the outcome, e.g., store/write the solution, if
   * any.
   *
   ...
   * TNLP::intermediate_callback returned false, i.e., the user code requested a
   * premature termination of the optimization.
  ...
  /** This method returns any metadata collected during the run of the
   * algorithm.
   *
   * This method is called just before finalize_solution is called.
   * The returned data includes the metadata provided by
   * TNLP::get_var_con_metadata. Each metadata can be of type string, integer,
   * or numeric. It can be associated to either the variables or the
   * constraints. The metadata that was associated with the primal variable
   * vector is stored in `var_..._md`.  The metadata associated with the
   * constraint multipliers is stored in `con_..._md`.  The metadata associated
   ...
   * the problem from `ip_data` and `ip_cq` objects into the TNLP
   * representation.
   *
   ...
   * The method expects the number of variables (dimension of x), number of
   * constraints (dimension of g(x)), and allocated arrays of appropriate
   * lengths as input.
   *
   * The method translates the x(), c(), d(), y_c(), y_d(), z_L(), and z_U()
   * vectors from IpoptData::curr() of the internal %NLP representation into the
   * form used by the TNLP. For the correspondence between scaled and unscaled
   * solutions, see the detailed description of OrigIpoptNLP. If %Ipopt is in
   * restoration mode, it maps the current iterate of restoration %NLP (see
   * RestoIpoptNLP) back to the original TNLP.
   *
   * If there are fixed variables and fixed_variable_treatment=make_parameter,
   * then requesting z_L and z_U can trigger a reevaluation of the Gradient of
   * the objective function and the Jacobian of the constraint functions.
   *
   ...
   * @param n       (in)  the number of variables \f$x\f$ in the problem; can be
   * arbitrary if skipping x, z_L, and z_U
   * @param x       (out) buffer to store value of primal variables \f$x\f$,
   * must have length at least n; pass NULL to skip retrieving x
   * @param z_L     (out) buffer to store the lower bound multipliers \f$z_L\f$,
   * must have length at least n; pass NULL to skip retrieving z_L and z_U
   * @param z_U     (out) buffer to store the upper bound multipliers \f$z_U\f$,
   * must have length at least n; pass NULL to skip retrieving z_U and z_U
   * @param m       (in)  the number of constraints \f$g(x)\f$; can be arbitrary
   * if skipping g and lambda
   * @param g       (out) buffer to store the constraint values \f$g(x)\f$, must
   * have length at least m; pass NULL to skip retrieving g
   * @param lambda  (out) buffer to store the constraint multipliers
   * \f$\lambda\f$, must have length at least m; pass NULL to skip retrieving
   * lambda
   *
  ...
  bool get_curr_iterate(const IpoptData* ip_data,
                        IpoptCalculatedQuantities* ip_cq, bool scaled, Index n,
                        Number* x, Number* z_L, Number* z_U, Index m, Number* g,
                        Number* lambda) const;
  // [TNLP_get_curr_iterate]

  /** Get primal and dual infeasibility of the current iterate.
   *
   * This method can be used to get the violations of constraints and optimality
   * conditions at the current iterate, e.g., during intermediate_callback().
   * The method expects the number of variables (dimension of x), number of
   * constraints (dimension of g(x)), and allocated arrays of appropriate
   * lengths as input.
   *
   * The method makes the vectors behind
   * (unscaled_)curr_orig_bounds_violation(),
   * (unscaled_)curr_nlp_constraint_violation(),
   * (unscaled_)curr_dual_infeasibility(), (unscaled_)curr_complementarity()
   * from IpoptCalculatedQuantities of the internal %NLP representation
   * available into the form used by the TNLP. If %Ipopt is in restoration mode,
   * it maps the current iterate of restoration %NLP (see RestoIpoptNLP) back to
   * the original TNLP.
   *
   * @note If in restoration phase, then requesting grad_lag_x can trigger a
   * call to eval_grad_f().
   *
   * @note Ipopt by default relaxes variable bounds (option bound_relax_factor >
   * 0.0). x_L_violation and x_U_violation report the violation of a solution
   * w.r.t. the original unrelaxed bounds. However, compl_x_L and compl_x_U use
   * the relaxed variable bounds to calculate the complementarity.
   *
   * @param ip_data    (in)  Ipopt Data
   * @param ip_cq      (in)  Ipopt Calculated Quantities
   * @param scaled     (in)  whether to retrieve scaled or unscaled violations
   * @param n          (in)  the number of variables \f$x\f$ in the problem; can
   * be arbitrary if skipping compl_x_L, compl_x_U, and grad_lag_x
   * @param x_L_violation (out) buffer to store violation of original lower
   * bounds on variables (max(orig_x_L-x,0)), must have length at least n; pass
   * NULL to skip retrieving orig_x_L
   * @param x_U_violation (out) buffer to store violation of original upper
   * bounds on variables (max(x-orig_x_U,0)), must have length at least n; pass
   * NULL to skip retrieving orig_x_U
   * @param compl_x_L  (out) buffer to store violation of complementarity for
   * lower bounds on variables (\f$(x-x_L)z_L\f$), must have length at least n;
   * pass NULL to skip retrieving compl_x_L
   * @param compl_x_U  (out) buffer to store violation of complementarity for
   * upper bounds on variables (\f$(x_U-x)z_U\f$), must have length at least n;
   * pass NULL to skip retrieving compl_x_U
   * @param grad_lag_x (out) buffer to store gradient of Lagrangian w.r.t.
   * variables \f$x\f$, must have length at least n; pass NULL to skip
   * retrieving grad_lag_x
   * @param m          (in)  the number of constraints \f$g(x)\f$; can be
   * arbitrary if skipping lambda
   * @param nlp_constraint_violation (out) buffer to store violation of
   * constraints \f$max(g_l-g(x),g(x)-g_u,0)\f$, must have length at least m;
   * pass NULL to skip retrieving constraint_violation
   * @param compl_g    (out) buffer to store violation of complementarity of
   * constraint (\f$(g(x)-g_l)*\lambda^+ + (g_l-g(x))*\lambda^-\f$, where
   * \f$\lambda^+=max(0,\lambda)\f$ and \f$\lambda^-=max(0,-\lambda)\f$
   * (componentwise)), must have length at least m; pass NULL to skip retrieving
   * compl_g
   *
   * @return Whether Ipopt has successfully filled the given arrays
   * @since 3.14.0
  ...
  bool get_curr_violations(const IpoptData* ip_data,
                           IpoptCalculatedQuantities* ip_cq, bool scaled,
                           Index n, Number* x_L_violation,
                           Number* x_U_violation, Number* compl_x_L,
                           Number* compl_x_U, Number* grad_lag_x, Index m,
