...
class SIPOPTLIB_EXPORT SensitivityStepCalculator
    : public AlgorithmStrategyObject {
 public:
  SensitivityStepCalculator() : driver_(NULL), do_boundcheck_(false) {}
  ...
  bool Do_Boundcheck() const { return do_boundcheck_; }
  ...
  void SetSchurDriver(SmartPtr<SchurDriver> driver) {
    ...
    if (IsValid(driver_->pcalc_nonconst())) {
      ...
      // when the schurdriver is set, the data in the pcalculator has to be
      // reset to its data?
    }
  }
