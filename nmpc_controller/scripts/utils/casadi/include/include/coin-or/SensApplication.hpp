...
class SIPOPTLIB_EXPORT SensApplication : public ReferencedObject {
 public:
  // constructor
  SensApplication(SmartPtr<Journalist> jnlst, SmartPtr<OptionsList> options,
  ...
  SmartPtr<Journalist> Jnlst() { return jnlst_; }

  SmartPtr<OptionsList> Options() { return options_; }

  /** Get the options list for setting options (const version) */
  SmartPtr<const OptionsList> Options() const { return ConstPtr(options_); }

  /** Copy over value of Directional Derivatives K^(-1)N_p(p-p0) */
  void GetDirectionalDerivatives(Number* SX, Number* SL, Number* SZL,
                                 Number* SZU) {
    if (GetRawPtr(controller) != NULL && NULL != DirectionalD_X &&
        NULL != DirectionalD_Z_L && NULL != DirectionalD_Z_U &&
        NULL != DirectionalD_L) {
  ...
  void GetSensitivityMatrix(Number* SX, Number* SL, Number* SZL, Number* SZU) {
    if (GetRawPtr(controller) != NULL && NULL != SensitivityM_X &&
        NULL != SensitivityM_Z_L && NULL != SensitivityM_Z_U &&
        NULL != SensitivityM_L) {
  ...
  /* place holders to keep the values of the directional derivatives for each
   * type of variable */
 ...
 private:
  ...
  // Pointers that are immediately passed from Ipopt and initialized by the
  // constructor
