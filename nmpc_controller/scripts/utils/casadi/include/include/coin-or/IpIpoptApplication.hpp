...
class IPOPTLIB_EXPORT IpoptApplication : public ReferencedObject {
 public:
   ...
   *  overwriting options that are set by the params file.
   */
  virtual ApplicationReturnStatus Initialize(const char* params_file,
                                             bool allow_clobber = false) {
  ...
  virtual ApplicationReturnStatus OptimizeNLP(
      const SmartPtr<NLP>& nlp, SmartPtr<AlgorithmBuilder>& alg_builder);
  ...
  virtual SmartPtr<Journalist> Jnlst() { return jnlst_; }

  /** Get a pointer to RegisteredOptions object to add new options */
  virtual SmartPtr<RegisteredOptions> RegOptions() { return reg_options_; }

  /** Get the options list for setting options */
  virtual SmartPtr<OptionsList> Options() { return options_; }
  ...
  /** Method to set whether non-ipopt non-bad_alloc non-overflow_error
   * exceptions are rethrown by Ipopt.
   *
   * By default, non-Ipopt and non-bad_alloc and non-overflow_error exceptions
   * are caught by Ipopts initialization and optimization methods and the status
   * NonIpopt_Exception_Thrown is returned. This function allows to enable
   * rethrowing of such exceptions.
   *
 ...
 private:
  ...
  /** Decide whether non-ipopt non-bad_alloc non-overflow_error exceptions
   * should be rethrown */
...
extern "C" IPOPTLIB_EXPORT class Ipopt::IpoptApplication* IPOPT_CALLCONV
