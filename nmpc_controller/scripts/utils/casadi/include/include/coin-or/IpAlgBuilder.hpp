...
class IPOPTLIB_EXPORT AlgorithmBuilder : public ReferencedObject {
 public:
  ...
  virtual SmartPtr<SymLinearSolver> SymLinearSolverFactory(
      const Journalist& jnlst, const OptionsList& options,
 ...
 protected:
  ...
  /// @since 3.14.0
  SmartPtr<LibraryLoader> GetHSLLoader(const OptionsList& options,
                                       const std::string& prefix);
  ...
  /// Creates new loader if not existing yet.
  /// @since 3.14.0
  SmartPtr<LibraryLoader> GetPardisoLoader(const OptionsList& options,
                                           const std::string& prefix);
