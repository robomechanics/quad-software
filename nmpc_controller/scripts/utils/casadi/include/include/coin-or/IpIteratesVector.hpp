...
class IPOPTLIB_EXPORT IteratesVector : public CompoundVector {
 public:
   ...
   */
  void Set_x(const Vector& vec) { SetComp(0, vec); }

  /** Set the x iterate (non-const).
   *
   * Sets the pointer, does NOT copy data.
   */
  void Set_x_NonConst(Vector& vec) { SetCompNonConst(0, vec); }

  /** Get the s iterate (const) */
  SmartPtr<const Vector> s() const { return GetIterateFromComp(1); }

  /** Get the s iterate (non-const) - this can only be called if the
   *  vector was created internally, or the Set_s_NonConst method was
   *  used.
   */
  SmartPtr<Vector> s_NonConst() { return GetNonConstIterateFromComp(1); }
   ...
   * Sets the pointer, does NOT copy data.
   */
  void Set_s(const Vector& vec) { SetComp(1, vec); }

  /** Set the s iterate (non-const).
   *
   * Sets the pointer, does NOT copy data.
   */
  void Set_s_NonConst(Vector& vec) { SetCompNonConst(1, vec); }

  /** Get the y_c iterate (const) */
  SmartPtr<const Vector> y_c() const { return GetIterateFromComp(2); }

  /** Get the y_c iterate (non-const) - this can only be called if
   *  the vector was created internally, or the Set_y_c_NonConst
   *  method was used.
   */
  SmartPtr<Vector> y_c_NonConst() { return GetNonConstIterateFromComp(2); }
   ...
   * Sets the pointer, does NOT copy data.
   */
  void Set_y_c(const Vector& vec) { SetComp(2, vec); }

  /** Set the y_c iterate (non-const).
   *
   * Sets the pointer, does NOT copy data.
   */
  void Set_y_c_NonConst(Vector& vec) { SetCompNonConst(2, vec); }

  /** Get the y_d iterate (const) */
  SmartPtr<const Vector> y_d() const { return GetIterateFromComp(3); }

  /** Get the y_d iterate (non-const) - this can only be called if
   *  the vector was created internally, or the Set_y_d_NonConst
   *  method was used. */
  SmartPtr<Vector> y_d_NonConst() { return GetNonConstIterateFromComp(3); }
   ...
   * Sets the pointer, does NOT copy data.
   */
  void Set_y_d(const Vector& vec) { SetComp(3, vec); }

  /** Set the y_d iterate (non-const).
   *
   * Sets the pointer, does NOT copy data.
   */
  void Set_y_d_NonConst(Vector& vec) { SetCompNonConst(3, vec); }

  /** Get the z_L iterate (const) */
  SmartPtr<const Vector> z_L() const { return GetIterateFromComp(4); }

  /** Get the z_L iterate (non-const) - this can only be called if
   *  the vector was created internally, or the Set_z_L_NonConst
   *  method was used.
   */
  SmartPtr<Vector> z_L_NonConst() { return GetNonConstIterateFromComp(4); }
   ...
   * Sets the pointer, does NOT copy data.
   */
  void Set_z_L(const Vector& vec) { SetComp(4, vec); }

  /** Set the z_L iterate (non-const).
   *
   * Sets the pointer, does NOT copy data.
   */
  void Set_z_L_NonConst(Vector& vec) { SetCompNonConst(4, vec); }

  /** Get the z_U iterate (const) */
  SmartPtr<const Vector> z_U() const { return GetIterateFromComp(5); }

  /** Get the z_U iterate (non-const) - this can only be called if
   *  the vector was created internally, or the Set_z_U_NonConst
   *  method was used. */
  SmartPtr<Vector> z_U_NonConst() { return GetNonConstIterateFromComp(5); }
   ...
   * Sets the pointer, does NOT copy data.
   */
  void Set_z_U(const Vector& vec) { SetComp(5, vec); }

  /** Set the z_U iterate (non-const).
   *
   * Sets the pointer, does NOT copy data.
   */
  void Set_z_U_NonConst(Vector& vec) { SetCompNonConst(5, vec); }

  /** Get the v_L iterate (const) */
  SmartPtr<const Vector> v_L() const { return GetIterateFromComp(6); }

  /** Get the v_L iterate (non-const) - this can only be called if
   *  the vector was created internally, or the Set_v_L_NonConst
   *  method was used.
   */
  SmartPtr<Vector> v_L_NonConst() { return GetNonConstIterateFromComp(6); }
   ...
   * Sets the pointer, does NOT copy data.
   */
  void Set_v_L(const Vector& vec) { SetComp(6, vec); }

  /** Set the v_L iterate (non-const).
   * Sets the pointer, does NOT copy data.
   */
  void Set_v_L_NonConst(Vector& vec) { SetCompNonConst(6, vec); }

  /** Get the v_U iterate (const) */
  SmartPtr<const Vector> v_U() const { return GetIterateFromComp(7); }

  /** Get the v_U iterate (non-const) - this can only be called if
   *  the vector was created intenally, or the Set_v_U_NonConst
   *  method was used.
   */
  SmartPtr<Vector> v_U_NonConst() { return GetNonConstIterateFromComp(7); }
   ...
   * Sets the pointer, does NOT copy data.
   */
  void Set_v_U(const Vector& vec) { SetComp(7, vec); }

  /** Set the v_U iterate (non-const).
   *
   * Sets the pointer, does NOT copy data.
   */
  void Set_v_U_NonConst(Vector& vec) { SetCompNonConst(7, vec); }
   ...
   *  Sets the pointers, does not copy data.
   */
  void Set_bound_mult(const Vector& z_L, const Vector& z_U, const Vector& v_L,
                      const Vector& v_U) {
    SetComp(4, z_L);
    SetComp(5, z_U);
    SetComp(6, v_L);
   ...
   *  Sets the pointers, does not copy data.
   */
  void Set_bound_mult_NonConst(Vector& z_L, Vector& z_U, Vector& v_L,
                               Vector& v_U) {
...
class IPOPTLIB_EXPORT IteratesVectorSpace : public CompoundVectorSpace {
 public:
  ...
  const SmartPtr<const IteratesVector> MakeNewIteratesVector(
      const Vector& x, const Vector& s, const Vector& y_c, const Vector& y_d,
      const Vector& z_L, const Vector& z_U, const Vector& v_L,
      const Vector& v_U) {
   ...
   */
  virtual Vector* MakeNew() const { return MakeNewIteratesVector(); }
  ///@}
  ...
  virtual void SetCompSpace(Index /*icomp*/, const VectorSpace& /*vec_space*/
    ...
    DBG_ASSERT(false &&
               "This is an IteratesVectorSpace - a special compound vector for "
               "Ipopt iterates. The contained spaces should not be modified.");
  }
