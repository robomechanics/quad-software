...
class IPOPTLIB_EXPORT CompoundMatrixSpace : public MatrixSpace {
 public:
   ...
   */
  CompoundMatrixSpace(Index ncomps_rows, Index ncomps_cols, Index total_nRows,
                      Index total_nCols);
   ...
   */
  void SetCompSpace(Index irow, Index jcol, const MatrixSpace& mat_space,
  ...
  Index NComps_Rows() const { return ncomps_rows_; }

  /** Number of block columns */
  Index NComps_Cols() const { return ncomps_cols_; }
  ...
  virtual Matrix* MakeNew() const { return MakeNewCompoundMatrix(); }
...
inline const Matrix* CompoundMatrix::ConstComp(Index irow, Index jcol) const {
  DBG_ASSERT(irow < NComps_Rows());
  DBG_ASSERT(jcol < NComps_Cols());
...
inline Matrix* CompoundMatrix::Comp(Index irow, Index jcol) {
  DBG_ASSERT(irow < NComps_Rows());
  DBG_ASSERT(jcol < NComps_Cols());
  return GetRawPtr(comps_[irow][jcol]);
}
