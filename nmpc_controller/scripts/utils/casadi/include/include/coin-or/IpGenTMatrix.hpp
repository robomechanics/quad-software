...
class IPOPTLIB_EXPORT GenTMatrix : public Matrix {
 public:
  ...
  const Number* Values() const { return values_; }
...
class IPOPTLIB_EXPORT GenTMatrixSpace : public MatrixSpace {
 public:
   ...
   *  structure.
   */
  GenTMatrixSpace(Index nRows, Index nCols, Index nonZeros, const Index* iRows,
                  const Index* jCols);
  ...
  GenTMatrix* MakeNewGenTMatrix() const { return new GenTMatrix(this); }

  virtual Matrix* MakeNew() const { return MakeNewGenTMatrix(); }

  /**@name Methods describing Matrix structure */
  ///@{
  /** Number of non-zeros in the sparse matrix */
  Index Nonzeros() const { return nonZeros_; }

  /** Row index of each non-zero element (counting starts at 1) */
  const Index* Irows() const { return iRows_; }

  /** Column index of each non-zero element (counting starts at 1) */
  const Index* Jcols() const { return jCols_; }
  ///@}
...
/* inline methods */
inline Index GenTMatrix::Nonzeros() const { return owner_space_->Nonzeros(); }

inline const Index* GenTMatrix::Irows() const { return owner_space_->Irows(); }

inline const Index* GenTMatrix::Jcols() const { return owner_space_->Jcols(); }
