 ...
 *  \f}
 *  where \f$y_c\f$, \f$y_d\f$, \f$z_L\f$, \f$z_U\f$ name the dual variables of
...
class IPOPTLIB_EXPORT IpoptNLP : public ReferencedObject {
 public:
  ...
  virtual SmartPtr<const SymMatrix> h(const Vector& x, Number obj_factor,
                                      const Vector& yc, const Vector& yd) = 0;
   ...
   *  this.
   */
  virtual bool objective_depends_on_mu() const { return false; }
