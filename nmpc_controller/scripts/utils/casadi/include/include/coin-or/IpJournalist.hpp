...
class IPOPTLIB_EXPORT Journalist : public ReferencedObject {
 public:
...
#endif
  virtual void
  Printf(EJournalLevel level, EJournalCategory category, const char* format,
         ...) const;
   ...
   *  documentation.
   */
  virtual void PrintStringOverLines(EJournalLevel level,
...
#endif
  virtual void
  PrintfIndented(EJournalLevel level, EJournalCategory category,
                 Index indent_level, const char* format, ...) const;

  /** Method to print a formatted string using the va_list argument. */
  virtual void VPrintf(EJournalLevel level, EJournalCategory category,
                       const char* pformat, va_list ap) const;

  /** Method to print a formatted string with indentation, using the va_list
   * argument. */
  virtual void VPrintfIndented(EJournalLevel level, EJournalCategory category,
                               Index indent_level, const char* pformat,
                               va_list ap) const;
   ...
   *
   *  @return the Journal pointer so you can set specific acceptance criteria,
   * or NULL if there was a problem creating a new Journal.
   */
  virtual SmartPtr<Journal> AddFileJournal(
      const std::string&
          location_name, /**< string identifier, which can be used to obtain the
                            pointer to the new Journal at a later point using
                            the GetJournal method */
      const std::string&
          fname, /**< name of the file to which this Journal corresponds; use
                    "stdout" for stdout and use "stderr" for stderr */
      EJournalLevel default_level =
          J_WARNING /**< default journal level used to initialize the printing
...
class IPOPTLIB_EXPORT Journal : public ReferencedObject {
 public:
  ...
  /** Flush output buffer.*/
  virtual void FlushBuffer() { FlushBufferImpl(); }
  ///@}
...
class IPOPTLIB_EXPORT FileJournal : public Journal {
 ...
 protected:
  ...
  /** Printf to the designated output location */
  virtual void PrintfImpl(EJournalCategory /*category*/,
...
/** StreamJournal class.
 *
 * This is a particular Journal implementation that writes to a stream for
 * output.
 */
