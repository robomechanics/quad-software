...
#define IPOPTLINEARSOLVER_ALLHSL                                              \
  (IPOPTLINEARSOLVER_MA27 | IPOPTLINEARSOLVER_MA57 | IPOPTLINEARSOLVER_MA77 | \
   IPOPTLINEARSOLVER_MA86 | IPOPTLINEARSOLVER_MA97 | IPOPTLINEARSOLVER_MC19)
...
/** get bitflags indicating which linear solvers (and mc19) are available to
 * Ipopt
 *
 * If buildinonly if set to a nonzero value, then only report linear solvers
 * that have been linked into Ipopt. Otherwise, also linear solvers are reported
 * which are loaded from a shared library at runtime, if this feature has been
 * compiled.
 * @since 3.14.0
 */
IPOPTLIB_EXPORT IpoptLinearSolver IPOPT_CALLCONV
IpoptGetAvailableLinearSolvers(int buildinonly);
