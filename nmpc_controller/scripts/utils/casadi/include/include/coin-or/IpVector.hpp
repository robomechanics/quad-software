...
class IPOPTLIB_EXPORT Vector : public TaggedObject {
 public:
   ...
   */
  inline Vector(const VectorSpace* owner_space);
  ...
  inline void Axpy(Number alpha, const Vector& x);

  /** Computes inner product of vector x with this (DDOT) */
  inline Number Dot(const Vector& x) const;
  ...
  inline void Set(Number alpha);

  /** Element-wise division  \f$y_i \gets y_i/x_i\f$ */
  inline void ElementWiseDivide(const Vector& x);

  /** Element-wise multiplication \f$y_i \gets y_i*x_i\f$ */
  inline void ElementWiseMultiply(const Vector& x);
   ...
   * @since 3.14.0
   */
  inline void ElementWiseSelect(const Vector& x);

  /** Element-wise max against entries in x */
  inline void ElementWiseMax(const Vector& x);

  /** Element-wise min against entries in x */
  inline void ElementWiseMin(const Vector& x);
  ...
  inline void AddScalar(Number scalar);
   ...
   */
  inline void AddOneVector(Number a, const Vector& v1, Number c);

  /** Add two vectors, y = a * v1 + b * v2 + c * y.
   *
   * Here, this vector is y.
   */
  inline void AddTwoVectors(Number a, const Vector& v1, Number b,
                            const Vector& v2, Number c);

  /** Fraction to the boundary parameter.
   *
   *  Computes \f$\alpha = \max\{\bar\alpha\in(0,1] : x + \bar\alpha \Delta \geq
   * (1-\tau)x\}\f$
   */
  inline Number FracToBound(const Vector& delta, Number tau) const;

  /** Add the quotient of two vectors, y = a * z/s + c * y. */
  inline void AddVectorQuotient(Number a, const Vector& z, const Vector& s,
                                Number c);
  ///@}

  /** Method for determining if all stored numbers are valid (i.e., no Inf or
   * Nan). */
  ...
  void Print(SmartPtr<const Journalist> jnlst, EJournalLevel level,
             EJournalCategory category, const std::string& name,
             Index indent = 0, const std::string& prefix = "") const;

  void Print(const Journalist& jnlst, EJournalLevel level,
             EJournalCategory category, const std::string& name,
             Index indent = 0, const std::string& prefix = "") const;
  ///@}
 ...
 protected:
   ...
   */
  virtual void AddTwoVectorsImpl(Number a, const Vector& v1, Number b,
                                 const Vector& v2, Number c);
  ...
  virtual void AddVectorQuotientImpl(Number a, const Vector& z, const Vector& s,
                                     Number c);
  ...
  /** Print the entire vector */
  virtual void PrintImpl(const Journalist& jnlst, EJournalLevel level,
                         EJournalCategory category, const std::string& name,
                         Index indent, const std::string& prefix) const = 0;
  ///@}
...
inline Vector::Vector(const VectorSpace* owner_space)
    : TaggedObject(),
...
inline void Vector::Axpy(Number alpha, const Vector& x) {
  AxpyImpl(alpha, x);
  ObjectChanged();
}

inline Number Vector::Dot(const Vector& x) const {
...
inline void Vector::Set(Number alpha) {
  // Could initialize caches here
  SetImpl(alpha);
  ObjectChanged();
}

inline void Vector::ElementWiseDivide(const Vector& x) {
  ElementWiseDivideImpl(x);
  ObjectChanged();
}

inline void Vector::ElementWiseMultiply(const Vector& x) {
  ElementWiseMultiplyImpl(x);
  ObjectChanged();
}

inline void Vector::ElementWiseSelect(const Vector& x) {
...
inline void Vector::ElementWiseMax(const Vector& x) {
  // Could initialize some caches here
  ElementWiseMaxImpl(x);
  ObjectChanged();
}

inline void Vector::ElementWiseMin(const Vector& x) {
  // Could initialize some caches here
...
inline void Vector::AddScalar(Number scalar) {
  // Could initialize some caches here
...
inline void Vector::AddOneVector(Number a, const Vector& v1, Number c) {
  AddTwoVectors(a, v1, 0., v1, c);
}

inline void Vector::AddTwoVectors(Number a, const Vector& v1, Number b,
                                  const Vector& v2, Number c) {
  AddTwoVectorsImpl(a, v1, b, v2, c);
  ObjectChanged();
}

inline Number Vector::FracToBound(const Vector& delta, Number tau) const {
...
inline void Vector::AddVectorQuotient(Number a, const Vector& z,
...
inline VectorSpace::VectorSpace(Index dim) : dim_(dim) {}
...
#define DBG_PRINT_VECTOR(__verbose_level, __vec_name, __vec)      \
  if (dbg_jrnl.Verbosity() >= (__verbose_level)) {                \
    if (dbg_jrnl.Jnlst() != NULL) {                               \
      (__vec).Print(dbg_jrnl.Jnlst(), J_ERROR, J_DBG, __vec_name, \
