...
class IPOPTLIB_EXPORT DenseVector : public Vector {
 public:
   ...
   */
  inline Number* ExpandedValues() { return Values(); }

  /** Indicates if the vector is homogeneous (i.e., all entries have
   *  the value Scalar().
   */
  bool IsHomogeneous() const { return homogeneous_; }
 ...
 protected:
   ...
   */
  void AddTwoVectorsImpl(Number a, const Vector& v1, Number b, const Vector& v2,
                         Number c);
  ...
  virtual void PrintImpl(const Journalist& jnlst, EJournalLevel level,
                         ...
                         Index indent, const std::string& prefix) const {
  ...
  void PrintImplOffset(const Journalist& jnlst, EJournalLevel level,
                       EJournalCategory category, const std::string& name,
                       Index indent, const std::string& prefix,
                       Index offset) const;
  ///@}
 ...
 private:
  ...
  /** Copy of the owner_space ptr as a DenseVectorSpace instead of a
   * VectorSpace. */
...
/** @name typedefs for the map variables that define meta data for the
 * DenseVectorSpace */
///@{
...
class IPOPTLIB_EXPORT DenseVectorSpace : public VectorSpace {
 public:
  ...
  DenseVectorSpace(Index dim) : VectorSpace(dim) {}
  ...
  virtual Vector* MakeNew() const { return MakeNewDenseVector(); }
  ...
  inline void FreeInternalStorage(Number* values) const;
  ///@}
  ...
  inline bool HasStringMetaData(const std::string& tag) const;

  /** Check if Integer meta exists for tag */
  inline bool HasIntegerMetaData(const std::string& tag) const;

  /** Check if Numeric meta exists for tag */
  inline bool HasNumericMetaData(const std::string& tag) const;
  ...
  inline void SetStringMetaData(const std::string& tag,
                                const std::vector<std::string>& meta_data);

  /** Set meta data of type Index by tag */
  inline void SetIntegerMetaData(const std::string& tag,
                                 const std::vector<Index>& meta_data);

  /** Set meta data of type Number by tag */
  inline void SetNumericMetaData(const std::string& tag,
...
inline void DenseVectorSpace::FreeInternalStorage(Number* values) const {
  delete[] values;
}
...
inline bool DenseVectorSpace::HasStringMetaData(const std::string& tag) const {
...
inline bool DenseVectorSpace::HasIntegerMetaData(const std::string& tag) const {
...
inline bool DenseVectorSpace::HasNumericMetaData(const std::string& tag) const {
...
inline void DenseVectorSpace::SetStringMetaData(
    const std::string& tag, const std::vector<std::string>& meta_data) {
  string_meta_data_[tag] = meta_data;
}

inline void DenseVectorSpace::SetIntegerMetaData(
    const std::string& tag, const std::vector<Index>& meta_data) {
  integer_meta_data_[tag] = meta_data;
}

inline void DenseVectorSpace::SetNumericMetaData(
    const std::string& tag, const std::vector<Number>& meta_data) {
  numeric_meta_data_[tag] = meta_data;
}

inline const StringMetaDataMapType& DenseVectorSpace::GetStringMetaData()
    const {
  return string_meta_data_;
}

inline const IntegerMetaDataMapType& DenseVectorSpace::GetIntegerMetaData()
    const {
  return integer_meta_data_;
}

inline const NumericMetaDataMapType& DenseVectorSpace::GetNumericMetaData()
    const {
