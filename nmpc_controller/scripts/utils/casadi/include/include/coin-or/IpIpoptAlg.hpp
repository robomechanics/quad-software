...
class IPOPTLIB_EXPORT IpoptAlgorithm : public AlgorithmStrategyObject {
 public:
  ...
  IpoptAlgorithm(
      ...
      const SmartPtr<IterateInitializer>&
      ...
      const SmartPtr<EqMultiplierCalculator>& eq_multiplier_calculator =
          NULL,  ///< calculator for multipliers
      const std::string& linear_solver_name =
          ""  ///< name of linear solver @since 3.14.0
  );
 ...
 private:
  ...
  ///@{
  void calc_number_of_bounds(const Vector& x, const Vector& x_L,
                             const Vector& x_U, const Matrix& Px_L,
