...
class IPOPTLIB_EXPORT Observer {
 ...
 protected:
   ...
   */
  inline void RequestAttach(NotifyType notify_type, const Subject* subject);
   ...
   * since further processing is done here
   */
  inline void RequestDetach(NotifyType notify_type, const Subject* subject);
 ...
 private:
   ...
   */
  inline void ProcessNotification(NotifyType notify_type,
...
class IPOPTLIB_EXPORT Subject {
 public:
  ...
  inline void AttachObserver(Observer::NotifyType notify_type,
                             Observer* observer) const;

  /** Detach the specified observer
   *  (i.e., no longer receive notifications). */
  inline void DetachObserver(Observer::NotifyType notify_type,
                             Observer* observer) const;
  ///@}

 protected:
  inline void Notify(Observer::NotifyType notify_type) const;
...
inline void Observer::RequestAttach(NotifyType notify_type,
                                    const Subject* subject) {
#ifdef IP_DEBUG_OBSERVER
...
inline void Observer::RequestDetach(NotifyType notify_type,
                                    const Subject* subject) {
#ifdef IP_DEBUG_OBSERVER
...
#ifdef IP_DEBUG_OBSERVER
      DBG_PRINT(
          (1, "Removing subject: %p from the list\n", (const void*)subject));
#endif
...
inline void Observer::ProcessNotification(NotifyType notify_type,
                                          const Subject* subject) {
#ifdef IP_DEBUG_OBSERVER
...
inline Subject::~Subject() {
  ...
  for (std::vector<Observer*>::iterator iter = observers_.begin();
       iter != observers_.end(); ++iter) {
    (*iter)->ProcessNotification(Observer::NT_BeingDestroyed, this);
  }
}

inline void Subject::AttachObserver(Observer::NotifyType /*notify_type*/,
                                    Observer* observer) const {
#ifdef IP_DEBUG_OBSERVER
...
inline void Subject::DetachObserver(Observer::NotifyType /*notify_type*/,
                                    Observer* observer) const {
#ifdef IP_DEBUG_OBSERVER
...
inline void Subject::Notify(Observer::NotifyType notify_type) const {
#ifdef IP_DEBUG_OBSERVER
