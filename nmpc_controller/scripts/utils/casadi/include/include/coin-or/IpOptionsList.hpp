...
class IPOPTLIB_EXPORT OptionsList : public ReferencedObject {
  ...
  class OptionValue {
   public:
    ...
    ///@{
    /** Default constructor */
    OptionValue() : initialized_(false) {}
 ...
 public:
  ...
  virtual void clear() { options_.clear(); }
  ...
  /// @since 3.14.1
  virtual bool SetBoolValue(const std::string& tag, bool value,
                            bool allow_clobber = true,
                            bool dont_print = false) {
  ...
  ///@{
  virtual bool SetStringValueIfUnset(const std::string& tag,
  ...
  /// @since 3.14.1
  virtual bool SetBoolValueIfUnset(const std::string& tag, bool value,
                                   bool allow_clobber = true,
                                   bool dont_print = false) {
 ...
 private:
   ...
   *
   *  @return true, if the option does not already exist or if the option exists
   * but is set to allow_clobber
   */
