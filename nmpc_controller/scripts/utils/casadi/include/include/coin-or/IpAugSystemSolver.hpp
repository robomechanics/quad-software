...
class IPOPTLIB_EXPORT AugSystemSolver : public AlgorithmStrategyObject {
 public:
  ...
  virtual ESymSolverStatus Solve(
      ...
      const Vector& rhs_x, const Vector& rhs_s, const Vector& rhs_c,
      const Vector& rhs_d, Vector& sol_x, Vector& sol_s, Vector& sol_c,
      Vector& sol_d, bool check_NegEVals, Index numberOfNegEVals) {
    ...
    sol_dV[0] = &sol_d;
    return MultiSolve(W, W_factor, D_x, delta_x, D_s, delta_s, J_c, D_c,
                      delta_c, J_d, D_d, delta_d, rhs_xV, rhs_sV, rhs_cV,
                      rhs_dV, sol_xV, sol_sV, sol_cV, sol_dV, check_NegEVals,
                      numberOfNegEVals);
  }
  ...
  virtual ESymSolverStatus MultiSolve(
    ...
    for (Index i = 0; i < nrhs; i++) {
      retval = Solve(W, W_factor, D_x, delta_x, D_s, delta_s, J_c, D_c, delta_c,
                     J_d, D_d, delta_d, *rhs_xV[i], *rhs_sV[i], *rhs_cV[i],
                     *rhs_dV[i], *sol_xV[i], *sol_sV[i], *sol_cV[i], *sol_dV[i],
