...
class IPOPTLIB_EXPORT RegisteredCategory : public ReferencedObject {
 ...
 public:
  /// Constructor
  ///
  /// Use negative value for priority to suppress it being included in
  /// documentation.
  ...
  /// name of category
  ///
  /// This one is for backward-compatibility with previous Ipopt versions where
  /// RegisteredCategory was a string.
  ...
  IPOPT_DEPRECATED
  bool operator<(const RegisteredCategory& other) const {
  ...
  class ComparePriority {
   public:
    bool operator()(const SmartPtr<RegisteredCategory>& lhs,
                    const SmartPtr<RegisteredCategory>& rhs) const {
...
class IPOPTLIB_EXPORT RegisteredOption : public ReferencedObject {
 ...
 public:
  ...
  RegisteredOption(
      ...
      const SmartPtr<RegisteredCategory>&
          registering_category,  ///< option category @since 3.14.0
  ...
  virtual const RegisteredOptionType& Type() const { return type_; }
  /** Set the Option's type */
  virtual void SetType(const RegisteredOptionType& type) { type_ = type; }

  /** Get the advanced flag
   * @since 3.14.0
   */
  virtual bool Advanced() const { return advanced_; }
  /** Set the advanced flag
   * @since 3.14.0
   */
  virtual void SetAdvanced(bool advanced = true) { advanced_ = advanced; }

  /** Counter */
  virtual Index Counter() const { return counter_; }
  ///@}
...
class IPOPTLIB_EXPORT RegisteredOptions : public ReferencedObject {
 public:
  ...
  /// @since 3.14.0
  typedef std::map<std::string, SmartPtr<RegisteredCategory> >
      RegCategoriesList;
  /// @since 3.14.0
  typedef std::set<SmartPtr<RegisteredCategory>,
                   RegisteredCategory::ComparePriority>
  ...
  ///@{
  /** Default Constructor */
  RegisteredOptions() : next_counter_(0) {}

  /** Destructor */
  virtual ~RegisteredOptions() {
    // break circular reference between registered options and registered
    // categories
    for (RegCategoriesList::iterator it(registered_categories_.begin());
         it != registered_categories_.end(); ++it) {
   ...
   *
   * If nonempty name, then all subsequent options will be added with the
   * registered category. If empty name, then all subsequent options will not be
   * added to any registered category.
   *
  ...
  /** set the registering class
   *
   * If not NULL, then all subsequent options will be added with the registered
   * category. If NULL, then all subsequent options will not be added to any
   * registered category.
   * @since 3.14.0
   */
  virtual void SetRegisteringCategory(
  ...
  virtual void AddNumberOption(
      ...
      const std::string& long_description = "",  ///< long description
      bool advanced =
          false  ///< whether option is for advanced users @since 3.14.0
  );
  ...
  void RegisteredCategoriesByPriority(
      RegCategoriesByPriority& categories) const;

  /** Output documentation
   *
   * Format is decided according to print_options_mode option.
   * Whether to print advanced options is decided according to
   * print_advanced_options option. All categories with priority equal or
   * greater minpriority are printed.
   * @since 3.14.0
   */
  virtual void OutputOptionDocumentation(const Journalist& jnlst,
  ...
  virtual void OutputOptionDocumentation(
      const Journalist& jnlst, const std::list<std::string>& categories =
  ...
  virtual void OutputLatexOptionDocumentation(
      const Journalist& jnlst, const std::list<std::string>& options_to_print =
                                   std::list<std::string>()) const;
