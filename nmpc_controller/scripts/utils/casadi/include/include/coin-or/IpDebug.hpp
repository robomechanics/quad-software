...
class IPOPTLIB_EXPORT DebugJournalistWrapper {
 public:
  ...
  ///@{
  DebugJournalistWrapper(const std::string& func_name, Index verbose_level);
  ...
  ///@{
  Index Verbosity() { return verbose_level_; }
  const Journalist* Jnlst() { return jrnl_; }
  Index IndentationLevel() { return indentation_level_; }
  ///@}
...
#endif
  void
  DebugPrintf(Index verbosity, const char* pformat, ...);
 ...
 private:
   ...
   * It is expected that this is only called by the IpoptApplication
   * constructor.
   */
...
#define DBG_PRINT(__args) dbg_jrnl.DebugPrintf __args;
...
#define DBG_VERBOSITY() dbg_jrnl.Verbosity()
