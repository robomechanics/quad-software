...
class SIPOPTLIB_EXPORT SchurData : public ReferencedObject {
 public:
  ...
  virtual void SetData_Flag(Index dim, const Index* flags,
                            const Number* values) = 0;

  virtual Index SetData_Index(Index dim, const Index* flags,
                              Number v = 1.0) = 0;

  virtual void SetData_List(const std::vector<Index>& list, Number v = 1.0) = 0;

  virtual void AddData_List(std::vector<Index> cols,
                            std::vector<Index>& delta_u_sort,
                            Index& new_du_size, Index v) = 0;

  /** Returns number of rows/columns in schur matrix */
  virtual Index GetNRowsAdded() const { return nrows_; }
   ...
   */
  virtual void GetMultiplyingVectors(Index row, std::vector<Index>& indices,
                                     std::vector<Number>& factors) const = 0;
 ...
 protected:
  virtual void Set_Initialized() { initialized_ = true; }

  virtual void Set_NRows(Index nrows) { nrows_ = nrows; }
