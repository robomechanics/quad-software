...
class IPOPTLIB_EXPORT SymScaledMatrixSpace : public SymMatrixSpace {
 public:
  ...
  SymScaledMatrix* MakeNewSymScaledMatrix(
      bool allocate_unscaled_matrix = false) const {
    SymScaledMatrix* ret = new SymScaledMatrix(this);
    if (allocate_unscaled_matrix) {
      SmartPtr<SymMatrix> unscaled_matrix =
          unscaled_matrix_space_->MakeNewSymMatrix();
  ...
  virtual Matrix* MakeNew() const { return MakeNewSymScaledMatrix(); }
