...
struct TensorEvaluator<const TensorChippingOp<DimId, ArgType>, Device> {
  ...
  enum {
    ...
    PreferBlockAccess =
        TensorEvaluator<ArgType, Device>::PreferBlockAccess || !IsOuterChipping,
  ...
  block(TensorBlockDesc& desc, TensorBlockScratch& scratch,
    ...
    ArgTensorBlock arg_block =
        m_impl.block(arg_desc, scratch, root_of_expr_ast);
 ...
 protected:
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE bool isOuterChipping() const {
    return IsOuterChipping ||
           (static_cast<int>(Layout) == ColMajor &&
            m_dim.actualDim() == NumInputDims - 1) ||
