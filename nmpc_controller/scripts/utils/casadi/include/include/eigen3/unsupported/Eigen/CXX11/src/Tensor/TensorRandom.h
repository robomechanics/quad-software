...
#ifdef EIGEN_USE_SYCL
    if (!m_exec_once) {
      // This is the second stage of adding thread Id to the CPU clock seed and
      // build unique seed per thread The (i * 6364136223846793005ULL) is the
      // remaining part of the PCG_XSH_RS_state on the GPU side
