...
struct h_skip {
  ...
  constexpr static EIGEN_STRONG_INLINE
      typename h_skip_helper_numeric<T, n, ii...>::type
      helper(numeric_list<T, ii...>) {
    return typename h_skip_helper_numeric<T, n, ii...>::type();
  }
  template <typename... tt>
  constexpr static EIGEN_STRONG_INLINE
      typename h_skip_helper_type<n, tt...>::type
      helper(type_list<tt...>) {
...
struct reduce<Reducer> {
  EIGEN_DEVICE_FUNC constexpr static EIGEN_STRONG_INLINE int run() {
...
struct reduce<Reducer, A> {
  EIGEN_DEVICE_FUNC constexpr static EIGEN_STRONG_INLINE A run(A a) {
    return a;
  }
};
...
struct sum_op {
  template <typename A, typename B>
  EIGEN_DEVICE_FUNC constexpr static EIGEN_STRONG_INLINE auto run(A a, B b)
      -> decltype(a + b) {
    return a + b;
  }
...
EIGEN_DEVICE_FUNC constexpr EIGEN_STRONG_INLINE decltype(reduce<product_op,
                                                                Ts...>::
                                                             run((
                                                                 *((Ts*)0))...))
arg_prod(Ts... ts) {
...
template <typename... Ts>
constexpr EIGEN_STRONG_INLINE decltype(reduce<sum_op, Ts...>::run(
    (*((Ts*)0))...))
arg_sum(Ts... ts) {
...
constexpr EIGEN_STRONG_INLINE Array h_array_reverse(Array arr,
                                                    numeric_list<int, n...>) {
...
struct h_array_reduce {
  EIGEN_DEVICE_FUNC constexpr static EIGEN_STRONG_INLINE auto run(
      array<T, N> arr, T identity)
...
struct h_array_reduce<Reducer, T, N, 0> {
  EIGEN_DEVICE_FUNC constexpr static EIGEN_STRONG_INLINE T
...
template <typename Reducer, typename T, std::size_t N>
EIGEN_DEVICE_FUNC constexpr EIGEN_STRONG_INLINE auto array_reduce(
    const array<T, N>& arr,
    T identity) -> decltype(h_array_reduce<Reducer, T, N>::run(arr, identity)) {
...
template <typename T, std::size_t N>
EIGEN_DEVICE_FUNC constexpr EIGEN_STRONG_INLINE auto array_sum(
    const array<T, N>& arr)
...
template <typename T, std::size_t N>
EIGEN_DEVICE_FUNC constexpr EIGEN_STRONG_INLINE auto array_prod(
    const array<T, N>& arr)
...
constexpr EIGEN_STRONG_INLINE array<decltype(Op::run(A(), B())), N> h_array_zip(
...
constexpr EIGEN_STRONG_INLINE array<decltype(Op::run(A(), B())), N> array_zip(
...
template <typename Reducer, typename Op, typename A, typename B, std::size_t N,
          int... n>
constexpr EIGEN_STRONG_INLINE auto h_array_zip_and_reduce(
    array<A, N> a, array<B, N> b, numeric_list<int, n...>)
...
template <typename Reducer, typename Op, typename A, typename B, std::size_t N>
constexpr EIGEN_STRONG_INLINE auto array_zip_and_reduce(array<A, N> a,
                                                        array<B, N> b)
...
constexpr EIGEN_STRONG_INLINE array<decltype(Op::run(A())), N> h_array_apply(
    array<A, N> a, numeric_list<int, n...>) {
  return array<decltype(Op::run(A())), N>{{Op::run(array_get<n>(a))...}};
}

template <typename Op, typename A, std::size_t N>
constexpr EIGEN_STRONG_INLINE array<decltype(Op::run(A())), N> array_apply(
    array<A, N> a) {
...
constexpr EIGEN_STRONG_INLINE auto h_array_apply_and_reduce(
    array<A, N> arr, numeric_list<int, n...>)
    -> decltype(reduce<Reducer,
...
struct h_repeat {
  ...
  constexpr static EIGEN_STRONG_INLINE array<t, n> run(
      t v, numeric_list<int, ii...>) {
