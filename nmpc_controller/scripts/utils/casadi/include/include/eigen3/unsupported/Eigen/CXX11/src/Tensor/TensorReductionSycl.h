 ...
 *
 * \brief:
 *  This is the specialization of the reduction operation. Two phase reduction
 *approach is used since the GPU does not have Global Synchronization for global
 *memory among different work-group/thread block. To solve the problem, we need
 *to create two kernels to reduce the data, where the first kernel reduce the
 *data locally and each local workgroup/thread-block save the input data into
 *global memory. In the second phase (global reduction) one work-group uses one
 *work-group/thread-block to reduces the intermediate data into one single
 *element. Here is an NVIDIA presentation explaining the optimized two phase
 *reduction algorithm on GPU:
...
template <typename Op, typename CoeffReturnType, typename Index,
          bool Vectorizable>
struct OpDefiner {
  typedef typename Vectorise<CoeffReturnType, Eigen::SyclDevice,
                             Vectorizable>::PacketReturnType PacketReturnType;
  typedef Op type;
  ...
  finalise_op(const PacketReturnType &accumulator, const Index &) {
    return accumulator;
  }
};

template <typename CoeffReturnType, typename Index>
struct OpDefiner<Eigen::internal::MeanReducer<CoeffReturnType>, CoeffReturnType,
                 Index, false> {
  typedef Eigen::internal::SumReducer<CoeffReturnType> type;
  static EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE type
  get_op(Eigen::internal::MeanReducer<CoeffReturnType> &) {
    return type();
  }

  static EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE CoeffReturnType
  finalise_op(const CoeffReturnType &accumulator, const Index &scale) {
...
struct OpDefiner<Eigen::internal::MeanReducer<CoeffReturnType>, CoeffReturnType,
                 Index, true> {
  typedef typename Vectorise<CoeffReturnType, Eigen::SyclDevice,
                             true>::PacketReturnType PacketReturnType;
  typedef Eigen::internal::SumReducer<CoeffReturnType> type;
  static EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE type
  get_op(Eigen::internal::MeanReducer<CoeffReturnType> &) {
    return type();
  ...
  static EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE PacketReturnType
  finalise_op(const PacketReturnType &accumulator, const Index &scale) {
    return ::Eigen::internal::pdiv(
        accumulator,
        ::Eigen::internal::pset1<PacketReturnType>(CoeffReturnType(scale)));
  }
};

template <typename CoeffReturnType, typename OpType, typename InputAccessor,
          typename OutputAccessor, typename Index, Index local_range>
struct SecondStepFullReducer {
  ...
  Op op;
  SecondStepFullReducer(LocalAccessor scratch_, InputAccessor aI_,
                        OutputAccessor outAcc_, OpType op_)
  ...
  void operator()(cl::sycl::nd_item<1> itemID) {
    ...
    for (Index offset = itemID.get_local_range(0) / 2; offset > 0;
         offset /= 2) {
...
// Full reduction first phase. In this version the vectorization is true and the
// reduction accept any generic reducerOp  e.g( max, min, sum, mean, iamax,
// iamin, etc ).
...
class FullReductionKernelFunctor {
 public:
  ...
  typedef OpDefiner<OpType, typename Evaluator::CoeffReturnType, Index,
                    (Evaluator::ReducerTraits::PacketAccess &
                     Evaluator::InputPacketAccess)>
      OpDef;
  ...
  typedef cl::sycl::accessor<OutType, 1, cl::sycl::access::mode::read_write,
                             cl::sycl::access::target::local>
      LocalAccessor;
  LocalAccessor scratch;
  Evaluator evaluator;
  ...
  FullReductionKernelFunctor(LocalAccessor scratch_, Evaluator evaluator_,
                             EvaluatorPointerType final_output_, Index rng_,
                             OpType op_)
      : scratch(scratch_),
  ...
  template <bool Vect = (Evaluator::ReducerTraits::PacketAccess &
                         Evaluator::InputPacketAccess)>
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE
      typename ::Eigen::internal::enable_if<Vect>::type
      compute_reduction(const cl::sycl::nd_item<1> &itemID) {
    auto output_ptr = final_output.get_pointer();
    Index VectorizedRange =
    ...
    // vectorizable parts
    PacketReturnType packetAccumulator =
    ...
    for (Index i = globalid; i < rng; i += itemID.get_global_range(0)) {
      op.template reducePacket<PacketReturnType>(
          ::Eigen::TensorSycl::internal::PacketWrapper<PacketReturnType,
                                                       Evaluator::PacketSize>::
              convert_to_packet_type(evaluator.impl().coeff(i),
                                     op.initialize()),
          &packetAccumulator);
    }
    scratch[localid] = packetAccumulator = OpDef::finalise_op(
        op.template finalizePacket<PacketReturnType>(packetAccumulator), rng);
    // reduction parts // Local size is always power of 2
    EIGEN_UNROLL_LOOP
    for (Index offset = local_range / 2; offset > 0; offset /= 2) {
      itemID.barrier(cl::sycl::access::fence_space::local_space);
      if (localid < offset) {
        op.template reducePacket<PacketReturnType>(scratch[localid + offset],
                                                   &packetAccumulator);
        scratch[localid] =
    ...
    if (localid == 0) {
      output_ptr[itemID.get_group(0)] = op.finalizeBoth(
          op.initialize(),
          op.template finalizePacket<PacketReturnType>(packetAccumulator));
    }
  }
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE
    ...
    scratch[localid] = accumulator =
        OpDef::finalise_op(op.finalize(accumulator), rng);
...
class GenericNondeterministicReducer {
 public:
  ...
  GenericNondeterministicReducer(Scratch, Evaluator evaluator_,
                                 EvaluatorPointerType output_accessor_,
                                 OpType functor_, Index range_,
  ...
  void operator()(cl::sycl::nd_item<1> itemID) {
    ...
    if (globalid < range) {
      ...
      Eigen::internal::GenericDimReducer<
          Evaluator::NumReducedDims - 1, Evaluator,
          Op>::reduce(evaluator, evaluator.firstInput(globalid), functor,
                      &accum);
      output_accessor_ptr[globalid] =
...
struct PartialReductionKernel {
  ...
  PartialReductionKernel(ScratchAcc scratch_, Evaluator evaluator_,
                         EvaluatorPointerType output_accessor_, OpType op_,
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE void element_wise_reduce(
      Index globalRId, Index globalPId, CoeffReturnType &accumulator) {
    if (globalPId >= num_coeffs_to_preserve) {
      return;
    }
    Index global_offset = rt == reduction_dim::outer_most
                              ? globalPId + (globalRId * num_coeffs_to_preserve)
                              : globalRId + (globalPId * num_coeffs_to_reduce);
    Index localOffset = globalRId;

    const Index per_thread_local_stride =
        PannelParameters::LocalThreadSizeR * reduce_elements_num_groups;
    const Index per_thread_global_stride =
        rt == reduction_dim::outer_most
            ? num_coeffs_to_preserve * per_thread_local_stride
            : per_thread_local_stride;
    for (Index i = globalRId; i < num_coeffs_to_reduce;
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE void operator()(
    ...
    Index pLocalThreadId =
        rt == reduction_dim::outer_most
            ? linearLocalThreadId % PannelParameters::LocalThreadSizeP
            : linearLocalThreadId / PannelParameters::LocalThreadSizeR;
    Index rLocalThreadId =
        rt == reduction_dim::outer_most
            ? linearLocalThreadId / PannelParameters::LocalThreadSizeP
            : linearLocalThreadId % PannelParameters::LocalThreadSizeR;
    const Index pGroupId =
        rt == reduction_dim::outer_most
            ? itemID.get_group(0) % preserve_elements_num_groups
    ...
    Index globalPId =
        pGroupId * PannelParameters::LocalThreadSizeP + pLocalThreadId;
    const Index globalRId =
        rGroupId * PannelParameters::LocalThreadSizeR + rLocalThreadId;
    auto scratchPtr = scratch.get_pointer().get();
    auto outPtr =
        output_accessor.get_pointer() + (reduce_elements_num_groups > 1
                                             ? rGroupId * num_coeffs_to_preserve
                                             : 0);
    CoeffReturnType accumulator = op.initialize();
    ...
    accumulator =
        OpDef::finalise_op(op.finalize(accumulator), num_coeffs_to_reduce);
    scratchPtr[pLocalThreadId +
               rLocalThreadId * (PannelParameters::LocalThreadSizeP +
                                 PannelParameters::BC)] = accumulator;
    ...
    EIGEN_UNROLL_LOOP
    for (Index offset = PannelParameters::LocalThreadSizeR >> 1; offset > 0;
         offset >>= 1) {
      if (rLocalThreadId < offset) {
        op.reduce(out_scratch_ptr[(PannelParameters::LocalThreadSizeP +
                                   PannelParameters::BC) *
                                  offset],
                  &accumulator);
...
template <typename OutScalar, typename Index, typename InputAccessor,
          typename OutputAccessor, typename OpType>
struct SecondStepPartialReduction {
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE SecondStepPartialReduction(
      ScratchAccessor, InputAccessor input_accessor_,
...
struct PartialReducerLauncher {
  ...
  typedef ReductionPannel<typename Self::Index, EIGEN_SYCL_LOCAL_THREAD_DIM0,
                          EIGEN_SYCL_LOCAL_THREAD_DIM1, true>
      PannelParameters;

  typedef PartialReductionKernel<Self, Op, PannelParameters, rt>
      SyclReducerKerneType;

  static bool run(const Self &self, const Op &reducer,
                  const Eigen::SyclDevice &dev, EvaluatorPointerType output,
                  Index num_coeffs_to_reduce, Index num_coeffs_to_preserve) {
    Index roundUpP =
        roundUp(num_coeffs_to_preserve, PannelParameters::LocalThreadSizeP);
    ...
    EIGEN_CONSTEXPR Index localRange =
        PannelParameters::LocalThreadSizeP * PannelParameters::LocalThreadSizeR;
    ...
    const Index rNumGroups =
        num_coeffs_to_reduce > reductionPerThread * localRange
            ? std::min(rGroups, localRange)
            : 1;
    const Index globalRange = pNumGroups * rNumGroups * localRange;

    EIGEN_CONSTEXPR Index scratchSize =
        PannelParameters::LocalThreadSizeR *
        (PannelParameters::LocalThreadSizeP + PannelParameters::BC);
    auto thread_range = cl::sycl::nd_range<1>(cl::sycl::range<1>(globalRange),
                                              cl::sycl::range<1>(localRange));
    if (rNumGroups > 1) {
      CoeffReturnType *temp_pointer =
          static_cast<CoeffReturnType *>(dev.allocate_temp(
              num_coeffs_to_preserve * rNumGroups * sizeof(CoeffReturnType)));
      ...
      dev.template unary_kernel_launcher<CoeffReturnType, SyclReducerKerneType>(
          self, temp_accessor, thread_range, scratchSize, reducer, pNumGroups,
          rNumGroups, num_coeffs_to_preserve, num_coeffs_to_reduce);

      typedef SecondStepPartialReduction<CoeffReturnType, Index,
                                         EvaluatorPointerType,
                                         EvaluatorPointerType, Op>
          SecondStepPartialReductionKernel;

      dev.template unary_kernel_launcher<CoeffReturnType,
                                         SecondStepPartialReductionKernel>(
          temp_accessor, output,
          cl::sycl::nd_range<1>(cl::sycl::range<1>(pNumGroups * localRange),
                                cl::sycl::range<1>(localRange)),
      ...
      dev.template unary_kernel_launcher<CoeffReturnType, SyclReducerKerneType>(
          self, output, thread_range, scratchSize, reducer, pNumGroups,
          rNumGroups, num_coeffs_to_preserve, num_coeffs_to_reduce);
    }
    return false;
  }
...
struct FullReducer<Self, Op, Eigen::SyclDevice, Vectorizable> {
  ...
  static void run(const Self &self, Op &reducer, const Eigen::SyclDevice &dev,
    ...
    Index reductionGroup =
        dev.getPowerOfTwo((inputSize + (reductionPerThread * local_range - 1)) /
                              (reductionPerThread * local_range),
                          true);
    ...
    typedef TensorSycl::internal::FullReductionKernelFunctor<Self, Op,
                                                             local_range>
        reduction_kernel_t;
    if (num_work_group > 1) {
      CoeffReturnType *temp_pointer = static_cast<CoeffReturnType *>(
          dev.allocate_temp(num_work_group * sizeof(CoeffReturnType)));
      typename Self::EvaluatorPointerType tmp_global_accessor =
          dev.get(temp_pointer);
      dev.template unary_kernel_launcher<OutType, reduction_kernel_t>(
          self, tmp_global_accessor, thread_range, local_range, inputSize,
          reducer);
      ...
      dev.template unary_kernel_launcher<CoeffReturnType, GenericRKernel>(
          ...
          cl::sycl::nd_range<1>(cl::sycl::range<1>(num_work_group),
                                cl::sycl::range<1>(num_work_group)),
          num_work_group, reducer);

      dev.deallocate_temp(temp_pointer);
    } else {
      dev.template unary_kernel_launcher<OutType, reduction_kernel_t>(
          self, data, thread_range, local_range, inputSize, reducer);
    }
  }
};
...
struct OuterReducer<Self, Op, Eigen::SyclDevice> {
  ...
  static bool run(const Self &self, const Op &reducer,
    ...
    return ::Eigen::TensorSycl::internal::PartialReducerLauncher<
        Self, Op, ::Eigen::TensorSycl::internal::reduction_dim::outer_most>::
        run(self, reducer, dev, output, num_coeffs_to_reduce,
...
struct InnerReducer<Self, Op, Eigen::SyclDevice> {
  ...
  static bool run(const Self &self, const Op &reducer,
                  ...
                  typename Self::Index num_coeffs_to_preserve) {
    return ::Eigen::TensorSycl::internal::PartialReducerLauncher<
        Self, Op, ::Eigen::TensorSycl::internal::reduction_dim::inner_most>::
        run(self, reducer, dev, output, num_coeffs_to_reduce,
            num_coeffs_to_preserve);
  }
};
...
struct GenericReducer<Self, Op, Eigen::SyclDevice> {
  ...
  static bool run(const Self &self, const Op &reducer,
    ...
    dev.template unary_kernel_launcher<
        ...
        Index(1), reducer, range,
        (num_values_to_reduce != 0) ? num_values_to_reduce
                                    : static_cast<Index>(1));
    return false;
  }
};
