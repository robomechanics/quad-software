 ...
 * method of the given Op. This allows the reduction to have a different type
 * for the accumulator than the input data type. If this is the case, the
 * functor needs to have two reduce method: one for reducing an element of the
 * input with the accumulator and the other for reducing two accumulators. Such
 * a reducer can be useful for instance when the accumulator is a boolean or a
 * bitset that checks for some properties of the input.
 */
...
struct TensorReductionEvaluatorBase<
  ...
  EIGEN_STRONG_INLINE TensorReductionEvaluatorBase(const XprType& op,
                                                   const Device& device)
...
struct TensorEvaluator<const TensorReductionOp<Op, Dims, ArgType, MakePointer_>,
  ...
  typedef TensorReductionEvaluatorBase<
      const TensorReductionOp<Op, Dims, ArgType, MakePointer_>, Device>
      Base;
  EIGEN_STRONG_INLINE TensorEvaluator(const typename Base::XprType& op,
                                      const Device& device)
      : Base(op, device) {}
};
