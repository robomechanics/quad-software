...
struct ALPAQA_EXPORT ALMParams {
  ...
  /// Minimum value for @ref penalty_update_factor after reduction because of
  /// inner convergence failure.
  ...
  /// Maximum value of @ref tolerance_update_factor after increase because of
  /// inner convergence failure.
  ...
  /// @ref initial_penalty_factor and the initial primal tolerance @ref
  /// initial_tolerance be reduced.
  unsigned max_num_initial_retries = 0;
  /// How many times can the penalty update factor @ref penalty_update_factor
  /// and the primal tolerance factor @ref tolerance_update_factor be reduced.
...
class ALPAQA_EXPORT ALMSolver {
 public:
  ...
  struct Stats {
    ...
    /// increased by @ref ALMParams::initial_tolerance_increase because the
    /// inner solver failed to converge in the first ALM iteration. If this
    /// number is greater than zero, consider lowering the initial penalty
    /// factor
    /// @ref ALMParams::initial_penalty or @ref
    /// ALMParams::initial_penalty_factor or increasing the initial tolerance
    /// @ref ALMParams::initial_tolerance (or both).
    unsigned initial_penalty_reduced = 0;
    /// The number of times that the penalty update factor @ref
    /// ALMParams::penalty_update_factor was reduced, that the tolerance update
    /// factor @ref ALMParams::tolerance_update_factor was increased, and that
    /// an ALM iteration had to be restarted with a lower penalty factor and a
    /// higher tolerance because the inner solver failed to converge (not
    /// applicable in the first ALM iteration). If this number is greater than
    /// zero, consider lowerering the penalty update factor @ref
    /// ALMParams::penalty_update_factor or increasing the tolerance update
    /// factor (or both). Lowering the initial penalty factor could help as
    /// well.
  ...
  ALMSolver(Params params, InnerSolver &&inner_solver)
      : params(params), inner_solver(std::forward<InnerSolver>(inner_solver)) {}
