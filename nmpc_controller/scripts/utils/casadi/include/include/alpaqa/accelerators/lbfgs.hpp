...
struct LBFGSStorage {
  ...
  auto s(index_t i) const { return std::as_const(sto).col(2 * i).topRows(n()); }
...
class ALPAQA_EXPORT LBFGS {
 public:
  ...
  void foreach_fwd(const F &fun) const {
    if (full)
      for (index_t i = idx; i < history(); ++i) fun(i);
    if (idx)
      for (index_t i = 0; i < idx; ++i) fun(i);
  }
  ...
  void foreach_rev(const F &fun) const {
    if (idx)
      for (index_t i = idx; i-- > 0;) fun(i);
    if (full)
      for (index_t i = history(); i-- > idx;) fun(i);
  }
