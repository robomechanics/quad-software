...
struct SteihaugCG {
  ...
  real_t solve(const auto &grad, const HessFun &hess_prod, real_t trust_radius,
               rvec step) const {
    ...
    real_t tolerance =
        std::fmin(params.tol_max,
                  params.tol_scale * grad_mag *
                      std::fmin(params.tol_scale_root, std::sqrt(grad_mag)));
    ...
    while (true) {
      ...
      if (dBd <= 0) {
        ...
        auto [ta, tb] = get_boundaries_intersections(z, d, trust_radius);
      ...
      if (std::sqrt(r_next_sq) < tolerance || i > max_iter) return eval(s);
