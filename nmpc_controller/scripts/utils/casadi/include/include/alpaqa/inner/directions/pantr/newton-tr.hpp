...
struct NewtonTRDirection {
  ...
  void initialize(const Problem &problem, [[maybe_unused]] crvec y,
    ...
    if (!direction_params.finite_diff && !problem.provides_eval_hess_ψ_prod())
  ...
  real_t apply([[maybe_unused]] real_t γₖ, [[maybe_unused]] crvec xₖ,
               ...
               [[maybe_unused]] crvec grad_ψxₖ, real_t radius, rvec qₖ) const {
    if (!std::isfinite(radius)) throw std::logic_error("Invalid trust radius");
    ...
    if (direction_params.hessian_vec_factor != 0) {
      if (direction_params.finite_diff) {
        real_t ε =
            (1 + grad_ψxₖ.norm()) * direction_params.finite_diff_stepsize;
        work = xₖ + ε * qₖ;
        problem->eval_grad_ψ(work, *y, *Σ, work_2, work_n_fd, work_m_fd);
        rJ.noalias() +=
            (work_2 - grad_ψxₖ)(J) * (direction_params.hessian_vec_factor / ε);
      } else {
