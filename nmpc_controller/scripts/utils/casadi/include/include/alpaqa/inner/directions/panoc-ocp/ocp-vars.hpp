...
struct OCPVariables {
  ...
  auto xuk(VectorRefLike<config_t> auto &&v, index_t t) const {
    return const_or_mut_rvec<config_t>(v.segment(t * indices.back(), nxu()));
  }
  ...
  auto ck(VectorRefLike<config_t> auto &&v, index_t t) const {
    return v.segment(
        t * indices.back() + (t < N ? indices[i_c] : indices_N[i_c_N]),
        (t < N ? nc() : nc_N()));
  }
  ...
  rmat ABk(rmat AB, index_t t) const { return AB.middleCols(t * nxu(), nxu()); }
  auto ABk(mat &AB, index_t t) const { return AB.middleCols(t * nxu(), nxu()); }
  ...
  crmat Ak(crmat AB, index_t t) const { return AB.middleCols(t * nxu(), nx()); }
...
struct OCPEvaluator {
  ...
  void forward_simulate(rvec storage) const {
    for (index_t t = 0; t < N(); ++t) {
      ...
      if (vars.nc() > 0) problem->eval_constr(t, xk, ck);
      problem->eval_f(t, xk, uk, vars.xk(storage, t + 1));
    }
    auto xN = vars.xk(storage, N());
    auto cN = vars.ck(storage, N());
    ...
    if (vars.nc_N() > 0) problem->eval_constr_N(xN, cN);
  }
    ...
    if (nc > 0 || nc_N > 0) {
      ...
      if (k < N) {
        for (index_t i = 0; i < nc; ++i)
          work_ck(i) =
              μk(i) * (ζ(i) < D.lowerbound(i) || ζ(i) > D.upperbound(i));
        problem->eval_add_gn_hess_constr(k, xk, work_ck, out);
      } else {
        for (index_t i = 0; i < nc_N; ++i)
          work_cN(i) =
              μk(i) * (ζ(i) < D_N.lowerbound(i) || ζ(i) > D_N.upperbound(i));
  ...
  void Rk_prod(crvec storage, index_t k, crindexvec mask_J, crindexvec mask_K,
    ...
    auto hk = vars.hk(storage, k);
    auto xuk = vars.xuk(storage, k);
    problem->eval_add_R_prod_masked(k, xuk, hk, mask_J, mask_K, v, out, work_R);
    check_finiteness(out.reshaped(), "Rk_prod output");
  }
  auto R_prod(crvec storage) const {
    return [=, this](index_t k) {
      return
          [=, this](crindexvec mask_J, crindexvec mask_K, crvec v, rvec out) {
