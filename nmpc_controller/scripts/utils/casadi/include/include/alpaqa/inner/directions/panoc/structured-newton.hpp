...
struct StructuredNewtonDirection {
  ...
  bool apply(real_t γₖ, crvec xₖ, [[maybe_unused]] crvec x̂ₖ, crvec pₖ,
    ...
    problem->eval_hess_ψ(xₖ, *y, *Σ, 1, inner_idx_H, outer_ptr_H, H_storage);
    ...
    if (nJ == n) {
      ...
      // Find the minimum eigenvalue to regularize the Hessian matrix and
      // make it positive definite.
      Eigen::SelfAdjointEigenSolver<mat> eig{H, Eigen::ComputeEigenvectors};
  ...
  std::string get_name() const {
    return "StructuredNewtonDirection<" + std::string(config_t::get_name()) +
           '>';
  }
