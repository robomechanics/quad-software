...
bool LBFGS<Conf>::update_valid(const Params &params, real_t yᵀs, real_t sᵀs,
  ...
  if (sᵀs <= params.min_abs_s) return false;
  if (not std::isfinite(yᵀs)) return false;
  real_t a_yᵀs = params.force_pos_def ? yᵀs : std::abs(yᵀs);
  if (a_yᵀs <= params.min_div_fac * sᵀs) return false;
  ...
  if (params.cbfgs) {
    ...
    if (not cbfgs_cond) return false;
  }
...
bool LBFGS<Conf>::update_sy_impl(const auto &s, const auto &y,
  ...
  if (not forced) {
    ...
    if (not update_valid(params, yᵀs, sᵀs, pₙₑₓₜᵀpₙₑₓₜ)) return false;
  }
...
bool LBFGS<Conf>::apply(rvec q, real_t γ) const {
  ...
  if (idx == 0 && not full) return false;
...
bool LBFGS<Conf>::apply_masked_impl(rvec q, real_t γ, const auto &J) const {
  ...
  if (params.stepsize == LBFGSStepSize::BasedOnCurvature) γ = -1;
  ...
  foreach_fwd([&](index_t i) {
    if (std::isnan(ρ(i))) return;  // continue foreach
