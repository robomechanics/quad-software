...
void assert_key_empty(ParamString s) {
  if (!s.key.empty())
    throw invalid_param("Type '" + demangled_typename(typeid(T)) +
                        "' cannot be indexed in '" + std::string(s.full_key) +
                        "'");
}
...
auto possible_keys() {
  ...
  if (tbl.empty()) return std::string{};
...
void set_param(T &t, ParamString s) {
  ...
  auto it = m.find(key);
  if (it == m.end())
    throw invalid_param("Invalid key '" + std::string(key) + "' for type '" +
                        demangled_typename(typeid(T)) + "' in '" +
                        std::string(s.full_key) +
