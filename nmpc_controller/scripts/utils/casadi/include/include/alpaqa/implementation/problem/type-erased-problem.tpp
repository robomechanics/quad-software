...
template <Config Conf>
auto ProblemVTable<Conf>::default_eval_inactive_indices_res_lna(
    const void *, real_t, crvec, crvec, rindexvec,
...
void ProblemVTable<Conf>::default_eval_jac_g(const void *, crvec, rindexvec,
  ...
  if (vtable.m != 0) throw not_implemented_error("eval_jac_g");
}

template <Config Conf>
auto ProblemVTable<Conf>::default_get_jac_g_num_nonzeros(
    const void *, const ProblemVTable &) -> length_t {
  return -1;
}
...
template <Config Conf>
auto ProblemVTable<Conf>::default_get_hess_L_num_nonzeros(
    const void *, const ProblemVTable &) -> length_t {
  return -1;
}
...
void ProblemVTable<Conf>::default_eval_hess_ψ_prod(
    ...
    const ProblemVTable &vtable) {
  if (y.size() == 0 &&
      vtable.eval_hess_L_prod != ProblemVTable<Conf>::default_eval_hess_L_prod)
...
void ProblemVTable<Conf>::default_eval_hess_ψ(const void *self, crvec x,
  ...
  if (y.size() == 0 && vtable.eval_hess_L != default_eval_hess_L)
    return vtable.eval_hess_L(self, x, y, scale, inner_idx, outer_ptr, H_values,
                              vtable);
  throw not_implemented_error("eval_hess_ψ");
}

template <Config Conf>
auto ProblemVTable<Conf>::default_get_hess_ψ_num_nonzeros(
    const void *, const ProblemVTable &) -> length_t {
  return 0;
}
...
auto ProblemVTable<Conf>::default_eval_f_grad_f(
    const void *self, crvec x, rvec grad_fx,
    const ProblemVTable &vtable) -> real_t {
...
auto ProblemVTable<Conf>::default_eval_f_g(
...
auto ProblemVTable<Conf>::default_eval_ψ(
    const void *self, crvec x, crvec y, crvec Σ, rvec ŷ,
    const ProblemVTable &vtable) -> real_t {
  if (y.size() == 0) /* [[unlikely]] */
    return vtable.eval_f(self, x);
...
auto ProblemVTable<Conf>::default_eval_ψ_grad_ψ(
    const void *self, crvec x, crvec y, crvec Σ, rvec grad_ψ, rvec work_n,
    rvec work_m, const ProblemVTable &vtable) -> real_t {
  if (y.size() == 0) /* [[unlikely]] */
