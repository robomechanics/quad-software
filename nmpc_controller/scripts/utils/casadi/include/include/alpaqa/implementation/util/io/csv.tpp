...
struct CSVReader {
    ...
    if (keep_reading) {
      if (!is.get(s.data() + bufidx, bufmaxsize - bufidx, end))
        throw read_error(
            ...
            " " + std::to_string(is.fail()) + " " + std::to_string(is.eof()));
      bufidx += is.gcount();
  ...
  static const char *read_single(const char *bufbegin, const char *bufend,
                                 F &v) {
    if (bufbegin != bufend && *bufbegin == '+') ++bufbegin;
  ...
  static const char *read_single(const char *bufbegin, char *bufend, F &v) {
    *bufend = '\0';
    ...
    if (errno || ptr == bufbegin)
      throw read_error("csv::read_row conversion failed '" +
                       std::string(bufbegin) + "': " + std::to_string(errno));
    return ptr;
  }
#endif
...
void read_row_impl(std::istream &is, Eigen::Ref<Eigen::VectorX<F>> v,
                   char sep) {
  CSVReader<F> reader;
  for (auto &vv : v) vv = reader.read(is, sep);
  reader.check_end(is);
}
...
std::vector<F> read_row_std_vector(std::istream &is, char sep) {
  CSVReader<F> reader;
  std::vector<F> v;
  while (!reader.done(is)) v.push_back(reader.read(is, sep));
  reader.check_end(is);
  return v;
}
