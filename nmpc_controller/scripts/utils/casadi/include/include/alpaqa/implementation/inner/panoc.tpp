...
auto PANOCSolver<DirectionProviderT>::operator()(
  ...
  while (true) {
    ...
    real_t εₖ = Helpers::calc_error_stop_crit(
        ...
        curr->grad_ψ, grad_ψx̂, work_n, next->p);
    ...
    if (k == 0) {  // Initialize L-BFGS
      ScopedMallocAllower ma;
      direction.initialize(problem, y, Σ, curr->γ, curr->x, curr->x̂, curr->p,
                           curr->grad_ψ);
      τ_init = 0;
    }
    if (k > 0 || direction.has_initial_direction()) {
      τ_init =
          direction.apply(curr->γ, curr->x, curr->x̂, curr->p, curr->grad_ψ, q)
              ? 1
              : 0;
