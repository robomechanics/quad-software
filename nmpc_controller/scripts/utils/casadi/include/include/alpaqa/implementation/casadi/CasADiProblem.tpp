...
struct CasADiFunctionsWithParam {
  ...
  std::optional<CasADiFunctionEvaluator<Conf, 5, 1>> hess_L_prod = std::nullopt;
  std::optional<CasADiFunctionEvaluator<Conf, 4, 1>> hess_L = std::nullopt;
  std::optional<CasADiFunctionEvaluator<Conf, 8, 1>> hess_ψ_prod = std::nullopt;
...
CasADiProblem<Conf>::CasADiProblem(const std::string &so_name)
  ...
  auto load_g_unknown_dims =
    ...
    if (gfun.n_in() != 2)
      throw std::invalid_argument("Invalid number of input arguments: got "s +
                                  std::to_string(gfun.n_in()) +
                                  ", should be 2.");
    if (gfun.n_out() > 1)
      throw std::invalid_argument("Invalid number of output arguments: got "s +
                                  std::to_string(gfun.n_in()) +
                                  ", should be 0 or 1.");
    ...
    if (gfun.n_out() == 1) m = static_cast<length_t>(gfun.size1_out(0));
    p = static_cast<length_t>(gfun.size1_in(1));
    if (gfun.n_out() == 0) {
      if (m != 0)
        throw std::invalid_argument("Function g has no outputs but m != 0");
      return std::nullopt;
    }
  ...
  if (fs::exists(data_filepath)) load_numerical_data(data_filepath);
}
...
void CasADiProblem<Conf>::load_numerical_data(
  ...
  if (!data_file)
    throw std::runtime_error("Unable to open data file \"" + filepath.string() +
                             '"');
...
template <Config Conf>
CasADiProblem<Conf> &CasADiProblem<Conf>::operator=(const CasADiProblem &) =
    default;
template <Config Conf>
CasADiProblem<Conf>::CasADiProblem(CasADiProblem &&) noexcept = default;
template <Config Conf>
CasADiProblem<Conf> &CasADiProblem<Conf>::operator=(CasADiProblem &&) noexcept =
    default;
...
void CasADiProblem<Conf>::eval_g(crvec x, rvec g) const {
  if (impl->constr) impl->constr->g({x.data(), param.data()}, {g.data()});
}
...
template <Config Conf>
typename CasADiProblem<Conf>::real_t CasADiProblem<Conf>::eval_ψ_grad_ψ(
    crvec x, crvec y, crvec Σ, rvec grad_ψ, rvec, rvec) const {
  real_t ψ;
  impl->ψ_grad_ψ({x.data(), param.data(), y.data(), Σ.data(),
                  this->D.lowerbound.data(), this->D.upperbound.data()},
                 {&ψ, grad_ψ.data()});
...
void CasADiProblem<Conf>::eval_grad_L(crvec x, crvec y, rvec grad_L,
                                      rvec) const {
  if (impl->constr)
    impl->constr->grad_L({x.data(), param.data(), y.data()}, {grad_L.data()});
  else
    eval_f_grad_f(x, grad_L);
}

template <Config Conf>
typename CasADiProblem<Conf>::real_t CasADiProblem<Conf>::eval_ψ(crvec x,
                                                                 crvec y,
                                                                 crvec Σ,
                                                                 rvec ŷ) const {
  real_t ψ;
...
auto CasADiProblem<Conf>::get_jac_g_num_nonzeros() const -> length_t {
  if (!impl->jac_g.has_value()) return -1;
...
void CasADiProblem<Conf>::eval_jac_g(crvec x, rindexvec inner_idx,
  ...
  if (J_values.size() > 0) {
    ...
    if (!sparsity.is_dense()) {
      ...
      std::transform(sparsity.colind(), sparsity.colind() + this->get_n() + 1,
...
auto CasADiProblem<Conf>::get_hess_L_num_nonzeros() const -> length_t {
  if (!impl->hess_L.has_value()) return -1;
...
void CasADiProblem<Conf>::eval_hess_ψ_prod(crvec x, crvec y, crvec Σ,
  ...
  (*impl->hess_ψ_prod)(
      {x.data(), param.data(), y.data(), Σ.data(), &scale,
       this->D.lowerbound.data(), this->D.upperbound.data(), v.data()},
      {Hv.data()});
}

template <Config Conf>
auto CasADiProblem<Conf>::get_hess_ψ_num_nonzeros() const -> length_t {
  if (!impl->hess_ψ.has_value()) return 0;
