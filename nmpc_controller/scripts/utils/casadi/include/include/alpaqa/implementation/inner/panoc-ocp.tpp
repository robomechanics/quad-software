...
auto PANOCOCPSolver<Conf>::operator()(
  ...
  struct Iterate {
    ...
    Iterate(const OCPVariables<config_t> &vars, bool enable_lbfgs)
        : xu{vars.create()},
          xû{vars.create()},
          grad_ψ{vars.N * vars.nu()},
          p{vars.N * vars.nu()},
          u{enable_lbfgs ? vars.N * vars.nu() : 0} {}
  } iterates[2]{
      {vars, enable_lbfgs},
      {vars, enable_lbfgs},
  };
  ...
  auto eval_prox_impl = [&](real_t γ, crvec xu, crvec grad_ψ, rvec x̂u, rvec p) {
  ...
  auto calc_error_stop_crit = [this, &eval_prox_impl](
                                  real_t γ, crvec xuₖ, crvec grad_ψₖ, crvec pₖ,
                                  real_t pₖᵀpₖ, rvec work_xu, rvec work_p) {
    switch (params.stop_crit) {
      ...
      case PANOCStopCrit::ProjGradUnitNorm2: {
        auto [pTp, gTp] = eval_prox_impl(1, xuₖ, grad_ψₖ, work_xu, work_p);
        return std::sqrt(pTp);
      }
  ...
  auto check_all_stop_conditions =
      [this, &opts](
        ...
        if (opts.max_time) max_time = std::min(max_time, *opts.max_time);
  ...
  auto linesearch_violated = [this](const Iterate &curr, const Iterate &next) {
  ...
  auto initial_lipschitz_estimate =
      [&](
        ...
        // work_xu = xu - h
        for (index_t t = 0; t < N; ++t)
          vars.uk(work_xu, t) = vars.uk(it->xu, t) - h.segment(t * nu, nu);
        ...
        {  // Calculate ∇ψ(x₀ + h)
          alpaqa::util::Timed t{s.time_backward};
          eval.backward(work_xu, work_grad_ψ, mut_qrk, mut_q_N, D, D_N, μ, y);
        }
  ...
  auto print_real3 = [&](real_t x) { return float_to_str_vw(print_buf, x, 3); };
  ...
  auto print_progress_n = [&](SolverStatus status) {
    *os << "└─ " << status << " ──" << std::endl;  // Flush for Python buffering
  };

  auto do_progress_cb = [this, &s, &problem, &lqr, &opts](
                            unsigned k, Iterate &curr, crvec q, real_t τ,
  ...
  while (true) {
    ...
    real_t εₖ = calc_error_stop_crit(curr->γ, curr->xu, curr->grad_ψ, curr->p,
                                     curr->pᵀp, next->xû, next->p);
    ...
    if (stop_status != SolverStatus::Busy) {
      ...
      if (!do_print && do_final_print)
        print_progress_1(k, curr->fbe(), curr->ψu, curr->grad_ψ, curr->pᵀp,
                         curr->γ, εₖ);
      if (do_print || do_final_print) print_progress_n(stop_status);
    ...
    if (params.disable_acceleration) {
      ...
      {  // evaluate the Jacobians
        ...
        for (index_t t = 0; t < N; ++t)
          problem.eval_jac_f(t, vars.xk(curr->xu, t), vars.uk(curr->xu, t),
                             vars.ABk(jacs, t));
      }
      {  // LQR factor
        alpaqa::util::Timed t{s.time_lqr_factor};
        lqr.factor_masked(ABk, Qk(curr->xu), Rk(curr->xu), Sk(curr->xu),
                          Rk_prod(curr->xu), Sk_prod(curr->xu), qk, rk, uk_eq,
                          Jk, Kk, params.lqr_factor_cholesky);
      }
      {  // LQR solve
      ...
      if (!enable_lbfgs) throw std::logic_error("enable_lbfgs");
      ...
      nJ = 0;
      {
        ...
        for (index_t t = 0; t < N; ++t)
          for (index_t i = 0; i < nu; ++i)
            if (is_constr_inactive(t, i)) J_idx(nJ++) = t * nu + i;
      }
    ...
    auto take_accelerated_step = [&](real_t τ) {
      if (τ == 1) {
        for (index_t t = 0; t < N; ++t)
          vars.uk(next->xu, t) = vars.uk(curr->xu, t) + q.segment(t * nu, nu);
      } else {
        do_gn_step = do_next_gn;
        for (index_t t = 0; t < N; ++t)
          vars.uk(next->xu, t) = vars.uk(curr->xu, t) +
    ...
    while (!stop_signal.stop_requested()) {
      ...
      if (τ > 0 && linesearch_violated(*curr, *next)) {
        τ /= 2;
        if (τ < params.min_linesearch_coefficient) τ = 0;
    ...
    if (enable_lbfgs) {
      ...
      if (!reset_because_gn) {  // TODO: this may be too restrictive
        ...
        s.lbfgs_rejected += dir_rejected =
            not lbfgs.update(curr->u, next->u, curr->grad_ψ, next->grad_ψ,
