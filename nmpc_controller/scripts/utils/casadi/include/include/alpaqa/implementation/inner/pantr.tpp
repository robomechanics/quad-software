...
auto PANTRSolver<DirectionProviderT>::operator()(
  ...
  if (params.Lipschitz.L_0 <= 0) {
    curr->L = Helpers::initial_lipschitz_estimate(
        ...
        /* in ⟹ out */ curr->ψx, curr->grad_ψ, curr->x̂, cand->grad_ψ, work_n,
        work_m);
  }
  ...
  if (!std::isfinite(Δ) || Δ == 0) Δ = real_t(0.1) * curr->grad_ψ.norm();
  ...
  while (true) {
    ...
    // Calculate ∇ψ(x̂ₖ)
    if (need_grad_ψx̂) eval_grad_ψx̂(*curr, grad_ψx̂);
    bool have_grad_ψx̂ = need_grad_ψx̂;

    real_t εₖ = Helpers::calc_error_stop_crit(
        problem, params.stop_crit, curr->p, curr->γ, curr->x, curr->x̂, curr->ŷx̂,
        curr->grad_ψ, grad_ψx̂, work_n, cand->p);
    ...
    if (stop_status != SolverStatus::Busy) {
      ...
      if (stop_status == SolverStatus::Converged ||
        ...
        auto &ŷ = curr->ŷx̂;
        if (err_z.size() > 0) err_z = Σ.asDiagonal().inverse() * (ŷ - y);
        x = std::move(curr->x̂);
        y = std::move(curr->ŷx̂);
      }
      // Save statistics
    ...
    auto compute_FBS_step = [&] {
      ...
      // Calculate ∇ψ(x̂ₖ)
      if (not have_grad_ψx̂) eval_grad_ψx̂(*curr, grad_ψx̂);
      have_grad_ψx̂ = true;
      prox->x = curr->x̂;
      prox->ψx = curr->ψx̂;
    ...
    if (k == 0) {
      ScopedMallocAllower ma;
      direction.initialize(problem, y, Σ, prox->γ, prox->x, prox->x̂, prox->p,
                           prox->grad_ψ);
    }
    ...
    if (accelerated_iteration && !params.disable_acceleration) {
      if (auto q_model = compute_trust_region_step(q, Δ); q_model < 0) {
        ...
        Δ = std::fmax(compute_updated_radius(q, ρ, Δ), params.min_radius);
      }
    }
    ...
    if (accept_candidate) {
      ...
      s.direction_update_rejected +=
          not direction.update(prox->γ, cand->γ, prox->x, cand->x, prox->p,
                               cand->p, prox->grad_ψ, cand->grad_ψ);

      if (do_print) print_progress_2(q, ρ, true, direction_duration);
      // Candidate becomes new iterate
      std::swap(curr, cand);
    }
    // Fall back to proximal gradient step
    else {
      if (accelerated_iteration) ++s.accelerated_step_rejected;
      ...
      if (params.update_direction_on_prox_step)
        s.direction_update_rejected +=
            not direction.update(curr->γ, prox->γ, curr->x, prox->x, curr->p,
                                 prox->p, curr->grad_ψ, prox->grad_ψ);
