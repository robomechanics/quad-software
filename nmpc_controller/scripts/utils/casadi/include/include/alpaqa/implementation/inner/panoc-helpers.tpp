...
struct PANOCHelpers {
  ...
  static real_t calc_error_stop_crit(
    ...
    switch (crit) {
      ...
      case PANOCStopCrit::ProjGradUnitNorm: {
        problem.eval_prox_grad_step(real_t(1), xₖ, grad_ψₖ, work_n1, work_n2);
      ...
      case PANOCStopCrit::Ipopt: {
        ...
        problem.eval_prox_grad_step(real_t(1), x̂ₖ, grad_̂ψₖ, work_n1, work_n2);
        ...
        real_t s_d = std::max(s_max, (C_lagr_mult + D_lagr_mult) / s_n) / s_max;
        return err / s_d;
      }
      case PANOCStopCrit::LBFGSBpp: {
        problem.eval_prox_grad_step(real_t(1), xₖ, grad_ψₖ, work_n1, work_n2);
        return vec_util::norm_inf(work_n2) / std::fmax(real_t(1), xₖ.norm());
      }
      default:;
    }
    throw std::out_of_range("Invalid PANOCStopCrit");
  ...
  /// @f] The projected gradient iterate @f$ \hat x^k @f$ and step @f$ p^k @f$
  /// are updated with the new step size @f$ \gamma^k @f$, and so are other
  /// quantities that depend on them, such as @f$ \nabla\psi(x^k)^\top p^k @f$
  /// and
  /// @f$ \|p\|^2 @f$. The intermediate vector @f$ \hat y(x^k) @f$ can be used
  /// to compute the gradient @f$ \nabla\psi(\hat x^k) @f$ or to update the
  ...
  static SolverStatus check_all_stop_conditions(
      /// [in]    Parameters including `max_iter`, `max_time` and
      /// `max_no_progress`
  ...
  static real_t initial_lipschitz_estimate(
      ...
      rvec work_m) {
    // Calculate ψ(x₀), ∇ψ(x₀)
    ψ = problem.eval_ψ_grad_ψ(x, y, Σ, /* in ⟹ out */ grad_ψ, work_n, work_m);
  ...
  static real_t initial_lipschitz_estimate(
    ...
    real_t norm_h = h.norm();
    // Calculate ∇ψ(x_0 + h)
    problem.eval_grad_ψ(work_n1, y, Σ, /* in ⟹ out */ work_n2, work_n3, work_m);
    // Calculate ∇ψ(x_0)
