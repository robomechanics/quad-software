...
auto ZeroFPRSolver<DirectionProviderT>::operator()(
  ...
  auto linesearch_violated = [this](const Iterate &curr, const Iterate &next) {
    if (params.force_linesearch) return false;
  ...
  auto eval_prox_grad_step_in_prox = [&problem, &prox](const Iterate &i) {
    prox->hx̂ =
        problem.eval_prox_grad_step(i.γ, i.x̂, prox->grad_ψ, prox->x̂, prox->p);
  ...
  if (params.Lipschitz.L_0 <= 0) {
    curr->L = Helpers::initial_lipschitz_estimate(
        ...
        /* in ⟹ out */ curr->ψx, curr->grad_ψ, curr->x̂, next->grad_ψ, work_n,
        work_m);
  }
  ...
  while (true) {
    ...
    real_t εₖ = Helpers::calc_error_stop_crit(
        ...
        curr->grad_ψ, prox->grad_ψ, work_n, next->p);
    ...
    if (k == 0) {  // Initialize L-BFGS
      ScopedMallocAllower ma;
      direction.initialize(problem, y, Σ, curr->γ, curr->x̂, prox->x̂, prox->p,
                           prox->grad_ψ);
      τ_init = 0;
    }
    if (k > 0 || direction.has_initial_direction()) {
      τ_init =
          direction.apply(curr->γ, curr->x̂, prox->x̂, prox->p, prox->grad_ψ, q)
              ? 1
              : 0;
      // Make sure quasi-Newton step is valid
      if (τ_init == 1 && not q.allFinite()) τ_init = 0;
      ...
      if (update_lbfgs_in_linesearch && !updated_lbfgs) {
        if (params.update_direction_from_prox_step) {
          s.lbfgs_rejected += dir_rejected =
              not direction.update(curr->γ, next->γ, curr->x̂, next->x, prox->p,
                                   next->p, prox->grad_ψ, next->grad_ψ);
        } else {
          s.lbfgs_rejected += dir_rejected =
