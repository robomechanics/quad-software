...
typename ALMSolver<InnerSolverT>::Stats ALMSolver<InnerSolverT>::operator()(
    const Problem &p, rvec x, rvec y) {
  ...
  for (unsigned i = 0; i < params.max_iter; ++i) {
    ...
    if (params.print_interval != 0 && i % params.print_interval == 0) {
      ...
      *os << "[\x1b[0;34mALM\x1b[0m]   " << std::setw(5) << i
          << ": ‖Σ‖ = " << print_real(Σ.norm())
          << ", ‖y‖ = " << print_real(y.norm()) << ", δ = " << print_real(δ)
          << ", ε = " << print_real(ps.ε) << ", Δ = " << print_real(Δ)
          << ", status = " << color << std::setw(13) << ps.status << color_end
    ...
    if (backtrack) {
      ...
      if (num_successful_iters > 0) {
        ...
        Helpers::update_penalty_weights(params, Δ, false, error_1, error_2,
    ...
    // If the inner solver did converge, increase penalty
      ...
      Helpers::update_penalty_weights(params, Δ, num_successful_iters == 0,
