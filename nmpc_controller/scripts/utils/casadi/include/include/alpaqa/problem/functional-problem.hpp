...
class FunctionalProblem : public BoxConstrProblem<Conf> {
 public:
  ...
  void eval_jac_g(crvec x, rindexvec, rindexvec, rvec J_values) const {
    ...
    if (J_values.size() > 0) jac_g(x, J_values.reshaped(this->m, this->n));
  }
  void eval_hess_L(crvec x, crvec y, real_t scale, rindexvec, rindexvec,
                   rvec H_values) const {
    ScopedMallocAllower ma;
    if (H_values.size() > 0)
      hess_L(x, y, scale, H_values.reshaped(this->n, this->n));
  }
  void eval_hess_ψ(crvec x, crvec y, crvec Σ, real_t scale, rindexvec,
                   rindexvec, rvec H_values) const {
    ScopedMallocAllower ma;
    if (H_values.size() > 0)
