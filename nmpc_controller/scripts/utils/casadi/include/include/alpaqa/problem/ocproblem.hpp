...
struct ControlProblemVTable : util::BasicVTable {
  ...
  using optional_function_t =
      util::BasicVTable::optional_function_t<F, ControlProblemVTable>;
  template <class F>
  ...
  ControlProblemVTable(std::in_place_t, P &p)
      : util::BasicVTable{std::in_place, p} {
  ...
  ALPAQA_EXPORT static void default_eval_add_Q_N(
      const void *self, crvec x, crvec h, rmat Q,
      const ControlProblemVTable &vtable);
  ALPAQA_EXPORT static void default_eval_add_R_prod_masked(
      const void *, index_t, crvec, crvec, crindexvec, crindexvec, crvec, rvec,
      rvec, const ControlProblemVTable &);
  ALPAQA_EXPORT static void default_eval_add_S_prod_masked(
      const void *, index_t, crvec, crvec, crindexvec, crvec, rvec, rvec,
      const ControlProblemVTable &);
  [[nodiscard]] ALPAQA_EXPORT static length_t default_get_R_work_size(
      const void *, const ControlProblemVTable &);
  [[nodiscard]] ALPAQA_EXPORT static length_t default_get_S_work_size(
      const void *, const ControlProblemVTable &);
  ALPAQA_EXPORT static void default_eval_constr_N(
      const void *self, crvec x, rvec c, const ControlProblemVTable &vtable);
  ALPAQA_EXPORT static void default_eval_grad_constr_prod_N(
      const void *self, crvec x, crvec p, rvec grad_cx_p,
      const ControlProblemVTable &vtable);
  ALPAQA_EXPORT static void default_eval_add_gn_hess_constr_N(
      const void *self, crvec x, crvec M, rmat out,
      const ControlProblemVTable &vtable);
};
 ...
 *     \begin{aligned}
 *         &\minimize_{u,x} && \sum_{k=0}^{N-1} \ell_k\big(h_k(x^k, u^k)\big) +
 * \ell_N\big(h_N(x^N)\big)\hspace{-0.8em} \\
 *         &\subjto && u^k \in \U \\
 *         &&& c_k(x^k) \in \D \\
 *         &&& c_N(x^N) \in \D_N \\
 *         &&& x^0 = \xinit \\
 ...
 * \\ \end{aligned} @f] See @cite pas2022gaussnewton for more details.
 *
...
class TypeErasedControlProblem
    : public util::TypeErased<ControlProblemVTable<Conf>, Allocator> {
 public:
  USING_ALPAQA_CONFIG(Conf);
 ...
 public:
  ...
  static TypeErasedControlProblem make(Args &&...args) {
    return TypeErased::template make<TypeErasedControlProblem, T>(
  ...
  /// Cost Hessian w.r.t. inputs and states @f$ S_k =
  /// \tp{\jac_{h_k}^u\!(\barxuk)} \partial^2\ell_k(\hbar^k)\,
  /// \jac_{h_k}^x\!(\barxuk) @f$, keeping only rows in the mask @f$ \mathcal J
  /// @f$, added to the given matrix @p S.
...
struct ControlProblemWithCounters {
  ...
  [[nodiscard, gnu::always_inline]] length_t get_N() const {
    return problem.get_N();
  }
  [[nodiscard, gnu::always_inline]] length_t get_nu() const {
    return problem.get_nu();
  }
  [[nodiscard, gnu::always_inline]] length_t get_nx() const {
    return problem.get_nx();
  }
  [[nodiscard, gnu::always_inline]] length_t get_nh() const {
    return problem.get_nh();
  }
  [[nodiscard, gnu::always_inline]] length_t get_nh_N() const {
    return problem.get_nh_N();
  }
  [[nodiscard, gnu::always_inline]] length_t get_nc() const {
    return problem.get_nc();
  }
  [[nodiscard, gnu::always_inline]] length_t get_nc_N() const {
    return problem.get_nc_N();
  }
  ...
  std::shared_ptr<OCPEvalCounter> evaluations =
      std::make_shared<OCPEvalCounter>();
  Problem problem;
  ...
  void decouple_evaluations() {
    evaluations = std::make_shared<OCPEvalCounter>(*evaluations);
  }
