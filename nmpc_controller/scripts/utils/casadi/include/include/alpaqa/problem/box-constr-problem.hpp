...
class BoxConstrProblem {
 public:
   ...
   * \end{aligned} @f] */
  static real_t eval_proj_grad_step_box(const Box &C, real_t γ, crvec x,
   ...
   *         \big)
   *     \big) \end{aligned} @f] */
  static void eval_prox_grad_step_box_l1_impl(const Box &C, const auto &λ,
                                              real_t γ, crvec x, crvec grad_ψ,
                                              rvec x̂, rvec p) {
  ...
  static real_t eval_prox_grad_step_box_l1(const Box &C, const auto &λ,
                                           real_t γ, crvec x, crvec grad_ψ,
                                           rvec x̂, rvec p) {
  ...
  /// @copydoc eval_prox_grad_step_box_l1_impl
  static real_t eval_prox_grad_step_box_l1_scal(const Box &C, real_t λ,
                                                real_t γ, crvec x, crvec grad_ψ,
                                                rvec x̂, rvec p) {
    auto n = x.size();
  ...
  void eval_proj_diff_g(crvec z, rvec p) const {
    p = alpaqa::projecting_difference(z, D);
  }
  ...
  index_t eval_inactive_indices_res_lna(real_t γ, crvec x, crvec grad_ψ,
    ...
    const auto add_to_J_if_in_box_interior = [&](real_t x_fw, index_t i) {
      if (C.lowerbound(i) < x_fw && x_fw < C.upperbound(i)) J(nJ++) = i;
    };
  ...
  void check() const {
    ...
    if (l1_reg.size() > 1)
      util::check_dim_msg<config_t>(l1_reg, n,
                                    "Length of problem.l1_reg does not match "
                                    "problem size problem.n, 1 or 0");
