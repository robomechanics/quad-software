...
struct ProblemVTable : util::BasicVTable {
  ...
  ALPAQA_EXPORT static real_t calc_ŷ_dᵀŷ(const void *self, rvec g_ŷ, crvec y,
                                         crvec Σ, const ProblemVTable &vtable);
  ALPAQA_EXPORT static index_t default_eval_inactive_indices_res_lna(
      const void *, real_t, crvec, crvec, rindexvec, const ProblemVTable &);
  ALPAQA_EXPORT static void default_eval_jac_g(const void *, crvec, rindexvec,
                                               rindexvec, rvec,
                                               const ProblemVTable &);
  ALPAQA_EXPORT static length_t default_get_jac_g_num_nonzeros(
      const void *, const ProblemVTable &);
  ALPAQA_EXPORT static void default_eval_grad_gi(const void *, crvec, index_t,
                                                 rvec, const ProblemVTable &);
  ALPAQA_EXPORT static void default_eval_hess_L_prod(const void *, crvec, crvec,
                                                     real_t, crvec, rvec,
                                                     const ProblemVTable &);
  ALPAQA_EXPORT static void default_eval_hess_L(const void *, crvec, crvec,
                                                real_t, rindexvec, rindexvec,
                                                rvec, const ProblemVTable &);
  ALPAQA_EXPORT static length_t default_get_hess_L_num_nonzeros(
      const void *, const ProblemVTable &);
  ALPAQA_EXPORT static void default_eval_hess_ψ_prod(
      const void *self, crvec x, crvec y, crvec, real_t scale, crvec v, rvec Hv,
      const ProblemVTable &vtable);
  ALPAQA_EXPORT static void default_eval_hess_ψ(const void *self, crvec x,
                                                crvec y, crvec, real_t scale,
                                                rindexvec inner_idx,
                                                rindexvec outer_ptr,
                                                rvec H_values,
                                                const ProblemVTable &vtable);
  ALPAQA_EXPORT static length_t default_get_hess_ψ_num_nonzeros(
      const void *, const ProblemVTable &);
  ALPAQA_EXPORT static real_t default_eval_f_grad_f(
      const void *self, crvec x, rvec grad_fx, const ProblemVTable &vtable);
  ALPAQA_EXPORT static real_t default_eval_f_g(const void *self, crvec x,
                                               rvec g,
                                               const ProblemVTable &vtable);
  ALPAQA_EXPORT static void default_eval_grad_f_grad_g_prod(
      const void *self, crvec x, crvec y, rvec grad_f, rvec grad_gxy,
      const ProblemVTable &vtable);
  ALPAQA_EXPORT static void default_eval_grad_L(const void *self, crvec x,
                                                crvec y, rvec grad_L,
                                                rvec work_n,
                                                const ProblemVTable &vtable);
  ALPAQA_EXPORT static real_t default_eval_ψ(const void *self, crvec x, crvec y,
                                             crvec Σ, rvec ŷ,
                                             const ProblemVTable &vtable);
  ALPAQA_EXPORT static void default_eval_grad_ψ(const void *self, crvec x,
                                                crvec y, crvec Σ, rvec grad_ψ,
                                                rvec work_n, rvec work_m,
                                                const ProblemVTable &vtable);
  ALPAQA_EXPORT static real_t default_eval_ψ_grad_ψ(
      const void *self, crvec x, crvec y, crvec Σ, rvec grad_ψ, rvec work_n,
      rvec work_m, const ProblemVTable &vtable);
  ALPAQA_EXPORT static const Box &default_get_box_C(const void *,
                                                    const ProblemVTable &);
  ALPAQA_EXPORT static const Box &default_get_box_D(const void *,
                                                    const ProblemVTable &);
...
/// Instead, [structural
/// typing](https://en.wikipedia.org/wiki/Structural_type_system) is used. The
/// @ref ProblemVTable constructor uses reflection to discover which member
/// functions are provided by the problem implementation. See
...
class TypeErasedProblem
 ...
 public:
  ...
  real_t eval_prox_grad_step(real_t γ, crvec x, crvec grad_ψ, rvec x̂,
                             rvec p) const;
  /// **[Optional]**
  ...
  index_t eval_inactive_indices_res_lna(real_t γ, crvec x, crvec grad_ψ,
                                        rindexvec J) const;
  ...
  bool provides_eval_inactive_indices_res_lna() const {
    return vtable.eval_inactive_indices_res_lna !=
           vtable.default_eval_inactive_indices_res_lna;
  }
  /// Returns true if the problem provides an implementation of
  /// @ref eval_jac_g.
  ...
  bool provides_get_jac_g_num_nonzeros() const {
    return vtable.get_jac_g_num_nonzeros !=
           vtable.default_get_jac_g_num_nonzeros;
  }
  /// Returns true if the problem provides an implementation of
  /// @ref eval_grad_gi.
  ...
  bool provides_get_hess_L_num_nonzeros() const {
    return vtable.get_hess_L_num_nonzeros !=
           vtable.default_get_hess_L_num_nonzeros;
  }
  /// Returns true if the problem provides an implementation of
  ...
  bool provides_get_hess_ψ_num_nonzeros() const {
    return vtable.get_hess_ψ_num_nonzeros !=
           vtable.default_get_hess_ψ_num_nonzeros;
  }
...
template <Config Conf, class Allocator>
void TypeErasedProblem<Conf, Allocator>::eval_proj_multipliers(rvec y,
                                                               real_t M) const {
  return call(vtable.eval_proj_multipliers, y, M);
}
template <Config Conf, class Allocator>
auto TypeErasedProblem<Conf, Allocator>::eval_prox_grad_step(
    real_t γ, crvec x, crvec grad_ψ, rvec x̂, rvec p) const -> real_t {
...
template <Config Conf, class Allocator>
void TypeErasedProblem<Conf, Allocator>::eval_jac_g(crvec x,
                                                    rindexvec inner_idx,
                                                    rindexvec outer_ptr,
                                                    rvec J_values) const {
...
template <Config Conf, class Allocator>
auto TypeErasedProblem<Conf, Allocator>::get_jac_g_num_nonzeros() const
    -> length_t {
...
template <Config Conf, class Allocator>
auto TypeErasedProblem<Conf, Allocator>::get_hess_L_num_nonzeros() const
    -> length_t {
  return call(vtable.get_hess_L_num_nonzeros);
}
template <Config Conf, class Allocator>
void TypeErasedProblem<Conf, Allocator>::eval_hess_ψ_prod(crvec x, crvec y,
...
template <Config Conf, class Allocator>
void TypeErasedProblem<Conf, Allocator>::eval_hess_ψ(crvec x, crvec y, crvec Σ,
                                                     real_t scale,
                                                     rindexvec inner_idx,
                                                     rindexvec outer_ptr,
                                                     rvec H_values) const {
  return call(vtable.eval_hess_ψ, x, y, Σ, scale, inner_idx, outer_ptr,
              H_values);
}
template <Config Conf, class Allocator>
auto TypeErasedProblem<Conf, Allocator>::get_hess_ψ_num_nonzeros() const
    -> length_t {
  return call(vtable.get_hess_ψ_num_nonzeros);
}
template <Config Conf, class Allocator>
auto TypeErasedProblem<Conf, Allocator>::eval_f_grad_f(
    crvec x, rvec grad_fx) const -> real_t {
  return call(vtable.eval_f_grad_f, x, grad_fx);
}
template <Config Conf, class Allocator>
auto TypeErasedProblem<Conf, Allocator>::eval_f_g(crvec x,
                                                  rvec g) const -> real_t {
...
template <Config Conf, class Allocator>
auto TypeErasedProblem<Conf, Allocator>::eval_ψ(crvec x, crvec y, crvec Σ,
                                                rvec ŷ) const -> real_t {
...
template <Config Conf, class Allocator>
auto TypeErasedProblem<Conf, Allocator>::eval_ψ_grad_ψ(
    crvec x, crvec y, crvec Σ, rvec grad_ψ, rvec work_n,
    rvec work_m) const -> real_t {
...
void print_provided_functions(std::ostream &os,
  ...
  os << "inactive_indices_res_lna: "
     ...
     << "      grad_f_grad_g_prod: "
     << problem.provides_eval_grad_f_grad_g_prod() << '\n'
