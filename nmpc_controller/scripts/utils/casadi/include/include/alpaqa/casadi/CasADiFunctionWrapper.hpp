...
class CasADiFunctionEvaluator {
 public:
  ...
  void validate_dimensions(const std::array<casadi_dim, N_in> &dim_in = {},
                           const std::array<casadi_dim, N_out> &dim_out = {}) {
    using namespace std::literals::string_literals;
    static constexpr std::array count{"first", "second", "third",   "fourth",
    ...
    for (size_t n = 0; n < N_in; ++n) {
      ...
      if (dim_in[n].first != 0 && dim_in[n] != fun.size_in(cs_n))
        throw std::invalid_argument(
            "Invalid dimension of "s + count[n] + " input argument: got " +
            to_string(fun.size_in(cs_n)) + ", should be " +
            to_string(dim_in[n]) + ".");
    }
    for (size_t n = 0; n < N_out; ++n) {
      ...
      if (dim_out[n].first != 0 && dim_out[n] != fun.size_out(cs_n))
        throw std::invalid_argument(
            "Invalid dimension of "s + count[n] + " output argument: got " +
            to_string(fun.size_out(cs_n)) + ", should be " +
            to_string(dim_out[n]) + ".");
    }
  }
