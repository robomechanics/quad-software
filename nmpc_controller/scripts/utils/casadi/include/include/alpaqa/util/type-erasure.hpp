...
struct BasicVTable {
  ...
  BasicVTable(std::in_place_t, T &) noexcept {
    ...
    move = [](void *self, void *storage) noexcept {
      new (storage) T(std::move(*std::launder(reinterpret_cast<T *>(self))));
    };
    destroy = [](void *self) {
...
struct Launderer {
 private:
  ...
  [[gnu::always_inline]] static constexpr auto do_invoke(V *self, Args... args,
                                                         ExtraArgs...) -> R {
...
class TypeErased {
 ...
 public:
  ...
  TypeErased &operator=(TypeErased &&other) noexcept {
    ...
    if constexpr (prop_alloc) allocator = std::move(other.allocator);
    // Only assign if other contains a value
    if (other.self == nullptr) return *this;
  ...
  template <class T>
  T &as() & {
    if (typeid(T) != type()) throw bad_type_erased_type(type(), typeid(T));
 ...
 private:
  ...
  void do_copy_assign(const TypeErased &other) {
    ...
    if constexpr (CopyAllocator && prop_alloc) allocator = other.allocator;
    if (!other) return;
    vtable = other.vtable;
 ...
 protected:
  ...
  /// @copydoc call
  template <class Ret>
  [[gnu::always_inline]] decltype(auto) call(Ret (*f)(void *, const VTable &)) {
    assert(f);
    assert(self);
    return f(self, vtable);
  }
