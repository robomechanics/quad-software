 ...
 **/
SLEQP_EXPORT SLEQP_ERROR_TYPE sleqp_error_type();

/**
 * Returns the message associated
 * with  the current error
 **/
SLEQP_EXPORT const char* sleqp_error_msg();

/**
 * Sets the current error. To be used by @ref sleqp_raise
 **/
SLEQP_EXPORT void sleqp_set_error(const char* file, int line, const char* func,
                                  SLEQP_ERROR_TYPE error_type, const char* fmt,
...
#define sleqp_raise(error_type, fmt, ...)                                     \
  do {                                                                        \
    sleqp_set_error(__FILE__, __LINE__, __PRETTY_FUNCTION__, error_type, fmt, \
                    ##__VA_ARGS__);                                           \
    return SLEQP_ERROR;                                                       \
  } while (false)
