...
SplitBeltController::SplitBeltController(ros::NodeHandle nh, int argc, char** argv) {
  ...
  quad_utils::loadROSParam(nh_, "/split_belt_controller/update_rate", update_rate_);

  nh.param<double>("split_belt_controller/left_belt_velocity", left_belt_velocity_, 0.0);
  nh.param<double>("split_belt_controller/right_belt_velocity", right_belt_velocity_, 0.0);

  ros::Publisher left_belt_pub_ = nh.advertise<std_msgs::Float64>("left_belt_velocity", left_belt_velocity_);
  ros::Publisher right_belt_pub_ = nh.advertise<std_msgs::Float64>("right_belt_velocity", right_belt_velocity_);
}
